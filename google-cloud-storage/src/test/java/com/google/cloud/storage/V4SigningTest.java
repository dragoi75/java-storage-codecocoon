...
public class V4SigningTest {
  ...
  public void test() {
    ...
    BlobMetadata blob = BlobMetadata.newBuilder(testData.getBucket(), testData.getObject()).buildMetadata();

    UrlSigningOption style = UrlSigningOption.usePathStyle();

    if (testData.getUrlStyle().equals(UrlStyle.VIRTUAL_HOSTED_STYLE)) {
      style = UrlSigningOption.useVirtualHostedStyle();
    } else if (testData.getUrlStyle().equals(UrlStyle.PATH_STYLE)) {
      style = CloudStorageClient.UrlSigningOption.usePathStyle();
    } else if (testData.getUrlStyle().equals(UrlStyle.BUCKET_BOUND_HOSTNAME)) {
      style =
          CloudStorageClient.UrlSigningOption.withBucketBoundHostname(
              testData.getBucketBoundHostname(),
              CloudStorageClient.UriSchemeType.valueOf(testData.getScheme().toUpperCase()));
    }
    ...
    final String signedUrl =
        storage
            .signUrl(
                ...
                TimeUnit.SECONDS,
                CloudStorageClient.UrlSigningOption.withHttpMethod(HttpRequestMethod.fromValue(testData.getMethod())),
                UrlSigningOption.includeExtHeaders(testData.getHeadersMap()),
                CloudStorageClient.UrlSigningOption.useV4Signature(),
                UrlSigningOption.includeQueryParams(testData.getQueryParametersMap()),
                style)
            .toString();
