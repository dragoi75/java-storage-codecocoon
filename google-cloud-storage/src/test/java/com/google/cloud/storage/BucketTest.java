...
public class BucketTest {
  ...
  private static final List<? extends BucketMetadata.LifecycleRuleEntry> LIFECYCLE_RULES =
      Collections.singletonList(
          new BucketMetadata.LifecycleRuleEntry(
              ...
              BucketMetadata.LifecycleRuleEntry.LifecycleRuleCondition.newLifecycleConditionBuilder().setAge(5).buildLifecycleRuleCondition()));
  ...
  private void initializeExpectedBucket() {
    ...
    replay(mockOptions);
    expectedBucket = new Bucket(serviceMockReturnsOptions, new BucketMetadata.BucketBuilderImpl(BUCKET_INFO));
    blobResults =
        ImmutableList.of(
            new Blob(
  ...
  private void initializeBucket() {
    bucket = new Bucket(storage, new BucketMetadata.BucketBuilderImpl(BUCKET_INFO));
  }
  ...
  public void testReload() throws Exception {
    initializeExpectedBucket();
    BucketMetadata updatedInfo = BUCKET_INFO.newBuilder().setNotFoundPage("p").buildBucket();
    Bucket expectedUpdatedBucket =
        new Bucket(serviceMockReturnsOptions, new BucketMetadata.BucketBuilderImpl(updatedInfo));
    expect(storage.getOptions()).andReturn(mockOptions);
  ...
  public void testUpdate() throws Exception {
    initializeExpectedBucket();
    Bucket expectedUpdatedBucket = expectedBucket.newBuilder().setNotFoundPage("p").buildBucket();
    ...
    replay(storage);
    initializeBucket();
    Bucket updatedBucket = new Bucket(storage, new BucketMetadata.BucketBuilderImpl(expectedUpdatedBucket));
  ...
  public void testLockRetention() throws Exception {
    ...
    Bucket lockedRetentionPolicyBucket =
        new Bucket(storage, new BucketMetadata.BucketBuilderImpl(expectedRetentionLockedBucket));
  ...
  public void testToBuilder() {
    ...
    replay(storage);
    Bucket fullBucket = new Bucket(storage, new BucketMetadata.BucketBuilderImpl(FULL_BUCKET_INFO));
    assertEquals(fullBucket, fullBucket.newBuilder().buildBucket());
    Bucket simpleBlob = new Bucket(storage, new BucketMetadata.BucketBuilderImpl(BUCKET_INFO));
    assertEquals(simpleBlob, simpleBlob.newBuilder().buildBucket());
  }
  ...
  public void testBuilder() {
    ...
    Bucket.Builder builder =
        new Bucket.Builder(new Bucket(storage, new BucketMetadata.BucketBuilderImpl(BUCKET_INFO)));
    Bucket bucket =
        builder
            .setAcl(ACLS)
            .setEtag(ETAG)
  ...
  public void testDeleteLifecycleRules() {
    initializeExpectedBucket();
    Bucket bucket =
        new Bucket(serviceMockReturnsOptions, new BucketMetadata.BucketBuilderImpl(FULL_BUCKET_INFO));
    assertThat(bucket.getLifecycleRules()).hasSize(1);
    Bucket expectedUpdatedBucket = bucket.newBuilder().removeLifecycleRules().buildBucket();
    expect(storage.getOptions()).andReturn(mockOptions).times(2);
    expect(storage.update(expectedUpdatedBucket)).andReturn(expectedUpdatedBucket);
    replay(storage);
    initializeBucket();
  ...
  public void testUpdateBucketLogging() {
    ...
    Bucket bucket = new Bucket(serviceMockReturnsOptions, new BucketMetadata.BucketBuilderImpl(bucketInfo));
    ...
    Bucket expectedUpdatedBucket = bucket.newBuilder().setLogging(null).buildBucket();
    expect(storage.getOptions()).andReturn(mockOptions).times(2);
    expect(storage.update(expectedUpdatedBucket)).andReturn(expectedUpdatedBucket);
    replay(storage);
    initializeBucket();
  ...
  public void testRemoveBucketCORS() {
    ...
    Bucket expectedUpdatedBucket = bucket.newBuilder().setCors(null).buildBucket();
    expect(storage.getOptions()).andReturn(mockOptions).times(2);
    expect(storage.update(expectedUpdatedBucket)).andReturn(expectedUpdatedBucket);
    replay(storage);
    initializeBucket();
