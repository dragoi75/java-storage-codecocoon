...
public class BucketTest {
  ...
  public void testGet() throws Exception {
    ...
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.get(BlobId.of(expectedBucket.getName(), "n"), new Storage.BlobGetOptions[0]))
  ...
  public void testCreateWithOptions() throws Exception {
    ...
    expect(
            storage.create(
                ...
                Storage.BlobTargetOptions.setPredefinedAcl(acl),
                Storage.BlobTargetOptions.customerSuppliedKey(BASE64_KEY),
                Storage.BlobTargetOptions.withUserProject(USER_PROJECT)))
        .andReturn(expectedBlob);
    replay(storage);
    initializeBucket();
    StorageObject blob =
  ...
  public void testCreateWithKmsKeyName() throws Exception {
    ...
    byte[] content = {0xD, 0xE, 0xA, 0xD};
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.create(info, content, Storage.BlobTargetOptions.setKmsKeyName(DEFAULT_KMS_KEY_NAME)))
        .andReturn(expectedBlob);
    replay(storage);
    initializeBucket();
    StorageObject blob =
  ...
  public void testCreateFromStreamWithOptions() throws Exception {
    ...
    expect(
            storage.create(
                ...
                Storage.BlobWriteOptions.setUserProject(USER_PROJECT)))
        .andReturn(expectedBlob);
    replay(storage);
    initializeBucket();
    StorageObject blob =
