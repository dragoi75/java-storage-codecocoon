...
public class BucketTest {
  ...
  public void testExists_True() throws Exception {
    ...
    Storage.BucketGetOptions[] expectedOptions = {Storage.BucketGetOptions.setFields()};
  ...
  public void testCreateWithOptions() throws Exception {
    ...
    Blob expectedBlob = new Blob(serviceMockReturnsOptions, new BlobInfo.BuilderImpl(info));
    byte[] content = {0xD, 0xE, 0xA, 0xD};
    Storage.PredefinedAclEntry acl = Storage.PredefinedAclEntry.ALL_AUTHENTICATED_USERS;
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(
            storage.create(
                info,
                ...
                Storage.BlobUploadOption.encryptionKeyOption(BASE64_KEY),
  ...
  public void testCreateWithEncryptionKey() throws Exception {
    ...
    byte[] content = {0xD, 0xE, 0xA, 0xD};
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.create(info, content, Storage.BlobUploadOption.encryptionKeyOption(KEY)))
        .andReturn(expectedBlob);
    replay(storage);
    initializeBucket();
    Blob blob =
  ...
  public void testCreateFromStreamWithOptions() throws Exception {
    ...
    expect(
            storage.create(
                ...
                Storage.ObjectWriteOption.withPredefinedAcl(acl),
                Storage.ObjectWriteOption.ifCrc32cMatch(),
                Storage.ObjectWriteOption.ifMd5Match(),
                Storage.ObjectWriteOption.customerSuppliedKey(BASE64_KEY),
                Storage.ObjectWriteOption.withUserProject(USER_PROJECT)))
        .andReturn(expectedBlob);
    replay(storage);
    initializeBucket();
    Blob blob =
  ...
  public void testCreateFromStreamWithEncryptionKey() throws Exception {
    ...
    InputStream streamContent = new ByteArrayInputStream(content);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.create(info, streamContent, Storage.ObjectWriteOption.customerSuppliedKey(KEY)))
        .andReturn(expectedBlob);
    replay(storage);
    initializeBucket();
    Blob blob =
  ...
  public void testCreateFromStreamNotExists() throws Exception {
    ...
    InputStream streamContent = new ByteArrayInputStream(content);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.create(info, streamContent, Storage.ObjectWriteOption.ifGenerationMatch()))
        .andReturn(expectedBlob);
    replay(storage);
    initializeBucket();
    Blob blob =
