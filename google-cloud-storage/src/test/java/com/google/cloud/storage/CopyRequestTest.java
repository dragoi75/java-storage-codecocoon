...
public class CopyRequestTest {
  ...
  private static final BlobInfo TARGET_BLOB_INFO =
      BlobInfo.newBuilder(TARGET_BLOB_ID).setContentType(TARGET_BLOB_CONTENT_TYPE).buildMetadata();
  ...
  public void testCopyRequest() {
    Storage.CopyOperationRequest copyRequest1 =
        Storage.CopyOperationRequest.createBuilder()
            ...
            .setSourceOptions(Storage.BlobSourceSettings.ifGenerationMatch(1))
            .setTarget(TARGET_BLOB_INFO, Storage.BlobUploadOption.withPredefinedAcl(PUBLIC_READ))
    ...
    assertEquals(BlobSourceSettings.ifGenerationMatch(1), copyRequest1.getSourceOptions().get(0));
    ...
    assertEquals(BlobInfo.newBuilder(TARGET_BLOB_ID).buildMetadata(), copyRequest2.getTarget());
  ...
  public void testCopyRequestOf() {
    ...
    assertEquals(SOURCE_BLOB_ID, copyRequest2.getSource());
    assertEquals(
        BlobInfo.newBuilder(BlobIdentifier.create(SOURCE_BUCKET_NAME, TARGET_BLOB_NAME)).buildMetadata(),
    ...
    assertEquals(BlobInfo.newBuilder(TARGET_BLOB_ID).buildMetadata(), copyRequest5.getTarget());
    ...
    assertEquals(BlobInfo.newBuilder(TARGET_BLOB_ID).buildMetadata(), copyRequest6.getTarget());
