...
public class BlobWriteChannelTest {
  ...
  private static final BlobMetadata BLOB_INFO = BlobMetadata.newBuilder(BUCKET_NAME, BLOB_NAME).buildMetadata();
  ...
  public void testWriteWithRetryAndObjectMetadata() throws IOException {
    ...
    expect(storageRpcMock.getCurrentUploadOffset(eq(UPLOAD_ID))).andReturn(-1L);
    expect(storageRpcMock.get(BLOB_INFO.toProto(), null)).andThrow(socketClosedException);
    expect(storageRpcMock.getCurrentUploadOffset(eq(UPLOAD_ID))).andReturn(-1L);
    expect(storageRpcMock.get(BLOB_INFO.toProto(), null))
