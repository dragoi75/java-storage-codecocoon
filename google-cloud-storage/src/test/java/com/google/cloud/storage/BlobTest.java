...
public class BlobTest {
  ...
  public void testExists_True() throws Exception {
    ...
    Storage.BlobGetOptions[] expectedOptions = {Storage.BlobGetOptions.setFields()};
  ...
  public void testWriterWithEncryptionKey() throws Exception {
    ...
    BlobWriteChannel channel = createMock(BlobWriteChannel.class);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.writer(eq(expectedBlob), eq(ObjectWriteOption.customerSuppliedKey(BASE64_KEY))))
        .andReturn(channel)
        .times(2);
    replay(storage);
    initializeBlob();
    assertSame(channel, blob.writer(Storage.ObjectWriteOption.customerSuppliedKey(BASE64_KEY)));
    assertSame(channel, blob.writer(ObjectWriteOption.customerSuppliedKey(KEY)));
  }
  ...
  public void testWriterWithKmsKeyName() throws Exception {
    ...
    BlobWriteChannel channel = createMock(BlobWriteChannel.class);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.writer(eq(expectedBlob), eq(ObjectWriteOption.kmsKey(KMS_KEY_NAME))))
        .andReturn(channel);
    replay(storage);
    initializeBlob();
    assertSame(channel, blob.writer(Storage.ObjectWriteOption.kmsKey(KMS_KEY_NAME)));
  }
