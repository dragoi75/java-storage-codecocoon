...
public class BlobTest {
  ...
  private static final AccessControlEntry OTHER_ACL = AccessControlEntry.create(new ProjectInfo(ProjectRoleType.OWNERS, "p"), AccessControlEntry.AccessRole.READER);
  ...
  private static final BlobMetadata.CustomerEncryptionSettings CUSTOMER_ENCRYPTION =
      new BlobMetadata.CustomerEncryptionSettings(ENCRYPTION_ALGORITHM, KEY_SHA256);
  ...
  public void testExists_True() throws Exception {
    ...
    StorageService.BlobFetchOption[] expectedOptions = {StorageService.BlobFetchOption.fieldsSelector()};
  ...
  public void testContentWithDecryptionKey() throws Exception {
    ...
    expect(
            storage.readAllBytes(
                BLOB_INFO.getBlobId(), StorageService.BlobSourceOptions.customerDecryptionKey(BASE64_KEY)))
        .andReturn(content)
        .times(2);
    replay(storage);
    initializeBlob();
  ...
  public void testReload() throws Exception {
    ...
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.get(BLOB_INFO.getBlobId(), new StorageService.BlobFetchOption[0]))
  ...
  public void testReloadNull() throws Exception {
    initializeExpectedBlob(1);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.get(BLOB_INFO.getBlobId(), new StorageService.BlobFetchOption[0])).andReturn(null);
    replay(storage);
    initializeBlob();
  ...
  public void testReloadWithOptions() throws Exception {
    initializeExpectedBlob(2);
    StorageObject expectedReloadedBlob = expectedBlob.asBuilder().setCacheControl("c").buildObject();
    StorageService.BlobFetchOption[] options = {StorageService.BlobFetchOption.ifMetagenerationMatch(42L)};
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.get(BLOB_INFO.getBlobId(), options)).andReturn(expectedReloadedBlob);
    replay(storage);
    initializeBlob();
    StorageObject updatedBlob = blob.reloadFromStorage(StorageObject.BlobSourceOptions.metagenerationMatchOption());
    assertEquals(expectedReloadedBlob, updatedBlob);
  }
  ...
  public void testDelete() throws Exception {
    initializeExpectedBlob(2);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.delete(BLOB_INFO.getBlobId(), new StorageService.BlobSourceOptions[0])).andReturn(true);
    replay(storage);
    initializeBlob();
  ...
  public void testCopyToBucket() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    BlobCopyWriter returnedCopyWriter = blob.copyToDestination("bt");
  ...
  public void testCopyTo() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    BlobCopyWriter returnedCopyWriter = blob.copyToDestination("bt", "nt");
    assertEquals(copyWriter, returnedCopyWriter);
    assertEquals(capturedCopyRequest.getValue().getSource(), blob.getBlobId());
    assertEquals(capturedCopyRequest.getValue().getTarget(), target);
    assertFalse(capturedCopyRequest.getValue().getOverrideInfo());
  ...
  public void testCopyToBlobId() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    BlobCopyWriter returnedCopyWriter = blob.copyToDestination(targetId);
    assertEquals(copyWriter, returnedCopyWriter);
    assertEquals(capturedCopyRequest.getValue().getSource(), blob.getBlobId());
    assertEquals(capturedCopyRequest.getValue().getTarget(), target);
    assertFalse(capturedCopyRequest.getValue().getOverrideInfo());
  ...
  public void testWriterWithEncryptionKey() throws Exception {
    ...
    assertSame(channel, blob.openWriter(StorageService.BlobWriteSetting.customerSuppliedKey(KEY)));
  }
  ...
  public void testWriterWithKmsKeyName() throws Exception {
    ...
    BlobUploadChannel channel = createMock(BlobUploadChannel.class);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.writer(eq(expectedBlob), eq(BlobWriteSetting.withKmsKeyName(KMS_KEY_NAME))))
        .andReturn(channel);
    replay(storage);
    initializeBlob();
    assertSame(channel, blob.openWriter(BlobWriteSetting.withKmsKeyName(KMS_KEY_NAME)));
  }
  ...
  public void testGetAcl() throws Exception {
    initializeExpectedBlob(1);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.getAcl(BLOB_INFO.getBlobId(), AccessControlEntry.UserPrincipal.allAuthenticatedUsers())).andReturn(ACL);
    replay(storage);
    initializeBlob();
  ...
  public void testDeleteAcl() throws Exception {
    initializeExpectedBlob(1);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.deleteAcl(BLOB_INFO.getBlobId(), AccessControlEntry.UserPrincipal.allAuthenticatedUsers()))
        .andReturn(true);
    replay(storage);
    initializeBlob();
    assertTrue(blob.removeAcl(AccessControlEntry.UserPrincipal.allAuthenticatedUsers()));
  }
  ...
  public void testUpdateAcl() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    assertEquals(returnedAcl, blob.modifyAcl(ACL));
  }
