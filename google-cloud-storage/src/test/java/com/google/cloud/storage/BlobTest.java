...
public class BlobTest {
  ...
  private void initializeExpectedBlob(int optionsCalls) {
    ...
    expectedBlob = new StorageObject(serviceMockReturnsOptions, new BlobInfo.BuilderImpl(BLOB_INFO));
  }

  private void initializeBlob() {
    blob = new StorageObject(storage, new BlobInfo.BuilderImpl(BLOB_INFO));
  }

  @Test
  public void testExists_True() throws Exception {
    initializeExpectedBlob(1);
    Storage.BlobFetchOption[] expectedOptions = {Storage.BlobFetchOption.withFields()};
  ...
  public void testContentWithDecryptionKey() throws Exception {
    ...
    expect(
            storage.readAllBytes(
                BLOB_INFO.getBlobId(), Storage.BlobReadOption.customerSuppliedKey(BASE64_KEY)))
        .andReturn(content)
        .times(2);
    replay(storage);
    initializeBlob();
    assertArrayEquals(content, blob.getContent(BlobSourceOptions.customerSuppliedKey(BASE64_KEY)));
  ...
  public void testReload() throws Exception {
    initializeExpectedBlob(2);
    StorageObject expectedReloadedBlob = expectedBlob.toBuilder().setCacheControl("c").build();
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.get(BLOB_INFO_NO_GENERATION.getBlobId(), new Storage.BlobFetchOption[0]))
        .andReturn(expectedReloadedBlob);
    replay(storage);
    initializeBlob();
    StorageObject updatedBlob = blob.refresh();
  ...
  public void testReloadNull() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    StorageObject reloadedBlob = blob.refresh();
  ...
  public void testReloadWithOptions() throws Exception {
    initializeExpectedBlob(2);
    StorageObject expectedReloadedBlob = expectedBlob.toBuilder().setCacheControl("c").build();
    Storage.BlobFetchOption[] options = {Storage.BlobFetchOption.matchMetageneration(42L)};
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.get(BLOB_INFO_NO_GENERATION.getBlobId(), options))
        .andReturn(expectedReloadedBlob);
    replay(storage);
    initializeBlob();
    StorageObject updatedBlob = blob.refresh(BlobSourceOptions.ifMetagenerationMatch());
    assertEquals(expectedReloadedBlob, updatedBlob);
  }
  ...
  public void testUpdate() throws Exception {
    initializeExpectedBlob(2);
    StorageObject expectedUpdatedBlob = expectedBlob.toBuilder().setCacheControl("c").build();
    expect(storage.getOptions()).andReturn(mockOptions).times(2);
    expect(storage.update(eq(expectedUpdatedBlob), new Storage.BlobTargetOptions[0]))
        .andReturn(expectedUpdatedBlob);
    replay(storage);
    initializeBlob();
    StorageObject updatedBlob = new StorageObject(storage, new BlobInfo.BuilderImpl(expectedUpdatedBlob));
    StorageObject actualUpdatedBlob = updatedBlob.save();
  ...
  public void testDelete() throws Exception {
    initializeExpectedBlob(2);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.delete(BLOB_INFO.getBlobId(), new Storage.BlobReadOption[0])).andReturn(true);
    replay(storage);
    initializeBlob();
    assertTrue(blob.remove());
  }
  ...
  public void testCopyToBucket() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    CopyWriter returnedCopyWriter = blob.downloadIntoInternal("bt");
  ...
  public void testCopyTo() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    CopyWriter returnedCopyWriter = blob.copyToBlob("bt", "nt");
    assertEquals(copyWriter, returnedCopyWriter);
    assertEquals(BLOB_INFO_NO_GENERATION.getBlobId(), capturedCopyRequest.getValue().getSource());
    assertEquals(target, capturedCopyRequest.getValue().getTarget());
    assertFalse(capturedCopyRequest.getValue().getOverrideInfo());
  ...
  public void testCopyToBlobId() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    CopyWriter returnedCopyWriter = blob.copyToBlob(targetId);
    assertEquals(copyWriter, returnedCopyWriter);
    assertEquals(BLOB_INFO_NO_GENERATION.getBlobId(), capturedCopyRequest.getValue().getSource());
    assertEquals(target, capturedCopyRequest.getValue().getTarget());
    assertFalse(capturedCopyRequest.getValue().getOverrideInfo());
  ...
  public void testReader() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    assertSame(channel, blob.getReader());
  }
  ...
  public void testReaderWithDecryptionKey() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    assertSame(channel, blob.getReader(BlobSourceOptions.customerSuppliedKey(BASE64_KEY)));
    assertSame(channel, blob.getReader(BlobSourceOptions.customerSuppliedKey(KEY)));
  }
  ...
  public void testWriter() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    assertSame(channel, blob.getWriter());
  }
  ...
  public void testWriterWithEncryptionKey() throws Exception {
    ...
    BlobWriteChannel channel = createMock(BlobWriteChannel.class);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.writer(eq(expectedBlob), eq(BlobWriteOptions.getEncryptionKey(BASE64_KEY))))
        .andReturn(channel)
        .times(2);
    replay(storage);
    initializeBlob();
    assertSame(channel, blob.getWriter(BlobWriteOptions.getEncryptionKey(BASE64_KEY)));
    assertSame(channel, blob.getWriter(BlobWriteOptions.getEncryptionKey(KEY)));
  }
  ...
  public void testWriterWithKmsKeyName() throws Exception {
    ...
    BlobWriteChannel channel = createMock(BlobWriteChannel.class);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.writer(eq(expectedBlob), eq(Storage.BlobWriteOptions.withKmsKeyName(KMS_KEY_NAME))))
        .andReturn(channel);
    replay(storage);
    initializeBlob();
    assertSame(channel, blob.getWriter(BlobWriteOptions.withKmsKeyName(KMS_KEY_NAME)));
  }
  ...
  public void testDeleteAcl() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    assertTrue(blob.removeAcl(User.ofAllAuthenticatedUsers()));
  }
  ...
  public void testCreateAcl() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    assertEquals(returnedAcl, blob.createBlobAcl(ACL));
  }
  ...
  public void testUpdateAcl() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    assertEquals(returnedAcl, blob.setAcl(ACL));
  }
  ...
  public void testListAcls() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    assertEquals(ACLS, blob.getAcls());
  }
  ...
  public void testToBuilder() {
    ...
    replay(storage);
    StorageObject fullBlob = new StorageObject(storage, new BlobInfo.BuilderImpl(FULL_BLOB_INFO));
    assertEquals(fullBlob, fullBlob.toBuilder().build());
    StorageObject simpleBlob = new StorageObject(storage, new BlobInfo.BuilderImpl(BLOB_INFO));
    assertEquals(simpleBlob, simpleBlob.toBuilder().build());
    StorageObject directory = new StorageObject(storage, new BlobInfo.BuilderImpl(DIRECTORY_INFO));
  ...
  public void testBuilder() {
    ...
    expect(storage.getOptions()).andReturn(mockOptions).times(6);
    replay(storage);
    StorageObject.BlobInfoBuilder builder = new StorageObject.BlobInfoBuilder(new StorageObject(storage, new BlobInfo.BuilderImpl(BLOB_INFO)));
    StorageObject blob =
        builder
            .setAcl(ACLS)
            .setComponentCount(COMPONENT_COUNT)
    ...
    builder = new StorageObject.BlobInfoBuilder(new StorageObject(storage, new BlobInfo.BuilderImpl(DIRECTORY_INFO)));
