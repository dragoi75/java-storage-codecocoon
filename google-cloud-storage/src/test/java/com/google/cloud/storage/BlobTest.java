...
public class BlobTest {
  ...
  private static final AclEntry ACL = AclEntry.ofEntry(UserPrincipal.allAuthenticatedUsers(), AccessRole.OWNER);
  private static final AclEntry OTHER_ACL = AclEntry.ofEntry(new ProjectInfo(ProjectRoleType.OWNERS, "p"), AccessRole.READER);
  ...
  private static final BlobAttributes.CustomerEncryptionInfo CUSTOMER_ENCRYPTION =
      new BlobAttributes.CustomerEncryptionInfo(ENCRYPTION_ALGORITHM, KEY_SHA256);
  ...
  private static final BlobAttributes FULL_BLOB_INFO =
  ...
  private static final BlobAttributes BLOB_INFO =
      BlobAttributes.newBuilder("b", "n", 12345678L).setMetageneration(42L).buildObject();
  private static final BlobAttributes BLOB_INFO_NO_GENERATION =
      BlobAttributes.newBuilder(BLOB_INFO.getBucket(), BLOB_INFO.getName())
  ...
  private void initializeExpectedBlob(int optionsCalls) {
    ...
    expectedBlob = new StorageObject(serviceMockReturnsOptions, new BlobAttributes.BlobInfoBuilderImpl(BLOB_INFO));
  }

  private void initializeBlob() {
    blob = new StorageObject(storage, new BlobAttributes.BlobInfoBuilderImpl(BLOB_INFO));
  }

  @Test
  public void testExists_True() throws Exception {
    initializeExpectedBlob(1);
    StorageClient.BlobGetOptions[] expectedOptions = {StorageClient.BlobGetOptions.selectFields()};
  ...
  public void testContentWithDecryptionKey() throws Exception {
    ...
    expect(
            storage.readAllBytes(
                BLOB_INFO.getBlobId(), StorageClient.BlobSourceOptions.customerSuppliedKey(BASE64_KEY)))
        .andReturn(content)
        .times(2);
    replay(storage);
    initializeBlob();
    assertArrayEquals(content, blob.getContent(StorageObject.BlobSourceOptions.withDecryptionKey(BASE64_KEY)));
  ...
  public void testReload() throws Exception {
    initializeExpectedBlob(2);
    StorageObject expectedReloadedBlob = expectedBlob.asBuilder().setCacheControl("c").buildObject();
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.get(BLOB_INFO_NO_GENERATION.getBlobId(), new StorageClient.BlobGetOptions[0]))
  ...
  public void testReloadWithOptions() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    StorageObject updatedBlob = blob.reloadFromStorage(StorageObject.BlobSourceOptions.ifMetagenerationMatch());
    assertEquals(expectedReloadedBlob, updatedBlob);
  }
  ...
  public void testUpdate() throws Exception {
    initializeExpectedBlob(2);
    StorageObject expectedUpdatedBlob = expectedBlob.asBuilder().setCacheControl("c").buildObject();
    ...
    replay(storage);
    initializeBlob();
    StorageObject updatedBlob = new StorageObject(storage, new BlobAttributes.BlobInfoBuilderImpl(expectedUpdatedBlob));
  ...
  public void testCopyToBucket() throws Exception {
    initializeExpectedBlob(2);
    BlobAttributes target = BlobAttributes.newBuilder(BlobIdentifier.create("bt", "n")).buildObject();
    BlobCopyWriter copyWriter = createMock(BlobCopyWriter.class);
    Capture<DataCopyRequest> capturedCopyRequest = Capture.newInstance();
    expect(storage.getOptions()).andReturn(mockOptions);
  ...
  public void testCopyTo() throws Exception {
    initializeExpectedBlob(2);
    BlobAttributes target = BlobAttributes.newBuilder(BlobIdentifier.create("bt", "nt")).buildObject();
    BlobCopyWriter copyWriter = createMock(BlobCopyWriter.class);
    Capture<DataCopyRequest> capturedCopyRequest = Capture.newInstance();
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.copy(capture(capturedCopyRequest))).andReturn(copyWriter);
  ...
  public void testCopyToBlobId() throws Exception {
    ...
    BlobCopyWriter copyWriter = createMock(BlobCopyWriter.class);
    Capture<StorageClient.DataCopyRequest> capturedCopyRequest = Capture.newInstance();
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.copy(capture(capturedCopyRequest))).andReturn(copyWriter);
    replay(storage);
    initializeBlob();
  ...
  public void testReaderWithDecryptionKey() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    assertSame(channel, blob.openReader(StorageObject.BlobSourceOptions.withDecryptionKey(BASE64_KEY)));
    assertSame(channel, blob.openReader(StorageObject.BlobSourceOptions.withDecryptionKey(KEY)));
  }
  ...
  public void testWriterWithEncryptionKey() throws Exception {
    ...
    BlobWriteChannel channel = createMock(BlobWriteChannel.class);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.writer(eq(expectedBlob), eq(StorageClient.BlobWriteOptions.customerSuppliedKey(BASE64_KEY))))
        .andReturn(channel)
        .times(2);
    replay(storage);
    initializeBlob();
    assertSame(channel, blob.openWriter(BlobWriteOptions.customerSuppliedKey(BASE64_KEY)));
    assertSame(channel, blob.openWriter(StorageClient.BlobWriteOptions.customerSuppliedKey(KEY)));
  }
  ...
  public void testWriterWithKmsKeyName() throws Exception {
    ...
    BlobWriteChannel channel = createMock(BlobWriteChannel.class);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.writer(eq(expectedBlob), eq(StorageClient.BlobWriteOptions.kmsKey(KMS_KEY_NAME))))
        .andReturn(channel);
    replay(storage);
    initializeBlob();
    assertSame(channel, blob.openWriter(StorageClient.BlobWriteOptions.kmsKey(KMS_KEY_NAME)));
  }
  ...
  public void testDeleteAcl() throws Exception {
    ...
    replay(storage);
    initializeBlob();
    assertTrue(blob.removeAcl(UserPrincipal.allAuthenticatedUsers()));
  }
  ...
  public void testToBuilder() {
    ...
    replay(storage);
    StorageObject fullBlob = new StorageObject(storage, new BlobAttributes.BlobInfoBuilderImpl(FULL_BLOB_INFO));
    assertEquals(fullBlob, fullBlob.asBuilder().buildObject());
    StorageObject simpleBlob = new StorageObject(storage, new BlobAttributes.BlobInfoBuilderImpl(BLOB_INFO));
    assertEquals(simpleBlob, simpleBlob.asBuilder().buildObject());
    StorageObject directory = new StorageObject(storage, new BlobAttributes.BlobInfoBuilderImpl(DIRECTORY_INFO));
    assertEquals(directory, directory.asBuilder().buildObject());
  }
  ...
  public void testBuilder() {
    ...
    expect(storage.getOptions()).andReturn(mockOptions).times(6);
    replay(storage);
    StorageObject.BlobInfoBuilder builder = new StorageObject.BlobInfoBuilder(new StorageObject(storage, new BlobAttributes.BlobInfoBuilderImpl(BLOB_INFO)));
    StorageObject blob =
        builder
            .setAcl(ACLS)
            .setComponentCount(COMPONENT_COUNT)
    ...
    builder = new StorageObject.BlobInfoBuilder(new StorageObject(storage, new BlobAttributes.BlobInfoBuilderImpl(DIRECTORY_INFO)));
  ...
  private CloudStorageRpcClient prepareForDownload() {
    CloudStorageRpcClient mockStorageRpc = createNiceMock(CloudStorageRpcClient.class);
    expect(storage.getOptions()).andReturn(mockOptions);
    replay(storage);
  ...
  public void testDownloadTo() throws Exception {
    ...
    CloudStorageRpcClient mockStorageRpc = prepareForDownload();
    expect(
            mockStorageRpc.read(
