...
public class BlobTest {
  ...
  public void testExists_True() throws Exception {
    ...
    StorageClient.BlobGetOptions[] expectedOptions = {StorageClient.BlobGetOptions.setFields()};
  ...
  public void testContentWithDecryptionKey() throws Exception {
    ...
    expect(
            storage.readAllBytes(
                BLOB_INFO.getBlobId(), StorageClient.BlobSourceOptions.withDecryptionKey(BASE64_KEY)))
        .andReturn(content)
        .times(2);
    replay(storage);
    initializeBlob();
  ...
  public void testCopyToBucket() throws Exception {
    ...
    Capture<StorageClient.CopyJobRequest> capturedCopyRequest = Capture.newInstance();
    expect(storage.getOptions()).andReturn(mockOptions);
  ...
  public void testWriterWithEncryptionKey() throws Exception {
    ...
    BlobWriteChannel channel = createMock(BlobWriteChannel.class);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.writer(eq(expectedBlob), eq(WriteBlobOption.customerSuppliedKey(BASE64_KEY))))
        .andReturn(channel)
        .times(2);
    replay(storage);
    initializeBlob();
    assertSame(channel, blob.writer(WriteBlobOption.customerSuppliedKey(BASE64_KEY)));
    assertSame(channel, blob.writer(WriteBlobOption.customerSuppliedKey(KEY)));
  }
  ...
  public void testWriterWithKmsKeyName() throws Exception {
    ...
    BlobWriteChannel channel = createMock(BlobWriteChannel.class);
    expect(storage.getOptions()).andReturn(mockOptions);
    expect(storage.writer(eq(expectedBlob), eq(WriteBlobOption.withKmsKeyName(KMS_KEY_NAME))))
        .andReturn(channel);
    replay(storage);
    initializeBlob();
    assertSame(channel, blob.writer(WriteBlobOption.withKmsKeyName(KMS_KEY_NAME)));
  }
