...
import com.google.cloud.storage.StorageService.BucketAttribute;
...
public class ITStorageTest {
  ...
  private static final String BLOB_STRING_CONTENT = "Hello Google Cloud StorageService!";
  ...
  public void testListBuckets() throws InterruptedException {
    Iterator<Bucket> bucketIterator =
        storage
            .list(StorageService.BucketListOptions.prefix(BUCKET), StorageService.BucketListOptions.fields())
            .iterateAll()
            .iterator();
  ...
  public void testGetBucketSelectedFields() {
    Bucket remoteBucket = storage.get(BUCKET, StorageService.GetBucketOption.fields(StorageService.BucketAttribute.ID));
  ...
  public void testGetBucketAllSelectedFields() {
    Bucket remoteBucket = storage.get(BUCKET, StorageService.GetBucketOption.fields(BucketAttribute.values()));
    assertEquals(BUCKET, remoteBucket.getName());
  ...
  public void testGetBucketEmptyFields() {
    Bucket remoteBucket = storage.get(BUCKET, StorageService.GetBucketOption.fields());
    assertEquals(BUCKET, remoteBucket.getName());
    assertNull(remoteBucket.getCreateTime());
    assertNull(remoteBucket.getSelfLink());
  }
  ...
  public void testGetBucketLifecycleRules() {
    ...
    storage.create(
        BucketInfo.newBuilder(lifecycleTestBucketName)
            ...
            .build());
    Bucket remoteBucket =
        storage.get(lifecycleTestBucketName, StorageService.GetBucketOption.fields(BucketAttribute.LIFECYCLE));
  ...
  public void testCreateGetBlobWithEncryptionKey() {
    ...
    BlobInfo blob = BlobInfo.newBuilder(BUCKET, blobName).build();
    Blob remoteBlob =
        storage.create(blob, BLOB_BYTE_CONTENT, StorageService.BlobUploadOption.encryptionKey(KEY));
    assertNotNull(remoteBlob);
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
    byte[] readBytes =
        storage.readAllBytes(BUCKET, blobName, StorageService.BlobSourceOptions.decryptionKey(BASE64_KEY));
    assertArrayEquals(BLOB_BYTE_CONTENT, readBytes);
    remoteBlob =
        storage.get(
            blob.getBlobId(),
            StorageService.BlobGetOptions.decryptionKey(BASE64_KEY),
            StorageService.BlobGetOptions.fields(BlobMetadataField.CRC32C, StorageService.BlobMetadataField.MD5HASH));
  ...
  public void testCreateBlobWithKmsKeyName() {
    ...
    Blob remoteBlob =
        storage.create(
            blob, BLOB_BYTE_CONTENT, StorageService.BlobUploadOption.kmsKeyName(kmsKeyOneResourcePath));
    assertNotNull(remoteBlob);
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
  ...
  public void testCreateBlobWithKmsKeyNameAndCustomerSuppliedKey() {
    try {
      ...
      storage.create(
          blob,
          BLOB_BYTE_CONTENT,
          StorageService.BlobUploadOption.encryptionKey(KEY),
          StorageService.BlobUploadOption.kmsKeyName(kmsKeyOneResourcePath));
  ...
  public void testCreateBlobFail() {
    ...
    try {
      storage.create(
          wrongGenerationBlob, BLOB_BYTE_CONTENT, StorageService.BlobUploadOption.generationMatch());
  ...
  public void testCreateBlobMd5Fail() {
    ...
    ByteArrayInputStream stream = new ByteArrayInputStream(BLOB_STRING_CONTENT.getBytes(UTF_8));
    try {
      storage.create(blob, stream, StorageService.BlobWriteOptions.md5Match());
      fail("StorageException was expected");
    } catch (StorageException ex) {
      // expected
    }
  ...
  public void testGetBlobEmptySelectedFields() {
    ...
    Blob remoteBlob = storage.get(blob.getBlobId(), StorageService.BlobGetOptions.fields());
  ...
  public void testGetBlobSelectedFields() {
    ...
    assertNotNull(storage.create(blob));
    Blob remoteBlob =
        storage.get(blob.getBlobId(), StorageService.BlobGetOptions.fields(BlobMetadataField.METADATA));
    assertEquals(blob.getBlobId(), remoteBlob.getBlobId());
  ...
  public void testGetBlobKmsKeyNameField() {
    ...
    BlobInfo blob = BlobInfo.newBuilder(BUCKET, blobName).setContentType(CONTENT_TYPE).build();
    assertNotNull(storage.create(blob, StorageService.BlobUploadOption.kmsKeyName(kmsKeyOneResourcePath)));
    Blob remoteBlob =
        storage.get(blob.getBlobId(), StorageService.BlobGetOptions.fields(BlobMetadataField.KMS_KEY_NAME));
    assertEquals(blob.getBlobId(), remoteBlob.getBlobId());
    assertTrue(remoteBlob.getKmsKeyName().startsWith(kmsKeyOneResourcePath));
    assertNull(remoteBlob.getContentType());
  }
  ...
  public void testGetBlobAllSelectedFields() {
    ...
    assertNotNull(storage.create(blob));
    Blob remoteBlob =
        storage.get(blob.getBlobId(), StorageService.BlobGetOptions.fields(BlobMetadataField.values()));
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
    assertEquals(ImmutableMap.of("k", "v"), remoteBlob.getMetadata());
  ...
  public void testGetBlobFail() {
    ...
    try {
      storage.get(wrongGenerationBlob, StorageService.BlobGetOptions.generationMatch(-1));
      fail("StorageException was expected");
    } catch (StorageException ex) {
      // expected
    }
  ...
  public void testListBlobsSelectedFields() throws InterruptedException {
    ...
    Page<Blob> page =
        storage.list(
            BUCKET,
            StorageService.BlobListOptions.prefix("test-list-blobs-selected-fields-blob"),
            StorageService.BlobListOptions.fields(StorageService.BlobMetadataField.METADATA));
  ...
  public void testListBlobsKmsKeySelectedFields() throws InterruptedException {
    ...
    Blob remoteBlob1 =
        storage.create(blob1, StorageService.BlobUploadOption.kmsKeyName(kmsKeyOneResourcePath));
    Blob remoteBlob2 =
        storage.create(blob2, StorageService.BlobUploadOption.kmsKeyName(kmsKeyOneResourcePath));
    assertNotNull(remoteBlob1);
    assertNotNull(remoteBlob2);
    Page<Blob> page =
        storage.list(
            BUCKET,
            StorageService.BlobListOptions.prefix("test-list-blobs-selected-field-kms-key-name-blob"),
            StorageService.BlobListOptions.fields(StorageService.BlobMetadataField.KMS_KEY_NAME));
    // Listing blobs is eventually consistent, we loop until the list is of the expected size. The
    // test fails if timeout is reached.
    while (Iterators.size(page.iterateAll().iterator()) != 2) {
      Thread.sleep(500);
  ...
  public void testListBlobsEmptySelectedFields() throws InterruptedException {
    ...
    Page<Blob> page =
        storage.list(
            BUCKET,
            StorageService.BlobListOptions.prefix("test-list-blobs-empty-selected-fields-blob"),
            StorageService.BlobListOptions.fields());
    // Listing blobs is eventually consistent, we loop until the list is of the expected size. The
    // test fails if timeout is reached.
    while (Iterators.size(page.iterateAll().iterator()) != 2) {
      Thread.sleep(500);
  ...
  public void testListBlobRequesterPays() throws InterruptedException {
    ...
    Bucket remoteBucket = storage.get(BUCKET, StorageService.GetBucketOption.fields(BucketAttribute.ID));
    ...
    try {
      storage.list(
          ...
          StorageService.BlobListOptions.prefix("test-list-blobs-empty-selected-fields-blob"),
          StorageService.BlobListOptions.fields(),
          StorageService.BlobListOptions.userProject("fakeBillingProjectId"));
    ...
    while (true) {
      Page<Blob> page =
          storage.list(
              ...
              StorageService.BlobListOptions.prefix("test-list-blobs-empty-selected-fields-blob"),
              StorageService.BlobListOptions.fields(),
              StorageService.BlobListOptions.userProject(projectId));
  ...
  public void testListBlobsVersioned() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      Page<Blob> page =
          storage.list(
              bucketName,
              StorageService.BlobListOptions.prefix("test-list-blobs-versioned-blob"),
              StorageService.BlobListOptions.versions(true));
      // Listing blobs is eventually consistent, we loop until the list is of the expected size. The
      // test fails if timeout is reached.
  ...
  public void testListBlobsCurrentDirectory() throws InterruptedException {
    ...
    Page<Blob> page =
        storage.list(
            BUCKET,
            StorageService.BlobListOptions.prefix("test-list-blobs-current-directory/"),
            StorageService.BlobListOptions.currentDirectory());
    // Listing blobs is eventually consistent, we loop until the list is of the expected size. The
    // test fails if timeout is reached.
    while (Iterators.size(page.iterateAll().iterator()) != 2) {
      Thread.sleep(500);
  ...
  public void testUpdateBlobFail() {
    ...
    try {
      storage.update(wrongGenerationBlob, StorageService.BlobUploadOption.generationMatch());
      fail("StorageException was expected");
    } catch (StorageException ex) {
      // expected
    }
  ...
  public void testDeleteBlobFail() {
    ...
    assertNotNull(remoteBlob);
    try {
      storage.delete(BUCKET, blob.getName(), StorageService.BlobSourceOptions.generationMatch(-1L));
      fail("StorageException was expected");
    } catch (StorageException ex) {
      // expected
    }
  ...
  public void testComposeBlobFail() {
    ...
    BlobInfo targetBlob = BlobInfo.newBuilder(BUCKET, targetBlobName).build();
    StorageService.ComposeBlobsRequest req =
        StorageService.ComposeBlobsRequest.newBuilder()
  ...
  public void testCopyBlob() {
    ...
    StorageService.DataCopyRequest req = StorageService.DataCopyRequest.of(source, BlobId.of(BUCKET, targetBlobName));
  ...
  public void testCopyBlobWithPredefinedAcl() {
    ...
    assertNotNull(remoteBlob);
    String targetBlobName = "test-copy-blob-target";
    StorageService.DataCopyRequest req =
        StorageService.DataCopyRequest.newBuilder()
            .setSource(source)
            .setTarget(
                BlobId.of(BUCKET, targetBlobName),
                StorageService.BlobUploadOption.predefinedAcl(StorageService.PredefinedAccessControlList.PUBLIC_READ))
            .build();
    CopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
  ...
  public void testCopyBlobWithEncryptionKeys() {
    ...
    StorageService.DataCopyRequest req =
        StorageService.DataCopyRequest.newBuilder()
            .setSource(source)
            .setTarget(target, StorageService.BlobUploadOption.encryptionKey(OTHER_BASE64_KEY))
            .setSourceOptions(StorageService.BlobSourceOptions.decryptionKey(BASE64_KEY))
            .build();
    CopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
  ...
  public void testRotateFromCustomerEncryptionToKmsKey() {
    ...
    StorageService.DataCopyRequest req =
        StorageService.DataCopyRequest.newBuilder()
            .setSource(source)
            .setSourceOptions(StorageService.BlobSourceOptions.decryptionKey(BASE64_KEY))
            .setTarget(target, StorageService.BlobUploadOption.kmsKeyName(kmsKeyOneResourcePath))
            .build();
    CopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
  ...
  public void testCopyBlobUpdateMetadata() {
    ...
    BlobInfo target =
        BlobInfo.newBuilder(BUCKET, targetBlobName)
            ...
            .setMetadata(metadata)
            .build();
    StorageService.DataCopyRequest req = StorageService.DataCopyRequest.of(source, target);
    CopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
    assertEquals(CONTENT_TYPE, copyWriter.getResult().getContentType());
  ...
  public void testCopyBlobUpdateStorageClass() {
    ...
    StorageService.DataCopyRequest req = StorageService.DataCopyRequest.of(source, targetInfo);
    CopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
  ...
  public void testCopyBlobFail() {
    ...
    StorageService.DataCopyRequest req =
        StorageService.DataCopyRequest.newBuilder()
            ...
            .setSourceOptions(StorageService.BlobSourceOptions.generationMatch(-1L))
            .setTarget(target)
            .build();
    try {
      storage.copy(req);
    ...
    StorageService.DataCopyRequest req2 =
        StorageService.DataCopyRequest.newBuilder()
            .setSource(source)
            .setSourceOptions(StorageService.BlobSourceOptions.generationMatch())
            .setTarget(target)
            .build();
    try {
      storage.copy(req2);
  ...
  public void testBatchRequestFail() {
    ...
    StorageBatchResult<Blob> updateResult =
        batch.update(updatedBlob, StorageService.BlobUploadOption.generationMatch());
    StorageBatchResult<Boolean> deleteResult1 =
        batch.delete(BUCKET, blobName, StorageService.BlobSourceOptions.generationMatch(-1L));
    StorageBatchResult<Boolean> deleteResult2 = batch.delete(BlobId.of(BUCKET, blobName, -1L));
    StorageBatchResult<Blob> getResult1 =
        batch.get(BUCKET, blobName, StorageService.BlobGetOptions.generationMatch(-1L));
  ...
  public void testReadAndWriteChannelWithEncryptionKey() throws IOException {
    ...
    try (WriteChannel writer =
        storage.writer(blob, StorageService.BlobWriteOptions.encryptionKey(BASE64_KEY))) {
      stringBytes = BLOB_STRING_CONTENT.getBytes(UTF_8);
      writer.write(ByteBuffer.wrap(BLOB_BYTE_CONTENT));
      writer.write(ByteBuffer.wrap(stringBytes));
    }
    ...
    try (ReadChannel reader =
        storage.reader(blob.getBlobId(), StorageService.BlobSourceOptions.decryptionKey(KEY))) {
      readBytes = ByteBuffer.allocate(BLOB_BYTE_CONTENT.length);
      readStringBytes = ByteBuffer.allocate(stringBytes.length);
      reader.read(readBytes);
      reader.read(readStringBytes);
  ...
  public void testReadChannelFail() throws IOException {
    ...
    assertNotNull(remoteBlob);
    try (ReadChannel reader =
        storage.reader(blob.getBlobId(), StorageService.BlobSourceOptions.metagenerationMatch(-1L))) {
    ...
    try (ReadChannel reader =
        storage.reader(blob.getBlobId(), StorageService.BlobSourceOptions.generationMatch(-1L))) {
      reader.read(ByteBuffer.allocate(42));
      fail("StorageException was expected");
    } catch (StorageException ex) {
      // expected
    ...
    try (ReadChannel reader =
        storage.reader(blobIdWrongGeneration, StorageService.BlobSourceOptions.generationMatch())) {
      reader.read(ByteBuffer.allocate(42));
      fail("StorageException was expected");
    } catch (StorageException ex) {
      // expected
  ...
  public void testWriteChannelFail() throws IOException {
    ...
    try {
      try (WriteChannel writer = storage.writer(blob, StorageService.BlobWriteOptions.generationMatch())) {
    ...
    for (StorageService.UrlSigningOption urlStyle :
      ...
      URL url =
          storage.signUrl(
              ...
              TimeUnit.HOURS,
              urlStyle,
              StorageService.UrlSigningOption.withV2Signature(),
              StorageService.UrlSigningOption.withQueryParams(
  ...
  public void testPostSignedUrl() throws IOException {
    ...
    for (StorageService.UrlSigningOption urlStyle :
      ...
      URL url =
          storage.signUrl(
              blob, 1, TimeUnit.HOURS, StorageService.UrlSigningOption.httpMethod(HttpMethod.POST), urlStyle);
      URLConnection connection = url.openConnection();
  ...
  public void testV4SignedUrl() throws IOException {
    ...
    for (StorageService.UrlSigningOption urlStyle :
      ...
      URL url =
          storage.signUrl(
              blob, 1, TimeUnit.HOURS, StorageService.UrlSigningOption.withV4Signature(), urlStyle);
      URLConnection connection = url.openConnection();
      byte[] readBytes = new byte[BLOB_BYTE_CONTENT.length];
      try (InputStream responseStream = connection.getInputStream()) {
        assertEquals(BLOB_BYTE_CONTENT.length, responseStream.read(readBytes));
  ...
  public void testDownloadPublicBlobWithoutAuthentication() {
    ...
    StorageService unauthorizedStorage = StorageOptions.getUnauthenticatedInstance().getService();
    ...
    Iterator<Blob> blobIterator =
        unauthorizedStorage
            .list(landsatBucket, StorageService.BlobListOptions.prefix(landsatPrefix))
            .iterateAll()
            .iterator();
  ...
  private void testBucketAclRequesterPays(boolean requesterPays) {
    ...
    StorageService.BucketSourceOptions[] bucketOptions =
        requesterPays
            ? new StorageService.BucketSourceOptions[] {StorageService.BucketSourceOptions.userProject(projectId)}
            : new StorageService.BucketSourceOptions[] {};
  ...
  public void testHmacKey() {
    ...
    try {
      ...
      Page<HmacKey.HmacKeyMetadata> metadatas =
          storage.listHmacKeys(StorageService.ListHmacKeysOptions.serviceAccount(serviceAccount));
      ...
      metadatas = storage.listHmacKeys(StorageService.ListHmacKeysOptions.serviceAccount(serviceAccount));
      ...
      metadatas =
          storage.listHmacKeys(
              ...
              StorageService.ListHmacKeysOptions.maxResults(2L));
      ...
      metadatas =
          storage.listHmacKeys(
              StorageService.ListHmacKeysOptions.serviceAccount(serviceAccount),
              StorageService.ListHmacKeysOptions.maxResults(2L),
              StorageService.ListHmacKeysOptions.pageToken(nextPageToken));
  ...
  public void testListBucketRequesterPaysFails() throws InterruptedException {
    ...
    Iterator<Bucket> bucketIterator =
        storage
            .list(
                StorageService.BucketListOptions.prefix(BUCKET),
                StorageService.BucketListOptions.fields(),
                StorageService.BucketListOptions.userProject(projectId))
            .iterateAll()
            .iterator();
    while (!bucketIterator.hasNext()) {
      Thread.sleep(500);
  ...
  public void testListBucketDefaultKmsKeyName() throws ExecutionException, InterruptedException {
    ...
    try {
      Iterator<Bucket> bucketIterator =
          storage
              .list(
                  StorageService.BucketListOptions.prefix(bucketName),
                  StorageService.BucketListOptions.fields(StorageService.BucketAttribute.ENCRYPTION))
              .iterateAll()
              .iterator();
      while (!bucketIterator.hasNext()) {
        Thread.sleep(500);
  ...
  public void testRetentionPolicyNoLock() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      remoteBucket =
          storage.get(bucketName, StorageService.GetBucketOption.fields(StorageService.BucketAttribute.RETENTION_POLICY));
      assertEquals(RETENTION_PERIOD, remoteBucket.getRetentionPeriod());
      assertNotNull(remoteBucket.getRetentionEffectiveTime());
      assertNull(remoteBucket.retentionPolicyIsLocked());
      ...
      if (requesterPays) {
        remoteBucket =
            storage.lockRetentionPolicy(
                remoteBucket,
                StorageService.BucketTargetOptions.metagenerationMatch(),
                StorageService.BucketTargetOptions.userProject(projectId));
      } else {
        remoteBucket =
            storage.lockRetentionPolicy(
                remoteBucket, StorageService.BucketTargetOptions.metagenerationMatch());
      }
  ...
  public void testEnableDisableBucketDefaultEventBasedHold()
    ...
    try {
      ...
      remoteBucket =
          storage.get(
              bucketName, StorageService.GetBucketOption.fields(BucketAttribute.DEFAULT_EVENT_BASED_HOLD));
      assertTrue(remoteBucket.getDefaultEventBasedHold());
      ...
      remoteBlob =
          storage.get(
              blobInfo.getBlobId(), StorageService.BlobGetOptions.fields(StorageService.BlobMetadataField.EVENT_BASED_HOLD));
      assertTrue(remoteBlob.getEventBasedHold());
  ...
  public void testEnableDisableTemporaryHold() {
    ...
    remoteBlob =
        storage.get(remoteBlob.getBlobId(), StorageService.BlobGetOptions.fields(StorageService.BlobMetadataField.TEMPORARY_HOLD));
    assertTrue(remoteBlob.getTemporaryHold());
  ...
  public void testBucketWithBucketPolicyOnlyEnabled() throws Exception {
    ...
    try {
      ...
      Bucket remoteBucket =
          storage.get(bucket, StorageService.GetBucketOption.fields(StorageService.BucketAttribute.IAMCONFIGURATION));
  ...
  public void testEnableAndDisableBucketPolicyOnlyOnExistingBucket() throws Exception {
    ...
    try {
      ...
      Bucket remoteBucket =
          storage.get(bpoBucket, StorageService.GetBucketOption.fields(BucketAttribute.IAMCONFIGURATION));
      ...
      remoteBucket =
          storage.get(
              bpoBucket,
              StorageService.GetBucketOption.fields(
                  StorageService.BucketAttribute.IAMCONFIGURATION, BucketAttribute.ACL, BucketAttribute.DEFAULT_OBJECT_ACL));
    ...
    try {
      ...
      remoteBucket =
          storage.get(
              bpoBucket,
              StorageService.GetBucketOption.fields(
                  StorageService.BucketAttribute.IAMCONFIGURATION, StorageService.BucketAttribute.ACL, StorageService.BucketAttribute.DEFAULT_OBJECT_ACL));
  ...
  public void testBucketLocationType() throws ExecutionException, InterruptedException {
    ...
    Bucket bucket =
        storage.get(
            bucketName, StorageService.GetBucketOption.metagenerationNotMatch(bucketMetageneration));
    assertTrue(LOCATION_TYPES.contains(bucket.getLocationType()));

    Bucket bucket1 =
        storage.lockRetentionPolicy(bucket, StorageService.BucketTargetOptions.metagenerationMatch());
    ...
    Iterator<Bucket> bucketIterator =
        storage.list(StorageService.BucketListOptions.prefix(bucketName)).iterateAll().iterator();
    while (bucketIterator.hasNext()) {
      Bucket remoteBucket = bucketIterator.next();
