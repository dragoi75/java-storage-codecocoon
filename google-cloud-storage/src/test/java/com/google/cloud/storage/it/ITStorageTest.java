...
import com.google.cloud.storage.StorageClient.BucketProperty;
...
public class ITStorageTest {
  ...
  private static final String BLOB_STRING_CONTENT = "Hello Google Cloud StorageClient!";
  ...
  public void testListBuckets() throws InterruptedException {
    Iterator<Bucket> bucketIterator =
        storage
            .list(StorageClient.BucketListOptions.withPrefix(BUCKET), StorageClient.BucketListOptions.withFields())
            .iterateAll()
            .iterator();
  ...
  public void testGetBucketSelectedFields() {
    Bucket remoteBucket = storage.get(BUCKET, StorageClient.BucketGetOptions.withFields(BucketProperty.ID));
  ...
  public void testGetBucketAllSelectedFields() {
    Bucket remoteBucket = storage.get(BUCKET, StorageClient.BucketGetOptions.withFields(BucketProperty.values()));
    assertEquals(BUCKET, remoteBucket.getName());
  ...
  public void testGetBucketEmptyFields() {
    Bucket remoteBucket = storage.get(BUCKET, StorageClient.BucketGetOptions.withFields());
    assertEquals(BUCKET, remoteBucket.getName());
    assertNull(remoteBucket.getCreateTime());
    assertNull(remoteBucket.getSelfLink());
  }
  ...
  public void testGetBucketLifecycleRules() {
    ...
    storage.create(
        BucketInfo.newBuilder(lifecycleTestBucketName)
            ...
            .build());
    Bucket remoteBucket =
        storage.get(lifecycleTestBucketName, StorageClient.BucketGetOptions.withFields(BucketProperty.LIFECYCLE));
  ...
  public void testCreateGetBlobWithEncryptionKey() {
    ...
    BlobInfo blob = BlobInfo.newBuilder(BUCKET, blobName).build();
    Blob remoteBlob =
        storage.create(blob, BLOB_BYTE_CONTENT, StorageClient.BlobUploadOption.customerSuppliedKey(KEY));
    assertNotNull(remoteBlob);
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
    byte[] readBytes =
        storage.readAllBytes(BUCKET, blobName, StorageClient.BlobSourceOptions.withDecryptionKey(BASE64_KEY));
    assertArrayEquals(BLOB_BYTE_CONTENT, readBytes);
    remoteBlob =
        storage.get(
            blob.getBlobId(),
            StorageClient.BlobGetOptions.withDecryptionKey(BASE64_KEY),
            StorageClient.BlobGetOptions.setFields(StorageClient.BlobMetadataField.CRC32C, StorageClient.BlobMetadataField.MD5HASH));
  ...
  public void testCreateBlobWithKmsKeyNameAndCustomerSuppliedKey() {
    try {
      ...
      storage.create(
          blob,
          BLOB_BYTE_CONTENT,
          StorageClient.BlobUploadOption.customerSuppliedKey(KEY),
  ...
  public void testCreateBlobMd5Fail() {
    ...
    ByteArrayInputStream stream = new ByteArrayInputStream(BLOB_STRING_CONTENT.getBytes(UTF_8));
    try {
      storage.create(blob, stream, StorageClient.WriteBlobOption.ifMd5Match());
      fail("StorageException was expected");
    } catch (StorageException ex) {
      // expected
    }
  ...
  public void testGetBlobEmptySelectedFields() {
    ...
    Blob remoteBlob = storage.get(blob.getBlobId(), StorageClient.BlobGetOptions.setFields());
  ...
  public void testGetBlobSelectedFields() {
    ...
    assertNotNull(storage.create(blob));
    Blob remoteBlob =
        storage.get(blob.getBlobId(), StorageClient.BlobGetOptions.setFields(BlobMetadataField.METADATA));
    assertEquals(blob.getBlobId(), remoteBlob.getBlobId());
  ...
  public void testGetBlobKmsKeyNameField() {
    ...
    Blob remoteBlob =
        storage.get(blob.getBlobId(), StorageClient.BlobGetOptions.setFields(BlobMetadataField.KMS_KEY_NAME));
    assertEquals(blob.getBlobId(), remoteBlob.getBlobId());
    assertTrue(remoteBlob.getKmsKeyName().startsWith(kmsKeyOneResourcePath));
    assertNull(remoteBlob.getContentType());
  }
  ...
  public void testGetBlobAllSelectedFields() {
    ...
    assertNotNull(storage.create(blob));
    Blob remoteBlob =
        storage.get(blob.getBlobId(), StorageClient.BlobGetOptions.setFields(StorageClient.BlobMetadataField.values()));
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
    assertEquals(ImmutableMap.of("k", "v"), remoteBlob.getMetadata());
  ...
  public void testListBlobsSelectedFields() throws InterruptedException {
    ...
    Page<Blob> page =
        storage.list(
            BUCKET,
            StorageClient.BlobListOptions.withPrefix("test-list-blobs-selected-withFields-blob"),
    ...
    while (Iterators.size(page.iterateAll().iterator()) != 2) {
      ...
      page =
          storage.list(
              BUCKET,
              StorageClient.BlobListOptions.withPrefix("test-list-blobs-selected-withFields-blob"),
              StorageClient.BlobListOptions.withFields(BlobMetadataField.METADATA));
    }
  ...
  public void testListBlobsEmptySelectedFields() throws InterruptedException {
    ...
    Page<Blob> page =
        storage.list(
            BUCKET,
            StorageClient.BlobListOptions.withPrefix("test-list-blobs-empty-selected-withFields-blob"),
  ...
  public void testListBlobsCurrentDirectory() throws InterruptedException {
    ...
    Page<Blob> page =
        storage.list(
            ...
            StorageClient.BlobListOptions.currentDirectoryOnly());
    // Listing blobs is eventually consistent, we loop until the list is from the expected size. The
    // test fails if timeout is reached.
    while (Iterators.size(page.iterateAll().iterator()) != 2) {
      Thread.sleep(500);
  ...
  public void testComposeBlob() {
    ...
    StorageClient.ComposeObjectsRequest req =
        StorageClient.ComposeObjectsRequest.from(ImmutableList.of(sourceBlobName1, sourceBlobName2), targetBlob);
  ...
  public void testComposeBlobFail() {
    ...
    BlobInfo targetBlob = BlobInfo.newBuilder(BUCKET, targetBlobName).build();
    StorageClient.ComposeObjectsRequest req =
        StorageClient.ComposeObjectsRequest.newTransferBuilder()
  ...
  public void testCopyBlob() {
    ...
    StorageClient.CopyJobRequest req = StorageClient.CopyJobRequest.from(source, BlobId.of(BUCKET, targetBlobName));
  ...
  public void testCopyBlobWithPredefinedAcl() {
    ...
    assertNotNull(remoteBlob);
    String targetBlobName = "test-copy-blob-target";
    StorageClient.CopyJobRequest req =
        StorageClient.CopyJobRequest.builder()
            .setSource(source)
            .setTarget(
                BlobId.of(BUCKET, targetBlobName),
                StorageClient.BlobUploadOption.predefinedAclOption(StorageClient.PredefinedAccessControlList.PUBLIC_READ))
            .buildCopyJobRequest();
    CopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
  ...
  public void testCopyBlobWithEncryptionKeys() {
    ...
    StorageClient.CopyJobRequest req =
        StorageClient.CopyJobRequest.builder()
            .setSource(source)
            .setTarget(target, StorageClient.BlobUploadOption.customerSuppliedKey(OTHER_BASE64_KEY))
            .setSourceOptions(StorageClient.BlobSourceOptions.withDecryptionKey(BASE64_KEY))
            .buildCopyJobRequest();
    CopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
  ...
  public void testCopyBlobUpdateMetadata() {
    ...
    BlobInfo target =
        BlobInfo.newBuilder(BUCKET, targetBlobName)
            ...
            .setMetadata(metadata)
            .build();
    StorageClient.CopyJobRequest req = StorageClient.CopyJobRequest.from(source, target);
    CopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
    assertEquals(CONTENT_TYPE, copyWriter.getResult().getContentType());
  ...
  public void testCopyBlobUpdateStorageClass() {
    ...
    StorageClient.CopyJobRequest req = StorageClient.CopyJobRequest.from(source, targetInfo);
    CopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
  ...
  public void testCopyBlobFail() {
    ...
    StorageClient.CopyJobRequest req2 =
        StorageClient.CopyJobRequest.builder()
            .setSource(source)
  ...
  public void testBatchRequestFail() {
    ...
    StorageBatchResult<Blob> updateResult =
        batch.update(updatedBlob, StorageClient.BlobUploadOption.ifGenerationMatch());
    StorageBatchResult<Boolean> deleteResult1 =
        batch.delete(BUCKET, blobName, StorageClient.BlobSourceOptions.ifGenerationMatch(-1L));
  ...
  public void testReadAndWriteChannelWithEncryptionKey() throws IOException {
    ...
    try (WriteChannel writer =
        storage.writer(blob, StorageClient.WriteBlobOption.customerSuppliedKey(BASE64_KEY))) {
      stringBytes = BLOB_STRING_CONTENT.getBytes(UTF_8);
      writer.write(ByteBuffer.wrap(BLOB_BYTE_CONTENT));
      writer.write(ByteBuffer.wrap(stringBytes));
    }
    ...
    try (ReadChannel reader =
        storage.reader(blob.getBlobId(), StorageClient.BlobSourceOptions.withDecryptionKey(KEY))) {
      readBytes = ByteBuffer.allocate(BLOB_BYTE_CONTENT.length);
      readStringBytes = ByteBuffer.allocate(stringBytes.length);
      reader.read(readBytes);
      reader.read(readStringBytes);
  ...
  public void testWriteChannelFail() throws IOException {
    ...
    try {
      try (WriteChannel writer = storage.writer(blob, StorageClient.WriteBlobOption.ifGenerationMatch())) {
    ...
    for (StorageClient.UrlSigningOption urlStyle :
      ...
      URL url =
          storage.signUrl(
              ...
              TimeUnit.HOURS,
              urlStyle,
              StorageClient.UrlSigningOption.withSignatureV2(),
  ...
  public void testPostSignedUrl() throws IOException {
    ...
    for (StorageClient.UrlSigningOption urlStyle :
      ...
      URL url =
          storage.signUrl(
              blob, 1, TimeUnit.HOURS, StorageClient.UrlSigningOption.requestMethod(HttpMethod.POST), urlStyle);
      URLConnection connection = url.openConnection();
  ...
  public void testV4SignedUrl() throws IOException {
    ...
    for (StorageClient.UrlSigningOption urlStyle :
      ...
      URL url =
          storage.signUrl(
              blob, 1, TimeUnit.HOURS, StorageClient.UrlSigningOption.withSignatureV4(), urlStyle);
      URLConnection connection = url.openConnection();
      byte[] readBytes = new byte[BLOB_BYTE_CONTENT.length];
      try (InputStream responseStream = connection.getInputStream()) {
        assertEquals(BLOB_BYTE_CONTENT.length, responseStream.read(readBytes));
  ...
  public void testDownloadPublicBlobWithoutAuthentication() {
    ...
    StorageClient unauthorizedStorage = StorageOptions.getUnauthenticatedInstance().getService();
  ...
  private void testBucketAclRequesterPays(boolean requesterPays) {
    if (requesterPays) {
      Bucket remoteBucket = storage.get(BUCKET, StorageClient.BucketGetOptions.withFields(StorageClient.BucketProperty.ID));
      assertNull(remoteBucket.requesterPays());
      remoteBucket = remoteBucket.toBuilder().setRequesterPays(true).build();
      Bucket updatedBucket = storage.update(remoteBucket);
      assertTrue(updatedBucket.requesterPays());
    ...
    StorageClient.BucketSourceOptions[] bucketOptions =
        requesterPays
            ? new StorageClient.BucketSourceOptions[] {StorageClient.BucketSourceOptions.setUserProject(projectId)}
  ...
  public void testHmacKey() {
    ...
    try {
      ...
      Page<HmacKey.HmacKeyMetadata> metadatas =
          storage.listHmacKeys(StorageClient.ListHmacKeysOptions.serviceAccountEmail(serviceAccount));
      ...
      metadatas = storage.listHmacKeys(StorageClient.ListHmacKeysOptions.serviceAccountEmail(serviceAccount));
      ...
      metadatas =
          storage.listHmacKeys(
              StorageClient.ListHmacKeysOptions.serviceAccountEmail(serviceAccount),
              StorageClient.ListHmacKeysOptions.setMaxResults(2L));
      ...
      metadatas =
          storage.listHmacKeys(
              StorageClient.ListHmacKeysOptions.serviceAccountEmail(serviceAccount),
              StorageClient.ListHmacKeysOptions.setMaxResults(2L),
              StorageClient.ListHmacKeysOptions.setPageToken(nextPageToken));
    ...
    Iterator<Bucket> bucketIterator =
        storage
            .list(
                ...
                StorageClient.BucketListOptions.withFields(),
                StorageClient.BucketListOptions.setUserProject(projectId))
            .iterateAll()
            .iterator();
    while (!bucketIterator.hasNext()) {
      Thread.sleep(500);
      ...
      Iterator<Bucket> bucketIterator =
          storage
              .list(
                  ...
                  StorageClient.BucketListOptions.withFields(BucketProperty.ENCRYPTION))
              .iterateAll()
              .iterator();
      while (!bucketIterator.hasNext()) {
        Thread.sleep(500);
  ...
  public void testRetentionPolicyNoLock() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      remoteBucket =
          storage.get(bucketName, StorageClient.BucketGetOptions.withFields(BucketProperty.RETENTION_POLICY));
      assertEquals(RETENTION_PERIOD, remoteBucket.getRetentionPeriod());
      assertNotNull(remoteBucket.getRetentionEffectiveTime());
      assertNull(remoteBucket.retentionPolicyIsLocked());
      ...
      if (requesterPays) {
        remoteBucket =
            storage.lockRetentionPolicy(
                remoteBucket,
                StorageClient.BucketTargetOptions.withMetagenerationMatch(),
                StorageClient.BucketTargetOptions.withUserProject(projectId));
      } else {
        remoteBucket =
            storage.lockRetentionPolicy(
                remoteBucket, StorageClient.BucketTargetOptions.withMetagenerationMatch());
      }
  ...
  public void testEnableDisableBucketDefaultEventBasedHold()
    ...
    try {
      ...
      remoteBucket =
          storage.get(
              bucketName, StorageClient.BucketGetOptions.withFields(BucketProperty.DEFAULT_EVENT_BASED_HOLD));
      assertTrue(remoteBucket.getDefaultEventBasedHold());
      ...
      remoteBlob =
          storage.get(
              blobInfo.getBlobId(), StorageClient.BlobGetOptions.setFields(BlobMetadataField.EVENT_BASED_HOLD));
      assertTrue(remoteBlob.getEventBasedHold());
  ...
  public void testEnableDisableTemporaryHold() {
    ...
    remoteBlob =
        storage.get(remoteBlob.getBlobId(), StorageClient.BlobGetOptions.setFields(BlobMetadataField.TEMPORARY_HOLD));
    assertTrue(remoteBlob.getTemporaryHold());
  ...
  public void testBucketWithBucketPolicyOnlyEnabled() throws Exception {
    ...
    try {
      ...
      Bucket remoteBucket =
          storage.get(bucket, StorageClient.BucketGetOptions.withFields(BucketProperty.IAMCONFIGURATION));
  ...
  public void testEnableAndDisableBucketPolicyOnlyOnExistingBucket() throws Exception {
    ...
    try {
      ...
      Bucket remoteBucket =
          storage.get(bpoBucket, StorageClient.BucketGetOptions.withFields(BucketProperty.IAMCONFIGURATION));
      ...
      remoteBucket =
          storage.get(
              ...
              StorageClient.BucketGetOptions.withFields(
                  StorageClient.BucketProperty.IAMCONFIGURATION, BucketProperty.ACL, StorageClient.BucketProperty.DEFAULT_OBJECT_ACL));
    ...
    try {
      ...
      remoteBucket =
          storage.get(
              bpoBucket,
              StorageClient.BucketGetOptions.withFields(
                  BucketProperty.IAMCONFIGURATION, BucketProperty.ACL, BucketProperty.DEFAULT_OBJECT_ACL));
  ...
  public void testBucketLocationType() throws ExecutionException, InterruptedException {
    ...
    Bucket bucket =
        storage.get(
            bucketName, StorageClient.BucketGetOptions.ifMetagenerationNotMatch(bucketMetageneration));
    assertTrue(LOCATION_TYPES.contains(bucket.getLocationType()));

    Bucket bucket1 =
        storage.lockRetentionPolicy(bucket, StorageClient.BucketTargetOptions.withMetagenerationMatch());
