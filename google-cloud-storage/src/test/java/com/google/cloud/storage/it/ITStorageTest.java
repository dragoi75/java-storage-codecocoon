...
import com.google.cloud.storage.StorageBatchOperation;
import com.google.cloud.storage.StorageBatchOutcome;
...
public class ITStorageTest {
  ...
  public void testListBuckets() throws InterruptedException {
    Iterator<StorageBucket> bucketIterator =
        storage
            .list(StorageService.BucketListOptions.withPrefix(BUCKET), StorageService.BucketListOptions.fieldsSelector())
            .iterateAll()
            .iterator();
  ...
  public void testGetBucketSelectedFields() {
    StorageBucket remoteBucket = storage.get(BUCKET, StorageService.GetBucketOption.selectFields(StorageService.BucketMetadataField.ID));
  ...
  public void testGetBucketAllSelectedFields() {
    StorageBucket remoteBucket = storage.get(BUCKET, StorageService.GetBucketOption.selectFields(StorageService.BucketMetadataField.values()));
    assertEquals(BUCKET, remoteBucket.getName());
  ...
  public void testGetBucketEmptyFields() {
    StorageBucket remoteBucket = storage.get(BUCKET, StorageService.GetBucketOption.selectFields());
    assertEquals(BUCKET, remoteBucket.getName());
    assertNull(remoteBucket.getCreateTime());
    assertNull(remoteBucket.getSelfLink());
  }
  ...
  public void testGetBucketLifecycleRules() {
    ...
    StorageBucket remoteBucket =
        storage.get(lifecycleTestBucketName, StorageService.GetBucketOption.selectFields(StorageService.BucketMetadataField.LIFECYCLE));
  ...
  public void testClearBucketDefaultKmsKeyName() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      StorageBucket updatedBucket = remoteBucket.toBucketBuilder().setDefaultKmsKeyName(null).buildInstance().modify();
  ...
  public void testUpdateBucketDefaultKmsKeyName() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      StorageBucket updatedBucket =
          remoteBucket.toBucketBuilder().setDefaultKmsKeyName(kmsKeyTwoResourcePath).buildInstance().modify();
  ...
  public void testCreateGetBlobWithEncryptionKey() {
    ...
    StorageObject remoteBlob =
        storage.create(blob, BLOB_BYTE_CONTENT, StorageService.BlobUploadOption.customerSuppliedKey(KEY));
    assertNotNull(remoteBlob);
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
    byte[] readBytes =
        storage.readAllBytes(BUCKET, blobName, StorageService.BlobSourceOptions.customerDecryptionKey(BASE64_KEY));
    assertArrayEquals(BLOB_BYTE_CONTENT, readBytes);
    remoteBlob =
        storage.get(
            blob.getBlobId(),
            StorageService.BlobFetchOption.customerDecryptionKey(BASE64_KEY),
            StorageService.BlobFetchOption.fieldsSelector(StorageService.BlobMetadataField.CRC32C, BlobMetadataField.MD5HASH));
  ...
  public void testCreateBlobWithKmsKeyName() {
    ...
    StorageObject remoteBlob =
        storage.create(
            blob, BLOB_BYTE_CONTENT, StorageService.BlobUploadOption.withKmsKeyName(kmsKeyOneResourcePath));
    assertNotNull(remoteBlob);
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
  ...
  public void testCreateBlobWithKmsKeyNameAndCustomerSuppliedKey() {
    try {
      ...
      storage.create(
          blob,
          BLOB_BYTE_CONTENT,
          StorageService.BlobUploadOption.customerSuppliedKey(KEY),
          StorageService.BlobUploadOption.withKmsKeyName(kmsKeyOneResourcePath));
  ...
  public void testCreateBlobMd5Fail() {
    ...
    ByteArrayInputStream stream = new ByteArrayInputStream(BLOB_STRING_CONTENT.getBytes(UTF_8));
    try {
      storage.create(blob, stream, StorageService.BlobWriteSetting.ifMd5Match());
      fail("StorageException was expected");
    } catch (StorageOperationException ex) {
      // expected
    }
  ...
  public void testGetBlobEmptySelectedFields() {
    ...
    StorageObject remoteBlob = storage.get(blob.getBlobId(), StorageService.BlobFetchOption.fieldsSelector());
  ...
  public void testGetBlobSelectedFields() {
    ...
    assertNotNull(storage.create(blob));
    StorageObject remoteBlob =
        storage.get(blob.getBlobId(), StorageService.BlobFetchOption.fieldsSelector(StorageService.BlobMetadataField.METADATA));
    assertEquals(blob.getBlobId(), remoteBlob.getBlobId());
  ...
  public void testGetBlobKmsKeyNameField() {
    ...
    BlobMetadata blob = BlobMetadata.newBuilder(BUCKET, blobName).setContentType(CONTENT_TYPE).buildObject();
    assertNotNull(storage.create(blob, StorageService.BlobUploadOption.withKmsKeyName(kmsKeyOneResourcePath)));
    StorageObject remoteBlob =
        storage.get(blob.getBlobId(), StorageService.BlobFetchOption.fieldsSelector(StorageService.BlobMetadataField.KMS_KEY_NAME));
    assertEquals(blob.getBlobId(), remoteBlob.getBlobId());
    assertTrue(remoteBlob.getKmsKeyName().startsWith(kmsKeyOneResourcePath));
    assertNull(remoteBlob.getContentType());
  }
  ...
  public void testGetBlobAllSelectedFields() {
    ...
    assertNotNull(storage.create(blob));
    StorageObject remoteBlob =
        storage.get(blob.getBlobId(), StorageService.BlobFetchOption.fieldsSelector(StorageService.BlobMetadataField.values()));
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
    assertEquals(ImmutableMap.of("k", "v"), remoteBlob.getMetadata());
  ...
  public void testGetBlobFail() {
    ...
    try {
      storage.get(wrongGenerationBlob, StorageService.BlobFetchOption.ifGenerationMatch(-1));
      fail("StorageException was expected");
    } catch (StorageOperationException ex) {
      // expected
    }
  ...
  public void testListBlobsSelectedFields() throws InterruptedException {
    ...
    Page<StorageObject> page =
        storage.list(
            ...
            StorageService.BlobListOptions.listFields(BlobMetadataField.METADATA));
  ...
  public void testListBlobsKmsKeySelectedFields() throws InterruptedException {
    ...
    StorageObject remoteBlob1 =
        storage.create(blob1, StorageService.BlobUploadOption.withKmsKeyName(kmsKeyOneResourcePath));
    StorageObject remoteBlob2 =
        storage.create(blob2, StorageService.BlobUploadOption.withKmsKeyName(kmsKeyOneResourcePath));
    assertNotNull(remoteBlob1);
    assertNotNull(remoteBlob2);
    Page<StorageObject> page =
        storage.list(
            ...
            StorageService.BlobListOptions.listFields(StorageService.BlobMetadataField.KMS_KEY_NAME));
    // Listing blobs is eventually consistent, we loop until the list is of the expected size. The
    // test fails if timeout is reached.
    while (Iterators.size(page.iterateAll().iterator()) != 2) {
      Thread.sleep(500);
  ...
  public void testListBlobsEmptySelectedFields() throws InterruptedException {
    ...
    Page<StorageObject> page =
        storage.list(
            ...
            StorageService.BlobListOptions.listFields());
    // Listing blobs is eventually consistent, we loop until the list is of the expected size. The
    // test fails if timeout is reached.
    while (Iterators.size(page.iterateAll().iterator()) != 2) {
      Thread.sleep(500);
  ...
  public void testListBlobRequesterPays() throws InterruptedException {
    ...
    remoteBucket = remoteBucket.toBucketBuilder().setRequesterPays(true).buildInstance();
  ...
  public void testListBlobsVersioned() throws ExecutionException, InterruptedException {
    String bucketName = RemoteStorageHelper.generateBucketName();
    StorageBucket bucket =
        storage.create(BucketMetadata.newBucketBuilder(bucketName).setVersioningEnabled(true).buildInstance());
    try {
  ...
  public void testListBlobsCurrentDirectory() throws InterruptedException {
    ...
    Page<StorageObject> page =
        storage.list(
            ...
            StorageService.BlobListOptions.currentDirectoryOnly());
    // Listing blobs is eventually consistent, we loop until the list is of the expected size. The
    // test fails if timeout is reached.
    while (Iterators.size(page.iterateAll().iterator()) != 2) {
      Thread.sleep(500);
  ...
  public void testDeleteBlobFail() {
    ...
    assertNotNull(remoteBlob);
    try {
      storage.delete(BUCKET, blob.getName(), StorageService.BlobSourceOptions.ifGenerationMatch(-1L));
      fail("StorageException was expected");
    } catch (StorageOperationException ex) {
      // expected
    }
  ...
  public void testComposeBlob() {
    ...
    StorageService.ComposeObjectRequest req =
        StorageService.ComposeObjectRequest.of(ImmutableList.of(sourceBlobName1, sourceBlobName2), targetBlob);
  ...
  public void testComposeBlobFail() {
    ...
    BlobMetadata targetBlob = BlobMetadata.newBuilder(BUCKET, targetBlobName).buildObject();
    StorageService.ComposeObjectRequest req =
        StorageService.ComposeObjectRequest.newBlobTransferBuilder()
  ...
  public void testCopyBlob() {
    ...
    StorageService.CopyOperationRequest req = StorageService.CopyOperationRequest.create(source, BlobIdentifier.create(BUCKET, targetBlobName));
  ...
  public void testCopyBlobWithEncryptionKeys() {
    ...
    StorageService.CopyOperationRequest req =
        StorageService.CopyOperationRequest.newCopyOperationBuilder()
            .setSource(source)
            .setTarget(target, StorageService.BlobUploadOption.customerSuppliedKey(OTHER_BASE64_KEY))
            .setSourceOptions(StorageService.BlobSourceOptions.customerDecryptionKey(BASE64_KEY))
            .buildRequest();
    BlobCopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
  ...
  public void testRotateFromCustomerEncryptionToKmsKey() {
    ...
    StorageService.CopyOperationRequest req =
        StorageService.CopyOperationRequest.newCopyOperationBuilder()
            .setSource(source)
            .setSourceOptions(StorageService.BlobSourceOptions.customerDecryptionKey(BASE64_KEY))
            .setTarget(target, StorageService.BlobUploadOption.withKmsKeyName(kmsKeyOneResourcePath))
            .buildRequest();
    BlobCopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
  ...
  public void testCopyBlobUpdateMetadata() {
    ...
    BlobMetadata target =
        BlobMetadata.newBuilder(BUCKET, targetBlobName)
            ...
            .setMetadata(metadata)
            .buildObject();
    StorageService.CopyOperationRequest req = StorageService.CopyOperationRequest.create(source, target);
    BlobCopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
    assertEquals(CONTENT_TYPE, copyWriter.getResult().getContentType());
  ...
  public void testCopyBlobUpdateStorageClass() {
    ...
    BlobMetadata sourceInfo =
        BlobMetadata.newBuilder(source).setStorageClass(StorageClassType.STANDARD).buildObject();
    ...
    BlobMetadata targetInfo =
        BlobMetadata.newBuilder(BUCKET, targetBlobName).setStorageClass(StorageClassType.COLDLINE).buildObject();
    StorageService.CopyOperationRequest req = StorageService.CopyOperationRequest.create(source, targetInfo);
    BlobCopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
  ...
  public void testCopyBlobFail() {
    ...
    StorageService.CopyOperationRequest req =
        StorageService.CopyOperationRequest.newCopyOperationBuilder()
            ...
            .setSourceOptions(StorageService.BlobSourceOptions.ifGenerationMatch(-1L))
            .setTarget(target)
            .buildRequest();
    try {
      storage.copy(req);
    ...
    StorageService.CopyOperationRequest req2 =
        StorageService.CopyOperationRequest.newCopyOperationBuilder()
            .setSource(source)
            .setSourceOptions(StorageService.BlobSourceOptions.ifGenerationMatch())
            .setTarget(target)
            .buildRequest();
    try {
      storage.copy(req2);
  ...
  public void testBatchRequest() {
    ...
    StorageBatchOperation updateBatch = storage.batch();
    StorageBatchOutcome<StorageObject> updateResult1 = updateBatch.updateBlob(updatedBlob1);
    StorageBatchOutcome<StorageObject> updateResult2 = updateBatch.updateBlob(updatedBlob2);
    ...
    // Batch get request
    StorageBatchOperation getBatch = storage.batch();
    StorageBatchOutcome<StorageObject> getResult1 = getBatch.get(BUCKET, sourceBlobName1);
    StorageBatchOutcome<StorageObject> getResult2 = getBatch.get(BUCKET, sourceBlobName2);
    ...
    StorageBatchOperation deleteBatch = storage.batch();
    StorageBatchOutcome<Boolean> deleteResult1 = deleteBatch.deleteBlob(BUCKET, sourceBlobName1);
    StorageBatchOutcome<Boolean> deleteResult2 = deleteBatch.deleteBlob(BUCKET, sourceBlobName2);
  ...
  public void testBatchRequestManyOperations() {
    List<StorageBatchOutcome<Boolean>> deleteResults =
        Lists.newArrayListWithCapacity(MAX_BATCH_SIZE);
    List<StorageBatchOutcome<StorageObject>> getResults = Lists.newArrayListWithCapacity(MAX_BATCH_SIZE / 2);
    List<StorageBatchOutcome<StorageObject>> updateResults =
        Lists.newArrayListWithCapacity(MAX_BATCH_SIZE / 2);
    StorageBatchOperation batch = storage.batch();
    ...
    StorageBatchOutcome<StorageObject> getResult = batch.get(BUCKET, sourceBlobName1);
    StorageBatchOutcome<StorageObject> updateResult = batch.updateBlob(updatedBlob2);

    batch.submitBatch();

    // Check deletes
    for (StorageBatchOutcome<Boolean> failedDeleteResult : deleteResults) {
      assertFalse(failedDeleteResult.get());
    }

    // Check gets
    for (StorageBatchOutcome<StorageObject> failedGetResult : getResults) {
    ...
    // Check updates
    for (StorageBatchOutcome<StorageObject> failedUpdateResult : updateResults) {
      try {
  ...
  public void testBatchRequestFail() {
    ...
    StorageBatchOperation batch = storage.batch();
    StorageBatchOutcome<StorageObject> updateResult =
        batch.updateBlob(updatedBlob, StorageService.BlobUploadOption.ifGenerationMatch());
    StorageBatchOutcome<Boolean> deleteResult1 =
        batch.deleteBlob(BUCKET, blobName, StorageService.BlobSourceOptions.ifGenerationMatch(-1L));
    StorageBatchOutcome<Boolean> deleteResult2 = batch.deleteBlob(BlobIdentifier.create(BUCKET, blobName, -1L));
    StorageBatchOutcome<StorageObject> getResult1 =
        batch.get(BUCKET, blobName, StorageService.BlobFetchOption.ifGenerationMatch(-1L));
    StorageBatchOutcome<StorageObject> getResult2 = batch.get(BlobIdentifier.create(BUCKET, blobName, -1L));
    batch.submitBatch();
    try {
      updateResult.get();
      fail("Expected StorageException");
  ...
  public void testReadAndWriteChannelWithEncryptionKey() throws IOException {
    ...
    try (WriteChannel writer =
        storage.writer(blob, StorageService.BlobWriteSetting.customerSuppliedKey(BASE64_KEY))) {
      stringBytes = BLOB_STRING_CONTENT.getBytes(UTF_8);
      writer.write(ByteBuffer.wrap(BLOB_BYTE_CONTENT));
      writer.write(ByteBuffer.wrap(stringBytes));
    }
    ...
    try (ReadChannel reader =
        storage.reader(blob.getBlobId(), StorageService.BlobSourceOptions.customerDecryptionKey(KEY))) {
      readBytes = ByteBuffer.allocate(BLOB_BYTE_CONTENT.length);
      readStringBytes = ByteBuffer.allocate(stringBytes.length);
      reader.read(readBytes);
      reader.read(readStringBytes);
  ...
  public void testReadChannelFail() throws IOException {
    ...
    assertNotNull(remoteBlob);
    try (ReadChannel reader =
        storage.reader(blob.getBlobId(), StorageService.BlobSourceOptions.ifMetagenerationMatch(-1L))) {
    ...
    try (ReadChannel reader =
        storage.reader(blob.getBlobId(), StorageService.BlobSourceOptions.ifGenerationMatch(-1L))) {
      reader.read(ByteBuffer.allocate(42));
      fail("StorageException was expected");
    } catch (StorageOperationException ex) {
      // expected
    ...
    try (ReadChannel reader =
        storage.reader(blobIdWrongGeneration, StorageService.BlobSourceOptions.ifGenerationMatch())) {
      reader.read(ByteBuffer.allocate(42));
      fail("StorageException was expected");
    } catch (StorageOperationException ex) {
      // expected
  ...
  public void testWriteChannelFail() throws IOException {
    ...
    try {
      try (WriteChannel writer = storage.writer(blob, StorageService.BlobWriteSetting.ifGenerationMatch())) {
  ...
  public void testGetSignedUrl() throws IOException {
    ...
    for (StorageService.UrlSigningOption urlStyle :
        Arrays.asList(
            StorageService.UrlSigningOption.enablePathStyle(),
            StorageService.UrlSigningOption.enableVirtualHostedStyle())) {
  ...
  public void testPostSignedUrl() throws IOException {
    ...
    for (StorageService.UrlSigningOption urlStyle :
      ...
      URL url =
          storage.signUrl(
              blob, 1, TimeUnit.HOURS, StorageService.UrlSigningOption.requestMethod(HttpRequestMethod.POST), urlStyle);
      URLConnection connection = url.openConnection();
  ...
  private void testBucketAclRequesterPays(boolean requesterPays) {
    ...
    StorageService.BucketFilterOption[] bucketOptions =
        requesterPays
            ? new StorageService.BucketFilterOption[] {StorageService.BucketFilterOption.withUserProject(projectId)}
            : new StorageService.BucketFilterOption[] {};
    ...
    AccessControlEntry acl = AccessControlEntry.create(AccessControlEntry.UserPrincipal.allAuthenticatedUsers(), AccessRole.READER);
    ...
    assertEquals(AccessControlEntry.AccessRole.WRITER, updatedAcl.getRole());
  ...
  public void testBucketDefaultAcl() {
    ...
    AccessControlEntry acl = AccessControlEntry.create(UserPrincipal.allAuthenticatedUsers(), AccessControlEntry.AccessRole.READER);
    assertNotNull(storage.createDefaultAcl(BUCKET, acl));
    AccessControlEntry updatedAcl = storage.updateDefaultAcl(BUCKET, acl.asBuilder().setRole(AccessControlEntry.AccessRole.OWNER).create());
  ...
  public void testHmacKey() {
    ...
    try {
      ...
      Page<HmacSecretKey.HmacKeyInfo> metadatas =
          storage.listHmacKeys(StorageService.ListHmacKeysOptions.serviceAccountEmail(serviceAccount));
      ...
      metadatas = storage.listHmacKeys(StorageService.ListHmacKeysOptions.serviceAccountEmail(serviceAccount));
      ...
      metadatas =
          storage.listHmacKeys(
              StorageService.ListHmacKeysOptions.serviceAccountEmail(serviceAccount),
              StorageService.ListHmacKeysOptions.setMaxResults(2L));
      ...
      metadatas =
          storage.listHmacKeys(
              StorageService.ListHmacKeysOptions.serviceAccountEmail(serviceAccount),
              StorageService.ListHmacKeysOptions.setMaxResults(2L),
              StorageService.ListHmacKeysOptions.setPageToken(nextPageToken));
  ...
  public void testUpdateBucketLabel() {
    ...
    remoteBucket = remoteBucket.toBucketBuilder().setLabels(BUCKET_LABELS).buildInstance();
    StorageBucket updatedBucket = storage.update(remoteBucket);
    assertEquals(BUCKET_LABELS, updatedBucket.getLabels());
    remoteBucket.toBucketBuilder().setLabels(REMOVE_BUCKET_LABELS).buildInstance().modify();
    ...
    Iterator<StorageBucket> bucketIterator =
        storage
            .list(
                ...
                StorageService.BucketListOptions.fieldsSelector(),
      ...
      Iterator<StorageBucket> bucketIterator =
          storage
              .list(
                  ...
                  StorageService.BucketListOptions.fieldsSelector(StorageService.BucketMetadataField.ENCRYPTION))
              .iterateAll()
              .iterator();
      while (!bucketIterator.hasNext()) {
        Thread.sleep(500);
        bucketIterator =
            storage
                .list(
                    StorageService.BucketListOptions.withPrefix(bucketName),
                    StorageService.BucketListOptions.fieldsSelector(BucketMetadataField.ENCRYPTION))
                .iterateAll()
                .iterator();
      }
      while (bucketIterator.hasNext()) {
  ...
  public void testRetentionPolicyNoLock() throws ExecutionException, InterruptedException {
    ...
    StorageBucket remoteBucket =
        storage.create(
            BucketMetadata.newBucketBuilder(bucketName).setRetentionPeriod(RETENTION_PERIOD).buildInstance());
    try {
      ...
      remoteBucket =
          storage.get(bucketName, StorageService.GetBucketOption.selectFields(BucketMetadataField.RETENTION_POLICY));
      assertEquals(RETENTION_PERIOD, remoteBucket.getRetentionPeriod());
      assertNotNull(remoteBucket.getRetentionEffectiveTime());
      assertNull(remoteBucket.isRetentionPolicyLocked());
      ...
      remoteBucket = remoteBucket.toBucketBuilder().setRetentionPeriod(null).buildInstance().modify();
  ...
  private void retentionPolicyLockRequesterPays(boolean requesterPays)
    ...
    if (requesterPays) {
      ...
      bucketInfo = BucketMetadata.newBucketBuilder(bucketName).setRetentionPeriod(RETENTION_PERIOD).buildInstance();
    }
      ...
      if (requesterPays) {
        remoteBucket =
            storage.lockRetentionPolicy(
                ...
                StorageService.BucketTargetOptions.withUserProject(projectId));
      } else {
        remoteBucket =
            storage.lockRetentionPolicy(
  ...
  public void testEnableDisableBucketDefaultEventBasedHold()
    ...
    String bucketName = RemoteStorageHelper.generateBucketName();
    StorageBucket remoteBucket =
        storage.create(BucketMetadata.newBucketBuilder(bucketName).setDefaultEventBasedHold(true).buildInstance());
    try {
      assertTrue(remoteBucket.getDefaultEventBasedHold());
      remoteBucket =
          storage.get(
              bucketName, StorageService.GetBucketOption.selectFields(StorageService.BucketMetadataField.DEFAULT_EVENT_BASED_HOLD));
      assertTrue(remoteBucket.getDefaultEventBasedHold());
      ...
      remoteBlob =
          storage.get(
              blobInfo.getBlobId(), StorageService.BlobFetchOption.fieldsSelector(BlobMetadataField.EVENT_BASED_HOLD));
      assertTrue(remoteBlob.getEventBasedHold());
      ...
      remoteBucket = remoteBucket.toBucketBuilder().setDefaultEventBasedHold(false).buildInstance().modify();
  ...
  public void testEnableDisableTemporaryHold() {
    ...
    remoteBlob =
        storage.get(remoteBlob.getBlobId(), StorageService.BlobFetchOption.fieldsSelector(BlobMetadataField.TEMPORARY_HOLD));
    assertTrue(remoteBlob.getTemporaryHold());
  ...
  public void testBucketWithBucketPolicyOnlyEnabled() throws Exception {
    ...
    try {
      ...
      StorageBucket remoteBucket =
          storage.get(bucket, StorageService.GetBucketOption.selectFields(BucketMetadataField.IAMCONFIGURATION));
  ...
  public void testBucketWithUniformBucketLevelAccessEnabled() throws Exception {
    ...
    try {
      ...
      StorageBucket remoteBucket =
          storage.get(bucket, StorageService.GetBucketOption.selectFields(StorageService.BucketMetadataField.IAMCONFIGURATION));
  ...
  public void testEnableAndDisableBucketPolicyOnlyOnExistingBucket() throws Exception {
    ...
    try {
      ...
      StorageBucket bucket =
          storage.create(
              StorageBucket.newBucketBuilder(bpoBucket)
                  .setAcl(ImmutableList.of(AccessControlEntry.create(UserPrincipal.allAuthenticatedUsers(), AccessControlEntry.AccessRole.READER)))
                  .setDefaultAcl(
                      ImmutableList.of(AccessControlEntry.create(UserPrincipal.allAuthenticatedUsers(), AccessControlEntry.AccessRole.READER)))
                  .buildInstance());
      ...
      StorageBucket remoteBucket =
          storage.get(bpoBucket, StorageService.GetBucketOption.selectFields(BucketMetadataField.IAMCONFIGURATION));
      ...
      remoteBucket =
          storage.get(
              bpoBucket,
              StorageService.GetBucketOption.selectFields(
                  StorageService.BucketMetadataField.IAMCONFIGURATION, StorageService.BucketMetadataField.ACL, StorageService.BucketMetadataField.DEFAULT_OBJECT_ACL));
  ...
  public void testEnableAndDisableUniformBucketLevelAccessOnExistingBucket() throws Exception {
    ...
    try {
      ...
      StorageBucket remoteBucket =
          storage.get(bpoBucket, StorageService.GetBucketOption.selectFields(StorageService.BucketMetadataField.IAMCONFIGURATION));

      assertTrue(remoteBucket.getIamConfiguration().isUniformBucketLevelAccessEnabled());
      assertNotNull(remoteBucket.getIamConfiguration().getUniformBucketLevelAccessLockedTime());

      remoteBucket.toBucketBuilder().setIamConfiguration(ublaDisabledIamConfiguration).buildInstance().modify();

      remoteBucket =
          storage.get(
              bpoBucket,
              StorageService.GetBucketOption.selectFields(
                  BucketMetadataField.IAMCONFIGURATION, StorageService.BucketMetadataField.ACL, StorageService.BucketMetadataField.DEFAULT_OBJECT_ACL));

      assertFalse(remoteBucket.getIamConfiguration().isUniformBucketLevelAccessEnabled());
      assertEquals(UserPrincipal.allAuthenticatedUsers(), remoteBucket.getDefaultAcl().get(0).getEntity());
      assertEquals(AccessControlEntry.AccessRole.READER, remoteBucket.getDefaultAcl().get(0).getRole());
      assertEquals(UserPrincipal.allAuthenticatedUsers(), remoteBucket.getAcl().get(0).getEntity());
      assertEquals(AccessRole.READER, remoteBucket.getAcl().get(0).getRole());
    } finally {
      RemoteStorageHelper.forceDelete(storage, bpoBucket, 1, TimeUnit.MINUTES);
  ...
  public void testBucketLocationType() throws ExecutionException, InterruptedException {
    ...
    StorageBucket bucket =
        storage.get(
            bucketName, StorageService.GetBucketOption.ifMetagenerationNotMatch(bucketMetageneration));
  ...
  public void testBucketLogging() throws ExecutionException, InterruptedException {
    ...
    try {
      assertNotNull(storage.create(BucketMetadata.newBucketBuilder(logsBucket).setLocation("us").buildInstance()));
      ...
      StorageBucket bucket =
          storage.create(
              BucketMetadata.newBucketBuilder(loggingBucket).setLocation("us").setLogging(logging).buildInstance());
