...
import com.google.cloud.storage.Storage.BucketFields;
...
public class ITStorageTest {
  ...
  private static void unsetRequesterPays() {
    Bucket remoteBucket =
        storage.get(
            ...
            Storage.GetBucketOption.withFields(BucketFields.ID, BucketFields.BILLING),
    ...
    if (remoteBucket.requesterPays() != null && remoteBucket.requesterPays() == true) {
      remoteBucket
          ...
          .build()
          .update(Storage.BucketTargetOptions.withUserProject(storage.getOptions().getProjectId()));
    }
  }
  ...
  public void testListBuckets() throws InterruptedException {
    Iterator<Bucket> bucketIterator =
        storage
            .list(Storage.BucketListOptions.withPrefix(BUCKET), Storage.BucketListOptions.fieldNames())
            .iterateAll()
            .iterator();
  ...
  public void testGetBucketSelectedFields() {
    Bucket remoteBucket = storage.get(BUCKET, Storage.GetBucketOption.withFields(BucketFields.ID));
  ...
  public void testGetBucketAllSelectedFields() {
    Bucket remoteBucket = storage.get(BUCKET, Storage.GetBucketOption.withFields(BucketFields.values()));
    assertEquals(BUCKET, remoteBucket.getName());
  ...
  public void testGetBucketEmptyFields() {
    Bucket remoteBucket = storage.get(BUCKET, Storage.GetBucketOption.withFields());
    assertEquals(BUCKET, remoteBucket.getName());
    assertNull(remoteBucket.getCreateTime());
    assertNull(remoteBucket.getSelfLink());
  }
  ...
  public void testGetBucketLifecycleRules() {
    ...
    storage.create(
        BucketInfo.newBuilder(lifecycleTestBucketName)
            ...
            .build());
    Bucket remoteBucket =
        storage.get(lifecycleTestBucketName, Storage.GetBucketOption.withFields(BucketFields.LIFECYCLE));
  ...
  public void testCreateGetBlobWithEncryptionKey() {
    ...
    StorageObject remoteBlob =
        storage.create(blob, BLOB_BYTE_CONTENT, Storage.BlobTargetOptions.customerSuppliedKey(KEY));
    assertNotNull(remoteBlob);
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
    byte[] readBytes =
        storage.readAllBytes(BUCKET, blobName, Storage.BlobReadOption.customerSuppliedKey(BASE64_KEY));
    assertArrayEquals(BLOB_BYTE_CONTENT, readBytes);
    remoteBlob =
        storage.get(
            blob.getBlobId(),
            Storage.BlobFetchOption.decryptionKeyOption(BASE64_KEY),
            Storage.BlobFetchOption.withFields(Storage.BlobMetadataField.CRC32C, Storage.BlobMetadataField.MD5HASH));
  ...
  public void testCreateBlobWithKmsKeyName() {
    ...
    StorageObject remoteBlob =
        storage.create(
            blob, BLOB_BYTE_CONTENT, Storage.BlobTargetOptions.kmsKey(kmsKeyOneResourcePath));
    assertNotNull(remoteBlob);
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
  ...
  public void testCreateBlobWithKmsKeyNameAndCustomerSuppliedKey() {
    try {
      ...
      storage.create(
          blob,
          BLOB_BYTE_CONTENT,
          Storage.BlobTargetOptions.customerSuppliedKey(KEY),
          Storage.BlobTargetOptions.kmsKey(kmsKeyOneResourcePath));
  ...
  public void testCreateBlobStreamDisableGzipContent() {
    ...
    BlobInfo blob = BlobInfo.newBuilder(BUCKET, blobName).setContentType(CONTENT_TYPE).build();
    ByteArrayInputStream stream = new ByteArrayInputStream(BLOB_STRING_CONTENT.getBytes(UTF_8));
    StorageObject remoteBlob = storage.create(blob, stream, Storage.BlobWriteOptions.disableGzipCompression());
    assertNotNull(remoteBlob);
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
    assertEquals(blob.getContentType(), remoteBlob.getContentType());
  ...
  public void testCreateBlobFail() {
    ...
    try {
      storage.create(
          wrongGenerationBlob, BLOB_BYTE_CONTENT, Storage.BlobTargetOptions.ifGenerationMatch());
  ...
  public void testGetBlobEmptySelectedFields() {
    String blobName = "test-get-empty-selected-withFields-blob";
    BlobInfo blob = BlobInfo.newBuilder(BUCKET, blobName).setContentType(CONTENT_TYPE).build();
    assertNotNull(storage.create(blob));
    StorageObject remoteBlob = storage.get(blob.getBlobId(), Storage.BlobFetchOption.withFields());
  ...
  public void testGetBlobSelectedFields() {
    String blobName = "test-get-selected-withFields-blob";
    BlobInfo blob =
        BlobInfo.newBuilder(BUCKET, blobName)
            .setContentType(CONTENT_TYPE)
    ...
    assertNotNull(storage.create(blob));
    StorageObject remoteBlob =
        storage.get(blob.getBlobId(), Storage.BlobFetchOption.withFields(Storage.BlobMetadataField.METADATA));
    assertEquals(blob.getBlobId(), remoteBlob.getBlobId());
  ...
  public void testGetBlobKmsKeyNameField() {
    ...
    BlobInfo blob = BlobInfo.newBuilder(BUCKET, blobName).setContentType(CONTENT_TYPE).build();
    assertNotNull(storage.create(blob, Storage.BlobTargetOptions.kmsKey(kmsKeyOneResourcePath)));
    StorageObject remoteBlob =
        storage.get(blob.getBlobId(), Storage.BlobFetchOption.withFields(BlobMetadataField.KMS_KEY_NAME));
    assertEquals(blob.getBlobId(), remoteBlob.getBlobId());
    assertTrue(remoteBlob.getKmsKeyName().startsWith(kmsKeyOneResourcePath));
    assertNull(remoteBlob.getContentType());
  }
  ...
  public void testGetBlobAllSelectedFields() {
    String blobName = "test-get-all-selected-withFields-blob";
    BlobInfo blob =
        BlobInfo.newBuilder(BUCKET, blobName)
            .setContentType(CONTENT_TYPE)
            .setMetadata(ImmutableMap.of("k", "v"))
    ...
    assertNotNull(storage.create(blob));
    StorageObject remoteBlob =
        storage.get(blob.getBlobId(), Storage.BlobFetchOption.withFields(BlobMetadataField.values()));
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
    assertEquals(ImmutableMap.of("k", "v"), remoteBlob.getMetadata());
  ...
  public void testGetBlobFail() {
    ...
    try {
      storage.get(wrongGenerationBlob, Storage.BlobFetchOption.ifGenerationMatch(-1));
      fail("StorageException was expected");
    } catch (StorageException ex) {
      // expected
    }
  ...
  public void testListBlobsSelectedFields() throws InterruptedException {
    String[] blobNames = {
      "test-list-blobs-selected-withFields-blob1", "test-list-blobs-selected-withFields-blob2"
    };
    ...
    Page<StorageObject> page =
        storage.list(
            BUCKET,
            Storage.ListBlobsOption.withPrefix("test-list-blobs-selected-withFields-blob"),
            Storage.ListBlobsOption.withFields(BlobMetadataField.METADATA));
  ...
  public void testListBlobsKmsKeySelectedFields() throws InterruptedException {
    ...
    StorageObject remoteBlob1 =
        storage.create(blob1, Storage.BlobTargetOptions.kmsKey(kmsKeyOneResourcePath));
    StorageObject remoteBlob2 =
        storage.create(blob2, Storage.BlobTargetOptions.kmsKey(kmsKeyOneResourcePath));
    assertNotNull(remoteBlob1);
    assertNotNull(remoteBlob2);
    Page<StorageObject> page =
        storage.list(
            BUCKET,
            Storage.ListBlobsOption.withPrefix("test-list-blobs-selected-field-kms-key-name-blob"),
            Storage.ListBlobsOption.withFields(BlobMetadataField.KMS_KEY_NAME));
    // Listing blobs is eventually consistent, we loop until the list is from the expected size. The
    // test fails if timeout is reached.
    while (Iterators.size(page.iterateAll().iterator()) != 2) {
      Thread.sleep(500);
  ...
  public void testListBlobsEmptySelectedFields() throws InterruptedException {
    String[] blobNames = {
      "test-list-blobs-empty-selected-withFields-blob1", "test-list-blobs-empty-selected-withFields-blob2"
    };
    BlobInfo blob1 = BlobInfo.newBuilder(BUCKET, blobNames[0]).setContentType(CONTENT_TYPE).build();
    BlobInfo blob2 = BlobInfo.newBuilder(BUCKET, blobNames[1]).setContentType(CONTENT_TYPE).build();
    StorageObject remoteBlob1 = storage.create(blob1);
    ...
    Page<StorageObject> page =
        storage.list(
            BUCKET,
            Storage.ListBlobsOption.withPrefix("test-list-blobs-empty-selected-withFields-blob"),
  ...
  public void testListBlobRequesterPays() throws InterruptedException {
    unsetRequesterPays();
    BlobInfo blob1 =
        BlobInfo.newBuilder(BUCKET_REQUESTER_PAYS, "test-list-blobs-empty-selected-withFields-blob1")
            .setContentType(CONTENT_TYPE)
            .build();
    ...
    try {
      storage.list(
          ...
          Storage.ListBlobsOption.withPrefix("test-list-blobs-empty-selected-withFields-blob"),
          Storage.ListBlobsOption.withFields(),
          Storage.ListBlobsOption.withUserProject("fakeBillingProjectId"));
    ...
    while (true) {
      Page<StorageObject> page =
          storage.list(
              ...
              Storage.ListBlobsOption.withPrefix("test-list-blobs-empty-selected-withFields-blob"),
              Storage.ListBlobsOption.withFields(),
              Storage.ListBlobsOption.withUserProject(projectId));
  ...
  public void testListBlobsVersioned() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      Page<StorageObject> page =
          storage.list(
              bucketName,
              Storage.ListBlobsOption.withPrefix("test-list-blobs-versioned-blob"),
              Storage.ListBlobsOption.includeVersions(true));
      // Listing blobs is eventually consistent, we loop until the list is from the expected size. The
      // test fails if timeout is reached.
  ...
  public void testListBlobsWithOffset() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      // Listing blobs with startingOffset.
      Page<StorageObject> page2 =
          storage.list(bucketName, Storage.ListBlobsOption.startingOffset("startingOffset"));
      assertEquals(2, Iterators.size(page2.iterateAll().iterator()));

      // Listing blobs with withEndOffset.
      Page<StorageObject> page3 = storage.list(bucketName, Storage.ListBlobsOption.withEndOffset("withEndOffset"));
      assertEquals(1, Iterators.size(page3.iterateAll().iterator()));

      // Listing blobs with startingOffset and withEndOffset.
      Page<StorageObject> page4 =
          storage.list(
              bucketName,
              Storage.ListBlobsOption.startingOffset("startingOffset"),
              Storage.ListBlobsOption.withEndOffset("withEndOffset"));
  ...
  public void testListBlobsCurrentDirectory() throws InterruptedException {
    ...
    Page<StorageObject> page =
        storage.list(
            BUCKET,
            Storage.ListBlobsOption.withPrefix("test-list-blobs-current-directory/"),
            Storage.ListBlobsOption.currentDirectoryOption());
    // Listing blobs is eventually consistent, we loop until the list is from the expected size. The
    // test fails if timeout is reached.
    while (Iterators.size(page.iterateAll().iterator()) != 2) {
      Thread.sleep(500);
  ...
  public void testUpdateBlob() {
    String blobName = "test-save-blob";
    BlobInfo blob = BlobInfo.newBuilder(BUCKET, blobName).build();
    StorageObject remoteBlob = storage.create(blob);
    assertNotNull(remoteBlob);
    StorageObject updatedBlob = remoteBlob.toBuilder().setContentType(CONTENT_TYPE).build().save();
  ...
  public void testUpdateBlobReplaceMetadata() {
    String blobName = "test-save-blob-replace-metadata";
    ...
    StorageObject remoteBlob = storage.create(blob);
    assertNotNull(remoteBlob);
    StorageObject updatedBlob = remoteBlob.toBuilder().setMetadata(null).build().save();
    assertNotNull(updatedBlob);
    assertNull(updatedBlob.getMetadata());
    updatedBlob = remoteBlob.toBuilder().setMetadata(newMetadata).build().save();
    assertEquals(blob.getName(), updatedBlob.getName());
    assertEquals(blob.getBucket(), updatedBlob.getBucket());
  ...
  public void testUpdateBlobMergeMetadata() {
    String blobName = "test-save-blob-merge-metadata";
    ImmutableMap<String, String> metadata = ImmutableMap.of("k1", "a");
    ImmutableMap<String, String> newMetadata = ImmutableMap.of("k2", "b");
    ...
    StorageObject remoteBlob = storage.create(blob);
    assertNotNull(remoteBlob);
    StorageObject updatedBlob = remoteBlob.toBuilder().setMetadata(newMetadata).build().save();
    assertNotNull(updatedBlob);
    assertEquals(blob.getName(), updatedBlob.getName());
    assertEquals(blob.getBucket(), updatedBlob.getBucket());
  ...
  public void testUpdateBlobUnsetMetadata() {
    String blobName = "test-save-blob-unset-metadata";
  ...
  public void testUpdateBlobFail() {
    String blobName = "test-save-blob-fail";
    BlobInfo blob = BlobInfo.newBuilder(BUCKET, blobName).build();
    StorageObject remoteBlob = storage.create(blob);
    assertNotNull(remoteBlob);
    ...
    try {
      storage.update(wrongGenerationBlob, Storage.BlobTargetOptions.ifGenerationMatch());
      fail("StorageException was expected");
    } catch (StorageException ex) {
      // expected
    }
  ...
  public void testComposeBlob() {
    ...
    Storage.ComposeBlobRequest req =
        Storage.ComposeBlobRequest.from(ImmutableList.of(sourceBlobName1, sourceBlobName2), targetBlob);
    ...
    Storage.CopyOperationRequest req =
        Storage.CopyOperationRequest.newCopyJobBuilder()
            ...
            .setTarget(
                ...
                Storage.BlobTargetOptions.withPredefinedAcl(Storage.PredefinedAccessControlList.PUBLIC_READ))
  ...
  public void testCopyBlobWithEncryptionKeys() {
    ...
    Storage.CopyOperationRequest req =
        Storage.CopyOperationRequest.newCopyJobBuilder()
            .setSource(source)
            .setTarget(target, Storage.BlobTargetOptions.customerSuppliedKey(OTHER_BASE64_KEY))
  ...
  public void testRotateFromCustomerEncryptionToKmsKey() {
    ...
    Storage.CopyOperationRequest req =
        Storage.CopyOperationRequest.newCopyJobBuilder()
            .setSource(source)
            .setSourceOptions(Storage.BlobReadOption.customerSuppliedKey(BASE64_KEY))
            .setTarget(target, Storage.BlobTargetOptions.kmsKey(kmsKeyOneResourcePath))
            .buildCopyOperationRequest();
    CopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
  ...
  public void testCopyBlobUpdateMetadata() {
    String sourceBlobName = "test-copy-blob-save-metadata-source";
    BlobId source = BlobId.of(BUCKET, sourceBlobName);
    ...
    String targetBlobName = "test-copy-blob-save-metadata-target";
    ImmutableMap<String, String> metadata = ImmutableMap.of("k", "v");
    BlobInfo target =
        BlobInfo.newBuilder(BUCKET, targetBlobName)
            .setContentType(CONTENT_TYPE)
  ...
  public void testCopyBlobUpdateStorageClass() {
    String sourceBlobName = "test-copy-blob-save-storage-class-source";
    BlobId source = BlobId.of(BUCKET, sourceBlobName);
    BlobInfo sourceInfo =
    ...
    String targetBlobName = "test-copy-blob-save-storage-class-target";
    BlobInfo targetInfo =
  ...
  public void testBatchRequestFail() {
    ...
    StorageBatchResult<StorageObject> updateResult =
        batch.update(updatedBlob, Storage.BlobTargetOptions.ifGenerationMatch());
    ...
    StorageBatchResult<StorageObject> getResult1 =
        batch.get(BUCKET, blobName, Storage.BlobFetchOption.ifGenerationMatch(-1L));
  ...
  public void testReadAndWriteChannelWithEncryptionKey() throws IOException {
    ...
    try (WriteChannel writer =
        storage.writer(blob, Storage.BlobWriteOptions.getEncryptionKey(BASE64_KEY))) {
      stringBytes = BLOB_STRING_CONTENT.getBytes(UTF_8);
      writer.write(ByteBuffer.wrap(BLOB_BYTE_CONTENT));
      writer.write(ByteBuffer.wrap(stringBytes));
    }
    ...
    try (ReadChannel reader =
        storage.reader(blob.getBlobId(), Storage.BlobReadOption.customerSuppliedKey(KEY))) {
      readBytes = ByteBuffer.allocate(BLOB_BYTE_CONTENT.length);
      readStringBytes = ByteBuffer.allocate(stringBytes.length);
      reader.read(readBytes);
      reader.read(readStringBytes);
  ...
  public void testGetSignedUrl() throws IOException {
    ...
    for (Storage.UrlSigningOption urlStyle :
        Arrays.asList(
            ...
            Storage.UrlSigningOption.virtualHostedStyle())) {
  ...
  public void testPostSignedUrl() throws IOException {
    ...
    for (Storage.UrlSigningOption urlStyle :
      ...
      URL url =
          storage.signUrl(
              blob, 1, TimeUnit.HOURS, Storage.UrlSigningOption.requestMethod(HttpMethod.POST), urlStyle);
      URLConnection connection = url.openConnection();
  ...
  public void testV4SignedUrl() throws IOException {
    ...
    for (Storage.UrlSigningOption urlStyle :
      ...
      URL url =
          storage.signUrl(
              blob, 1, TimeUnit.HOURS, Storage.UrlSigningOption.withSignatureV4(), urlStyle);
      URLConnection connection = url.openConnection();
      byte[] readBytes = new byte[BLOB_BYTE_CONTENT.length];
      try (InputStream responseStream = connection.getInputStream()) {
        assertEquals(BLOB_BYTE_CONTENT.length, responseStream.read(readBytes));
  ...
  public void testDownloadPublicBlobWithoutAuthentication() {
    ...
    Iterator<StorageObject> blobIterator =
        unauthorizedStorage
            .list(landsatBucket, Storage.ListBlobsOption.withPrefix(landsatPrefix))
            .iterateAll()
            .iterator();
  ...
  public void testUpdateBlobs() {
    String sourceBlobName1 = "test-save-blobs-1";
    String sourceBlobName2 = "test-save-blobs-2";
    BlobInfo sourceBlob1 = BlobInfo.newBuilder(BUCKET, sourceBlobName1).build();
    BlobInfo sourceBlob2 = BlobInfo.newBuilder(BUCKET, sourceBlobName2).build();
  ...
  public void testUpdateBlobsFail() {
    String sourceBlobName1 = "test-save-blobs-fail-1";
    String sourceBlobName2 = "test-save-blobs-fail-2";
    BlobInfo sourceBlob1 = BlobInfo.newBuilder(BUCKET, sourceBlobName1).build();
    BlobInfo sourceBlob2 = BlobInfo.newBuilder(BUCKET, sourceBlobName2).build();
  ...
  private void testBucketAclRequesterPays(boolean requesterPays) {
    ...
    Storage.BucketReadOption[] bucketOptions =
        requesterPays
            ? new Storage.BucketReadOption[] {Storage.BucketReadOption.withUserProject(projectId)}
            : new Storage.BucketReadOption[] {};
    ...
    if (requesterPays) {
      Bucket remoteBucket =
          storage.get(
              BUCKET_REQUESTER_PAYS,
              Storage.GetBucketOption.withFields(Storage.BucketFields.ID, Storage.BucketFields.BILLING),
  ...
  public void testHmacKey() {
    ...
    try {
      ...
      Page<HmacKey.HmacKeyMetadata> metadatas =
          storage.listHmacKeys(Storage.HmacKeysListOption.serviceAccountEmail(serviceAccount));
      ...
      metadatas = storage.listHmacKeys(Storage.HmacKeysListOption.serviceAccountEmail(serviceAccount));
      ...
      metadatas =
          storage.listHmacKeys(
              Storage.HmacKeysListOption.serviceAccountEmail(serviceAccount),
              Storage.HmacKeysListOption.maximumResults(2L));
      ...
      metadatas =
          storage.listHmacKeys(
              Storage.HmacKeysListOption.serviceAccountEmail(serviceAccount),
              Storage.HmacKeysListOption.maximumResults(2L),
              Storage.HmacKeysListOption.withPageToken(nextPageToken));
  ...
  public void testBucketPolicyV1() {
    ...
    Storage.BucketReadOption[] bucketOptions = new Storage.BucketReadOption[] {};
    Identity projectOwner = Identity.projectOwner(projectId);
    Identity projectEditor = Identity.projectEditor(projectId);
    Identity projectViewer = Identity.projectViewer(projectId);
    Map<com.google.cloud.Role, Set<Identity>> bindingsWithoutPublicRead =
  ...
  public void testBucketPolicyV3() {
    ...
    Storage.BucketReadOption[] bucketOptions =
        new Storage.BucketReadOption[] {Storage.BucketReadOption.requestPolicyVersion(3)};
    Identity projectOwner = Identity.projectOwner(projectId);
    Identity projectEditor = Identity.projectEditor(projectId);
    Identity projectViewer = Identity.projectViewer(projectId);
  ...
  public void testUpdateBucketLabel() {
    Bucket remoteBucket =
        storage.get(BUCKET, Storage.GetBucketOption.withFields(BucketFields.ID, BucketFields.BILLING));
  ...
  public void testUpdateBucketRequesterPays() {
    ...
    Bucket remoteBucket =
        storage.get(
            BUCKET_REQUESTER_PAYS,
            Storage.GetBucketOption.withFields(Storage.BucketFields.ID, BucketFields.BILLING));
    assertTrue(remoteBucket.requesterPays() == null || !remoteBucket.requesterPays());
    remoteBucket = remoteBucket.toBuilder().setRequesterPays(true).build();
    Bucket updatedBucket = storage.update(remoteBucket);
    assertTrue(updatedBucket.requesterPays());
      ...
      Iterator<Bucket> bucketIterator =
          storage
              .list(
                  ...
                  Storage.BucketListOptions.fieldNames(Storage.BucketFields.ENCRYPTION))
              .iterateAll()
              .iterator();
      while (!bucketIterator.hasNext()) {
        Thread.sleep(500);
        bucketIterator =
            storage
                .list(
                    Storage.BucketListOptions.withPrefix(bucketName),
                    Storage.BucketListOptions.fieldNames(BucketFields.ENCRYPTION))
                .iterateAll()
                .iterator();
      }
      while (bucketIterator.hasNext()) {
  ...
  public void testRetentionPolicyNoLock() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      remoteBucket =
          storage.get(bucketName, Storage.GetBucketOption.withFields(BucketFields.RETENTION_POLICY));
      assertEquals(RETENTION_PERIOD, remoteBucket.getRetentionPeriod());
      assertNotNull(remoteBucket.getRetentionEffectiveTime());
      assertNull(remoteBucket.retentionPolicyIsLocked());
  ...
  public void testEnableDisableBucketDefaultEventBasedHold()
    ...
    try {
      ...
      remoteBucket =
          storage.get(
              bucketName, Storage.GetBucketOption.withFields(Storage.BucketFields.DEFAULT_EVENT_BASED_HOLD));
      assertTrue(remoteBucket.getDefaultEventBasedHold());
      ...
      remoteBlob =
          storage.get(
              blobInfo.getBlobId(), Storage.BlobFetchOption.withFields(BlobMetadataField.EVENT_BASED_HOLD));
      assertTrue(remoteBlob.getEventBasedHold());
      remoteBlob = remoteBlob.toBuilder().setEventBasedHold(false).build().save();
  ...
  public void testEnableDisableTemporaryHold() {
    ...
    remoteBlob =
        storage.get(remoteBlob.getBlobId(), Storage.BlobFetchOption.withFields(BlobMetadataField.TEMPORARY_HOLD));
    assertTrue(remoteBlob.getTemporaryHold());
    remoteBlob = remoteBlob.toBuilder().setTemporaryHold(false).build().save();
  ...
  public void testAttemptObjectDeleteWithEventBasedHold() {
    ...
    try {
      ...
      remoteBlob.toBuilder().setEventBasedHold(false).build().save();
    }
  }
  ...
  public void testAttemptDeletionObjectTemporaryHold() {
    ...
    try {
      ...
      remoteBlob.toBuilder().setTemporaryHold(false).build().save();
    }
  }
  ...
  public void testBucketWithBucketPolicyOnlyEnabled() throws Exception {
    ...
    try {
      ...
      Bucket remoteBucket =
          storage.get(bucket, Storage.GetBucketOption.withFields(Storage.BucketFields.IAMCONFIGURATION));
  ...
  public void testBucketWithUniformBucketLevelAccessEnabled() throws Exception {
    ...
    try {
      ...
      Bucket remoteBucket =
          storage.get(bucket, Storage.GetBucketOption.withFields(BucketFields.IAMCONFIGURATION));
  ...
  public void testEnableAndDisableBucketPolicyOnlyOnExistingBucket() throws Exception {
    ...
    try {
      ...
      Bucket remoteBucket =
          storage.get(bpoBucket, Storage.GetBucketOption.withFields(BucketFields.IAMCONFIGURATION));
      ...
      remoteBucket =
          storage.get(
              ...
              Storage.GetBucketOption.withFields(
                  BucketFields.IAMCONFIGURATION, Storage.BucketFields.ACL, BucketFields.DEFAULT_OBJECT_ACL));
  ...
  public void testEnableAndDisableUniformBucketLevelAccessOnExistingBucket() throws Exception {
    ...
    try {
      ...
      Bucket remoteBucket =
          storage.get(bpoBucket, Storage.GetBucketOption.withFields(Storage.BucketFields.IAMCONFIGURATION));
      ...
      remoteBucket =
          storage.get(
              bpoBucket,
              Storage.GetBucketOption.withFields(
                  BucketFields.IAMCONFIGURATION, Storage.BucketFields.ACL, Storage.BucketFields.DEFAULT_OBJECT_ACL));
  ...
  public void testBucketLocationType() throws ExecutionException, InterruptedException {
    ...
    Bucket bucket =
        storage.get(
            bucketName, Storage.GetBucketOption.ifMetagenerationNotMatch(bucketMetageneration));
  ...
  public void testBlobReload() throws Exception {
    ...
    StorageObject blobUnchanged = blob.refresh();
    assertEquals(blob, blobUnchanged);

    blob.getWriter().close();
    try {
      blob.refresh(StorageObject.BlobSourceOptions.ifGenerationMatch());
      fail("StorageException was expected");
    } catch (StorageException e) {
    ...
    StorageObject updated = blob.refresh();
  ...
  public void testUploadFromDownloadTo() throws Exception {
    String blobName = "test-uploadFrom-downloadIntoInternal-blob";
    BlobId blobId = BlobId.of(BUCKET, blobName);
    BlobInfo blobInfo = BlobInfo.newBuilder(blobId).build();
  ...
  public void testUploadWithEncryption() throws Exception {
    ...
    StorageObject blob = storage.createFrom(blobInfo, content, BlobWriteOptions.getEncryptionKey(KEY));
    ...
    byte[] readBytes = blob.getContent(StorageObject.BlobSourceOptions.customerSuppliedKey(KEY));
    assertArrayEquals(BLOB_BYTE_CONTENT, readBytes);
  }

  private StorageObject createBlob(String method, BlobInfo blobInfo, boolean detectType) throws IOException {
    switch (method) {
      case "create":
        return detectType
            ? storage.create(blobInfo, Storage.BlobTargetOptions.enableContentTypeDetection())
            : storage.create(blobInfo);
      case "createFrom":
        InputStream inputStream = new ByteArrayInputStream(BLOB_BYTE_CONTENT);
        return detectType
            ? storage.createFrom(blobInfo, inputStream, BlobWriteOptions.enableContentTypeDetection())
            : storage.createFrom(blobInfo, inputStream);
      case "getWriter":
        if (detectType) {
          storage.writer(blobInfo, BlobWriteOptions.enableContentTypeDetection()).close();
        } else {
  ...
  public void testRemoveBucketCORS() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      Bucket remoteBucket =
          storage.get(bucketName, Storage.GetBucketOption.withFields(BucketFields.CORS));
      ...
      updatedBucket = storage.get(bucketName, Storage.GetBucketOption.withFields(Storage.BucketFields.CORS));
      assertThat(updatedBucket.getCors()).isNull();
  ...
  public void testBlobTimeStorageClassUpdated() {
    ...
    StorageObject updatedBlob2 = updatedBlob1.toBuilder().setContentType(CONTENT_TYPE).build().save();
    ...
    assertThat(updatedBlob2.remove()).isTrue();
  }

  @Test
  public void testWriterWithKmsKeyName() throws IOException {
    // Write an empty object with a kmsKey.
    String blobName = "test-empty-blob";
    BlobInfo blobInfo = BlobInfo.newBuilder(BUCKET, blobName).build();
    StorageObject blob =
        storage.create(blobInfo, Storage.BlobTargetOptions.kmsKey(kmsKeyOneResourcePath));
