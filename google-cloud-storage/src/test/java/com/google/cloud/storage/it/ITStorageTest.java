...
import com.google.cloud.storage.ServiceAccountIdentity;
...
import com.google.cloud.storage.FormPostPolicyV4;
...
import com.google.cloud.storage.CloudStorageClient.BucketProperty;
...
public class ITStorageTest {
  ...
  private static final BucketInfo.LifecycleRuleDefinition LIFECYCLE_RULE_1 =
      new BucketInfo.LifecycleRuleDefinition(
          AbstractLifecycleAction.newSetStorageClassLifecycleAction(StorageTier.COLDLINE),
          LifecycleRuleDefinition.LifecycleRuleCondition.newLifecycleRuleBuilder()
              .setAge(1)
              .setNumberOfNewerVersions(3)
              .setIsLive(false)
              .setMatchesStorageClass(ImmutableList.of(StorageTier.COLDLINE))
              .buildLifecycleCondition());
  private static final BucketInfo.LifecycleRuleDefinition LIFECYCLE_RULE_2 =
      new LifecycleRuleDefinition(
          AbstractLifecycleAction.newRemoveAction(), BucketInfo.LifecycleRuleDefinition.LifecycleRuleCondition.newLifecycleRuleBuilder().setAge(1).buildLifecycleCondition());
            ...
            .setLifecycleRules(
                ImmutableList.of(
                    new LifecycleRuleDefinition(
                        ...
                        LifecycleRuleDefinition.LifecycleRuleCondition.newLifecycleRuleBuilder().setAge(1).buildLifecycleCondition())))
            .buildBucketInfo());
  ...
  private static void unsetRequesterPays() {
    StorageBucket remoteBucket =
        storage.get(
            ...
            CloudStorageClient.BucketGetOptions.selectFields(BucketProperty.ID, BucketProperty.BILLING),
            CloudStorageClient.BucketGetOptions.withUserProject(storage.getOptions().getProjectId()));
    ...
    if (remoteBucket.isRequesterPays() != null && remoteBucket.isRequesterPays() == true) {
      remoteBucket
          ...
          .buildBucketInfo()
          .updateBucket(CloudStorageClient.BucketTargetOptions.withUserProject(storage.getOptions().getProjectId()));
    }
  }
  ...
  public void testGetBucketSelectedFields() {
    StorageBucket remoteBucket = storage.get(BUCKET, CloudStorageClient.BucketGetOptions.selectFields(CloudStorageClient.BucketProperty.ID));
  ...
  public void testGetBucketAllSelectedFields() {
    StorageBucket remoteBucket = storage.get(BUCKET, CloudStorageClient.BucketGetOptions.selectFields(BucketProperty.values()));
    assertEquals(BUCKET, remoteBucket.getName());
  ...
  public void testGetBucketEmptyFields() {
    StorageBucket remoteBucket = storage.get(BUCKET, CloudStorageClient.BucketGetOptions.selectFields());
    assertEquals(BUCKET, remoteBucket.getName());
    assertNull(remoteBucket.getCreateTime());
    assertNull(remoteBucket.getSelfLink());
  }
  ...
  public void testGetBucketLifecycleRules() {
    ...
    storage.create(
        BucketInfo.newBucketBuilder(lifecycleTestBucketName)
            ...
            .buildBucketInfo());
    StorageBucket remoteBucket =
        storage.get(lifecycleTestBucketName, CloudStorageClient.BucketGetOptions.selectFields(BucketProperty.LIFECYCLE));
    ...
    try {
      assertTrue(
          lifecycleRule
              .getAction()
              .getActionType()
              .equals(BucketInfo.LifecycleRuleDefinition.SetStorageClassLifecycleOperation.TYPE));
  ...
  public void testClearBucketDefaultKmsKeyName() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      StorageBucket updatedBucket = remoteBucket.toBucketBuilder().setDefaultKmsKeyName(null).buildBucketInfo().updateBucket();
  ...
  public void testUpdateBucketDefaultKmsKeyName() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      StorageBucket updatedBucket =
          remoteBucket.toBucketBuilder().setDefaultKmsKeyName(kmsKeyTwoResourcePath).buildBucketInfo().updateBucket();
  ...
  public void testCreateBlob() {
    ...
    BlobMetadata blob =
        BlobMetadata.newBuilder(BUCKET, blobName).setCustomTime(System.currentTimeMillis()).buildObject();
    StorageBlob remoteBlob = storage.create(blob, BLOB_BYTE_CONTENT);
  ...
  public void testCreateGetBlobWithEncryptionKey() {
    ...
    StorageBlob remoteBlob =
        storage.create(blob, BLOB_BYTE_CONTENT, CloudStorageClient.BlobUploadOptions.customerSuppliedKey(KEY));
    assertNotNull(remoteBlob);
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
    byte[] readBytes =
        storage.readAllBytes(BUCKET, blobName, CloudStorageClient.BlobReadOption.customerSuppliedKey(BASE64_KEY));
    assertArrayEquals(BLOB_BYTE_CONTENT, readBytes);
    remoteBlob =
        storage.get(
            blob.getBlobId(),
            CloudStorageClient.BlobGetOptions.decryptionKeyBase64(BASE64_KEY),
            CloudStorageClient.BlobGetOptions.selectFields(BlobMetadataField.CRC32C, CloudStorageClient.BlobMetadataField.MD5HASH));
  ...
  public void testCreateBlobWithKmsKeyName() {
    ...
    StorageBlob remoteBlob =
        storage.create(
            blob, BLOB_BYTE_CONTENT, CloudStorageClient.BlobUploadOptions.withKmsKeyName(kmsKeyOneResourcePath));
    assertNotNull(remoteBlob);
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
  ...
  public void testCreateBlobWithKmsKeyNameAndCustomerSuppliedKey() {
    try {
      ...
      storage.create(
          blob,
          BLOB_BYTE_CONTENT,
          CloudStorageClient.BlobUploadOptions.customerSuppliedKey(KEY),
          CloudStorageClient.BlobUploadOptions.withKmsKeyName(kmsKeyOneResourcePath));
  ...
  public void testCreateEmptyBlob() {
    ...
    BlobMetadata blob = BlobMetadata.newBuilder(BUCKET, blobName).buildObject();
    StorageBlob remoteBlob = storage.create(blob);
    assertNotNull(remoteBlob);
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
    byte[] readBytes = storage.readAllBytes(BUCKET, blobName);
  ...
  public void testCreateBlobStream() {
    ...
    StorageBlob remoteBlob = storage.create(blob, stream);
    assertNotNull(remoteBlob);
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
  ...
  public void testCreateBlobStreamDisableGzipContent() {
    ...
    BlobMetadata blob = BlobMetadata.newBuilder(BUCKET, blobName).setContentType(CONTENT_TYPE).buildObject();
    ByteArrayInputStream stream = new ByteArrayInputStream(BLOB_STRING_CONTENT.getBytes(UTF_8));
    StorageBlob remoteBlob = storage.create(blob, stream, BlobWriteOptions.disableGzip());
    assertNotNull(remoteBlob);
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
    assertEquals(blob.getContentType(), remoteBlob.getContentType());
  ...
  public void testCreateBlobFail() {
    ...
    try {
      storage.create(
          wrongGenerationBlob, BLOB_BYTE_CONTENT, CloudStorageClient.BlobUploadOptions.ifGenerationMatch());
  ...
  public void testGetBlobEmptySelectedFields() {
    ...
    StorageBlob remoteBlob = storage.get(blob.getBlobId(), CloudStorageClient.BlobGetOptions.selectFields());
  ...
  public void testGetBlobKmsKeyNameField() {
    ...
    BlobMetadata blob = BlobMetadata.newBuilder(BUCKET, blobName).setContentType(CONTENT_TYPE).buildObject();
    assertNotNull(storage.create(blob, CloudStorageClient.BlobUploadOptions.withKmsKeyName(kmsKeyOneResourcePath)));
    StorageBlob remoteBlob =
  ...
  public void testListBlobsSelectedFields() throws InterruptedException {
    ...
    BlobMetadata blob2 =
        BlobMetadata.newBuilder(BUCKET, blobNames[1])
            ...
            .setMetadata(metadata)
            .buildObject();
    StorageBlob remoteBlob1 = storage.create(blob1);
    StorageBlob remoteBlob2 = storage.create(blob2);
    assertNotNull(remoteBlob1);
    assertNotNull(remoteBlob2);
    Page<StorageBlob> page =
        storage.list(
            BUCKET,
    ...
    Iterator<StorageBlob> iterator = page.iterateAll().iterator();
    while (iterator.hasNext()) {
      StorageBlob remoteBlob = iterator.next();
  ...
  public void testListBlobsKmsKeySelectedFields() throws InterruptedException {
    ...
    StorageBlob remoteBlob1 =
        storage.create(blob1, CloudStorageClient.BlobUploadOptions.withKmsKeyName(kmsKeyOneResourcePath));
    StorageBlob remoteBlob2 =
        storage.create(blob2, CloudStorageClient.BlobUploadOptions.withKmsKeyName(kmsKeyOneResourcePath));
    assertNotNull(remoteBlob1);
    assertNotNull(remoteBlob2);
    Page<StorageBlob> page =
        storage.list(
  ...
  public void testListBlobRequesterPays() throws InterruptedException {
    ...
    while (true) {
      ...
      List<StorageBlob> blobs = Lists.newArrayList(page.iterateAll());
  ...
  public void testListBlobsVersioned() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      StorageBlob remoteBlob1 = storage.create(blob1);
      StorageBlob remoteBlob2 = storage.create(blob2);
      StorageBlob remoteBlob3 = storage.create(blob2);
      assertNotNull(remoteBlob1);
      assertNotNull(remoteBlob2);
  ...
  public void testListBlobsWithOffset() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      BlobMetadata blob1 =
          BlobMetadata.newBuilder(bucket, blobNames.get(0)).setContentType(CONTENT_TYPE).buildObject();
      BlobMetadata blob2 =
          BlobMetadata.newBuilder(bucket, blobNames.get(1)).setContentType(CONTENT_TYPE).buildObject();
      BlobMetadata blob3 =
          BlobMetadata.newBuilder(bucket, blobNames.get(2)).setContentType(CONTENT_TYPE).buildObject();

      StorageBlob remoteBlob1 = storage.create(blob1);
      StorageBlob remoteBlob2 = storage.create(blob2);
      StorageBlob remoteBlob3 = storage.create(blob3);
      assertNotNull(remoteBlob1);
      assertNotNull(remoteBlob2);
      assertNotNull(remoteBlob3);

      // Listing blobs without BlobListOptions.
      Page<StorageBlob> page1 = storage.list(bucketName);
      assertEquals(3, Iterators.size(page1.iterateAll().iterator()));

      // Listing blobs with startOffset.
      Page<StorageBlob> page2 =
      ...
      Page<StorageBlob> page3 = storage.list(bucketName, CloudStorageClient.BlobListOptions.withEndOffset("endOffset"));
      assertEquals(1, Iterators.size(page3.iterateAll().iterator()));

      // Listing blobs with startOffset and endOffset.
      Page<StorageBlob> page4 =
          storage.list(
              bucketName,
  ...
  public void testListBlobsCurrentDirectory() throws InterruptedException {
    ...
    BlobMetadata blob1 = BlobMetadata.newBuilder(BUCKET, blobNames[0]).setContentType(CONTENT_TYPE).buildObject();
    BlobMetadata blob2 = BlobMetadata.newBuilder(BUCKET, blobNames[1]).setContentType(CONTENT_TYPE).buildObject();
    StorageBlob remoteBlob1 = storage.create(blob1, BLOB_BYTE_CONTENT);
    StorageBlob remoteBlob2 = storage.create(blob2, BLOB_BYTE_CONTENT);
    assertNotNull(remoteBlob1);
    assertNotNull(remoteBlob2);
    Page<StorageBlob> page =
        storage.list(
            ...
            CloudStorageClient.BlobListOptions.restrictToCurrentDirectory());
    // Listing blobs is eventually consistent, we loop until the list is of the expected size. The
    // test fails if timeout is reached.
    while (Iterators.size(page.iterateAll().iterator()) != 2) {
      Thread.sleep(500);
  ...
  public void testUpdateBlob() {
    ...
    StorageBlob remoteBlob = storage.create(blob);
    assertNotNull(remoteBlob);
    StorageBlob updatedBlob = remoteBlob.asBuilder().setContentType(CONTENT_TYPE).buildObject().updateInStorage();
  ...
  public void testUpdateBlobReplaceMetadata() {
    ...
    StorageBlob remoteBlob = storage.create(blob);
    assertNotNull(remoteBlob);
    StorageBlob updatedBlob = remoteBlob.asBuilder().setMetadata(null).buildObject().updateInStorage();
    assertNotNull(updatedBlob);
  ...
  public void testUpdateBlobMergeMetadata() {
    ...
    StorageBlob remoteBlob = storage.create(blob);
    assertNotNull(remoteBlob);
    StorageBlob updatedBlob = remoteBlob.asBuilder().setMetadata(newMetadata).buildObject().updateInStorage();
    assertNotNull(updatedBlob);
    assertEquals(blob.getName(), updatedBlob.getName());
    assertEquals(blob.getBucket(), updatedBlob.getBucket());
  ...
  public void testUpdateBlobFail() {
    ...
    try {
      storage.update(wrongGenerationBlob, CloudStorageClient.BlobUploadOptions.ifGenerationMatch());
      fail("StorageException was expected");
    } catch (StorageServiceException ex) {
      // expected
    }
  ...
  public void testComposeBlob() {
    ...
    StorageBlob remoteSourceBlob1 = storage.create(sourceBlob1, BLOB_BYTE_CONTENT);
    StorageBlob remoteSourceBlob2 = storage.create(sourceBlob2, BLOB_BYTE_CONTENT);
    ...
    StorageBlob remoteTargetBlob = storage.compose(req);
  ...
  public void testComposeBlobFail() {
    ...
    BlobMetadata sourceBlob1 = BlobMetadata.newBuilder(BUCKET, sourceBlobName1).buildObject();
    BlobMetadata sourceBlob2 = BlobMetadata.newBuilder(BUCKET, sourceBlobName2).buildObject();
    StorageBlob remoteSourceBlob1 = storage.create(sourceBlob1);
    StorageBlob remoteSourceBlob2 = storage.create(sourceBlob2);
    assertNotNull(remoteSourceBlob1);
    assertNotNull(remoteSourceBlob2);
  ...
  public void testCopyBlob() {
    ...
    CloudStorageClient.CopyOperationRequest req = CloudStorageClient.CopyOperationRequest.create(source, BlobIdentifier.create(BUCKET, targetBlobName));
  ...
  public void testCopyBlobWithPredefinedAcl() {
    ...
    CloudStorageClient.CopyOperationRequest req =
        CloudStorageClient.CopyOperationRequest.builder()
            ...
            .setTarget(
                ...
                CloudStorageClient.BlobUploadOptions.withPredefinedAcl(CloudStorageClient.PredefinedAccessControlList.PUBLIC_READ))
            .create();
    BlobCopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
    assertEquals(CONTENT_TYPE, copyWriter.getResult().getContentType());
    assertEquals(metadata, copyWriter.getResult().getMetadata());
    assertNotNull(copyWriter.getResult().getAcl(AccessControlEntry.UserPrincipal.allUsers()));
    assertTrue(copyWriter.isDone());
    assertTrue(remoteBlob.deleteFromStorage());
    assertTrue(storage.delete(BUCKET, targetBlobName));
  }
  ...
  public void testCopyBlobWithEncryptionKeys() {
    ...
    CloudStorageClient.CopyOperationRequest req =
        CloudStorageClient.CopyOperationRequest.builder()
            .setSource(source)
            .setTarget(target, CloudStorageClient.BlobUploadOptions.customerSuppliedKey(OTHER_BASE64_KEY))
    ...
    assertArrayEquals(
        BLOB_BYTE_CONTENT,
        copyWriter.getResult().getContent(StorageBlob.BlobSourceOptions.decryptionKeyBase64(OTHER_BASE64_KEY)));
    assertEquals(metadata, copyWriter.getResult().getMetadata());
    assertTrue(copyWriter.isDone());
    req =
        CloudStorageClient.CopyOperationRequest.builder()
  ...
  public void testRotateFromCustomerEncryptionToKmsKey() {
    ...
    CloudStorageClient.CopyOperationRequest req =
        CloudStorageClient.CopyOperationRequest.builder()
            .setSource(source)
            .setSourceOptions(CloudStorageClient.BlobReadOption.customerSuppliedKey(BASE64_KEY))
            .setTarget(target, CloudStorageClient.BlobUploadOptions.withKmsKeyName(kmsKeyOneResourcePath))
            .create();
    BlobCopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
  ...
  public void testCopyBlobUpdateMetadata() {
    ...
    BlobIdentifier source = BlobIdentifier.create(BUCKET, sourceBlobName);
    StorageBlob remoteSourceBlob = storage.create(BlobMetadata.newBuilder(source).buildObject(), BLOB_BYTE_CONTENT);
  ...
  public void testCopyBlobUpdateStorageClass() {
    ...
    BlobMetadata sourceInfo =
        BlobMetadata.newBuilder(source).setStorageClass(StorageTier.STANDARD).buildObject();
    StorageBlob remoteSourceBlob = storage.create(sourceInfo, BLOB_BYTE_CONTENT);
    assertNotNull(remoteSourceBlob);
    ...
    BlobMetadata targetInfo =
        BlobMetadata.newBuilder(BUCKET, targetBlobName).setStorageClass(StorageTier.COLDLINE).buildObject();
  ...
  public void testBatchRequest() {
    ...
    BlobMetadata updatedBlob1 = sourceBlob1.asBuilder().setContentType(CONTENT_TYPE).buildObject();
    BlobMetadata updatedBlob2 = sourceBlob2.asBuilder().setContentType(CONTENT_TYPE).buildObject();
    StorageOperationBatch updateBatch = storage.batch();
    StorageBatchResult<StorageBlob> updateResult1 = updateBatch.patch(updatedBlob1);
    StorageBatchResult<StorageBlob> updateResult2 = updateBatch.patch(updatedBlob2);
    updateBatch.execute();
    StorageBlob remoteUpdatedBlob1 = updateResult1.get();
    StorageBlob remoteUpdatedBlob2 = updateResult2.get();
    ...
    StorageBatchResult<StorageBlob> getResult1 = getBatch.get(BUCKET, sourceBlobName1);
    StorageBatchResult<StorageBlob> getResult2 = getBatch.get(BUCKET, sourceBlobName2);
    getBatch.execute();
    StorageBlob remoteBlob1 = getResult1.get();
    StorageBlob remoteBlob2 = getResult2.get();
  ...
  public void testBatchRequestManyOperations() {
    ...
    List<StorageBatchResult<StorageBlob>> getResults = Lists.newArrayListWithCapacity(MAX_BATCH_SIZE / 2);
    List<StorageBatchResult<StorageBlob>> updateResults =
    ...
    StorageBatchResult<StorageBlob> getResult = batch.get(BUCKET, sourceBlobName1);
    StorageBatchResult<StorageBlob> updateResult = batch.patch(updatedBlob2);
    ...
    // Check gets
    for (StorageBatchResult<StorageBlob> failedGetResult : getResults) {
      assertNull(failedGetResult.get());
    }
    StorageBlob remoteBlob1 = getResult.get();
    assertEquals(sourceBlob1.getBucket(), remoteBlob1.getBucket());
    assertEquals(sourceBlob1.getName(), remoteBlob1.getName());

    // Check updates
    for (StorageBatchResult<StorageBlob> failedUpdateResult : updateResults) {
      try {
    ...
    StorageBlob remoteUpdatedBlob2 = updateResult.get();
    assertEquals(sourceBlob2.getBucket(), remoteUpdatedBlob2.getBucket());
    assertEquals(sourceBlob2.getName(), remoteUpdatedBlob2.getName());
    assertEquals(updatedBlob2.getContentType(), remoteUpdatedBlob2.getContentType());
  }
  ...
  public void testBatchRequestFail() {
    ...
    StorageOperationBatch batch = storage.batch();
    StorageBatchResult<StorageBlob> updateResult =
        batch.patch(updatedBlob, CloudStorageClient.BlobUploadOptions.ifGenerationMatch());
    ...
    StorageBatchResult<StorageBlob> getResult1 =
        batch.get(BUCKET, blobName, CloudStorageClient.BlobGetOptions.ifGenerationMatch(-1L));
    StorageBatchResult<StorageBlob> getResult2 = batch.get(BlobIdentifier.create(BUCKET, blobName, -1L));
    batch.execute();
    try {
      updateResult.get();
      fail("Expected StorageException");
  ...
  public void testReadAndWriteChannelWithEncryptionKey() throws IOException {
    ...
    try (WriteChannel writer =
        storage.writer(blob, BlobWriteOptions.customerSuppliedKey(BASE64_KEY))) {
      stringBytes = BLOB_STRING_CONTENT.getBytes(UTF_8);
      writer.write(ByteBuffer.wrap(BLOB_BYTE_CONTENT));
      writer.write(ByteBuffer.wrap(stringBytes));
    }
    ...
    try (ReadChannel reader =
        storage.reader(blob.getBlobId(), CloudStorageClient.BlobReadOption.customerSuppliedKey(KEY))) {
      readBytes = ByteBuffer.allocate(BLOB_BYTE_CONTENT.length);
      readStringBytes = ByteBuffer.allocate(stringBytes.length);
      reader.read(readBytes);
      reader.read(readStringBytes);
  ...
  public void testReadChannelFailUpdatedGeneration() throws IOException {
    ...
    StorageBlob remoteBlob = storage.create(blob, content);
    assertNotNull(remoteBlob);
  ...
  public void testWriteChannelWithConnectionPool() throws IOException {
    ...
    CloudStorageClient storageWithPool =
        StorageClientOptions.createBuilder().setTransportOptions(transportOptions).build().getService();
  ...
  public void testPostSignedUrl() throws IOException {
    ...
    for (CloudStorageClient.UrlSigningOption urlStyle :
      ...
      connection.connect();
      StorageBlob remoteBlob = storage.get(BUCKET, blobName);
      assertNotNull(remoteBlob);
      assertEquals(blob.getBucket(), remoteBlob.getBucket());
      assertEquals(blob.getName(), remoteBlob.getName());
    }
  ...
  public void testGetBlobs() {
    ...
    assertNotNull(storage.create(sourceBlob1));
    assertNotNull(storage.create(sourceBlob2));
    List<StorageBlob> remoteBlobs = storage.get(sourceBlob1.getBlobId(), sourceBlob2.getBlobId());
  ...
  public void testDownloadPublicBlobWithoutAuthentication() {
    ...
    int numBlobs = 0;
    Iterator<StorageBlob> blobIterator =
        unauthorizedStorage
  ...
  public void testUpdateBlobs() {
    ...
    BlobMetadata sourceBlob1 = BlobMetadata.newBuilder(BUCKET, sourceBlobName1).buildObject();
    BlobMetadata sourceBlob2 = BlobMetadata.newBuilder(BUCKET, sourceBlobName2).buildObject();
    StorageBlob remoteBlob1 = storage.create(sourceBlob1);
    StorageBlob remoteBlob2 = storage.create(sourceBlob2);
    assertNotNull(remoteBlob1);
    assertNotNull(remoteBlob2);
    List<StorageBlob> updatedBlobs =
        storage.update(
  ...
  private void testBucketAclRequesterPays(boolean requesterPays) {
    ...
    CloudStorageClient.BucketSourceRequestOption[] bucketOptions =
        requesterPays
            ? new CloudStorageClient.BucketSourceRequestOption[] {CloudStorageClient.BucketSourceRequestOption.withUserProject(projectId)}
            : new CloudStorageClient.BucketSourceRequestOption[] {};

    assertNull(
        storage.getAcl(BUCKET_REQUESTER_PAYS, AccessControlEntry.UserPrincipal.allAuthenticatedUsers(), bucketOptions));
    assertFalse(
        storage.deleteAcl(BUCKET_REQUESTER_PAYS, UserPrincipal.allAuthenticatedUsers(), bucketOptions));
    AccessControlEntry acl = AccessControlEntry.create(UserPrincipal.allAuthenticatedUsers(), UserRole.READER);
    assertNotNull(storage.createAcl(BUCKET_REQUESTER_PAYS, acl, bucketOptions));
    AccessControlEntry updatedAcl =
        storage.updateAcl(
            BUCKET_REQUESTER_PAYS, acl.toEntityBuilder().setRole(AccessControlEntry.UserRole.WRITER).buildEntry(), bucketOptions);
    assertEquals(AccessControlEntry.UserRole.WRITER, updatedAcl.getRole());
    Set<AccessControlEntry> acls = new HashSet<>();
    acls.addAll(storage.listAcls(BUCKET_REQUESTER_PAYS, bucketOptions));
    assertTrue(acls.contains(updatedAcl));
    assertTrue(
        storage.deleteAcl(BUCKET_REQUESTER_PAYS, AccessControlEntry.UserPrincipal.allAuthenticatedUsers(), bucketOptions));
    assertNull(
        storage.getAcl(BUCKET_REQUESTER_PAYS, UserPrincipal.allAuthenticatedUsers(), bucketOptions));
    if (requesterPays) {
      StorageBucket remoteBucket =
          storage.get(
              BUCKET_REQUESTER_PAYS,
              CloudStorageClient.BucketGetOptions.selectFields(CloudStorageClient.BucketProperty.ID, BucketProperty.BILLING),
              CloudStorageClient.BucketGetOptions.withUserProject(projectId));
      assertTrue(remoteBucket.isRequesterPays());
      remoteBucket = remoteBucket.toBucketBuilder().setRequesterPays(false).buildBucketInfo();
      StorageBucket updatedBucket =
          storage.update(remoteBucket, CloudStorageClient.BucketTargetOptions.withUserProject(projectId));
  ...
  public void testBucketDefaultAcl() {
    assertNull(storage.getDefaultAcl(BUCKET, AccessControlEntry.UserPrincipal.allAuthenticatedUsers()));
    assertFalse(storage.deleteDefaultAcl(BUCKET, AccessControlEntry.UserPrincipal.allAuthenticatedUsers()));
    AccessControlEntry acl = AccessControlEntry.create(UserPrincipal.allAuthenticatedUsers(), AccessControlEntry.UserRole.READER);
    ...
    assertTrue(acls.contains(updatedAcl));
    assertTrue(storage.deleteDefaultAcl(BUCKET, UserPrincipal.allAuthenticatedUsers()));
    assertNull(storage.getDefaultAcl(BUCKET, AccessControlEntry.UserPrincipal.allAuthenticatedUsers()));
  }
  ...
  public void testBlobAcl() {
    ...
    storage.create(blob);
    assertNull(storage.getAcl(blobId, UserPrincipal.allAuthenticatedUsers()));
    AccessControlEntry acl = AccessControlEntry.create(AccessControlEntry.UserPrincipal.allAuthenticatedUsers(), AccessControlEntry.UserRole.READER);
    assertNotNull(storage.createAcl(blobId, acl));
    AccessControlEntry updatedAcl = storage.updateAcl(blobId, acl.toEntityBuilder().setRole(UserRole.OWNER).buildEntry());
    assertEquals(UserRole.OWNER, updatedAcl.getRole());
    Set<AccessControlEntry> acls = new HashSet<>(storage.listAcls(blobId));
    assertTrue(acls.contains(updatedAcl));
    assertTrue(storage.deleteAcl(blobId, AccessControlEntry.UserPrincipal.allAuthenticatedUsers()));
    ...
    try {
      assertNull(storage.getAcl(otherBlobId, UserPrincipal.allAuthenticatedUsers()));
      fail("Expected an 'Invalid argument' exception");
    } catch (StorageServiceException e) {
      assertThat(e.getMessage()).contains("Invalid argument");
    }
    ...
    try {
      assertFalse(storage.deleteAcl(otherBlobId, UserPrincipal.allAuthenticatedUsers()));
      fail("Expected an 'Invalid argument' exception");
    } catch (StorageServiceException e) {
      assertThat(e.getMessage()).contains("Invalid argument");
    }
  ...
  public void testHmacKey() {
    ServiceAccountIdentity serviceAccount = ServiceAccountIdentity.create(System.getenv("IT_SERVICE_ACCOUNT_EMAIL"));
    try {
      ...
      Page<HmacSecretKey.HmacKeyInfo> metadatas =
          storage.listHmacKeys(CloudStorageClient.HmacKeysListOption.withServiceAccount(serviceAccount));
      ...
      metadatas = storage.listHmacKeys(CloudStorageClient.HmacKeysListOption.withServiceAccount(serviceAccount));
      ...
      metadatas =
          storage.listHmacKeys(
              CloudStorageClient.HmacKeysListOption.withServiceAccount(serviceAccount),
              CloudStorageClient.HmacKeysListOption.withMaxResults(2L));
      ...
      metadatas =
          storage.listHmacKeys(
              CloudStorageClient.HmacKeysListOption.withServiceAccount(serviceAccount),
              CloudStorageClient.HmacKeysListOption.withMaxResults(2L),
              CloudStorageClient.HmacKeysListOption.withPageToken(nextPageToken));
  ...
  public void testReadCompressedBlob() throws IOException {
    ...
    BlobMetadata blobInfo =
        BlobMetadata.newBuilder(BlobIdentifier.create(BUCKET, blobName))
            ...
            .buildObject();
    StorageBlob blob = storage.create(blobInfo, COMPRESSED_CONTENT);
  ...
  public void testBucketPolicyV1RequesterPays() throws ExecutionException, InterruptedException {
    ...
    StorageBucket bucketFalse =
        storage.update(
            ...
            CloudStorageClient.BucketTargetOptions.withUserProject(projectId));
  ...
  public void testBucketPolicyV1() {
    ...
    CloudStorageClient.BucketSourceRequestOption[] bucketOptions = new CloudStorageClient.BucketSourceRequestOption[] {};
    Identity projectOwner = Identity.projectOwner(projectId);
    Identity projectEditor = Identity.projectEditor(projectId);
    Identity projectViewer = Identity.projectViewer(projectId);
    Map<com.google.cloud.Role, Set<Identity>> bindingsWithoutPublicRead =
  ...
  public void testBucketPolicyV3() {
    ...
    CloudStorageClient.BucketSourceRequestOption[] bucketOptions =
        new CloudStorageClient.BucketSourceRequestOption[] {CloudStorageClient.BucketSourceRequestOption.withRequestedPolicyVersion(3)};
    Identity projectOwner = Identity.projectOwner(projectId);
    Identity projectEditor = Identity.projectEditor(projectId);
    Identity projectViewer = Identity.projectViewer(projectId);
  ...
  public void testUpdateBucketLabel() {
    StorageBucket remoteBucket =
        storage.get(BUCKET, CloudStorageClient.BucketGetOptions.selectFields(CloudStorageClient.BucketProperty.ID, CloudStorageClient.BucketProperty.BILLING));
    ...
    remoteBucket.toBucketBuilder().setLabels(REMOVE_BUCKET_LABELS).buildBucketInfo().updateBucket();
  ...
  public void testUpdateBucketRequesterPays() {
    ...
    StorageBlob remoteBlob = updatedBucket.createBlob(blobName, BLOB_BYTE_CONTENT, option);
    assertNotNull(remoteBlob);
    byte[] readBytes =
    ...
    assertArrayEquals(BLOB_BYTE_CONTENT, readBytes);
    remoteBucket = remoteBucket.toBucketBuilder().setRequesterPays(false).buildBucketInfo();
    updatedBucket = storage.update(remoteBucket, CloudStorageClient.BucketTargetOptions.withUserProject(projectId));
    assertFalse(updatedBucket.isRequesterPays());
  }
      ...
      Iterator<StorageBucket> bucketIterator =
          storage
              .list(
                  ...
                  CloudStorageClient.BucketListOptions.selectFields(BucketProperty.ENCRYPTION))
              .iterateAll()
              .iterator();
      while (!bucketIterator.hasNext()) {
        Thread.sleep(500);
  ...
  public void testRetentionPolicyNoLock() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      remoteBucket =
          storage.get(bucketName, CloudStorageClient.BucketGetOptions.selectFields(BucketProperty.RETENTION_POLICY));
      assertEquals(RETENTION_PERIOD, remoteBucket.getRetentionPeriod());
      assertNotNull(remoteBucket.getRetentionEffectiveTime());
      assertNull(remoteBucket.isRetentionPolicyLocked());
      ...
      StorageBlob remoteBlob = storage.create(blobInfo);
      assertNotNull(remoteBlob.getRetentionExpirationTime());
      remoteBucket = remoteBucket.toBucketBuilder().setRetentionPeriod(null).buildBucketInfo().updateBucket();
    ...
    try {
      ...
      if (requesterPays) {
        ...
        StorageBucket updateBucket =
            storage.update(bucketInfo, CloudStorageClient.BucketTargetOptions.withUserProject(projectId));
  ...
  public void testEnableDisableBucketDefaultEventBasedHold()
    ...
    try {
      ...
      remoteBucket =
          storage.get(
              bucketName, CloudStorageClient.BucketGetOptions.selectFields(BucketProperty.DEFAULT_EVENT_BASED_HOLD));
      assertTrue(remoteBucket.getDefaultEventBasedHold());
      ...
      remoteBucket = remoteBucket.toBucketBuilder().setDefaultEventBasedHold(false).buildBucketInfo().updateBucket();
  ...
  public void testAttemptDeletionObjectTemporaryHold() {
    ...
    try {
      ...
      remoteBlob.asBuilder().setTemporaryHold(false).buildObject().updateInStorage();
    }
  }
  ...
  public void testBucketWithBucketPolicyOnlyEnabled() throws Exception {
    ...
    try {
      ...
      StorageBucket remoteBucket =
          storage.get(bucket, CloudStorageClient.BucketGetOptions.selectFields(BucketProperty.IAMCONFIGURATION));
  ...
  public void testEnableAndDisableBucketPolicyOnlyOnExistingBucket() throws Exception {
    ...
    try {
      ...
      StorageBucket bucket =
          storage.create(
              StorageBucket.newBucketBuilder(bpoBucket)
                  .setAcl(ImmutableList.of(AccessControlEntry.create(UserPrincipal.allAuthenticatedUsers(), AccessControlEntry.UserRole.READER)))
                  .setDefaultAcl(
                      ImmutableList.of(AccessControlEntry.create(UserPrincipal.allAuthenticatedUsers(), AccessControlEntry.UserRole.READER)))
                  .buildBucketInfo());

      BucketInfo.IamSettings bpoEnabledIamConfiguration =
          BucketInfo.IamSettings.newIamSettingsBuilder().setIsBucketPolicyOnlyEnabled(true).buildIamSettings();
      bucket
          .toBucketBuilder()
          .setAcl(null)
          .setDefaultAcl(null)
      ...
      StorageBucket remoteBucket =
          storage.get(bpoBucket, CloudStorageClient.BucketGetOptions.selectFields(CloudStorageClient.BucketProperty.IAMCONFIGURATION));
      ...
      remoteBucket
          .toBucketBuilder()
          .setIamConfiguration(
              bpoEnabledIamConfiguration.toIamSettingsBuilder().setIsBucketPolicyOnlyEnabled(false).buildIamSettings())
          .buildBucketInfo()
          .updateBucket();
      ...
      remoteBucket =
          storage.get(
              ...
              CloudStorageClient.BucketGetOptions.selectFields(
                  BucketProperty.IAMCONFIGURATION, CloudStorageClient.BucketProperty.ACL, BucketProperty.DEFAULT_OBJECT_ACL));
      ...
      assertEquals(AccessControlEntry.UserRole.READER, remoteBucket.getAcl().get(0).getRole());
    } finally {
  ...
  public void testEnableAndDisableUniformBucketLevelAccessOnExistingBucket() throws Exception {
    String bpoBucket = RemoteStorageHelper.generateBucketName();
    try {
      BucketInfo.IamSettings ublaDisabledIamConfiguration =
          BucketInfo.IamSettings.newIamSettingsBuilder()
              .setIsUniformBucketLevelAccessEnabled(false)
              .buildIamSettings();
      StorageBucket bucket =
          storage.create(
              StorageBucket.newBucketBuilder(bpoBucket)
                  ...
                  .setAcl(ImmutableList.of(AccessControlEntry.create(AccessControlEntry.UserPrincipal.allAuthenticatedUsers(), UserRole.READER)))
                  .setDefaultAcl(
                      ImmutableList.of(AccessControlEntry.create(UserPrincipal.allAuthenticatedUsers(), UserRole.READER)))
                  .buildBucketInfo());
      ...
      remoteBucket.toBucketBuilder().setIamConfiguration(ublaDisabledIamConfiguration).buildBucketInfo().updateBucket();

      remoteBucket =
          storage.get(
              bpoBucket,
              CloudStorageClient.BucketGetOptions.selectFields(
                  CloudStorageClient.BucketProperty.IAMCONFIGURATION, BucketProperty.ACL, BucketProperty.DEFAULT_OBJECT_ACL));

      assertFalse(remoteBucket.getIamConfiguration().isUniformBucketLevelAccessEnabled());
      assertEquals(UserPrincipal.allAuthenticatedUsers(), remoteBucket.getDefaultAcl().get(0).getEntity());
      assertEquals(AccessControlEntry.UserRole.READER, remoteBucket.getDefaultAcl().get(0).getRole());
      assertEquals(UserPrincipal.allAuthenticatedUsers(), remoteBucket.getAcl().get(0).getEntity());
  ...
  public void testUploadUsingSignedURL() throws Exception {
    ...
    for (CloudStorageClient.UrlSigningOption urlStyle :
      ...
      StorageBlob blobToRead = storage.get(blobId);
  ...
  public void testBucketLogging() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      StorageBucket updatedBucket = bucket.toBucketBuilder().setLogging(null).buildBucketInfo().updateBucket();
  ...
  public void testSignedPostPolicyV4() throws Exception {
    PostFieldsMapV4 fields = PostFieldsMapV4.builder().setAcl("public-read").create();

    FormPostPolicyV4 policy =
        storage.generateSignedPostPolicyV4(
            BlobMetadata.newBuilder(BUCKET, "my-object").buildObject(), 7, TimeUnit.DAYS, fields);
  ...
  public void testBlobReload() throws Exception {
    ...
    BlobIdentifier blobId = BlobIdentifier.create(BUCKET, blobName);
    BlobMetadata blobInfo = BlobMetadata.newBuilder(blobId).buildObject();
    StorageBlob blob = storage.create(blobInfo, new byte[] {0, 1, 2});

    StorageBlob blobUnchanged = blob.reloadFromStorage();
    assertEquals(blob, blobUnchanged);

    blob.openWriter().close();
    try {
      blob.reloadFromStorage(StorageBlob.BlobSourceOptions.ifGenerationMatch());
      fail("StorageException was expected");
    } catch (StorageServiceException e) {
    ...
    StorageBlob updated = blob.reloadFromStorage();
  ...
  public void testDeleteLifecycleRules() throws ExecutionException, InterruptedException {
    ...
    try {
      StorageBucket updatedBucket = bucket.toBucketBuilder().deleteLifecycleRules().buildBucketInfo().updateBucket();
  ...
  public void testUploadFromDownloadTo() throws Exception {
    ...
    StorageBlob blob = storage.createFrom(blobInfo, tempFileFrom);
  ...
  public void testUploadWithEncryption() throws Exception {
    ...
    StorageBlob blob = storage.createFrom(blobInfo, content, CloudStorageClient.BlobWriteOptions.customerSuppliedKey(KEY));
    ...
    byte[] readBytes = blob.getContent(StorageBlob.BlobSourceOptions.decryptionKeyBase64(KEY));
    assertArrayEquals(BLOB_BYTE_CONTENT, readBytes);
  }

  private StorageBlob createBlob(String method, BlobMetadata blobInfo, boolean detectType) throws IOException {
    switch (method) {
      case "create":
        return detectType
            ? storage.create(blobInfo, CloudStorageClient.BlobUploadOptions.autoDetectContentType())
            : storage.create(blobInfo);
      case "createFrom":
        InputStream inputStream = new ByteArrayInputStream(BLOB_BYTE_CONTENT);
        return detectType
            ? storage.createFrom(blobInfo, inputStream, BlobWriteOptions.enableDetectContentType())
            : storage.createFrom(blobInfo, inputStream);
      case "writer":
        if (detectType) {
          storage.writer(blobInfo, BlobWriteOptions.enableDetectContentType()).close();
        } else {
  ...
  private void testAutoContentType(String method) throws IOException {
    ...
    for (int i = 0; i < names.length; i++) {
      ...
      BlobMetadata blobInfo = BlobMetadata.newBuilder(blobId).buildObject();
      StorageBlob blob_true = createBlob(method, blobInfo, true);
      assertEquals(types[i], blob_true.getContentType());

      StorageBlob blob_false = createBlob(method, blobInfo, false);
    ...
    BlobMetadata blobInfo = BlobMetadata.newBuilder(blobId).setContentType(customType).buildObject();
    StorageBlob blob = createBlob(method, blobInfo, true);
  ...
  public void testRemoveBucketCORS() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      StorageBucket remoteBucket =
          storage.get(bucketName, CloudStorageClient.BucketGetOptions.selectFields(BucketProperty.CORS));
      ...
      StorageBucket updatedBucket = remoteBucket.toBucketBuilder().setCors(null).buildBucketInfo().updateBucket();
      assertThat(updatedBucket.getCors()).isNull();

      // case-3 : Cors are not set and field selector is selected then returns null.
      updatedBucket = storage.get(bucketName, CloudStorageClient.BucketGetOptions.selectFields(BucketProperty.CORS));
      assertThat(updatedBucket.getCors()).isNull();
  ...
  public void testBucketUpdateTime() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      StorageBucket updatedBucket = bucket.toBucketBuilder().setVersioningEnabled(false).buildBucketInfo().updateBucket();
  ...
  public void testBlobTimeStorageClassUpdated() {
    ...
    StorageTier storageClass = StorageTier.COLDLINE;
    BlobMetadata blob = BlobMetadata.newBuilder(BUCKET, blobName).setStorageClass(storageClass).buildObject();
    StorageBlob remoteBlob = storage.create(blob);
    ...
    CloudStorageClient.CopyOperationRequest request =
        CloudStorageClient.CopyOperationRequest.builder()
            .setSource(blobId)
            .setTarget(BlobMetadata.newBuilder(blobId).setStorageClass(StorageTier.STANDARD).buildObject())
            .create();
    StorageBlob updatedBlob1 = storage.copy(request).getResult();
    ...
    StorageBlob updatedBlob2 = updatedBlob1.asBuilder().setContentType(CONTENT_TYPE).buildObject().updateInStorage();
  ...
  public void testWriterWithKmsKeyName() throws IOException {
    ...
    BlobMetadata blobInfo = BlobMetadata.newBuilder(BUCKET, blobName).buildObject();
    StorageBlob blob =
        storage.create(blobInfo, CloudStorageClient.BlobUploadOptions.withKmsKeyName(kmsKeyOneResourcePath));

    // Create a writer using blob that already has metadata received from Storage API.
    int numberOfBytes;
    try (WriteChannel writer = blob.openWriter()) {
