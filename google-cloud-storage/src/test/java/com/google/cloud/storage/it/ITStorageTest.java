...
import com.google.cloud.storage.StorageClient.BucketMetadataField;
...
public class ITStorageTest {
            ...
            .setLifecycleRules(
                ImmutableList.of(
                    new BucketMetadata.LifecycleRuleDefinition(
                        ...
                        LifecycleRuleCondition.newLifecycleConditionBuilder().setAge(1).buildLifecycleCondition())))
            .buildBucket());
  ...
  public void beforeEach() {
    StorageBucket remoteBucket =
        storage.get(
            BUCKET,
            StorageClient.BucketGetOptions.withFields(StorageClient.BucketMetadataField.ID, BucketMetadataField.BILLING),
    ...
    if (remoteBucket.isRequesterPays() != null && remoteBucket.isRequesterPays() == true) {
      remoteBucket
          ...
          .buildBucket()
          .updateBucket(StorageClient.BucketTargetRequestOption.withUserProject(storage.getOptions().getProjectId()));
    }
  }
  ...
  public void testListBuckets() throws InterruptedException {
    Iterator<StorageBucket> bucketIterator =
        storage
            .list(StorageClient.BucketListOptions.namePrefix(BUCKET), StorageClient.BucketListOptions.fieldsSelector())
            .iterateAll()
            .iterator();
  ...
  public void testGetBucketSelectedFields() {
    StorageBucket remoteBucket = storage.get(BUCKET, StorageClient.BucketGetOptions.withFields(BucketMetadataField.ID));
  ...
  public void testGetBucketAllSelectedFields() {
    StorageBucket remoteBucket = storage.get(BUCKET, StorageClient.BucketGetOptions.withFields(BucketMetadataField.values()));
    assertEquals(BUCKET, remoteBucket.getName());
  ...
  public void testGetBucketLifecycleRules() {
    ...
    storage.create(
        BucketMetadata.newBucketBuilder(lifecycleTestBucketName)
            ...
            .buildBucket());
    StorageBucket remoteBucket =
        storage.get(lifecycleTestBucketName, StorageClient.BucketGetOptions.withFields(BucketMetadataField.LIFECYCLE));
  ...
  public void testCreateBlob() {
    ...
    BlobMetadata blob = BlobMetadata.newBuilder(BUCKET, blobName).buildStorageObject();
    ...
    assertTrue(remoteBlob.deleteBlob());
  }
  ...
  public void testCreateGetBlobWithEncryptionKey() {
    ...
    assertEquals(blob.getName(), remoteBlob.getName());
    byte[] readBytes =
        storage.readAllBytes(BUCKET, blobName, StorageClient.BlobSourceOptions.decryptionKeyBase64(BASE64_KEY));
    assertArrayEquals(BLOB_BYTE_CONTENT, readBytes);
    remoteBlob =
        storage.get(
            blob.getBlobId(),
            StorageClient.BlobGetOptions.decryptionKeyBase64(BASE64_KEY),
            StorageClient.BlobGetOptions.fieldsSelector(BlobMetadataField.CRC32C, BlobMetadataField.MD5HASH));
  ...
  public void testCreateBlobStream() {
    ...
    BlobMetadata blob = BlobMetadata.newBuilder(BUCKET, blobName).setContentType(CONTENT_TYPE).buildStorageObject();
  ...
  public void testCreateBlobStreamDisableGzipContent() {
    ...
    BlobMetadata blob = BlobMetadata.newBuilder(BUCKET, blobName).setContentType(CONTENT_TYPE).buildStorageObject();
    ByteArrayInputStream stream = new ByteArrayInputStream(BLOB_STRING_CONTENT.getBytes(UTF_8));
    StorageBlob remoteBlob = storage.create(blob, stream, StorageClient.BlobWriteSetting.disableGzip());
    assertNotNull(remoteBlob);
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
    assertEquals(blob.getContentType(), remoteBlob.getContentType());
  ...
  public void testCreateBlobFail() {
    ...
    StorageBlob remoteBlob = storage.create(blob);
    assertNotNull(remoteBlob);
    BlobMetadata wrongGenerationBlob = BlobMetadata.newBuilder(BUCKET, blobName, -1L).buildStorageObject();
    try {
      storage.create(
          wrongGenerationBlob, BLOB_BYTE_CONTENT, StorageClient.BlobUploadOption.ifGenerationMatch());
  ...
  public void testCreateBlobMd5Fail() {
    ...
    ByteArrayInputStream stream = new ByteArrayInputStream(BLOB_STRING_CONTENT.getBytes(UTF_8));
    try {
      storage.create(blob, stream, BlobWriteSetting.md5Equals());
      fail("StorageException was expected");
    } catch (StorageOperationException ex) {
      // expected
    }
  ...
  public void testGetBlobEmptySelectedFields() {
    ...
    StorageBlob remoteBlob = storage.get(blob.getBlobId(), StorageClient.BlobGetOptions.fieldsSelector());
  ...
  public void testGetBlobSelectedFields() {
    ...
    assertNotNull(storage.create(blob));
    StorageBlob remoteBlob =
        storage.get(blob.getBlobId(), StorageClient.BlobGetOptions.fieldsSelector(BlobMetadataField.METADATA));
    assertEquals(blob.getBlobId(), remoteBlob.getBlobId());
  ...
  public void testGetBlobKmsKeyNameField() {
    ...
    StorageBlob remoteBlob =
        storage.get(blob.getBlobId(), StorageClient.BlobGetOptions.fieldsSelector(BlobMetadataField.KMS_KEY_NAME));
    assertEquals(blob.getBlobId(), remoteBlob.getBlobId());
    assertTrue(remoteBlob.getKmsKeyName().startsWith(kmsKeyOneResourcePath));
    assertNull(remoteBlob.getContentType());
  }
  ...
  public void testGetBlobAllSelectedFields() {
    ...
    assertNotNull(storage.create(blob));
    StorageBlob remoteBlob =
        storage.get(blob.getBlobId(), StorageClient.BlobGetOptions.fieldsSelector(BlobMetadataField.values()));
    assertEquals(blob.getBucket(), remoteBlob.getBucket());
    assertEquals(blob.getName(), remoteBlob.getName());
    assertEquals(ImmutableMap.of("k", "v"), remoteBlob.getMetadata());
  ...
  public void testListBlobsSelectedFields() throws InterruptedException {
    ...
    Page<StorageBlob> page =
        storage.list(
            BUCKET,
            StorageClient.BlobListOptions.namePrefix("test-list-blobs-selected-fields-blob"),
            StorageClient.BlobListOptions.selectFields(BlobMetadataField.METADATA));
  ...
  public void testListBlobsKmsKeySelectedFields() throws InterruptedException {
    ...
    BlobMetadata blob1 = BlobMetadata.newBuilder(BUCKET, blobNames[0]).setContentType(CONTENT_TYPE).buildStorageObject();
    BlobMetadata blob2 = BlobMetadata.newBuilder(BUCKET, blobNames[1]).setContentType(CONTENT_TYPE).buildStorageObject();
    ...
    Page<StorageBlob> page =
        storage.list(
            BUCKET,
            StorageClient.BlobListOptions.namePrefix("test-list-blobs-selected-field-kms-key-name-blob"),
            StorageClient.BlobListOptions.selectFields(BlobMetadataField.KMS_KEY_NAME));
    // Listing blobs is eventually consistent, we loop until the list is of the expected size. The
    // test fails if timeout is reached.
    while (Iterators.size(page.iterateAll().iterator()) != 2) {
      Thread.sleep(500);
  ...
  public void testListBlobsEmptySelectedFields() throws InterruptedException {
    ...
    Page<StorageBlob> page =
        storage.list(
            BUCKET,
            StorageClient.BlobListOptions.namePrefix("test-list-blobs-empty-selected-fields-blob"),
            StorageClient.BlobListOptions.selectFields());
    // Listing blobs is eventually consistent, we loop until the list is of the expected size. The
    // test fails if timeout is reached.
    while (Iterators.size(page.iterateAll().iterator()) != 2) {
      Thread.sleep(500);
  ...
  public void testListBlobRequesterPays() throws InterruptedException {
    ...
    StorageBucket remoteBucket =
        storage.get(BUCKET, StorageClient.BucketGetOptions.withFields(BucketMetadataField.ID, BucketMetadataField.BILLING));
    ...
    try {
      storage.list(
          ...
          StorageClient.BlobListOptions.namePrefix("test-list-blobs-empty-selected-fields-blob"),
          StorageClient.BlobListOptions.selectFields(),
          StorageClient.BlobListOptions.userProjectId("fakeBillingProjectId"));
    ...
    while (true) {
      Page<StorageBlob> page =
          storage.list(
              ...
              StorageClient.BlobListOptions.namePrefix("test-list-blobs-empty-selected-fields-blob"),
              StorageClient.BlobListOptions.selectFields(),
              StorageClient.BlobListOptions.userProjectId(projectId));
  ...
  public void testListBlobsVersioned() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      BlobMetadata blob1 =
          BlobMetadata.newBuilder(bucket, blobNames[0]).setContentType(CONTENT_TYPE).buildStorageObject();
      BlobMetadata blob2 =
          BlobMetadata.newBuilder(bucket, blobNames[1]).setContentType(CONTENT_TYPE).buildStorageObject();
      StorageBlob remoteBlob1 = storage.create(blob1);
      StorageBlob remoteBlob2 = storage.create(blob2);
      ...
      Page<StorageBlob> page =
          storage.list(
              bucketName,
              StorageClient.BlobListOptions.namePrefix("test-list-blobs-versioned-blob"),
  ...
  public void testListBlobsCurrentDirectory() throws InterruptedException {
    ...
    Page<StorageBlob> page =
        storage.list(
            BUCKET,
            StorageClient.BlobListOptions.namePrefix("test-list-blobs-current-directory/"),
            StorageClient.BlobListOptions.currentDir());
    // Listing blobs is eventually consistent, we loop until the list is of the expected size. The
    // test fails if timeout is reached.
    while (Iterators.size(page.iterateAll().iterator()) != 2) {
      Thread.sleep(500);
  ...
  public void testUpdateBlob() {
    ...
    StorageBlob remoteBlob = storage.create(blob);
    assertNotNull(remoteBlob);
    StorageBlob updatedBlob = remoteBlob.toBlobInfoBuilder().setContentType(CONTENT_TYPE).buildStorageObject().updateBlob();
  ...
  public void testUpdateBlobReplaceMetadata() {
    ...
    StorageBlob remoteBlob = storage.create(blob);
    assertNotNull(remoteBlob);
    StorageBlob updatedBlob = remoteBlob.toBlobInfoBuilder().setMetadata(null).buildStorageObject().updateBlob();
    assertNotNull(updatedBlob);
    assertNull(updatedBlob.getMetadata());
    updatedBlob = remoteBlob.toBlobInfoBuilder().setMetadata(newMetadata).buildStorageObject().updateBlob();
    assertEquals(blob.getName(), updatedBlob.getName());
    assertEquals(blob.getBucket(), updatedBlob.getBucket());
  ...
  public void testUpdateBlobMergeMetadata() {
    ...
    StorageBlob remoteBlob = storage.create(blob);
    assertNotNull(remoteBlob);
    StorageBlob updatedBlob = remoteBlob.toBlobInfoBuilder().setMetadata(newMetadata).buildStorageObject().updateBlob();
    assertNotNull(updatedBlob);
    assertEquals(blob.getName(), updatedBlob.getName());
    assertEquals(blob.getBucket(), updatedBlob.getBucket());
  ...
  public void testUpdateBlobFail() {
    ...
    BlobMetadata wrongGenerationBlob =
        BlobMetadata.newBuilder(BUCKET, blobName, -1L).setContentType(CONTENT_TYPE).buildStorageObject();
    try {
      storage.update(wrongGenerationBlob, StorageClient.BlobUploadOption.ifGenerationMatch());
      fail("StorageException was expected");
    } catch (StorageOperationException ex) {
      // expected
    }
  ...
  public void testComposeBlob() {
    ...
    BlobMetadata sourceBlob1 = BlobMetadata.newBuilder(BUCKET, sourceBlobName1).buildStorageObject();
    BlobMetadata sourceBlob2 = BlobMetadata.newBuilder(BUCKET, sourceBlobName2).buildStorageObject();
    ...
    BlobMetadata targetBlob = BlobMetadata.newBuilder(BUCKET, targetBlobName).buildStorageObject();
  ...
  public void testComposeBlobWithContentType() {
    ...
    BlobMetadata targetBlob =
        BlobMetadata.newBuilder(BUCKET, targetBlobName).setContentType(CONTENT_TYPE).buildStorageObject();
    StorageClient.ComposeBlobsRequest req =
        StorageClient.ComposeBlobsRequest.of(ImmutableList.of(sourceBlobName1, sourceBlobName2), targetBlob);
    StorageBlob remoteTargetBlob = storage.compose(req);
    assertNotNull(remoteTargetBlob);
  ...
  public void testComposeBlobFail() {
    ...
    BlobMetadata targetBlob = BlobMetadata.newBuilder(BUCKET, targetBlobName).buildStorageObject();
    StorageClient.ComposeBlobsRequest req =
        StorageClient.ComposeBlobsRequest.newTargetBuilder()
  ...
  public void testCopyBlob() {
    ...
    ImmutableMap<String, String> metadata = ImmutableMap.of("k", "v");
    BlobMetadata blob =
        BlobMetadata.newBuilder(source).setContentType(CONTENT_TYPE).setMetadata(metadata).buildStorageObject();
    StorageBlob remoteBlob = storage.create(blob, BLOB_BYTE_CONTENT);
    assertNotNull(remoteBlob);
    String targetBlobName = "test-copy-blob-target";
    StorageClient.CopyOperationRequest req = StorageClient.CopyOperationRequest.create(source, BlobIdentifier.create(BUCKET, targetBlobName));
  ...
  public void testCopyBlobWithPredefinedAcl() {
    ...
    assertNotNull(remoteBlob);
    String targetBlobName = "test-copy-blob-target";
    StorageClient.CopyOperationRequest req =
  ...
  public void testCopyBlobWithEncryptionKeys() {
    ...
    StorageBlob remoteBlob =
        storage.create(
            BlobMetadata.newBuilder(source).buildStorageObject(),
            BLOB_BYTE_CONTENT,
            StorageClient.BlobUploadOption.withEncryptionKey(KEY));
    assertNotNull(remoteBlob);
    ...
    StorageClient.CopyOperationRequest req =
        StorageClient.CopyOperationRequest.newCopyOperationBuilder()
            ...
            .setSourceOptions(StorageClient.BlobSourceOptions.decryptionKeyBase64(BASE64_KEY))
            .buildCopyOperation();
    BlobCopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
    ...
    assertArrayEquals(
        BLOB_BYTE_CONTENT,
        copyWriter.getResult().getContent(StorageBlob.BlobSourceOptions.withDecryptionKey(OTHER_BASE64_KEY)));
    assertEquals(metadata, copyWriter.getResult().getMetadata());
    assertTrue(copyWriter.isDone());
    req =
        StorageClient.CopyOperationRequest.newCopyOperationBuilder()
  ...
  public void testCopyBlobUpdateMetadata() {
    ...
    BlobIdentifier source = BlobIdentifier.create(BUCKET, sourceBlobName);
    StorageBlob remoteSourceBlob = storage.create(BlobMetadata.newBuilder(source).buildStorageObject(), BLOB_BYTE_CONTENT);
    ...
    BlobMetadata target =
        BlobMetadata.newBuilder(BUCKET, targetBlobName)
            ...
            .setMetadata(metadata)
            .buildStorageObject();
    StorageClient.CopyOperationRequest req = StorageClient.CopyOperationRequest.create(source, target);
    BlobCopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
    assertEquals(CONTENT_TYPE, copyWriter.getResult().getContentType());
    assertEquals(metadata, copyWriter.getResult().getMetadata());
    assertTrue(copyWriter.isDone());
    assertTrue(remoteSourceBlob.deleteBlob());
    assertTrue(storage.delete(BUCKET, targetBlobName));
  }
  ...
  public void testCopyBlobUpdateStorageClass() {
    ...
    BlobMetadata sourceInfo =
        BlobMetadata.newBuilder(source).setStorageClass(StorageTier.STANDARD).buildStorageObject();
    ...
    BlobMetadata targetInfo =
        BlobMetadata.newBuilder(BUCKET, targetBlobName).setStorageClass(StorageTier.COLDLINE).buildStorageObject();
    StorageClient.CopyOperationRequest req = StorageClient.CopyOperationRequest.create(source, targetInfo);
    BlobCopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
  ...
  public void testCopyBlobNoContentType() {
    ...
    ImmutableMap<String, String> metadata = ImmutableMap.of("k", "v");
    BlobMetadata target = BlobMetadata.newBuilder(BUCKET, targetBlobName).setMetadata(metadata).buildStorageObject();
    StorageClient.CopyOperationRequest req = StorageClient.CopyOperationRequest.create(source, target);
    BlobCopyWriter copyWriter = storage.copy(req);
    assertEquals(BUCKET, copyWriter.getResult().getBucket());
    assertEquals(targetBlobName, copyWriter.getResult().getName());
  ...
  public void testCopyBlobFail() {
    ...
    StorageClient.CopyOperationRequest req2 =
        StorageClient.CopyOperationRequest.newCopyOperationBuilder()
            .setSource(source)
  ...
  public void testBatchRequest() {
    ...
    BlobMetadata updatedBlob1 = sourceBlob1.toBlobInfoBuilder().setContentType(CONTENT_TYPE).buildStorageObject();
    BlobMetadata updatedBlob2 = sourceBlob2.toBlobInfoBuilder().setContentType(CONTENT_TYPE).buildStorageObject();
    StorageOperationBatch updateBatch = storage.batch();
    StorageBatchResult<StorageBlob> updateResult1 = updateBatch.updateBlob(updatedBlob1);
    StorageBatchResult<StorageBlob> updateResult2 = updateBatch.updateBlob(updatedBlob2);
  ...
  public void testBatchRequestManyOperations() {
    ...
    StorageBatchResult<StorageBlob> updateResult = batch.updateBlob(updatedBlob2);
  ...
  public void testBatchRequestFail() {
    ...
    StorageBlob remoteBlob = storage.create(blob);
    assertNotNull(remoteBlob);
    BlobMetadata updatedBlob = BlobMetadata.newBuilder(BUCKET, blobName, -1L).buildStorageObject();
    StorageOperationBatch batch = storage.batch();
    StorageBatchResult<StorageBlob> updateResult =
        batch.updateBlob(updatedBlob, StorageClient.BlobUploadOption.ifGenerationMatch());
    StorageBatchResult<Boolean> deleteResult1 =
        batch.deleteBlob(BUCKET, blobName, StorageClient.BlobSourceOptions.ifGenerationMatch(-1L));
  ...
  public void testReadAndWriteChannelWithEncryptionKey() throws IOException {
    ...
    try (WriteChannel writer =
        storage.writer(blob, BlobWriteSetting.customerSuppliedKey(BASE64_KEY))) {
      stringBytes = BLOB_STRING_CONTENT.getBytes(UTF_8);
      writer.write(ByteBuffer.wrap(BLOB_BYTE_CONTENT));
      writer.write(ByteBuffer.wrap(stringBytes));
    }
    ...
    try (ReadChannel reader =
        storage.reader(blob.getBlobId(), StorageClient.BlobSourceOptions.decryptionKeyBase64(KEY))) {
      readBytes = ByteBuffer.allocate(BLOB_BYTE_CONTENT.length);
      readStringBytes = ByteBuffer.allocate(stringBytes.length);
      reader.read(readBytes);
      reader.read(readStringBytes);
  ...
  public void testWriteChannelFail() throws IOException {
    ...
    BlobMetadata blob = BlobMetadata.newBuilder(BUCKET, blobName, -1L).buildStorageObject();
    try {
      try (WriteChannel writer = storage.writer(blob, BlobWriteSetting.ifGenerationMatch())) {
  ...
  public void testWriteChannelWithConnectionPool() throws IOException {
    ...
    StorageClient storageWithPool =
        StorageSettings.newClientBuilder().setTransportOptions(transportOptions).build().getService();
    ...
    for (StorageClient.UrlSigningOption urlStyle :
      ...
      URL url =
          storage.signUrl(
              ...
              StorageClient.UrlSigningOption.withQueryParameters(
  ...
  public void testPostSignedUrl() throws IOException {
    ...
    for (StorageClient.UrlSigningOption urlStyle :
      ...
      URL url =
          storage.signUrl(
              blob, 1, TimeUnit.HOURS, StorageClient.UrlSigningOption.setHttpMethod(HttpRequestMethod.POST), urlStyle);
      URLConnection connection = url.openConnection();
  ...
  public void testDownloadPublicBlobWithoutAuthentication() {
    ...
    Iterator<StorageBlob> blobIterator =
        unauthorizedStorage
            .list(landsatBucket, StorageClient.BlobListOptions.namePrefix(landsatPrefix))
            .iterateAll()
            .iterator();
    ...
    BlobMetadata sourceBlob = BlobMetadata.newBuilder(BUCKET, sourceBlobName).buildStorageObject();
    ...
    assertThat(storage.get(sourceBlob.getBlobId()).deleteBlob()).isTrue();
  ...
  public void testUpdateBlobs() {
    ...
    List<StorageBlob> updatedBlobs =
        storage.update(
            remoteBlob1.toBlobInfoBuilder().setContentType(CONTENT_TYPE).buildStorageObject(),
            remoteBlob2.toBlobInfoBuilder().setContentType(CONTENT_TYPE).buildStorageObject());
  ...
  public void testUpdateBlobsFail() {
    ...
    List<StorageBlob> updatedBlobs =
        storage.update(
            remoteBlob1.toBlobInfoBuilder().setContentType(CONTENT_TYPE).buildStorageObject(),
            sourceBlob2.toBlobInfoBuilder().setContentType(CONTENT_TYPE).buildStorageObject());
    assertEquals(sourceBlob1.getBucket(), updatedBlobs.get(0).getBucket());
    assertEquals(sourceBlob1.getName(), updatedBlobs.get(0).getName());
    assertEquals(CONTENT_TYPE, updatedBlobs.get(0).getContentType());
  ...
  private void testBucketAclRequesterPays(boolean requesterPays) {
    if (requesterPays) {
      StorageBucket remoteBucket =
          storage.get(BUCKET, StorageClient.BucketGetOptions.withFields(BucketMetadataField.ID, StorageClient.BucketMetadataField.BILLING));
    ...
    StorageClient.BucketSourceOptions[] bucketOptions =
        requesterPays
            ? new StorageClient.BucketSourceOptions[] {StorageClient.BucketSourceOptions.withUserProject(projectId)}
            : new StorageClient.BucketSourceOptions[] {};

    assertNull(storage.getAcl(BUCKET, UserPrincipal.allAuthenticatedUsers(), bucketOptions));
    assertFalse(storage.deleteAcl(BUCKET, UserPrincipal.allAuthenticatedUsers(), bucketOptions));
    AccessControlEntry acl = AccessControlEntry.create(UserPrincipal.allAuthenticatedUsers(), AccessRole.READER);
    assertNotNull(storage.createAcl(BUCKET, acl, bucketOptions));
    AccessControlEntry updatedAcl =
        storage.updateAcl(BUCKET, acl.asBuilder().setRole(AccessRole.WRITER).create(), bucketOptions);
    ...
    assertTrue(storage.deleteAcl(BUCKET, UserPrincipal.allAuthenticatedUsers(), bucketOptions));
    assertNull(storage.getAcl(BUCKET, UserPrincipal.allAuthenticatedUsers(), bucketOptions));
    if (requesterPays) {
      StorageBucket remoteBucket =
          storage.get(
              BUCKET,
              StorageClient.BucketGetOptions.withFields(StorageClient.BucketMetadataField.ID, StorageClient.BucketMetadataField.BILLING),
      ...
      StorageBucket updatedBucket =
          storage.update(remoteBucket, StorageClient.BucketTargetRequestOption.withUserProject(projectId));
  ...
  public void testBucketDefaultAcl() {
    assertNull(storage.getDefaultAcl(BUCKET, UserPrincipal.allAuthenticatedUsers()));
    ...
    AccessControlEntry updatedAcl = storage.updateDefaultAcl(BUCKET, acl.asBuilder().setRole(AccessRole.OWNER).create());
    assertEquals(AccessControlEntry.AccessRole.OWNER, updatedAcl.getRole());
    Set<AccessControlEntry> acls = new HashSet<>();
  ...
  public void testBlobAcl() {
    ...
    BlobMetadata blob = BlobMetadata.newBuilder(blobId).buildStorageObject();
    storage.create(blob);
    assertNull(storage.getAcl(blobId, UserPrincipal.allAuthenticatedUsers()));
    AccessControlEntry acl = AccessControlEntry.create(UserPrincipal.allAuthenticatedUsers(), AccessRole.READER);
    assertNotNull(storage.createAcl(blobId, acl));
    AccessControlEntry updatedAcl = storage.updateAcl(blobId, acl.asBuilder().setRole(AccessRole.OWNER).create());
  ...
  public void testBucketPolicyV1RequesterPays() {
    ...
    assertEquals(
        ...
        storage.testIamPermissions(
            ...
            bucketOptions));
    remoteBucket = remoteBucket.toBucketBuilder().setRequesterPays(false).buildBucket();
    updatedBucket = storage.update(remoteBucket, StorageClient.BucketTargetRequestOption.withUserProject(projectId));
    assertFalse(updatedBucket.isRequesterPays());
  }
  ...
  public void testBucketPolicyV1() {
    ...
    StorageClient.BucketSourceOptions[] bucketOptions = new StorageClient.BucketSourceOptions[] {};
    Identity projectOwner = Identity.projectOwner(projectId);
    Identity projectEditor = Identity.projectEditor(projectId);
    Identity projectViewer = Identity.projectViewer(projectId);
    Map<com.google.cloud.Role, Set<Identity>> bindingsWithoutPublicRead =
  ...
  public void testBucketPolicyV3() {
    ...
    StorageClient.BucketSourceOptions[] bucketOptions =
        new StorageClient.BucketSourceOptions[] {StorageClient.BucketSourceOptions.withRequestedPolicyVersion(3)};
    Identity projectOwner = Identity.projectOwner(projectId);
    Identity projectEditor = Identity.projectEditor(projectId);
    Identity projectViewer = Identity.projectViewer(projectId);
  ...
  public void testUpdateBucketLabel() {
    StorageBucket remoteBucket =
        storage.get(BUCKET, StorageClient.BucketGetOptions.withFields(StorageClient.BucketMetadataField.ID, BucketMetadataField.BILLING));
  ...
  public void testUpdateBucketRequesterPays() {
    ...
    assertNotNull(remoteBlob);
    byte[] readBytes =
        storage.readAllBytes(BUCKET, blobName, StorageClient.BlobSourceOptions.userProjectId(projectId));
    assertArrayEquals(BLOB_BYTE_CONTENT, readBytes);
    remoteBucket = remoteBucket.toBucketBuilder().setRequesterPays(false).buildBucket();
    updatedBucket = storage.update(remoteBucket, StorageClient.BucketTargetRequestOption.withUserProject(projectId));
    assertFalse(updatedBucket.isRequesterPays());
  ...
  public void testListBucketRequesterPaysFails() throws InterruptedException {
    ...
    Iterator<StorageBucket> bucketIterator =
        storage
            .list(
                StorageClient.BucketListOptions.namePrefix(BUCKET),
                StorageClient.BucketListOptions.fieldsSelector(),
                StorageClient.BucketListOptions.userProjectId(projectId))
            .iterateAll()
            .iterator();
    while (!bucketIterator.hasNext()) {
      Thread.sleep(500);
  ...
  public void testListBucketDefaultKmsKeyName() throws ExecutionException, InterruptedException {
    ...
    try {
      Iterator<StorageBucket> bucketIterator =
          storage
              .list(
                  StorageClient.BucketListOptions.namePrefix(bucketName),
                  StorageClient.BucketListOptions.fieldsSelector(BucketMetadataField.ENCRYPTION))
              .iterateAll()
              .iterator();
      while (!bucketIterator.hasNext()) {
        Thread.sleep(500);
  ...
  public void testRetentionPolicyNoLock() throws ExecutionException, InterruptedException {
    ...
    try {
      ...
      remoteBucket =
          storage.get(bucketName, StorageClient.BucketGetOptions.withFields(BucketMetadataField.RETENTION_POLICY));
      assertEquals(RETENTION_PERIOD, remoteBucket.getRetentionPeriod());
      assertNotNull(remoteBucket.getRetentionEffectiveTime());
      assertNull(remoteBucket.isRetentionPolicyLocked());
      String blobName = "test-create-with-retention-policy-hold";
      BlobMetadata blobInfo = BlobMetadata.newBuilder(bucketName, blobName).buildStorageObject();
    ...
    try {
      ...
      if (requesterPays) {
        remoteBucket =
            storage.lockRetentionPolicy(
                ...
                StorageClient.BucketTargetRequestOption.withUserProject(projectId));
      } else {
        remoteBucket =
            storage.lockRetentionPolicy(
                remoteBucket, StorageClient.BucketTargetRequestOption.ifMetagenerationMatch());
      }
      ...
      if (requesterPays) {
        ...
        StorageBucket updateBucket =
            storage.update(bucketInfo, StorageClient.BucketTargetRequestOption.withUserProject(projectId));
  ...
  public void testEnableDisableBucketDefaultEventBasedHold()
    ...
    try {
      ...
      remoteBucket =
          storage.get(
              bucketName, StorageClient.BucketGetOptions.withFields(StorageClient.BucketMetadataField.DEFAULT_EVENT_BASED_HOLD));
      assertTrue(remoteBucket.getDefaultEventBasedHold());
      ...
      remoteBlob =
          storage.get(
              blobInfo.getBlobId(), StorageClient.BlobGetOptions.fieldsSelector(BlobMetadataField.EVENT_BASED_HOLD));
      assertTrue(remoteBlob.getEventBasedHold());
      remoteBlob = remoteBlob.toBlobInfoBuilder().setEventBasedHold(false).buildStorageObject().updateBlob();
  ...
  public void testEnableDisableTemporaryHold() {
    ...
    BlobMetadata blobInfo = BlobMetadata.newBuilder(BUCKET, blobName).setTemporaryHold(true).buildStorageObject();
    StorageBlob remoteBlob = storage.create(blobInfo);
    assertTrue(remoteBlob.getTemporaryHold());
    remoteBlob =
        storage.get(remoteBlob.getBlobId(), StorageClient.BlobGetOptions.fieldsSelector(BlobMetadataField.TEMPORARY_HOLD));
    assertTrue(remoteBlob.getTemporaryHold());
    remoteBlob = remoteBlob.toBlobInfoBuilder().setTemporaryHold(false).buildStorageObject().updateBlob();
  ...
  public void testAttemptObjectDeleteWithEventBasedHold() {
    String blobName = "test-create-with-event-based-hold";
    BlobMetadata blobInfo = BlobMetadata.newBuilder(BUCKET, blobName).setEventBasedHold(true).buildStorageObject();
    StorageBlob remoteBlob = storage.create(blobInfo);
    assertTrue(remoteBlob.getEventBasedHold());
    try {
      remoteBlob.deleteBlob();
      ...
      remoteBlob.toBlobInfoBuilder().setEventBasedHold(false).buildStorageObject().updateBlob();
    }
  }
  ...
  public void testBucketWithBucketPolicyOnlyEnabled() throws Exception {
    ...
    try {
      ...
      StorageBucket remoteBucket =
          storage.get(bucket, StorageClient.BucketGetOptions.withFields(BucketMetadataField.IAMCONFIGURATION));
  ...
  public void testBucketWithUniformBucketLevelAccessEnabled() throws Exception {
    ...
    try {
      ...
      StorageBucket remoteBucket =
          storage.get(bucket, StorageClient.BucketGetOptions.withFields(StorageClient.BucketMetadataField.IAMCONFIGURATION));
  ...
  public void testEnableAndDisableBucketPolicyOnlyOnExistingBucket() throws Exception {
    ...
    try {
      ...
      StorageBucket bucket =
          storage.create(
              StorageBucket.newBucketBuilder(bpoBucket)
                  .setAcl(ImmutableList.of(AccessControlEntry.create(UserPrincipal.allAuthenticatedUsers(), AccessRole.READER)))
                  .setDefaultAcl(
                      ImmutableList.of(AccessControlEntry.create(UserPrincipal.allAuthenticatedUsers(), AccessRole.READER)))
                  .buildBucket());

      BucketMetadata.BucketIamConfiguration bpoEnabledIamConfiguration =
          BucketMetadata.BucketIamConfiguration.newUniformBucketLevelAccessBuilder().setIsBucketPolicyOnlyEnabled(true).buildBucketIamConfiguration();
      bucket
          .toBucketBuilder()
          .setAcl(null)
          .setDefaultAcl(null)
      ...
      StorageBucket remoteBucket =
          storage.get(bpoBucket, StorageClient.BucketGetOptions.withFields(BucketMetadataField.IAMCONFIGURATION));
      ...
      remoteBucket =
          storage.get(
              ...
              StorageClient.BucketGetOptions.withFields(
                  BucketMetadataField.IAMCONFIGURATION, StorageClient.BucketMetadataField.ACL, BucketMetadataField.DEFAULT_OBJECT_ACL));
  ...
  public void testEnableAndDisableUniformBucketLevelAccessOnExistingBucket() throws Exception {
    ...
    try {
      ...
      StorageBucket remoteBucket =
          storage.get(bpoBucket, StorageClient.BucketGetOptions.withFields(StorageClient.BucketMetadataField.IAMCONFIGURATION));
      ...
      remoteBucket =
          storage.get(
              bpoBucket,
              StorageClient.BucketGetOptions.withFields(
                  StorageClient.BucketMetadataField.IAMCONFIGURATION, BucketMetadataField.ACL, StorageClient.BucketMetadataField.DEFAULT_OBJECT_ACL));

      assertFalse(remoteBucket.getIamConfiguration().isUniformBucketLevelAccessEnabled());
      assertEquals(UserPrincipal.allAuthenticatedUsers(), remoteBucket.getDefaultAcl().get(0).getEntity());
      assertEquals(AccessRole.READER, remoteBucket.getDefaultAcl().get(0).getRole());
      assertEquals(AccessControlEntry.UserPrincipal.allAuthenticatedUsers(), remoteBucket.getAcl().get(0).getEntity());
      assertEquals(AccessRole.READER, remoteBucket.getAcl().get(0).getRole());
    } finally {
      RemoteStorageHelper.forceDelete(storage, bpoBucket, 1, TimeUnit.MINUTES);
    }
  ...
  public void testBucketLocationType() throws ExecutionException, InterruptedException {
    ...
    StorageBucket bucket1 =
        storage.lockRetentionPolicy(bucket, StorageClient.BucketTargetRequestOption.ifMetagenerationMatch());
    ...
    Iterator<StorageBucket> bucketIterator =
        storage.list(StorageClient.BucketListOptions.namePrefix(bucketName)).iterateAll().iterator();
    while (bucketIterator.hasNext()) {
      StorageBucket remoteBucket = bucketIterator.next();
