...
import static com.google.cloud.storage.AccessControlEntry.RoleType.READER;
...
import com.google.cloud.storage.BlobMetadata.CustomerEncryptionMetadata;
...
public class BlobInfoTest {
  ...
  private static final List<AccessControlEntry> ACL =
      ImmutableList.of(
          AccessControlEntry.create(UserIdentity.allAuthenticatedUsers(), READER),
          AccessControlEntry.create(new AccessControlEntry.ProjectInfo(VIEWERS, "p1"), WRITER));
  ...
  private static final CustomerEncryptionMetadata CUSTOMER_ENCRYPTION =
      new CustomerEncryptionMetadata(ENCRYPTION_ALGORITHM, KEY_SHA256);
  ...
  public void testToBuilder() {
    compareBlobs(BLOB_INFO, BLOB_INFO.toInfoBuilder().buildObject());
    BlobMetadata blobInfo =
        BLOB_INFO.toInfoBuilder().setBlobId(BlobId.from("b2", "n2")).setSize(200L).buildObject();
    assertEquals("n2", blobInfo.getName());
    assertEquals("b2", blobInfo.getBucket());
    assertEquals(Long.valueOf(200), blobInfo.getSize());
    blobInfo =
        blobInfo.toInfoBuilder().setBlobId(BlobId.from("b", "n", GENERATION)).setSize(SIZE).buildObject();
  ...
  public void testToBuilderSetMd5FromHexString() {
    BlobMetadata blobInfo =
        BlobMetadata.newBuilder(BlobId.from("b2", "n2")).setMd5FromHexString(MD5_HEX_STRING).buildObject();
  ...
  public void testToBuilderSetMd5FromHexStringLeadingZeros() {
    BlobMetadata blobInfo =
        BlobMetadata.newBuilder(BlobId.from("b2", "n2"))
  ...
  public void testToBuilderSetCrc32cFromHexString() {
    BlobMetadata blobInfo =
        BlobMetadata.newBuilder(BlobId.from("b2", "n2")).setCrc32cFromHexString(CRC32_HEX_STRING).buildObject();
  ...
  public void testToBuilderIncomplete() {
    BlobMetadata incompleteBlobInfo = BlobMetadata.newBuilder(BlobId.from("b2", "n2")).buildObject();
    compareBlobs(incompleteBlobInfo, incompleteBlobInfo.toInfoBuilder().buildObject());
  }
  ...
  private void compareBlobs(BlobMetadata expected, BlobMetadata value) {
  ...
  private void compareCustomerEncryptions(BlobMetadata.CustomerEncryptionMetadata expected, CustomerEncryptionMetadata value) {
    assertEquals(expected, value);
  ...
  public void testToPbAndFromPb() {
    compareCustomerEncryptions(
        CUSTOMER_ENCRYPTION, CustomerEncryptionMetadata.fromProto(CUSTOMER_ENCRYPTION.toProto()));
    compareBlobs(BLOB_INFO, BlobMetadata.fromProto(BLOB_INFO.toProto()));
    BlobMetadata blobInfo = BlobMetadata.newBuilder(BlobId.from("b", "n")).buildObject();
    compareBlobs(blobInfo, BlobMetadata.fromProto(blobInfo.toProto()));
    ...
    blobInfo = BlobMetadata.fromProto(object);
  ...
  public void testBlobId() {
    assertEquals(BlobId.from("b", "n", GENERATION), BLOB_INFO.getBlobId());
  }
}
