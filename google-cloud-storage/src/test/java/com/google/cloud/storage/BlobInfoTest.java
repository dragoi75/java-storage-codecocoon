...
import static com.google.cloud.storage.AccessControlEntry.UserRole.READER;
...
public class BlobInfoTest {
  ...
  private static final List<AccessControlEntry> ACL =
      ImmutableList.of(
          AccessControlEntry.create(AccessControlEntry.UserIdentity.allAuthenticatedUsers(), READER),
          AccessControlEntry.create(new ProjectInfo(VIEWERS, "p1"), WRITER));
  ...
  public void testToBuilder() {
    compareBlobs(BLOB_INFO, BLOB_INFO.toBuilderCopy().buildMetadata());
    BlobMetadata blobInfo =
        BLOB_INFO.toBuilderCopy().setBlobId(BlobIdentifier.from("b2", "n2")).setSize(200L).buildMetadata();
    assertEquals("n2", blobInfo.getName());
    assertEquals("b2", blobInfo.getBucket());
    assertEquals(Long.valueOf(200), blobInfo.getSize());
    blobInfo =
        blobInfo.toBuilderCopy().setBlobId(BlobIdentifier.from("b", "n", GENERATION)).setSize(SIZE).buildMetadata();
  ...
  public void testToBuilderSetMd5FromHexString() {
    BlobMetadata blobInfo =
        BlobMetadata.newBuilder(BlobIdentifier.from("b2", "n2")).setMd5FromHexString(MD5_HEX_STRING).buildMetadata();
  ...
  public void testToBuilderSetMd5FromHexStringLeadingZeros() {
    BlobMetadata blobInfo =
        BlobMetadata.newBuilder(BlobIdentifier.from("b2", "n2"))
  ...
  public void testToBuilderSetCrc32cFromHexString() {
    BlobMetadata blobInfo =
        BlobMetadata.newBuilder(BlobIdentifier.from("b2", "n2")).setCrc32cFromHexString(CRC32_HEX_STRING).buildMetadata();
  ...
  public void testToBuilderIncomplete() {
    BlobMetadata incompleteBlobInfo = BlobMetadata.newBuilder(BlobIdentifier.from("b2", "n2")).buildMetadata();
    compareBlobs(incompleteBlobInfo, incompleteBlobInfo.toBuilderCopy().buildMetadata());
  }
  ...
  public void testToPbAndFromPb() {
    ...
    BlobMetadata blobInfo = BlobMetadata.newBuilder(BlobIdentifier.from("b", "n")).buildMetadata();
  ...
  public void testBlobId() {
    assertEquals(BlobIdentifier.from("b", "n", GENERATION), BLOB_INFO.getBlobId());
  }
}
