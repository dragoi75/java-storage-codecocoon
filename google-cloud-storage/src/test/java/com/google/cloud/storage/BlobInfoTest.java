...
public class BlobInfoTest {
  ...
  private static final List<AclEntry> ACL =
      ImmutableList.of(
          AclEntry.ofEntry(AclEntry.UserIdentity.allAuthenticatedUsers(), READER),
  ...
  public void testToBuilder() {
    compareBlobs(BLOB_INFO, BLOB_INFO.toBlobBuilder().buildObject());
    BlobMetadata blobInfo =
        BLOB_INFO.toBlobBuilder().setBlobId(BlobIdentifier.create("b2", "n2")).setSize(200L).buildObject();
    assertEquals("n2", blobInfo.getName());
    assertEquals("b2", blobInfo.getBucket());
    assertEquals(Long.valueOf(200), blobInfo.getSize());
    blobInfo =
        blobInfo.toBlobBuilder().setBlobId(BlobIdentifier.create("b", "n", GENERATION)).setSize(SIZE).buildObject();
  ...
  public void testToBuilderSetMd5FromHexString() {
    BlobMetadata blobInfo =
        BlobMetadata.newBuilder(BlobIdentifier.create("b2", "n2")).setMd5FromHexString(MD5_HEX_STRING).buildObject();
  ...
  public void testToBuilderSetCrc32cFromHexString() {
    BlobMetadata blobInfo =
        BlobMetadata.newBuilder(BlobIdentifier.create("b2", "n2")).setCrc32cFromHexString(CRC32_HEX_STRING).buildObject();
    assertEquals(CRC32, blobInfo.getCrc32c());
  }

  @Test
  public void testToBuilderIncomplete() {
    BlobMetadata incompleteBlobInfo = BlobMetadata.newBuilder(BlobIdentifier.create("b2", "n2")).buildObject();
    compareBlobs(incompleteBlobInfo, incompleteBlobInfo.toBlobBuilder().buildObject());
  }
  ...
  private void compareCustomerEncryptions(CustomerEncryptionInfo expected, BlobMetadata.CustomerEncryptionInfo value) {
    assertEquals(expected, value);
  ...
  public void testToPbAndFromPb() {
    ...
    BlobMetadata blobInfo = BlobMetadata.newBuilder(BlobIdentifier.create("b", "n")).buildObject();
