  ...
  public void test() {
    ...
    if (!Strings.isNullOrEmpty(fields.get("success_action_redirect"))) {
      builder.addSuccessActionRedirect(
          S3PostPolicyV4.ConditionV4Kind.MATCHES, fields.get("success_action_redirect"));
    }

    if (!Strings.isNullOrEmpty(fields.get("success_action_status"))) {
      builder.addSuccessActionStatus(
          S3PostPolicyV4.ConditionV4Kind.MATCHES,
    ...
    if (conditions != null) {
      if (!conditions.getStartsWithList().isEmpty()) {
        builder.addCustom(
            S3PostPolicyV4.ConditionV4Kind.STARTS_WITH,
    ...
    S3PostPolicyV4.PostFieldsMapV4 v4Fields = S3PostPolicyV4.PostFieldsMapV4.from(fields);

    CloudStorageClient.PostPolicyV4FormField style = CloudStorageClient.PostPolicyV4FormField.usePathStyle();

    if (policyInput.getUrlStyle().equals(UrlStyle.VIRTUAL_HOSTED_STYLE)) {
      style = CloudStorageClient.PostPolicyV4FormField.useVirtualHostedStyle();
    } else if (policyInput.getUrlStyle().equals(UrlStyle.PATH_STYLE)) {
      style = CloudStorageClient.PostPolicyV4FormField.usePathStyle();
    } else if (policyInput.getUrlStyle().equals(UrlStyle.BUCKET_BOUND_HOSTNAME)) {
      style =
          CloudStorageClient.PostPolicyV4FormField.withBucketBoundHostname(
              policyInput.getBucketBoundHostname(),
              CloudStorageClient.UriSchemeType.valueOf(policyInput.getScheme().toUpperCase()));
    }
