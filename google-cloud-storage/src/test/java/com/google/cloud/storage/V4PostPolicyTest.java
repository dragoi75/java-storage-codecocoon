  ...
  public void test() {
    ...
    if (!Strings.isNullOrEmpty(fields.get("success_action_redirect"))) {
      builder.addSuccessActionRedirect(
          FormPostPolicyV4.ConditionV4Operator.MATCHES, fields.get("success_action_redirect"));
    }

    if (!Strings.isNullOrEmpty(fields.get("success_action_status"))) {
      builder.addSuccessActionStatus(
          FormPostPolicyV4.ConditionV4Operator.MATCHES,
    ...
    if (conditions != null) {
      if (!conditions.getStartsWithList().isEmpty()) {
        builder.addCustom(
            FormPostPolicyV4.ConditionV4Operator.STARTS_WITH,
    ...
    FormPostPolicyV4.PostFieldsMapV4 v4Fields = FormPostPolicyV4.PostFieldsMapV4.create(fields);

    CloudStorageClient.PostPolicyV4Parameter style = CloudStorageClient.PostPolicyV4Parameter.usePathStyle();

    if (policyInput.getUrlStyle().equals(UrlStyle.VIRTUAL_HOSTED_STYLE)) {
      style = CloudStorageClient.PostPolicyV4Parameter.useVirtualHostedStyle();
    } else if (policyInput.getUrlStyle().equals(UrlStyle.PATH_STYLE)) {
      style = CloudStorageClient.PostPolicyV4Parameter.usePathStyle();
    } else if (policyInput.getUrlStyle().equals(UrlStyle.BUCKET_BOUND_HOSTNAME)) {
      style =
          CloudStorageClient.PostPolicyV4Parameter.withBucketBoundHostname(
              policyInput.getBucketBoundHostname(),
              CloudStorageClient.UriProtocol.valueOf(policyInput.getScheme().toUpperCase()));
    }
