...
import com.google.cloud.storage.SigningInfo.RequestSignatureBuilder;
import java.net.URI;
...
public class SignatureInfoTest {
  ...
  public void requireHttpVerb() {
    ...
    new RequestSignatureBuilder(null, 0L, URI.create(RESOURCE)).buildSigningInfo();
  }

  @Test(expected = IllegalArgumentException.class)
  public void requireResource() {

    new SigningInfo.RequestSignatureBuilder(HttpRequestMethod.GET, 0L, null).buildSigningInfo();
  }

  @Test
  public void constructUnsignedPayload() {

    SigningInfo.RequestSignatureBuilder builder = new SigningInfo.RequestSignatureBuilder(HttpRequestMethod.PUT, 0L, URI.create(RESOURCE));

    String unsignedPayload = builder.buildSigningInfo().createUnsignedPayload();
  ...
  public void constructV4UnsignedPayload() {
    SigningInfo.RequestSignatureBuilder builder = new SigningInfo.RequestSignatureBuilder(HttpRequestMethod.PUT, 10L, URI.create(RESOURCE));

    builder.setSignatureVersion(StorageService.UrlSigningOption.SigningVersion.V4);
  ...
  public void constructV4QueryString() {
    SigningInfo.RequestSignatureBuilder builder = new RequestSignatureBuilder(HttpRequestMethod.PUT, 10L, URI.create(RESOURCE));

    builder.setSignatureVersion(StorageService.UrlSigningOption.SigningVersion.V4);
    builder.setAccountEmail("me@google.com");
    builder.setTimestamp(1000000000000L);

    String queryString = builder.buildSigningInfo().buildV4QueryString();
    assertEquals(
