...
public class CopyWriterTest {
  ...
  private static final StorageRpcClient.ObjectRewriteRequest REQUEST_WITH_OBJECT =
      new ObjectRewriteRequest(
          BLOB_ID.toProto(), EMPTY_OPTIONS, true, BLOB_INFO.toProto(), EMPTY_OPTIONS, null);
  private static final StorageRpcClient.ObjectRewriteRequest REQUEST_WITHOUT_OBJECT =
      new ObjectRewriteRequest(
          BLOB_ID.toProto(), EMPTY_OPTIONS, false, BLOB_INFO.toProto(), EMPTY_OPTIONS, null);
  private static final StorageRpcClient.RewriteOperationResponse RESPONSE_WITH_OBJECT =
      new StorageRpcClient.RewriteOperationResponse(REQUEST_WITH_OBJECT, null, 42L, false, "token", 21L);
  private static final StorageRpcClient.RewriteOperationResponse RESPONSE_WITHOUT_OBJECT =
      new StorageRpcClient.RewriteOperationResponse(REQUEST_WITHOUT_OBJECT, null, 42L, false, "token", 21L);
  private static final StorageRpcClient.RewriteOperationResponse RESPONSE_WITH_OBJECT_DONE =
      new StorageRpcClient.RewriteOperationResponse(REQUEST_WITH_OBJECT, RESULT_INFO.toProto(), 42L, true, "token", 42L);
  private static final StorageRpcClient.RewriteOperationResponse RESPONSE_WITHOUT_OBJECT_DONE =
      new StorageRpcClient.RewriteOperationResponse(REQUEST_WITHOUT_OBJECT, RESULT_INFO.toProto(), 42L, true, "token", 42L);

  private StorageSettings options;
  private StorageRpcProvider rpcFactoryMock;
  private StorageRpcClient storageRpcMock;
  private ResumableCopyWriter copyWriter;
  private CloudStorageObject result;
  ...
  public void setUp() {
    ...
    options =
        StorageSettings.newSettingsBuilder()
            ...
            .build();
    result = new CloudStorageObject(options.getService(), new BlobMetadata.BlobInfoBuilderImpl(RESULT_INFO));
  }
  ...
  public void testRewriteWithObject() {
    ...
    copyWriter = new ResumableCopyWriter(options, RESPONSE_WITH_OBJECT);
  ...
  public void testRewriteWithoutObject() {
    ...
    EasyMock.replay(storageRpcMock);
    copyWriter = new ResumableCopyWriter(options, RESPONSE_WITHOUT_OBJECT);
    assertEquals(result, copyWriter.getResult());
    assertTrue(copyWriter.isDone());
    assertEquals(42L, copyWriter.getTotalBytesCopied());
    assertEquals(42L, copyWriter.getBlobSize());
  ...
  public void testSaveAndRestoreWithObject() {
    ...
    assertEquals(42L, copyWriter.getBlobSize());
    RestorableState<ResumableCopyWriter> rewriterState = copyWriter.capture();
    ResumableCopyWriter restoredRewriter = rewriterState.restore();
