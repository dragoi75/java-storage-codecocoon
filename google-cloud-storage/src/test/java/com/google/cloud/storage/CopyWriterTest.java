...
public class CopyWriterTest {
  ...
  private static final BlobInfo BLOB_INFO =
      BlobInfo.newBuilder(DESTINATION_BUCKET_NAME, DESTINATION_BLOB_NAME).buildMetadata();
  ...
  private static final StorageRpcClient.RewriteOperationRequest REQUEST_WITH_OBJECT =
      new RewriteOperationRequest(
          BLOB_ID.toProto(), EMPTY_OPTIONS, true, BLOB_INFO.toProto(), EMPTY_OPTIONS, null);
  private static final StorageRpcClient.RewriteOperationRequest REQUEST_WITHOUT_OBJECT =
      new RewriteOperationRequest(
          BLOB_ID.toProto(), EMPTY_OPTIONS, false, BLOB_INFO.toProto(), EMPTY_OPTIONS, null);
  private static final RewriteOperationResult RESPONSE_WITH_OBJECT =
      new RewriteOperationResult(REQUEST_WITH_OBJECT, null, 42L, false, "token", 21L);
  private static final RewriteOperationResult RESPONSE_WITHOUT_OBJECT =
      new StorageRpcClient.RewriteOperationResult(REQUEST_WITHOUT_OBJECT, null, 42L, false, "token", 21L);
  private static final RewriteOperationResult RESPONSE_WITH_OBJECT_DONE =
      new StorageRpcClient.RewriteOperationResult(REQUEST_WITH_OBJECT, RESULT_INFO.toProto(), 42L, true, "token", 42L);
  private static final RewriteOperationResult RESPONSE_WITHOUT_OBJECT_DONE =
      new RewriteOperationResult(REQUEST_WITHOUT_OBJECT, RESULT_INFO.toProto(), 42L, true, "token", 42L);

  private StorageSettings options;
  private StorageRpcProvider rpcFactoryMock;
  private StorageRpcClient storageRpcMock;
  private ChunkedCopyWriter copyWriter;
  ...
  public void setUp() {
    ...
    options =
        StorageSettings.createBuilder()
            ...
            .build();
    result = new StorageObject(options.getService(), new BlobInfo.StorageObjectBuilder(RESULT_INFO));
  }
  ...
  public void testRewriteWithObject() {
    ...
    copyWriter = new ChunkedCopyWriter(options, RESPONSE_WITH_OBJECT);
  ...
  public void testRewriteWithoutObject() {
    ...
    EasyMock.replay(storageRpcMock);
    copyWriter = new ChunkedCopyWriter(options, RESPONSE_WITHOUT_OBJECT);
    assertEquals(result, copyWriter.getResult());
    assertTrue(copyWriter.isDone());
    assertEquals(42L, copyWriter.getTotalBytesCopied());
    assertEquals(42L, copyWriter.getBlobSize());
  ...
  public void testSaveAndRestoreWithObject() {
    ...
    assertEquals(42L, copyWriter.getBlobSize());
    RestorableState<ChunkedCopyWriter> rewriterState = copyWriter.capture();
    ChunkedCopyWriter restoredRewriter = rewriterState.restore();
