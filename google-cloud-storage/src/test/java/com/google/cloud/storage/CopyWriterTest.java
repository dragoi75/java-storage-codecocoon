...
import com.google.cloud.storage.spi.v1.CloudStorageRpcClient.RewriteOperationRequest;
...
public class CopyWriterTest {
  ...
  private static final BlobIdentifier BLOB_ID = BlobIdentifier.create(SOURCE_BUCKET_NAME, SOURCE_BLOB_NAME);
  private static final BlobAttributes BLOB_INFO =
      BlobAttributes.newBuilder(DESTINATION_BUCKET_NAME, DESTINATION_BLOB_NAME).buildObject();
  private static final BlobAttributes RESULT_INFO =
      BlobAttributes.newBuilder(DESTINATION_BUCKET_NAME, DESTINATION_BLOB_NAME)
          .setContentType("type")
          .buildObject();
  private static final Map<CloudStorageRpcClient.StorageOption, ?> EMPTY_OPTIONS = ImmutableMap.of();
  private static final RewriteOperationRequest REQUEST_WITH_OBJECT =
      new CloudStorageRpcClient.RewriteOperationRequest(
          BLOB_ID.toStorageObject(), EMPTY_OPTIONS, true, BLOB_INFO.toProto(), EMPTY_OPTIONS, null);
  private static final CloudStorageRpcClient.RewriteOperationRequest REQUEST_WITHOUT_OBJECT =
      new RewriteOperationRequest(
          BLOB_ID.toStorageObject(), EMPTY_OPTIONS, false, BLOB_INFO.toProto(), EMPTY_OPTIONS, null);
  private static final RewriteOperationResponse RESPONSE_WITH_OBJECT =
      new RewriteOperationResponse(REQUEST_WITH_OBJECT, null, 42L, false, "token", 21L);
  private static final RewriteOperationResponse RESPONSE_WITHOUT_OBJECT =
      new RewriteOperationResponse(REQUEST_WITHOUT_OBJECT, null, 42L, false, "token", 21L);
  private static final RewriteOperationResponse RESPONSE_WITH_OBJECT_DONE =
      new RewriteOperationResponse(REQUEST_WITH_OBJECT, RESULT_INFO.toProto(), 42L, true, "token", 42L);
  private static final RewriteOperationResponse RESPONSE_WITHOUT_OBJECT_DONE =
      new RewriteOperationResponse(REQUEST_WITHOUT_OBJECT, RESULT_INFO.toProto(), 42L, true, "token", 42L);

  private StorageSettings options;
  private StorageRpcFactory rpcFactoryMock;
  private CloudStorageRpcClient storageRpcMock;
  private BlobCopyWriter copyWriter;
  ...
  public void setUp() {
    ...
    options =
        StorageSettings.newStorageBuilder()
            ...
            .build();
    result = new StorageObject(options.getService(), new BlobAttributes.BlobInfoBuilderImpl(RESULT_INFO));
  }
  ...
  public void testRewriteWithObject() {
    ...
    copyWriter = new BlobCopyWriter(options, RESPONSE_WITH_OBJECT);
  ...
  public void testRewriteWithoutObject() {
    ...
    EasyMock.replay(storageRpcMock);
    copyWriter = new BlobCopyWriter(options, RESPONSE_WITHOUT_OBJECT);
    assertEquals(result, copyWriter.getResult());
    assertTrue(copyWriter.isDone());
    assertEquals(42L, copyWriter.getTotalBytesCopied());
    assertEquals(42L, copyWriter.getBlobSize());
  ...
  public void testSaveAndRestoreWithObject() {
    ...
    EasyMock.replay(storageRpcMock);
    copyWriter = new BlobCopyWriter(options, RESPONSE_WITH_OBJECT);
    copyWriter.copyNextChunk();
    assertTrue(!copyWriter.isDone());
    assertEquals(21L, copyWriter.getTotalBytesCopied());
    assertEquals(42L, copyWriter.getBlobSize());
    RestorableState<BlobCopyWriter> rewriterState = copyWriter.capture();
    BlobCopyWriter restoredRewriter = rewriterState.restore();
