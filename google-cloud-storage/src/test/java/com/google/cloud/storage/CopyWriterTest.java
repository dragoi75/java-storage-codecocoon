...
public class CopyWriterTest {
  ...
  private static final BlobIdentifier BLOB_ID = BlobIdentifier.from(SOURCE_BUCKET_NAME, SOURCE_BLOB_NAME);
  private static final BlobMetadata BLOB_INFO =
      BlobMetadata.newBuilder(DESTINATION_BUCKET_NAME, DESTINATION_BLOB_NAME).buildMetadata();
  ...
  private static final RewriteResult RESPONSE_WITHOUT_OBJECT =
      new StorageRpcClient.RewriteResult(REQUEST_WITHOUT_OBJECT, null, 42L, false, "token", 21L);
  ...
  private BlobRewriteWriter copyWriter;
  ...
  public void setUp() {
    ...
    options =
        StorageClientOptions.newServiceClientBuilder()
            ...
            .build();
    result = new StorageObject(options.getService(), new BlobMetadata.BlobMetadataBuilderImpl(RESULT_INFO));
  }
  ...
  public void testRewriteWithObject() {
    ...
    copyWriter = new BlobRewriteWriter(options, RESPONSE_WITH_OBJECT);
  ...
  public void testRewriteWithoutObject() {
    ...
    EasyMock.replay(storageRpcMock);
    copyWriter = new BlobRewriteWriter(options, RESPONSE_WITHOUT_OBJECT);
    assertEquals(result, copyWriter.getResult());
    assertTrue(copyWriter.isDone());
    assertEquals(42L, copyWriter.getTotalBytesCopied());
    assertEquals(42L, copyWriter.getBlobSize());
  ...
  public void testSaveAndRestoreWithObject() {
    ...
    assertEquals(42L, copyWriter.getBlobSize());
    RestorableState<BlobRewriteWriter> rewriterState = copyWriter.capture();
    BlobRewriteWriter restoredRewriter = rewriterState.restore();
