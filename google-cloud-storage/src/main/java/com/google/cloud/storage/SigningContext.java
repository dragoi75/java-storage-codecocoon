...
public class SigningContext {
  ...
  private static final List<String> RESERVED_PARAMETER_NAMES_LOWERCASE =
  ...
  private SigningContext(SignedUrlBuilder signedUrlCreator) {
    ...
    ImmutableMap.Builder<String, String> headerCollector =
        new ImmutableMap.Builder<String, String>().putAll(signedUrlCreator.canonicalExtensionHeaders);
    ...
    if (StorageService.UrlSigningOption.SigningVersion.V4.equals(signingVersion)
        && (!signedUrlCreator.canonicalExtensionHeaders.containsKey("host"))) {
  ...
  private String computeV4CanonicalRequestHash() {
    ...
    CanonicalExtensionHeadersSerializer headerSerializer =
    ...
    canonicalReq
        .append(headerSerializer.serialize(canonicalExtensionHeaders))
        .append(COMPONENT_SEPARATOR);
    canonicalReq
        .append(headerSerializer.serializeHeaderNames(canonicalExtensionHeaders))
        .append(COMPONENT_SEPARATOR);

    String providedHash = canonicalExtensionHeaders.get("X-Goog-Content-SHA256");
    canonicalReq.append(providedHash == null ? "UNSIGNED-PAYLOAD" : providedHash);
  ...
  public String buildV4QueryString() {
    ...
    orderedParamMap.put(
        "X-Goog-Credential", rfc3986UriEncode(userEmail + "/" + ymdString + SCOPE, true));
  ...
  public static final class SignedUrlBuilder {
    ...
    public SignedUrlBuilder setCanonicalizedExtensionHeaders(
    ...
    public SigningContext buildSignedUrl() {
      ...
      checkArgument(expiryMillis >= 0, "Expiration must be greater than or equal to zero");
      ...
      if (StorageService.UrlSigningOption.SigningVersion.V4.equals(signingVersion)) {
        ...
        checkArgument(
            expiryMillis <= 604800, "Expiration can't be longer than 7 days to use V4 signing");
      }
