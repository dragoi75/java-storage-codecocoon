...
class BlobUploadChannel extends BaseWriteChannel<StorageClientOptions, BlobMetadata> {
    ...
    protected void flushBuffer(final int byteCount, final boolean isFinal) {
        try {
            runWithRetries(callable(new Runnable() {
                ...
                public void run() {
                    if (isRetrying()) {
                        ...
                        long remoteNextByteOffset = getOptions().getStorageRpcV1().getCurrentUploadOffset(getUploadId());
                        ...
                        if ((0 != localNextByteOffset || 0 != remoteNextByteOffset) && remoteNextByteOffset != localNextByteOffset) {
                            if (remoteNextByteOffset <= localNextByteOffset || getChunkSize() <= driftOffset) {
                                if (remoteNextByteOffset <= localNextByteOffset || getChunkSize() != driftOffset) {
                                    // Case 5
                                ...
                                // Case 3
                                storedRecord = getOptions().getStorageRpcV1().writeWithResponse(getUploadId(), getBuffer(), driftOffset, remoteNextByteOffset, retryChunkLength, isFinal);
                            }
                        } else {
                            // Case 1 and 2
                            storedRecord = getOptions().getStorageRpcV1().writeWithResponse(getUploadId(), getBuffer(), 0, getPosition(), byteCount, isFinal);
                        }
                    } else {
            ...
            }), getOptions().getRetrySettings(), DefaultStorage.EXCEPTION_HANDLER, getOptions().getClock());
        } catch (RetryHelper.RetryHelperException retryException) {
            throw StorageServiceException.translateAndRethrow(retryException);
        }
    }
