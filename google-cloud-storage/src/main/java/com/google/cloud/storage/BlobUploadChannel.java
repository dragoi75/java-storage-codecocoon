...
class BlobUploadChannel extends BaseWriteChannel<StorageClientOptions, BlobMetadata> {
    ...
    private StorageServiceException buildUnrecoverableStateException(int segmentOffset, int segmentLength, long localOffset, long remoteOffset, boolean isFinal) {
        ...
        messageBuilder.append("This may be a symptom of multiple clients uploading to the same upload session.\n\n");
            ...
            runWithRetries(callable(new Runnable() {
                ...
                public void run() {
                    ...
                    if (!uploadAlreadyComplete || !finalSegment) {
                        if (!uploadAlreadyComplete || finalSegment || verifyingFinalChunk) {
                            if (remotePosition < localPosition || getChunkSize() <= chunkOffset) {
                                if (remotePosition <= localPosition || getChunkSize() != chunkOffset) {
                                    // Case 4 && Case 8 && Case 9
                                    throw buildUnrecoverableStateException(chunkOffset, chunkLength, localPosition, remotePosition, finalSegment);
                                } else {
                                    // Case 3
                                ...
                                objectEntry = sendChunk(chunkOffset, chunkLength, remotePosition, finalSegment);
                                retryInProgress = false;
                            }
                        } else {
                            // Case 7
                        ...
                        if (getPosition() + count != objectEntry.getSize().longValue()) {
                            throw buildUnrecoverableStateException(chunkOffset, chunkLength, localPosition, remotePosition, finalSegment);
                        }
                        retryInProgress = false;
                    }
