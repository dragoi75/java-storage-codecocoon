...
public class HttpStorageClient implements CloudStorageRpc {
  ...
  public HttpStorageClient(StorageSettings storageSettings) {
    ...
    backendStore =
        new Storage.Builder(httpClient, new JacksonFactory(), requestInit)
  ...
  private class RpcBatchProcessor implements RpcRequestBatch {
    ...
    public void addDeleteRequest(
            StorageObject objectMetadata, ResultHandler<Void> resultHandler, Map<StorageOption, ?> storageSettings) {
      try {
        if (currentSize == BATCH_SIZE_LIMIT) {
    ...
    public void addPatchRequest(
        ...
        ResultHandler<StorageObject> resultHandler,
  ...
  private static void setEncryptionHeaders(
          HttpHeaders httpMetadata, String encryptionPrefix, Map<StorageOption, ?> storageSettings) {
    ...
    if (secretValue != null) {
      ...
      httpMetadata.set(
          encryptionPrefix + "key-sha256",
          binaryEncoder.encode(hasher.hashBytes(binaryEncoder.decode(secretValue)).asBytes()));
    }
  }
  ...
  private Span createSpan(String operationName) {
    return telemetry
        ...
        .setRecordEvents(metricsModule.isRecordEvents())
        .startSpan();
  }
  ...
  public StorageObject create(
          StorageObject objectMetadata, final InputStream inputStream, Map<StorageOption, ?> storageSettings) {
    Span traceContext = createSpan(HttpStorageRpcSpans.SPAN_NAME_CREATE_OBJECT);
    Scope contextHandle = telemetry.withSpan(traceContext);
    try {
      Storage.Objects.Insert createRequest =
          backendStore
              .objects()
              .insert(
                  objectMetadata.getBucket(),
                      objectMetadata,
                  new InputStreamContent(objectMetadata.getContentType(), inputStream));
      ...
      Iterable<StorageObject> objectIterable =
          Iterables.concat(
              ...
              objectCollection.getPrefixes() != null
                  ? Lists.transform(objectCollection.getPrefixes(), storageObjectFromPrefix(container))
  ...
  public Bucket get(Bucket container, Map<StorageOption, ?> storageSettings) {
    ...
    try {
      ...
      if (operationError.getCode() == HTTP_NOT_FOUND) {
        return null;
      }
  ...
  private Storage.Objects.Get getCall(StorageObject storageItem, Map<StorageOption, ?> storageSettings)
    ...
    return retrievalRequest.setGeneration(storageItem.getGeneration())
        .setProjection(DEFAULT_PROJECTION)
        .setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(storageSettings))
        .setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(storageSettings))
        .setIfGenerationMatch(StorageOption.IF_GENERATION_MATCH.getLong(storageSettings))
  ...
  private Storage.Objects.Patch buildPatchCall(StorageObject objectMetadata, Map<StorageOption, ?> storageSettings)
      throws IOException {
    return backendStore
        .objects()
  ...
  private Storage.Objects.Delete buildDeleteCall(StorageObject deleteTarget, Map<StorageOption, ?> storageSettings)
      throws IOException {
    return backendStore
        .objects()
        .delete(deleteTarget.getBucket(), deleteTarget.getName())
        .setGeneration(deleteTarget.getGeneration())
        .setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(storageSettings))
        .setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(storageSettings))
        .setIfGenerationMatch(StorageOption.IF_GENERATION_MATCH.getLong(storageSettings))
        .setIfGenerationNotMatch(StorageOption.IF_GENERATION_NOT_MATCH.getLong(storageSettings))
  ...
  public StorageObject compose(
          Iterable<StorageObject> inputItems, StorageObject destinationItem, Map<StorageOption, ?> destinationParams) {
    ...
    for (StorageObject storageItem : inputItems) {
      ...
      if (versionNumber != null) {
        ...
        objectElement.setObjectPreconditions(
  ...
  public byte[] load(StorageObject originObject, Map<StorageOption, ?> storageSettings) {
    ...
    Scope contextHandle = telemetry.withSpan(traceContext);
    try {
      Storage.Objects.Get retrievalRequest =
          backendStore
              .objects()
  ...
  @Override
  public long read(
          StorageObject originObject, Map<StorageOption, ?> storageSettings, long offset, OutputStream destStream) {
    ...
    try {
      ...
      traceContext.setStatus(Status.UNKNOWN.withDescription(ioEx.getMessage()));
      StorageOperationException operationError = translateException(ioEx);
      if (operationError.getCode() == STATUS_REQUESTED_RANGE_NOT_SATISFIABLE) {
        return 0;
      }
      throw operationError;
    } finally {
  ...
  public Tuple<String, byte[]> read(
          StorageObject originObject, Map<StorageOption, ?> storageSettings, long offset, int length) {
    Span traceContext = createSpan(HttpStorageRpcSpans.SPAN_NAME_READ);
    Scope contextHandle = telemetry.withSpan(traceContext);
    try {
  ...
  public void write(
    ...
    try {
      ...
      HttpRequest request =
          backendStore
              .getRequestFactory()
              .buildPutRequest(endpoint, new ByteArrayContent(null, writeBuffer, bufferOffset, byteCount));
  ...
  public String open(StorageObject storageItem, Map<StorageOption, ?> storageSettings) {
    ...
    try {
      ...
      HttpRequest request =
          requestMaker.buildPostRequest(endpoint, new JsonHttpContent(jsonParser, storageItem));
      HttpHeaders httpHeaderCollection = request.getHeaders();
      httpHeaderCollection.set(
  ...
  public String open(String signedUrl) {
    ...
    try {
      ...
      HttpRequestFactory requestMaker = backendStore.getRequestFactory();
      HttpRequest request =
          requestMaker.buildPostRequest(
                  endpoint, new ByteArrayContent("", rawBytes, 0, rawBytes.length));
      HttpHeaders httpHeaderCollection = request.getHeaders();
  ...
  private RewriteOperationResponse performRewrite(RewriteOperationRequest readRequest, String authToken) {
    try {
      ...
      Long maxBytesPerCall =
          readRequest.megabytesRewrittenPerCall != null ? readRequest.megabytesRewrittenPerCall * BYTES_PER_MB : null;
      Storage.Objects.Rewrite rewriteOp =
          backendStore
              ...
              .setIfSourceMetagenerationMatch(
                  StorageOption.IF_SOURCE_METAGENERATION_MATCH.getLong(readRequest.sourceOptions))
              .setIfSourceMetagenerationNotMatch(
                  StorageOption.IF_SOURCE_METAGENERATION_NOT_MATCH.getLong(readRequest.sourceOptions))
              .setIfSourceGenerationMatch(
                  StorageOption.IF_SOURCE_GENERATION_MATCH.getLong(readRequest.sourceOptions))
              .setIfSourceGenerationNotMatch(
                  StorageOption.IF_SOURCE_GENERATION_NOT_MATCH.getLong(readRequest.sourceOptions))
              .setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(readRequest.targetOptions))
              .setIfMetagenerationNotMatch(
                  StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(readRequest.targetOptions))
              .setIfGenerationMatch(StorageOption.IF_GENERATION_MATCH.getLong(readRequest.targetOptions))
              .setIfGenerationNotMatch(StorageOption.IF_GENERATION_NOT_MATCH.getLong(readRequest.targetOptions))
              .setDestinationPredefinedAcl(StorageOption.PREDEFINED_ACL.getString(readRequest.targetOptions))
              .setUserProject(projectOwner)
              .setDestinationKmsKeyName(StorageOption.KMS_KEY_NAME.getString(readRequest.targetOptions));
  ...
  public ObjectAccessControl getAcl(String container, String storageItem, Long versionNumber, String entityId) {
    ...
    try {
      return backendStore
          ...
          .get(container, storageItem, entityId)
  ...
  public boolean deleteAcl(String container, String storageItem, Long versionNumber, String entityId) {
    ...
    try {
      backendStore
          .objectAccessControls()
          .delete(container, storageItem, entityId)
          .setGeneration(versionNumber)
          .execute();
      return true;
    } catch (IOException ioEx) {
  ...
  public Tuple<String, Iterable<HmacKeyMetadata>> listHmacKeys(Map<StorageOption, ?> storageSettings) {
    ...
    try {
      HmacKeysMetadata hmacCollectionInfo =
          backendStore
              .projects()
              .hmacKeys()
      ...
      return Tuple.<String, Iterable<HmacKeyMetadata>>of(
          hmacCollectionInfo.getNextPageToken(), hmacCollectionInfo.getItems());
    } catch (IOException ioEx) {
      traceContext.setStatus(Status.UNKNOWN.withDescription(ioEx.getMessage()));
      throw translateException(ioEx);
    } finally {
  ...
  public Policy setIamPolicy(String container, Policy iamPolicy, Map<StorageOption, ?> storageSettings) {
    ...
    try {
      return backendStore
          .buckets()
          .setIamPolicy(container, iamPolicy)
          .setUserProject(StorageOption.USER_PROJECT.getString(storageSettings))
          .execute();
    } catch (IOException ioEx) {
      traceContext.setStatus(Status.UNKNOWN.withDescription(ioEx.getMessage()));
  ...
  public TestIamPermissionsResponse testIamPermissions(
          String container, List<String> permissionList, Map<StorageOption, ?> storageSettings) {
