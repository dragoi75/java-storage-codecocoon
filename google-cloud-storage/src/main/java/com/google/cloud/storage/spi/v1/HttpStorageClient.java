...
public class HttpStorageClient implements StorageRpcClient {
  ...
  public HttpStorageClient(StorageClientOptions clientConfig) {
    ...
    dataStore =
        new Storage.Builder(httpConnection, new JacksonFactory(), requestInit)
  ...
  private class DefaultRpcBatcher implements RpcRequestBatch {
    ...
    public void addDeleteOperation(
            StorageObject objectMetadata, ResultCallback<Void> completionHandler, Map<StorageOption, ?> clientConfig) {
      try {
    ...
    public void addPatchOperation(
        ...
        ResultCallback<StorageObject> completionHandler,
  ...
  private static void setEncryptionHeaders(
          HttpHeaders httpFields, String encKeyTag, Map<StorageOption, ?> clientConfig) {
    ...
    if (secretId != null) {
      ...
      httpFields.set(
          encKeyTag + "key-sha256",
          b64Encoder.encode(digestFunc.hashBytes(b64Encoder.decode(secretId)).asBytes()));
    }
  }
  ...
  private Span createSpan(String operationName) {
    return telemetry
        ...
        .setRecordEvents(monitoringModule.isRecordEvents())
        .startSpan();
  }
  ...
  public Bucket create(Bucket container, Map<StorageOption, ?> clientConfig) {
    ...
    try {
      return dataStore
          .buckets()
          .insert(this.clientConfig.getProjectId(), container)
  ...
  public StorageObject create(
          StorageObject objectMetadata, final InputStream inputStream, Map<StorageOption, ?> clientConfig) {
    Span spanContext = createSpan(HttpStorageRpcSpans.SPAN_NAME_CREATE_OBJECT);
    Scope tracingScope = telemetry.withSpan(spanContext);
    try {
      Storage.Objects.Insert uploadAction =
          dataStore
              .objects()
              .insert(
                  objectMetadata.getBucket(),
                      objectMetadata,
                  new InputStreamContent(resolveContentType(objectMetadata, clientConfig), inputStream));
  ...
  private Storage.Objects.Get getCall(StorageObject obj, Map<StorageOption, ?> clientConfig)
      throws IOException {
    Storage.Objects.Get fetchRequest = dataStore.objects().get(obj.getBucket(), obj.getName());
    setEncryptionHeaders(fetchRequest.getRequestHeaders(), ENCRYPTION_PREFIX, clientConfig);
    return fetchRequest.setGeneration(obj.getGeneration())
        .setProjection(DEFAULT_PROJECTION)
        .setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(clientConfig))
        .setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(clientConfig))
        .setIfGenerationMatch(StorageOption.IF_GENERATION_MATCH.getLong(clientConfig))
  ...
  public Bucket patch(Bucket container, Map<StorageOption, ?> clientConfig) {
    ...
    try {
      ...
      return dataStore
          ...
          .setProjection(selectFields == null ? DEFAULT_PROJECTION : selectFields)
          .setPredefinedAcl(StorageOption.PREDEFINED_ACL.getString(clientConfig))
          .setPredefinedDefaultObjectAcl(StorageOption.PREDEFINED_DEFAULT_OBJECT_ACL.getString(clientConfig))
          .setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(clientConfig))
          .setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(clientConfig))
  ...
  private Storage.Objects.Patch buildPatchCall(StorageObject objectMetadata, Map<StorageOption, ?> clientConfig)
      throws IOException {
    return dataStore
        .objects()
  ...
  private Storage.Objects.Delete buildDeleteCall(StorageObject storageEntry, Map<StorageOption, ?> clientConfig)
      throws IOException {
    return dataStore
        .objects()
        .delete(storageEntry.getBucket(), storageEntry.getName())
        .setGeneration(storageEntry.getGeneration())
        .setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(clientConfig))
        .setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(clientConfig))
        .setIfGenerationMatch(StorageOption.IF_GENERATION_MATCH.getLong(clientConfig))
        .setIfGenerationNotMatch(StorageOption.IF_GENERATION_NOT_MATCH.getLong(clientConfig))
  ...
  public StorageObject compose(
          Iterable<StorageObject> storageObjects, StorageObject destinationObject, Map<StorageOption, ?> destinationParams) {
    ...
    for (StorageObject originObject : storageObjects) {
      ...
      if (version != null) {
        ...
        objectEntry.setObjectPreconditions(
            new ObjectPreconditions().setIfGenerationMatch(version));
      }
    ...
    try {
      return dataStore
          .objects()
          .compose(destinationObject.getBucket(), destinationObject.getName(), composePayload)
  ...
  public byte[] load(StorageObject sourceObject, Map<StorageOption, ?> clientConfig) {
    ...
    Scope tracingScope = telemetry.withSpan(spanContext);
    try {
      Storage.Objects.Get readOperation =
          dataStore
              .objects()
  ...
  @Override
  public long read(
          StorageObject sourceObject, Map<StorageOption, ?> clientConfig, long offset, OutputStream destinationStream) {
    ...
    try {
      ...
      spanContext.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
      StorageServiceException storageFailure = toStorageServiceException(ioException);
      if (storageFailure.getCode() == SC_RANGE_NOT_SATISFIABLE) {
        return 0;
      }
      throw storageFailure;
    } finally {
  ...
  public Tuple<String, byte[]> read(
          StorageObject sourceObject, Map<StorageOption, ?> clientConfig, long offset, int length) {
    Span spanContext = createSpan(HttpStorageRpcSpans.SPAN_NAME_READ);
    Scope tracingScope = telemetry.withSpan(spanContext);
    try {
  ...
  public long getCurrentUploadOffset(String transferId) {
    try {
      ...
      HttpRequest httpCall =
          dataStore.getRequestFactory().buildPutRequest(endpoint, new EmptyContent());
  ...
  public StorageObject writeWithResponse(
    ...
    try {
      ...
      HttpRequest httpCall =
          dataStore
              .getRequestFactory()
              .buildPutRequest(endpoint, new ByteArrayContent(null, dataChunk, sourceOffset, count));
  ...
  public String open(StorageObject obj, Map<StorageOption, ?> clientConfig) {
    ...
    try {
      ...
      if (kmsKey != null && kmsKey.contains("cryptoKeyVersions")) {
        obj.setKmsKeyName("");
      }
      Insert getOperation =
          dataStore
              .objects()
              .insert(obj.getBucket(), obj)
              .setName(obj.getName())
              .setProjection(StorageOption.PROJECTION.getString(clientConfig))
              .setPredefinedAcl(StorageOption.PREDEFINED_ACL.getString(clientConfig))
              .setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(clientConfig))
              .setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(clientConfig))
              .setIfGenerationMatch(StorageOption.IF_GENERATION_MATCH.getLong(clientConfig))
      ...
      HttpRequest httpCall =
          httpRequestFactory.buildPostRequest(endpoint, new JsonHttpContent(jsonFactoryInstance, obj));
    ...
    try {
      ...
      HttpRequest httpCall =
          httpRequestFactory.buildPostRequest(
                  endpoint, new ByteArrayContent("", dataBytes, 0, dataBytes.length));
      HttpHeaders httpMetadata = httpCall.getHeaders();
    ...
    try {
      ...
      Storage.Objects.Rewrite rewriteOperation =
          dataStore
              ...
              .setUserProject(projectOverride)
      ...
      return new RewriteResult(
              getOperation,
          responsePayload.getResource(),
          responsePayload.getObjectSize().longValue(),
          responsePayload.getDone(),
          responsePayload.getRewriteToken(),
          responsePayload.getTotalBytesRewritten().longValue());
    } catch (IOException ioException) {
  ...
  public boolean deleteAcl(String container, String obj, Long version, String principal) {
    ...
    try {
      dataStore
          .objectAccessControls()
          .delete(container, obj, principal)
          .setGeneration(version)
          .execute();
      return true;
    } catch (IOException ioException) {
  ...
  public HmacKey createHmacKey(String principalEmail, Map<StorageOption, ?> clientConfig) {
    ...
    try {
      return dataStore
          .projects()
          .hmacKeys()
          .create(tenantId, principalEmail)
          .setUserProject(StorageOption.USER_PROJECT.getString(clientConfig))
          .execute();
    } catch (IOException ioException) {
      spanContext.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
  ...
  public Tuple<String, Iterable<HmacKeyMetadata>> listHmacKeys(Map<StorageOption, ?> clientConfig) {
    ...
    try {
      ...
      return Tuple.<String, Iterable<HmacKeyMetadata>>of(
          hmacMetadata.getNextPageToken(), hmacMetadata.getItems());
    } catch (IOException ioException) {
      spanContext.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
      throw toStorageServiceException(ioException);
    } finally {
  ...
  public HmacKeyMetadata updateHmacKey(HmacKeyMetadata hmacMetadata, Map<StorageOption, ?> clientConfig) {
    ...
    try {
      return dataStore
          .projects()
          .hmacKeys()
          .update(tenantId, hmacMetadata.getAccessId(), hmacMetadata)
          .setUserProject(StorageOption.USER_PROJECT.getString(clientConfig))
          .execute();
    } catch (IOException ioException) {
      spanContext.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
  ...
  public void deleteHmacKey(HmacKeyMetadata hmacMetadata, Map<StorageOption, ?> clientConfig) {
    ...
    try {
      dataStore
          .projects()
          .hmacKeys()
          .delete(tenantId, hmacMetadata.getAccessId())
          .setUserProject(StorageOption.USER_PROJECT.getString(clientConfig))
          .execute();
    } catch (IOException ioException) {
      spanContext.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
  ...
  public TestIamPermissionsResponse testIamPermissions(
          String container, List<String> accessList, Map<StorageOption, ?> clientConfig) {
    Span spanContext = createSpan(HttpStorageRpcSpans.SPAN_NAME_TEST_BUCKET_IAM_PERMISSIONS);
    Scope tracingScope = telemetry.withSpan(spanContext);
    try {
      return dataStore
          .buckets()
          .testIamPermissions(container, accessList)
          .setUserProject(StorageOption.USER_PROJECT.getString(clientConfig))
          .execute();
    } catch (IOException ioException) {
      spanContext.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
  ...
  public Bucket lockRetentionPolicy(Bucket container, Map<StorageOption, ?> clientConfig) {
    ...
    try {
      return dataStore
          .buckets()
          .lockRetentionPolicy(container.getName(), StorageOption.IF_METAGENERATION_MATCH.getLong(clientConfig))
          .setUserProject(StorageOption.USER_PROJECT.getString(clientConfig))
          .execute();
    } catch (IOException ioException) {
      spanContext.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
