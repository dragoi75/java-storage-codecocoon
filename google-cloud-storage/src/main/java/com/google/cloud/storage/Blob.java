...
public class Blob extends BlobInfo {
   ...
   * want to change, buildIamSettings the new {@code Blob} instance, and then call {@link
   ...
   * BlobId blobId = BlobId.from(bucketName, blobName);
   * Blob blob = storage.get(blobId);
   * blob.toUniformBucketLevelAccessBuilder().setContentType("text/plain").buildIamSettings().update();
   * }</pre>
   *
