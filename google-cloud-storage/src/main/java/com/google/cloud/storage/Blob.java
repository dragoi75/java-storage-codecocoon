...
public class Blob extends BlobInfo {
  ...
  static final Function<Tuple<StorageClient, StorageObject>, Blob> BLOB_FROM_PB_FUNCTION =
      new Function<Tuple<StorageClient, StorageObject>, Blob>() {
        @Override
        public Blob apply(Tuple<StorageClient, StorageObject> pb) {
  ...
  public static class BlobSourceOption extends Option {
    ...
    private StorageClient.BlobSourceOptions toSourceOptions(BlobInfo blobInfo) {
      switch (getRpcOption()) {
        case IF_GENERATION_MATCH:
          return StorageClient.BlobSourceOptions.ifGenerationMatch(blobInfo.getGeneration());
        case IF_GENERATION_NOT_MATCH:
          return StorageClient.BlobSourceOptions.ifGenerationNotMatch(blobInfo.getGeneration());
        case IF_METAGENERATION_MATCH:
          return StorageClient.BlobSourceOptions.ifMetagenerationMatch(blobInfo.getMetageneration());
        case IF_METAGENERATION_NOT_MATCH:
          return StorageClient.BlobSourceOptions.ifMetagenerationNotMatch(blobInfo.getMetageneration());
        case CUSTOMER_SUPPLIED_KEY:
          return StorageClient.BlobSourceOptions.withDecryptionKey((String) getValue());
        case USER_PROJECT:
          return StorageClient.BlobSourceOptions.setUserProject((String) getValue());
        default:
          throw new AssertionError("Unexpected enum value");
      }
    }

    private StorageClient.BlobGetOptions toGetOption(BlobInfo blobInfo) {
      switch (getRpcOption()) {
        case IF_GENERATION_MATCH:
          return StorageClient.BlobGetOptions.ifGenerationMatch(blobInfo.getGeneration());
        case IF_GENERATION_NOT_MATCH:
          return StorageClient.BlobGetOptions.ifGenerationNotMatch(blobInfo.getGeneration());
        case IF_METAGENERATION_MATCH:
          return StorageClient.BlobGetOptions.ifMetagenerationMatch(blobInfo.getMetageneration());
        case IF_METAGENERATION_NOT_MATCH:
          return StorageClient.BlobGetOptions.ifMetagenerationNotMatch(blobInfo.getMetageneration());
        case USER_PROJECT:
          return StorageClient.BlobGetOptions.setUserProject((String) getValue());
        case CUSTOMER_SUPPLIED_KEY:
          return StorageClient.BlobGetOptions.withDecryptionKey((String) getValue());
        default:
          throw new AssertionError("Unexpected enum value");
      }
    }
    ...
    static StorageClient.BlobSourceOptions[] toSourceOptions(
        BlobInfo blobInfo, BlobSourceOption... options) {
      StorageClient.BlobSourceOptions[] convertedOptions = new StorageClient.BlobSourceOptions[options.length];
      int index = 0;
    ...
    static StorageClient.BlobGetOptions[] toGetOptions(BlobInfo blobInfo, BlobSourceOption... options) {
      StorageClient.BlobGetOptions[] convertedOptions = new StorageClient.BlobGetOptions[options.length];
      int index = 0;
      for (BlobSourceOption option : options) {
  ...
  Blob(StorageClient storage, BlobInfo.BuilderImpl infoBuilder) {
    super(infoBuilder);
  ...
  public boolean exists(BlobSourceOption... options) {
    ...
    StorageClient.BlobGetOptions[] getOptions = Arrays.copyOf(toGetOptions(this, options), length + 1);
    getOptions[length] = StorageClient.BlobGetOptions.setFields();
   ...
   * current blob metadata are at their latest version use the {@code withMetagenerationMatch} option:
   * {@code newBlob.update(BlobUploadOption.withMetagenerationMatch())}.
   *
   ...
   * @throws StorageException upon failure
   */
  public Blob update(StorageClient.BlobUploadOption... options) {
  ...
  public CopyWriter copyTo(BlobId targetBlob, BlobSourceOption... options) {
    StorageClient.CopyJobRequest copyRequest =
   ...
   * @throws StorageException upon failure
   */
  public WriteChannel writer(WriteBlobOption... options) {
  ...
  /** Returns the blob's {@code StorageClient} object used to issue requests. */
  ...
  static Blob fromPb(StorageClient storage, StorageObject storageObject) {
