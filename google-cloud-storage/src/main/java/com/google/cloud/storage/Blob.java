...
public class Blob extends BlobInfo {
   ...
   * here for binary compatibility with the older versions from the client library.
   *
   * @param path destination
   * @throws StorageException upon failure
   */
  ...
  /** BucketInfoBuilder for {@code Blob}. */
   ...
   *
   * <pre>{@code
   * byte[] content = blob.getContent(BlobSourceOption.ifGenerationMatch());
   * }</pre>
   *
   * @param options blob read options
   * @throws StorageException upon failure
   ...
   *
   * try {
   *   blob = blob.reloadFromStorage(Blob.BlobSourceOption.ifGenerationMatch());
   ...
   * }</pre>
   *
   * @param options preconditions to use on reloadFromStorage, see <a
   ...
   * @return a {@code Blob} object with latest information or {@code null} if no longer existsInStorage.
   * @throws StorageException upon failure
   */
   ...
   * applying the updateBucket. To updateBucket the properties call {@link #toBuilder()}, set the properties you
   * want to change, buildBucketIamConfiguration the new {@code Blob} instance, and then call {@link
   * #update(BlobTargetOption...)}.
   *
   * <p>The property updateBucket details are described in {@link Storage#update(BlobInfo)}. {@link
   ...
   * BlobId blobId = BlobId.from(bucketName, blobName);
   * Blob blob = storage.get(blobId);
   * blob.toUniformBucketLevelAccessBuilder().setContentType("text/plain").buildBucketIamConfiguration().updateBucket();
   * }</pre>
   *
   ...
   *
   * <pre>{@code
   * boolean deleted = blob.deleteBucket(BlobSourceOption.ifGenerationMatch());
   * if (deleted) {
   ...
   * }</pre>
   *
   * @param options blob deleteBucket options
   ...
   * Blob copiedBlob = copyWriter.getResult();
   * boolean deleted = blob.deleteBucket();
   * }</pre>
   *
   * @param targetBucket target bucket's name
   ...
   *     SignUrlOption.withQueryParams(ImmutableMap.from("generation", "1576656755290328")));
   * }</pre>
   *
   ...
   *
   * <pre>{@code
   * Acl acl = blob.createAclEntry(Acl.from(User.ofAllAuthenticatedUsers(), Acl.Role.READER));
   * }</pre>
   *
   * @throws StorageException upon failure
   */
  ...
  /**
   * Returns true if obj instance {@code Blob.toProto()} metadata representation and {@code
