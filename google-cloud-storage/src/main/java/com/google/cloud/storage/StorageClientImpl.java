...
final class StorageClientImpl extends BaseService<StorageSettings> implements StorageClient {
  ...
  private static final Function<Tuple<StorageClient, Boolean>, Boolean> DELETE_FUNCTION =
  ...
  public StorageBlob create(BlobMetadata blobMetadata, BlobUploadOption... settings) {
    BlobMetadata updatedMetadata =
        blobMetadata
            .toBlobInfoBuilder()
            .setMd5(ZERO_LENGTH_BYTES_MD5)
            .setCrc32c(ZERO_LENGTH_BYTES_CRC32C)
            .buildStorageObject();
  ...
  public StorageBlob create(
          BlobMetadata blobMetadata, byte[] dataBytes, int startOffset, int chunkLength, BlobUploadOption... settings) {
    dataBytes = firstNonNull(dataBytes, ZERO_LENGTH_BYTES);
  ...
  public StorageBlob create(BlobMetadata blobMetadata, InputStream dataBytes, BlobWriteSetting... settings) {
    ...
    return com.google.cloud.storage.StorageBlob.fromProto(this, cloudStorageClient.create(objectProto, inputStreamRef, storageOptionMap));
  }
  ...
  private StorageBlob createInternal(BlobMetadata blobMetadata, final byte[] dataBytes, BlobUploadOption... settings) {
    ...
    try {
      return com.google.cloud.storage.StorageBlob.fromProto(
          this,
          runWithRetries(
    ...
    try {
      StorageObject objectRecord =
          runWithRetries(
              ...
              EXCEPTION_HANDLER,
              getOptions().getClock());
      return objectRecord == null ? null : com.google.cloud.storage.StorageBlob.fromProto(this, objectRecord);
    } catch (RetryHelperException retryEx) {
      throw StorageOperationException.translateAndRethrow(retryEx);
    }
  }
  ...
  private static class BucketPageRetriever implements NextPageFetcher<StorageBucket> {
    ...
    BucketPageRetriever(
            StorageSettings serviceSettings, String pageToken, Map<CloudStorageRpc.StorageOption, ?> settingsMap) {
      this.requestSettings =
          PageImpl.nextRequestOptions(CloudStorageRpc.StorageOption.PAGE_TOKEN, pageToken, settingsMap);
  ...
  private static class BlobPageIterator implements NextPageFetcher<StorageBlob> {
    ...
    BlobPageIterator(
        ...
        StorageSettings serviceSettings,
        String pageToken,
        Map<CloudStorageRpc.StorageOption, ?> settingsMap) {
      this.requestSettings =
          PageImpl.nextRequestOptions(CloudStorageRpc.StorageOption.PAGE_TOKEN, pageToken, settingsMap);
      this.serviceSettings = serviceSettings;
  ...
  private static Page<StorageBucket> listAllBuckets(
          final StorageSettings serviceSettings, final Map<CloudStorageRpc.StorageOption, ?> storageOptionMap) {
    try {
      Tuple<String, Iterable<com.google.api.services.storage.model.Bucket>> pageCursorAndBuckets =
          runWithRetries(
              new Callable<
      ...
      Iterable<StorageBucket> storageBuckets =
          pageCursorAndBuckets.y() == null
              ? ImmutableList.<StorageBucket>of()
              : Iterables.transform(
                  pageCursorAndBuckets.y(),
      ...
      return new PageImpl<>(
          new BucketPageRetriever(serviceSettings, pageToken, storageOptionMap), pageToken, storageBuckets);
    } catch (RetryHelperException retryEx) {
      throw StorageOperationException.translateAndRethrow(retryEx);
    }
  }
  ...
  private static Page<StorageBlob> listAllBlobs(
      ...
      final StorageSettings serviceSettings,
      final Map<CloudStorageRpc.StorageOption, ?> storageOptionMap) {
    try {
      Tuple<String, Iterable<StorageObject>> pageCursorAndBuckets =
          runWithRetries(
              ...
              serviceSettings.getClock());
      String pageToken = pageCursorAndBuckets.x();
      Iterable<StorageBlob> storageBlobs =
          pageCursorAndBuckets.y() == null
              ...
              : Iterables.transform(
                  ...
                  new Function<StorageObject, StorageBlob>() {
                    ...
                    public StorageBlob apply(StorageObject storageObject) {
                      return com.google.cloud.storage.StorageBlob.fromProto(serviceSettings.getService(), storageObject);
                    }
                  });
      return new PageImpl<>(
          new BlobPageIterator(containerName, serviceSettings, pageToken, storageOptionMap), pageToken, storageBlobs);
    } catch (RetryHelperException retryEx) {
      throw StorageOperationException.translateAndRethrow(retryEx);
    }
  }
  ...
  public StorageBlob compose(final ComposeBlobsRequest mergeBlobsSpec) {
    final List<StorageObject> sourceList =
        Lists.newArrayListWithCapacity(mergeBlobsSpec.getSourceBlobs().size());
    for (ComposeBlobsRequest.StorageBlob inputBlob : mergeBlobsSpec.getSourceBlobs()) {
      sourceList.add(
          BlobMetadata.newBuilder(
                  BlobIdentifier.create(
                      mergeBlobsSpec.getTarget().getBucket(),
                      inputBlob.getName(),
                      inputBlob.getGeneration()))
              .buildStorageObject()
              .toProto());
    }
          ...
          runWithRetries(
              new Callable<RewriteOperationResponse>() {
                ...
                public CloudStorageRpc.RewriteOperationResponse call() {
                  return cloudStorageClient.openRewrite(
                      new CloudStorageRpc.RewriteOperationRequest(
                              originObject,
                              originOptions,
                          copyOperationSpec.getOverrideInfo(),
                              destinationObj,
                              targetPair,
                          copyOperationSpec.getMegabytesCopiedPerChunk()));
                }
              },
              getOptions().getRetrySettings(),
              EXCEPTION_HANDLER,
  ...
  public URL signUrl(BlobMetadata blobMetadata, long timeout, TimeUnit timeUnit, UrlSigningOption... settings) {
    ...
    boolean useV2 =
        getPreferredSignatureVersion(settingsMap).equals(UrlSigningOption.SignatureProtocolVersion.V2);
    boolean useV4 =
        getPreferredSignatureVersion(settingsMap).equals(UrlSigningOption.SignatureProtocolVersion.V4);

    ServiceAccountSigner serviceAccountSigner =
        (ServiceAccountSigner) settingsMap.get(UrlSigningOption.RequestOption.SERVICE_ACCOUNT_CRED);
    ...
    long expiryMillis =
        useV4
            ? TimeUnit.SECONDS.convert(timeUnit.toMillis(timeout), TimeUnit.MILLISECONDS)
            : TimeUnit.SECONDS.convert(
                getOptions().getClock().millisTime() + timeUnit.toMillis(timeout),
                TimeUnit.MILLISECONDS);

    checkArgument(
        !(settingsMap.containsKey(UrlSigningOption.RequestOption.VIRTUAL_HOSTED_STYLE)
            && settingsMap.containsKey(UrlSigningOption.RequestOption.PATH_STYLE)
            && settingsMap.containsKey(UrlSigningOption.RequestOption.BUCKET_BOUND_HOST_NAME)),
    ...
    String xmlHostName =
        pathStyleEnabled
            ? STORAGE_XML_SCHEME + "://" + getBaseStorageHostName(settingsMap)
            : STORAGE_XML_SCHEME + "://" + bucketId + "." + getBaseStorageHostName(settingsMap);
    ...
    String signedPath =
        pathStyleEnabled
            ? buildResourceUriPath(bucketId, encodedObjectName, settingsMap)
            : buildResourceUriPath("", encodedObjectName, settingsMap);
    ...
    URI signingUri =
        useV2 ? URI.create(buildResourceUriPath(bucketId, encodedObjectName, settingsMap)) : resourceUri;

    try {
      SignatureMetadata signatureMetadata =
          buildSignatureMetadata(
                  settingsMap, blobMetadata, expiryMillis, signingUri, serviceAccountSigner.getAccount());
  ...
  private String buildResourceUriPath(
      ...
      String encodedObjectName,
      EnumMap<UrlSigningOption.RequestOption, Object> settingsMap) {
  ...
  private SignatureMetadata buildSignatureMetadata(
      Map<UrlSigningOption.RequestOption, Object> settingsMap,
    ...
    HttpRequestMethod requestMethod =
        settingsMap.containsKey(UrlSigningOption.RequestOption.HTTP_METHOD)
            ? (HttpRequestMethod) settingsMap.get(UrlSigningOption.RequestOption.HTTP_METHOD)
            : HttpRequestMethod.GET;

    SignatureMetadata.CanonicalRequestBuilder canonicalBuilder =
        new SignatureMetadata.CanonicalRequestBuilder(requestMethod, expiryMillis, resourceUri);
    ...
    canonicalBuilder.setSignatureVersion(
        (UrlSigningOption.SignatureProtocolVersion) settingsMap.get(UrlSigningOption.RequestOption.SIGNATURE_VERSION));
    ...
    boolean useV4 =
        UrlSigningOption.SignatureProtocolVersion.V4.equals(
            settingsMap.get(UrlSigningOption.RequestOption.SIGNATURE_VERSION));
    if (useV4) { // We don't sign the host header for V2 signed URLs; only do this for V4.
      ...
      if (settingsMap.containsKey(UrlSigningOption.RequestOption.VIRTUAL_HOSTED_STYLE)) {
        extraHeadersBuilder.put(
            "host",
            bucketNameWithoutSlashes(blobMetadata) + "." + getBaseStorageHostName(settingsMap));
      } else if (settingsMap.containsKey(UrlSigningOption.RequestOption.HOST_NAME)
          || settingsMap.containsKey(UrlSigningOption.RequestOption.BUCKET_BOUND_HOST_NAME)) {
    ...
    if (settingsMap.containsKey(UrlSigningOption.RequestOption.EXT_HEADERS)) {
      extraHeadersBuilder.putAll(
          (Map<String, String>) settingsMap.get(UrlSigningOption.RequestOption.EXT_HEADERS));
    }
    ...
    if (settingsMap.containsKey(UrlSigningOption.RequestOption.QUERY_PARAMS)) {
      paramsBuilder.putAll(
          (Map<String, String>) settingsMap.get(UrlSigningOption.RequestOption.QUERY_PARAMS));
    }
  ...
  private String getBaseStorageHostName(Map<UrlSigningOption.RequestOption, Object> settingsMap) {
    ...
    String bucketScopedHost =
        (String) settingsMap.get(UrlSigningOption.RequestOption.BUCKET_BOUND_HOST_NAME);
  ...
  public List<StorageBlob> get(Iterable<BlobIdentifier> identifiers) {
    ...
    for (BlobIdentifier objectName : identifiers) {
      operationBatch
          .get(objectName)
          .notify(
              new BatchResult.Callback<StorageBlob, StorageOperationException>() {
                @Override
  ...
  public List<Boolean> delete(Iterable<BlobIdentifier> identifiers) {
    ...
    for (BlobIdentifier objectName : identifiers) {
      operationBatch
          ...
          .notify(
              new BatchResult.Callback<Boolean, StorageOperationException>() {
                @Override
    ...
    try {
      ...
      BucketAccessControl resultBucket =
          runWithRetries(
              ...
              EXCEPTION_HANDLER,
              getOptions().getClock());
      return resultBucket == null ? null : AccessControlEntry.fromProto(resultBucket);
    } catch (RetryHelperException retryEx) {
      throw StorageOperationException.translateAndRethrow(retryEx);
    }
  }
  ...
  public boolean deleteAcl(
          final String containerName, final TypedEntity aclEntity, BucketSourceOptions... settings) {
    try {
      final Map<CloudStorageRpc.StorageOption, ?> storageOptionMap = buildOptionMap(settings);
      return runWithRetries(
          new Callable<Boolean>() {
  ...
  @Override
  public boolean deleteAcl(final String containerName, final AccessControlEntry.TypedEntity aclEntity) {
  ...
  @Override
  public AccessControlEntry getDefaultAcl(final String containerName, final AccessControlEntry.TypedEntity aclEntity) {
    try {
      ...
      ObjectAccessControl resultBucket =
          runWithRetries(
              new Callable<ObjectAccessControl>() {
                @Override
                public ObjectAccessControl call() {
                  return cloudStorageClient.getAcl(
                      objectName.getBucket(), objectName.getName(), objectName.getGeneration(), aclEntity.toProto());
                }
              },
              getOptions().getRetrySettings(),
              EXCEPTION_HANDLER,
    ...
    try {
      return runWithRetries(
          new Callable<Boolean>() {
            @Override
            public Boolean call() {
              return cloudStorageClient.deleteAcl(
                  objectName.getBucket(), objectName.getName(), objectName.getGeneration(), aclEntity.toProto());
            }
          },
          getOptions().getRetrySettings(),
      ...
      List<ObjectAccessControl> resultBucket =
          runWithRetries(
              new Callable<List<ObjectAccessControl>>() {
                @Override
                public List<ObjectAccessControl> call() {
                  return cloudStorageClient.listAcls(
  ...
  public HmacSecretKey createHmacKey(
          final ServiceAccountInfo accountInfo, final CreateHmacKeyRequestOption... settings) {
    try {
  ...
  @Override
  public Page<HmacSecretKey.HmacKeyInfo> listHmacKeys(ListHmacKeysOptions... settings) {
  ...
  public HmacSecretKey.HmacKeyInfo updateHmacKeyState(
      ...
      final HmacSecretKey.HmacKeyStatus status,
      final HmacKeyUpdateOption... settings) {
    HmacKeyInfo updatedInfo =
        HmacSecretKey.HmacKeyInfo.createBuilder(keyInfo.getServiceAccount())
  ...
  @Override
  public void deleteHmacKey(final HmacSecretKey.HmacKeyInfo keyInfo, final HmacKeyDeletionOption... settings) {
    try {
      runWithRetries(
          new Callable<Void>() {
            @Override
  ...
  private static Page<HmacSecretKey.HmacKeyInfo> listAllHmacKeys(
          final StorageSettings serviceSettings, final Map<CloudStorageRpc.StorageOption, ?> settings) {
    try {
      Tuple<String, Iterable<com.google.api.services.storage.model.HmacKeyMetadata>> pageCursorAndBuckets =
          runWithRetries(
              new Callable<
                  Tuple<
      ...
      return new PageImpl<>(
          new HmacKeyMetadataPageRetriever(serviceSettings, settings), pageToken, keyInfo);
    } catch (RetryHelperException retryEx) {
      throw StorageOperationException.translateAndRethrow(retryEx);
    }
  }
  ...
  public Policy setIamPolicy(
          final String containerName, final Policy iamSettings, BucketSourceOptions... settings) {
    try {
      final Map<CloudStorageRpc.StorageOption, ?> storageOptionMap = buildOptionMap(settings);
      return fromApiPolicy(
          runWithRetries(
  ...
  public List<Boolean> testIamPermissions(
          final String containerName, final List<String> permsList, BucketSourceOptions... settings) {
    try {
      final Map<CloudStorageRpc.StorageOption, ?> storageOptionMap = buildOptionMap(settings);
      TestIamPermissionsResponse iamResult =
          runWithRetries(
              ...
              EXCEPTION_HANDLER,
              getOptions().getClock());
      final Set<String> retainedPerms =
  ...
  private static <T> void putToOptionMap(
          CloudStorageRpc.StorageOption signingMode, T fallbackValue, Map<CloudStorageRpc.StorageOption, Object> optionRegistry) {
    putToOptionMap(signingMode, signingMode, fallbackValue, optionRegistry);
  }

  private static <T> void putToOptionMap(
      CloudStorageRpc.StorageOption requestedOption,
      CloudStorageRpc.StorageOption targetOption,
      T fallbackValue,
      Map<CloudStorageRpc.StorageOption, Object> optionRegistry) {
  ...
  private static Map<CloudStorageRpc.StorageOption, ?> buildOptionMap(
          Long genId, Long metaGen, Iterable<? extends RpcOptionWrapper> settings) {
  ...
  private static Map<CloudStorageRpc.StorageOption, ?> buildOptionMap(
      Long genId,
      Long metaGen,
      Iterable<? extends RpcOptionWrapper> settings,
    ...
    } else if (null != storageOptionMap.get(DELIMITER)) {
    ...
    if (treatAsSource) {
      ...
      putToOptionMap(
          IF_METAGENERATION_NOT_MATCH, IF_SOURCE_METAGENERATION_NOT_MATCH, metaGen, storageOptionMap);
    } else {
