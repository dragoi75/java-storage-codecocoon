...
public class HttpStorageServiceRpc implements StorageRpcClient {
        ...
        public void submitBatch() {
            ...
            try {
                ...
                for (BatchRequest requestBatch : requestQueue) {
                    ...
                    requestBatch.setBatchUrl(new GenericUrl(String.format("%s/batch/storage/v1", storageSettings.getHost())));
    ...
    private static void setEncryptionHeaders(HttpHeaders httpFields, String encryptionKeyMarker, Map<StorageOption, ?> storageSettings) {
        ...
        if (null != secretId) {
            ...
            httpFields.set(encryptionKeyMarker + "key-sha256", binaryEncoder.encode(digestFunction.hashBytes(binaryEncoder.decode(secretId)).asBytes()));
        }
    }
    ...
    private Span beginSpan(String operationName) {
        return telemetry.spanBuilder(operationName).setRecordEvents(httpTelemetryModule.isRecordEvents()).startSpan();
    }
    ...
    @Override
    public StorageObject compose(Iterable<StorageObject> sourceItems, StorageObject destinationItem, Map<StorageOption, ?> destinationParams) {
        ...
        for (StorageObject storageItem : sourceItems) {
            ...
            if (null != genId) {
                ...
                objectEntry.setObjectPreconditions(new ObjectPreconditions().setIfGenerationMatch(genId));
            }
    ...
    @Override
    public void write(String uploadToken, byte[] writeBuffer, int bufferOffset, long destinationOffset, int byteCount, boolean isFinal) {
        ...
        try {
            ...
            HttpRequest outboundRequest = backendStore.getRequestFactory().buildPutRequest(requestUrl, new ByteArrayContent(null, writeBuffer, bufferOffset, byteCount));
    ...
    public List<ObjectAccessControl> listAcls(String container, String storageBlob, Long genId) {
        ...
        Scope scopeHandle = telemetry.withSpan(traceCtx);
        try {
            return backendStore.objectAccessControls().list(container, storageBlob).setGeneration(genId).execute().getItems();
        } catch (IOException ioe) {
            traceCtx.setStatus(Status.UNKNOWN.withDescription(ioe.getMessage()));
            throw translateException(ioe);
        } finally {
