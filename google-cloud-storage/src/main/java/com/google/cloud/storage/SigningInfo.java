...
public class SigningInfo {
  ...
  private SigningInfo(RequestSignatureBuilder signatureCreator) {
    ...
    ImmutableMap.Builder<String, String> headerAssembler =
        new ImmutableMap.Builder<String, String>().putAll(signatureCreator.canonicalExtensionHeadersMap);
    ...
    if (StorageService.UrlSigningOption.SigningVersion.V4.equals(signingVersion)
        && (!signatureCreator.canonicalExtensionHeadersMap.containsKey("host"))) {
  ...
  private String createV2UnsignedPayload() {
    ...
    if (canonicalExtensionHeadersMap.size() > 0) {
      bodyBuilderV2.append(
          new CanonicalExtensionHeadersFormatter(StorageService.UrlSigningOption.SigningVersion.V2)
              .serializeExtensionHeaders(canonicalExtensionHeadersMap));
    }
  ...
  private String computeV4CanonicalRequestHash() {
    ...
    CanonicalExtensionHeadersFormatter extHeaderFormatter =
    ...
    canonicalReq
        .append(extHeaderFormatter.serializeExtensionHeaders(canonicalExtensionHeadersMap))
        .append(COMPONENT_SEPARATOR);
    canonicalReq
        .append(extHeaderFormatter.joinHeaderNames(canonicalExtensionHeadersMap))
        .append(COMPONENT_SEPARATOR);
    canonicalReq.append("UNSIGNED-PAYLOAD");

    return Hashing.sha256()
        .hashString(canonicalReq.toString(), StandardCharsets.UTF_8)
        .toString();
  }
  ...
  private TreeMap<String, String> getNonReservedUserQueryParams() {
    ...
    for (Map.Entry<String, String> paramEntry : queryParameters.entrySet()) {
      ...
      if (!RESERVED_PARAM_NAMES.contains(paramEntry.getKey().toLowerCase())) {
        ...
        paramTreeMap.put(
            encodeRfc3986Uri(paramEntry.getKey(), true), encodeRfc3986Uri(paramEntry.getValue(), true));
      }
    }
  ...
  public String buildV4QueryString() {
    ...
    paramTreeMap.put(
        "X-Goog-Credential", encodeRfc3986Uri(accountAddress + "/" + dateYmd + SCOPE, true));
    ...
    StringBuilder signedHeadersBuf =
        new CanonicalExtensionHeadersFormatter(StorageService.UrlSigningOption.SigningVersion.V4)
            .joinHeaderNames(canonicalExtensionHeadersMap);
    paramTreeMap.put(
        "X-Goog-SignedHeaders", encodeRfc3986Uri(signedHeadersBuf.toString(), true));
  ...
  public static final class RequestSignatureBuilder {
    ...
    public RequestSignatureBuilder setCanonicalizedExtensionHeaders(
        Map<String, String> canonicalExtensionHeadersMap) {
    ...
    public SigningInfo buildSigningInfo() {
      checkArgument(requestMethod != null, "Required HTTP method");
      checkArgument(canonicalResource != null, "Required canonicalized resource");
      checkArgument(expiry >= 0, "Expiration must be greater than or equal to zero");

      if (StorageService.UrlSigningOption.SigningVersion.V4.equals(signingVersion)) {
        checkArgument(accountAddress != null, "Account email required to use V4 signing");
        checkArgument(epochSeconds > 0, "Timestamp required to use V4 signing");
        checkArgument(
            expiry <= 604800, "Expiration can't be longer than 7 days to use V4 signing");
      }

      if (canonicalExtensionHeadersMap == null) {
