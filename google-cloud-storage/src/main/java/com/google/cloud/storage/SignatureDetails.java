...
public class SignatureDetails {
  ...
  private static final List<String> SIGNATURE_RESERVED_KEYS =
  ...
  private SignatureDetails(CanonicalStringBuilder canonicalString) {
    ...
    ImmutableMap.Builder<String, String> headerFactory =
        new ImmutableMap.Builder<String, String>().putAll(canonicalString.canonicalExtHeaders);
    ...
    if (CloudStorageClient.UrlSigningOption.SignatureProtocolVersion.V4.equals(protocolVersion)
        && (!canonicalString.canonicalExtHeaders.containsKey("host"))) {
  ...
  private String buildV2UnsignedPayload() {
    ...
    if (canonicalExtHeaders.size() > 0) {
      dataBuilder.append(
          new CanonicalExtensionHeadersSerializer(CloudStorageClient.UrlSigningOption.SignatureProtocolVersion.V2)
  ...
  private String computeV4CanonicalRequestHash() {
    ...
    CanonicalExtensionHeadersSerializer extHeadersSerializer =
        new CanonicalExtensionHeadersSerializer(CloudStorageClient.UrlSigningOption.SignatureProtocolVersion.V4);
    ...
    canonicalReqBuilder
        .append(extHeadersSerializer.serialize(canonicalExtHeaders))
        .append(COMPONENT_SEPARATOR);
    canonicalReqBuilder
        .append(extHeadersSerializer.serializeHeaderNames(canonicalExtHeaders))
        .append(COMPONENT_SEPARATOR);
  ...
  public String buildV4QueryString() {
    ...
    paramsTreeMap.put(
        "X-Goog-Credential", rfc3986UriEncode(accountAddress + "/" + yyyymmddString + SCOPE, true));
  ...
  public static final class CanonicalStringBuilder {
    ...
    public SignatureDetails buildCanonicalString() {
      ...
      checkArgument(expiryTimestamp >= 0, "Expiration must be greater than or equal to zero");

      if (CloudStorageClient.UrlSigningOption.SignatureProtocolVersion.V4.equals(protocolVersion)) {
        checkArgument(accountAddress != null, "Account email required to use V4 signing");
        checkArgument(timeMillis > 0, "Timestamp required to use V4 signing");
        checkArgument(
            expiryTimestamp <= 604800, "Expiration can't be longer than 7 days to use V4 signing");
      }
