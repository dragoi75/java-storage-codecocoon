   ...
   */
    ...
    public FormPostPolicyV4 generateSignedPostPolicyV4(BlobMetadata blobMetadata, long expiryDuration, TimeUnit timeScale, PostFieldsMapV4 formData, PostConditionsVersion4 constraints, PostPolicyV4Parameter... clientParams) {
        ...
        for (Map.Entry<String, String> mapEntry : formData.getFieldsMap().entrySet()) {
            ...
            conditionBuilder.addCustom(FormPostPolicyV4.ConditionV4Operator.MATCHES, mapEntry.getKey(), mapEntry.getValue());
            policyMap.put(mapEntry.getKey(), mapEntry.getValue());
        }
        FormPostPolicyV4.PostConditionsVersion4 conditionsV4 = conditionBuilder.addBucket(ConditionV4Operator.MATCHES, blobMetadata.getBucket()).addKey(ConditionV4Operator.MATCHES, blobMetadata.getName()).addCustom(ConditionV4Operator.MATCHES, "x-goog-date", dateStr).addCustom(ConditionV4Operator.MATCHES, "x-goog-credential", signingKey).addCustom(ConditionV4Operator.MATCHES, "x-goog-algorithm", "GOOG4-RSA-SHA256").create();
        FormPostPolicyV4.PostPolicyV4DocumentModel doc = PostPolicyV4DocumentModel.create(expiryFormat.format(epochMillis + timeScale.toMillis(expiryDuration)), conditionsV4);
        ...
        for (FormPostPolicyV4.BinaryConditionV4 binaryCondition : conditionsV4.getConditions()) {
            if (FormPostPolicyV4.ConditionV4Operator.MATCHES == binaryCondition.type) {
    ...
    public FormPostPolicyV4 generateSignedPostPolicyV4(BlobMetadata blobMetadata, long expiryDuration, TimeUnit timeScale, FormPostPolicyV4.PostFieldsMapV4 formData, PostPolicyV4Parameter... clientParams) {
        return generateSignedPostPolicyV4(blobMetadata, expiryDuration, timeScale, formData, FormPostPolicyV4.PostConditionsVersion4.builder().create(), clientParams);
    }
