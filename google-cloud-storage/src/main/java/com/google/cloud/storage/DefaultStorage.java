...
final class DefaultStorage extends BaseService<StorageClientOptions> implements CloudStorageClient {
  ...
  public StorageBlob create(BlobMetadata blobMetadata, BlobUploadOptions... clientParams) {
    BlobMetadata updatedMetadata =
        blobMetadata
            .asBuilder()
            .setMd5(MD5_EMPTY_HASH)
            .setCrc32c(CRC32C_EMPTY_HASH)
            .buildObject();
  ...
  public StorageBlob create(
          BlobMetadata blobMetadata, byte[] dataBytes, int startIndex, int totalCount, BlobUploadOptions... clientParams) {
    dataBytes = firstNonNull(dataBytes, ZERO_LENGTH_BYTES);
    BlobMetadata updatedMetadata =
        blobMetadata
            .asBuilder()
            .setMd5(
                BaseEncoding.base64()
                    .encode(Hashing.md5().hashBytes(dataBytes, startIndex, totalCount).asBytes()))
            .setCrc32c(
                BaseEncoding.base64()
                    .encode(
                        Ints.toByteArray(
                            Hashing.crc32c().hashBytes(dataBytes, startIndex, totalCount).asInt())))
            .buildObject();
  ...
  public StorageBlob create(BlobMetadata blobMetadata, InputStream dataBytes, BlobWriteOptions... clientParams) {
    ...
    InputStream inStream =
        firstNonNull(dataBytes, new ByteArrayInputStream(ZERO_LENGTH_BYTES));
  ...
  private StorageBlob createInternal(
      ...
      BlobUploadOptions... clientParams) {
    ...
    try {
      return StorageBlob.fromProto(
          this,
          runWithRetries(
              new Callable<StorageObject>() {
                @Override
                public StorageObject call() {
                  return storageServiceClient.create(
                          blobProto, new ByteArrayInputStream(dataBytes, startIndex, totalCount), optionsMapping);
                }
              },
              getOptions().getRetrySettings(),
              EXCEPTION_HANDLER,
  ...
  @Override
  public StorageBlob createFrom(BlobMetadata blobMetadata, Path fileLocation, BlobWriteOptions... clientParams)
      throws IOException {
    return createFrom(blobMetadata, fileLocation, DEFAULT_IO_BUFFER_SIZE, clientParams);
  }

  @Override
  public StorageBlob createFrom(BlobMetadata blobMetadata, Path fileLocation, int chunkSize, BlobWriteOptions... clientParams)
      throws IOException {
  ...
  @Override
  public StorageBlob createFrom(BlobMetadata blobMetadata, InputStream dataBytes, BlobWriteOptions... clientParams)
      throws IOException {
  ...
  public StorageBlob createFrom(
          BlobMetadata blobMetadata, InputStream dataBytes, int chunkSize, BlobWriteOptions... clientParams)
      throws IOException {
   ...
   */
  private static void uploadStreamHelper(ReadableByteChannel inputChannel, WriteChannel outputChannel, int chunkSize)
      throws IOException {
    ...
    while (inputChannel.read(byteContainer) >= 0) {
      byteContainer.flip();
    ...
    try {
      StorageObject storedItem =
          runWithRetries(
              ...
              EXCEPTION_HANDLER,
              getOptions().getClock());
      return storedItem == null ? null : StorageBlob.fromProto(this, storedItem);
    } catch (RetryHelperException retryException) {
      throw StorageServiceException.translateAndRethrow(retryException);
    }
  }
  ...
  private static class BucketPageRetriever implements NextPageFetcher<StorageBucket> {
    ...
    BucketPageRetriever(
            StorageClientOptions clientOptions, String pageToken, Map<StorageServiceRpc.StorageOption, ?> optionsMap) {
      this.requestParams =
          PageImpl.nextRequestOptions(StorageServiceRpc.StorageOption.PAGE_TOKEN, pageToken, optionsMap);
  ...
  private static class BlobPageIterator implements NextPageFetcher<StorageBlob> {
    ...
    BlobPageIterator(
        ...
        String pageToken,
        Map<StorageServiceRpc.StorageOption, ?> optionsMap) {
      this.requestParams =
          PageImpl.nextRequestOptions(StorageServiceRpc.StorageOption.PAGE_TOKEN, pageToken, optionsMap);
      this.clientOptions = clientOptions;
  ...
  private static Page<StorageBucket> listAllBuckets(
          final StorageClientOptions clientOptions, final Map<StorageServiceRpc.StorageOption, ?> optionsMapping) {
    try {
      Tuple<String, Iterable<com.google.api.services.storage.model.Bucket>> fetchResult =
          runWithRetries(
              new Callable<
      ...
      Iterable<StorageBucket> storageEntries =
          fetchResult.y() == null
      ...
      return new PageImpl<>(
          new BucketPageRetriever(clientOptions, pageToken, optionsMapping), pageToken, storageEntries);
    } catch (RetryHelperException retryException) {
      throw StorageServiceException.translateAndRethrow(retryException);
    }
  }

  private static Page<StorageBlob> listAllBlobs(
      final String containerName,
      final StorageClientOptions clientOptions,
      final Map<StorageServiceRpc.StorageOption, ?> optionsMapping) {
    try {
      Tuple<String, Iterable<StorageObject>> fetchResult =
          runWithRetries(
              ...
              clientOptions.getClock());
      String pageToken = fetchResult.x();
      Iterable<StorageBlob> storageEntries =
          fetchResult.y() == null
              ? ImmutableList.<StorageBlob>of()
              : Iterables.transform(
                  fetchResult.y(),
                  new Function<StorageObject, StorageBlob>() {
                    @Override
      ...
      return new PageImpl<>(
          new BlobPageIterator(containerName, clientOptions, pageToken, optionsMapping), pageToken, storageEntries);
    } catch (RetryHelperException retryException) {
      throw StorageServiceException.translateAndRethrow(retryException);
    }
  }
      ...
      sourceList.add(
          BlobMetadata.newBuilder(
                  BlobIdentifier.create(
                      ...
                      versionedSource.getName(),
                      versionedSource.getGeneration()))
              .buildObject()
              .toProto());
    }
          ...
          runWithRetries(
              new Callable<StorageServiceRpc.RewriteResult>() {
                ...
                public StorageServiceRpc.RewriteResult call() {
                  return storageServiceClient.openRewrite(
  ...
  public URL signUrl(BlobMetadata blobMetadata, long expiryDuration, TimeUnit timeScale, UrlSigningOption... clientParams) {
    ...
    ServiceAccountSigner signer =
        (ServiceAccountSigner) optionsMap.get(UrlSigningOption.ServiceOption.SERVICE_ACCOUNT_CRED);
    if (signer == null) {
      checkState(
    ...
    long expiryTimestamp =
        useVersion4
            ? TimeUnit.SECONDS.convert(timeScale.toMillis(expiryDuration), TimeUnit.MILLISECONDS)
            : TimeUnit.SECONDS.convert(
                getOptions().getClock().millisTime() + timeScale.toMillis(expiryDuration),
                TimeUnit.MILLISECONDS);

    checkArgument(
        !(optionsMap.containsKey(UrlSigningOption.ServiceOption.VIRTUAL_HOSTED_STYLE)
            && optionsMap.containsKey(UrlSigningOption.ServiceOption.PATH_STYLE)
            && optionsMap.containsKey(UrlSigningOption.ServiceOption.BUCKET_BOUND_HOST_NAME)),
    ...
    String xmlHost =
        enablePathStyle
            ...
            : STORAGE_XML_SCHEME + "://" + containerName + "." + getBaseStorageHostName(optionsMap);
    ...
    String storagePath =
        enablePathStyle
            ? buildResourceUriPath(containerName, escapedObjectName, optionsMap)
            : buildResourceUriPath("", escapedObjectName, optionsMap);
    ...
    URI signingUri =
        useVersion2 ? URI.create(buildResourceUriPath(containerName, escapedObjectName, optionsMap)) : fileLocation;

    try {
      SigningContext signingContext =
          buildSigningInfo(
                  optionsMap, blobMetadata, expiryTimestamp, signingUri, signer.getAccount());
  ...
  @Override
  public FormPostPolicyV4 generateSignedPostPolicyV4(
      BlobMetadata blobMetadata,
      long expiryDuration,
      TimeUnit timeScale,
      PostFieldsMapV4 formData,
      PostConditionsVersion4 constraints,
      PostPolicyV4Parameter... clientParams) {
    EnumMap<UrlSigningOption.ServiceOption, Object> optionsMap = Maps.newEnumMap(UrlSigningOption.ServiceOption.class);
    ...
    String signingKey =
        signer.getAccount()
            + "/"
            + ymdFormatter.format(epochMillis)
            + "/auto/storage/goog4_request";

    Map<String, String> policyMap = new HashMap<>();

    FormPostPolicyV4.PostConditionsVersion4.ConditionsBuilder conditionBuilder = constraints.asBuilder();
    ...
    for (Map.Entry<String, String> mapEntry : formData.getFieldsMap().entrySet()) {
      ...
      conditionBuilder.addCustom(
          FormPostPolicyV4.ConditionV4Operator.MATCHES, mapEntry.getKey(), mapEntry.getValue());

      policyMap.put(mapEntry.getKey(), mapEntry.getValue());
    }

    FormPostPolicyV4.PostConditionsVersion4 conditionsV4 =
        conditionBuilder
            .addBucket(ConditionV4Operator.MATCHES, blobMetadata.getBucket())
            .addKey(ConditionV4Operator.MATCHES, blobMetadata.getName())
            .addCustom(ConditionV4Operator.MATCHES, "x-goog-date", dateStr)
            .addCustom(ConditionV4Operator.MATCHES, "x-goog-credential", signingKey)
            .addCustom(ConditionV4Operator.MATCHES, "x-goog-algorithm", "GOOG4-RSA-SHA256")
            .create();
    FormPostPolicyV4.PostPolicyV4DocumentModel doc =
        PostPolicyV4DocumentModel.create(
            expiryFormat.format(epochMillis + timeScale.toMillis(expiryDuration)), conditionsV4);
    String policyJson = BaseEncoding.base64().encode(doc.toJsonString().getBytes());
    String sigValue =
        BaseEncoding.base16().encode(signer.sign(policyJson.getBytes())).toLowerCase();

    for (FormPostPolicyV4.BinaryConditionV4 binaryCondition : conditionsV4.getConditions()) {
      if (binaryCondition.type == FormPostPolicyV4.ConditionV4Operator.MATCHES) {
  ...
  public FormPostPolicyV4 generateSignedPostPolicyV4(
      ...
      long expiryDuration,
      TimeUnit timeScale,
      FormPostPolicyV4.PostFieldsMapV4 formData,
      PostPolicyV4Parameter... clientParams) {
    return generateSignedPostPolicyV4(
            blobMetadata, expiryDuration, timeScale, formData, FormPostPolicyV4.PostConditionsVersion4.builder().create(), clientParams);
  }
  ...
  public FormPostPolicyV4 generateSignedPostPolicyV4(
      ...
      PostPolicyV4Parameter... clientParams) {
    return generateSignedPostPolicyV4(
            blobMetadata, expiryDuration, timeScale, PostFieldsMapV4.builder().create(), constraints, clientParams);
  }

  public FormPostPolicyV4 generateSignedPostPolicyV4(
          BlobMetadata blobMetadata, long expiryDuration, TimeUnit timeScale, PostPolicyV4Parameter... clientParams) {
    return generateSignedPostPolicyV4(
            blobMetadata, expiryDuration, timeScale, PostFieldsMapV4.builder().create(), clientParams);
  }

  private String buildResourceUriPath(
      String sanitizedBucketName,
      String escapedObjectName,
      EnumMap<UrlSigningOption.ServiceOption, Object> optionsMap) {
   ...
   */
  private SigningContext buildSigningInfo(
      Map<UrlSigningOption.ServiceOption, Object> optionsMap,
      BlobMetadata blobMetadata,
      long expiryTimestamp,
      URI fileLocation,
    ...
    HttpRequestMethod httpMethod =
        optionsMap.containsKey(UrlSigningOption.ServiceOption.HTTP_METHOD)
            ? (HttpRequestMethod) optionsMap.get(UrlSigningOption.ServiceOption.HTTP_METHOD)
            : HttpRequestMethod.GET;

    SigningContext.RequestSignatureBuilder signatureBuilder =
        new SigningContext.RequestSignatureBuilder(httpMethod, expiryTimestamp, fileLocation);
    ...
    signatureBuilder.setSignatureVersion(
        (UrlSigningOption.SignatureSchemeVersion) optionsMap.get(UrlSigningOption.ServiceOption.SIGNATURE_VERSION));
    ...
    boolean useVersion4 =
        UrlSigningOption.SignatureSchemeVersion.V4.equals(
            optionsMap.get(UrlSigningOption.ServiceOption.SIGNATURE_VERSION));
    if (useVersion4) { // We don't sign the host header for V2 signed URLs; only do this for V4.
      // Add the host here first, allowing it to be overridden in the EXT_HEADERS option below.
      if (optionsMap.containsKey(UrlSigningOption.ServiceOption.VIRTUAL_HOSTED_STYLE)) {
        extraHeadersBuilder.put(
            "host",
            slashlessBucketNameFrom(blobMetadata) + "." + getBaseStorageHostName(optionsMap));
      } else if (optionsMap.containsKey(UrlSigningOption.ServiceOption.HOST_NAME)
          || optionsMap.containsKey(UrlSigningOption.ServiceOption.BUCKET_BOUND_HOST_NAME)) {
    ...
    if (optionsMap.containsKey(UrlSigningOption.ServiceOption.EXT_HEADERS)) {
      extraHeadersBuilder.putAll(
          (Map<String, String>) optionsMap.get(UrlSigningOption.ServiceOption.EXT_HEADERS));
    }
    ...
    if (optionsMap.containsKey(UrlSigningOption.ServiceOption.QUERY_PARAMS)) {
      queryBuilder.putAll(
          (Map<String, String>) optionsMap.get(UrlSigningOption.ServiceOption.QUERY_PARAMS));
    }
  ...
  private String getBaseStorageHostName(Map<UrlSigningOption.ServiceOption, Object> optionsMap) {
    ...
    String bucketScopedHost =
        (String) optionsMap.get(UrlSigningOption.ServiceOption.BUCKET_BOUND_HOST_NAME);
  ...
  public List<StorageBlob> get(Iterable<BlobIdentifier> identifiers) {
    ...
    for (BlobIdentifier objectName : identifiers) {
      operationBatch
          .get(objectName)
          .notify(
              new BatchResult.Callback<StorageBlob, StorageServiceException>() {
                @Override
  ...
  public boolean deleteAcl(
          final String containerName, final TypedEntity principal, BucketSourceRequestOption... clientParams) {
    try {
      final Map<StorageServiceRpc.StorageOption, ?> optionsMapping = buildOptionMap(clientParams);
      return runWithRetries(
          new Callable<Boolean>() {
  ...
  @Override
  public boolean deleteAcl(final String containerName, final AccessControlEntry.TypedEntity principal) {
      ...
      ObjectAccessControl result =
          runWithRetries(
              new Callable<ObjectAccessControl>() {
                @Override
                public ObjectAccessControl call() {
                  return storageServiceClient.getAcl(
                      objectName.getBucket(), objectName.getName(), objectName.getGeneration(), principal.toProto());
                }
              },
              getOptions().getRetrySettings(),
              EXCEPTION_HANDLER,
    ...
    try {
      return runWithRetries(
          new Callable<Boolean>() {
            @Override
            public Boolean call() {
              return storageServiceClient.deleteAcl(
                  objectName.getBucket(), objectName.getName(), objectName.getGeneration(), principal.toProto());
            }
          },
          getOptions().getRetrySettings(),
      ...
      List<ObjectAccessControl> result =
          runWithRetries(
              new Callable<List<ObjectAccessControl>>() {
                @Override
                public List<ObjectAccessControl> call() {
                  return storageServiceClient.listAcls(
  ...
  public HmacSecretKey createHmacKey(
          final ServiceAccountIdentity accountIdentity, final HmacKeyCreationOption... clientParams) {
    try {
  ...
  private HmacSecretKey.HmacKeyInfo modifyHmacKey(
          final HmacSecretKey.HmacKeyInfo keyInfo, final UpdateHmacKeyRpcOption... clientParams) {
    try {
      return HmacSecretKey.HmacKeyInfo.fromProto(
          runWithRetries(
              new Callable<com.google.api.services.storage.model.HmacKeyMetadata>() {
  ...
  public HmacSecretKey.HmacKeyInfo updateHmacKeyState(
      final HmacSecretKey.HmacKeyInfo keyInfo,
      final HmacSecretKey.HmacKeyStatus keyStatus,
      final UpdateHmacKeyRpcOption... clientParams) {
    HmacSecretKey.HmacKeyInfo modifiedInfo =
        HmacSecretKey.HmacKeyInfo.newServiceAccountKeyBuilder(keyInfo.getServiceAccount())
  ...
  private static Page<HmacSecretKey.HmacKeyInfo> listAllHmacKeys(
          final StorageClientOptions clientOptions, final Map<StorageServiceRpc.StorageOption, ?> clientParams) {
    try {
      Tuple<String, Iterable<com.google.api.services.storage.model.HmacKeyMetadata>> fetchResult =
          runWithRetries(
              new Callable<
                  Tuple<
              ...
              clientOptions.getClock());
      String pageToken = fetchResult.x();
      final Iterable<HmacSecretKey.HmacKeyInfo> keyInfo =
          fetchResult.y() == null
      ...
      return new PageImpl<>(
          new HmacKeyMetadataPaginator(clientOptions, clientParams), pageToken, keyInfo);
    } catch (RetryHelperException retryException) {
      throw StorageServiceException.translateAndRethrow(retryException);
    }
  }
  ...
  public Policy setIamPolicy(
          final String containerName, final Policy policyJson, BucketSourceRequestOption... clientParams) {
    try {
      final Map<StorageServiceRpc.StorageOption, ?> optionsMapping = buildOptionMap(clientParams);
      return buildPolicyFromApi(
          runWithRetries(
  ...
  public List<Boolean> testIamPermissions(
          final String containerName, final List<String> requestedPerms, BucketSourceRequestOption... clientParams) {
    try {
      final Map<StorageServiceRpc.StorageOption, ?> optionsMapping = buildOptionMap(clientParams);
    ...
    try {
      com.google.api.services.storage.model.ServiceAccount result =
          runWithRetries(
              ...
              EXCEPTION_HANDLER,
              getOptions().getClock());
      return result == null ? null : ServiceAccountIdentity.fromServiceAccountModel(result);
    } catch (RetryHelperException retryException) {
      throw StorageServiceException.translateAndRethrow(retryException);
    }
  }

  private static <T> void putOptionIntoMap(
          StorageServiceRpc.StorageOption signingChoice, T fallbackValue, Map<StorageServiceRpc.StorageOption, Object> storageOptionLookup) {
    putOptionIntoMap(signingChoice, signingChoice, fallbackValue, storageOptionLookup);
  }

  private static <T> void putOptionIntoMap(
      StorageServiceRpc.StorageOption requestedOption,
      StorageServiceRpc.StorageOption optionToStore,
      T fallbackValue,
      Map<StorageServiceRpc.StorageOption, Object> storageOptionLookup) {
    if (storageOptionLookup.containsKey(requestedOption)) {
      @SuppressWarnings("unchecked")
      T resolvedVal = (T) storageOptionLookup.remove(requestedOption);
      checkArgument(
          resolvedVal != null || fallbackValue != null,
  ...
  private static Map<StorageServiceRpc.StorageOption, ?> buildOptionMap(
          Long genNumber, Long metaGen, Iterable<? extends OptionDescriptor> clientParams) {
  ...
  private static Map<StorageServiceRpc.StorageOption, ?> buildOptionMap(
      Long genNumber,
      Long metaGen,
      Iterable<? extends OptionDescriptor> clientParams,
    ...
    for (OptionDescriptor signingChoice : clientParams) {
      ...
      checkArgument(previousValue == null, "Duplicate option %s", signingChoice);
    }
    ...
    } else if (null != intermediateMap.get(DELIMITER)) {
    ...
    if (treatAsSource) {
      ...
      putOptionIntoMap(
          IF_METAGENERATION_NOT_MATCH, IF_SOURCE_METAGENERATION_NOT_MATCH, metaGen, intermediateMap);
    } else {
