...
public final class StorageServiceException extends BaseHttpServiceException {
  ...
  private static final Set<Error> RETRIABLE_FAILURES =
      ImmutableSet.of(
          new Error(504, null),
          new Error(503, null),
          new Error(502, null),
          ...
          new Error(429, null),
          new Error(408, null),
          new Error(null, INTERNAL_FAILURE),
          new Error(null, CONNECTION_TERMINATED_PREMATURELY));
  ...
  public static StorageServiceException translateException(IOException ioFailure) {
    if (ioFailure.getMessage().contains("Connection closed prematurely")) {
      return new StorageServiceException(
          0, ioFailure.getMessage(), CONNECTION_TERMINATED_PREMATURELY, ioFailure);
    } else {
      // default
