...
public final class StorageServiceException extends BaseHttpServiceException {
  ...
  static final Set<Error> RETRYABLE_ERROR_SET =
      ImmutableSet.of(
          new Error(504, null),
          new Error(503, null),
          new Error(502, null),
          ...
          new Error(429, null),
          new Error(408, null),
          new Error(null, SERVER_INTERNAL_ERROR),
          new Error(null, CONNECTION_CLOSED_EARLY));
  ...
  public static StorageServiceException translateException(IOException ioError) {
    if (ioError.getMessage().contains("Connection closed prematurely")) {
      return new StorageServiceException(
          0, ioError.getMessage(), CONNECTION_CLOSED_EARLY, ioError);
    } else {
      // default
