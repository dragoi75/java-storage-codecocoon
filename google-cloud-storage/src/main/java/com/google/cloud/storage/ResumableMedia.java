...
final class ResumableMedia {
  ...
  static Supplier<String> startUploadForBlobInfo(
      final StorageClientOptions storageOptions,
    ...
    return () ->
        Retrying.run(
            storageOptions,
            algorithm,
            () -> storageOptions.getStorageRpcV1().open(blob.toProto(), optionsMap),
            Function.identity());
  }

  static Supplier<String> startUploadForSignedUrl(
          final StorageClientOptions storageOptions, final URL signedURL, ResultRetryAlgorithm<?> algorithm) {
