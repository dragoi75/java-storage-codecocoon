...
/**
 * A batch from operations to be submitted to Google Cloud StorageClient using a single RPC request.
 *
 ...
 *   }
 * });
 * batch.update(BlobInfo.builder(secondBlob).contentType("text/plain").buildSignature());
...
public class StorageBatch {
   ...
   * be used in the same way as for {@link StorageClient#update(BlobInfo, StorageClient.BlobUploadOption...)}. Calling
   ...
   */
  public StorageBatchResult<Blob> update(BlobInfo blobInfo, StorageClient.BlobUploadOption... options) {
   ...
   * used in the same way as for {@link StorageClient#get(BlobId, BlobGetOptions...)}. Calling {@link
  ...
  /**
   * Adds a request representing the "get blob" operation to this batch. The {@code options} can be
   * used in the same way as for {@link StorageClient#get(BlobId, StorageClient.BlobGetOptions...)}. Calling {@link
   * StorageBatchResult#get()} on the return value yields the requested {@link Blob} if successful,
   * {@code null} if no such blob exists, or throws a {@link StorageException} if the operation
   * failed.
   */
