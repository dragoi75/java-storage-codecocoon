 ...
 *   }
 * });
 * batch.update(BlobInfo.builder(secondBlob).contentType("text/plain").buildPostFieldsMap());
...
public class StorageBatch {
   ...
   * {@link StorageBatchResult#get()} on the return getValue yields the updated {@link Blob} if
   ...
   * StorageBatchResult#get()} on the return getValue yields the requested {@link Blob} if successful,
   * {@code null} if no such blob exists, or throws a {@link StorageException} if the operation
   * failed.
   */
  public StorageBatchResult<Blob> get(String bucket, String blob, Storage.BlobGetOptions... options) {
