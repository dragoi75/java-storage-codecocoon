        ...
        try {
            runWithRetries(callable(new Runnable() {
                ...
                public void run() {
                    ...
                    final boolean uploadAlreadyComplete = -1 == remotePosition;
                    ...
                    if (!uploadAlreadyComplete || !lastChunk) {
                        if (!uploadAlreadyComplete || lastChunk || checkingForLastChunk) {
                            if (remotePosition < localPosition || getChunkSize() <= chunkOffset) {
                                if (remotePosition <= localPosition || getChunkSize() != chunkOffset) {
                                    // Case 4 && Case 8 && Case 9
                                    throw unrecoverableState(getUploadId(), chunkOffset, chunkLength, localPosition, remotePosition, lastChunk);
                                } else {
                                    // Case 3
                                ...
                                storageObject = transmitChunk(chunkOffset, chunkLength, remotePosition, lastChunk);
                                retrying = false;
                            }
                        } else {
                            // Case 7
                        ...
                        if (null == storageObject) {
                            storageObject = getOptions().getStorageRpcV1().queryCompletedResumableUpload(getUploadId(), totalBytes);
                        }
                        ...
                        if (null == storageObject) {
                            throw errorResolvingMetadataLastChunk(getUploadId(), chunkOffset, chunkLength, localPosition, remotePosition, lastChunk);
                        }
                        ...
                        if (totalBytes != size.longValue()) {
                            throw unrecoverableState(getUploadId(), chunkOffset, chunkLength, localPosition, remotePosition, lastChunk);
                        }
                        retrying = false;
                    }
                }
            }), getOptions().getRetrySettings(), algorithmForWrite, getOptions().getClock());
