...
final class DefaultStorageImpl extends BaseService<StorageSettings> implements StorageService {
  ...
  public StorageObject create(BlobMetadata blobMetadata, byte[] inputData, BlobUploadOption... storageSettings) {
    ...
    BlobMetadata updatedMetadata =
        blobMetadata
            .asBuilder()
            .setMd5(BaseEncoding.base64().encode(Hashing.md5().hashBytes(inputData).asBytes()))
            .setCrc32c(
                BaseEncoding.base64()
                    .encode(Ints.toByteArray(Hashing.crc32c().hashBytes(inputData).asInt())))
            .buildObject();
  ...
  public StorageObject create(
          BlobMetadata blobMetadata, byte[] inputData, int startIndex, int count, BlobUploadOption... storageSettings) {
    inputData = firstNonNull(inputData, ZERO_LENGTH_BYTE_ARRAY);
  ...
  public StorageObject create(BlobMetadata blobMetadata, InputStream inputData, BlobWriteSetting... storageSettings) {
    ...
    InputStream inStream =
        firstNonNull(inputData, new ByteArrayInputStream(ZERO_LENGTH_BYTE_ARRAY));
  ...
  public StorageObject get(BlobIdentifier objectIdentifier, BlobFetchOption... storageSettings) {
    ...
    try {
      com.google.api.services.storage.model.StorageObject objectEntity =
          runWithRetries(
              new Callable<com.google.api.services.storage.model.StorageObject>() {
                @Override
                public com.google.api.services.storage.model.StorageObject call() {
              ...
              EXCEPTION_HANDLER,
              getOptions().getClock());
      return objectEntity == null ? null : StorageObject.fromProto(this, objectEntity);
    } catch (RetryHelperException retryException) {
      throw StorageOperationException.translateAndRethrow(retryException);
    }
  }
  ...
  private static class BucketPageIterator implements NextPageFetcher<StorageBucket> {
    ...
    BucketPageIterator(
            StorageSettings serviceSettings, String pageCursor, Map<StorageRpcClient.StorageOption, ?> storageOptionMap) {
      this.requestOptionMap =
  ...
  private static Page<StorageBucket> listAllBuckets(
          final StorageSettings serviceSettings, final Map<StorageRpcClient.StorageOption, ?> optionMap) {
    try {
      Tuple<String, Iterable<com.google.api.services.storage.model.Bucket>> bucketTuple =
          runWithRetries(
              new Callable<
      ...
      Iterable<StorageBucket> bucketList =
          bucketTuple.y() == null
      ...
      return new PageImpl<>(
          new BucketPageIterator(serviceSettings, pageCursor, optionMap), pageCursor, bucketList);
    } catch (RetryHelperException retryException) {
      throw StorageOperationException.translateAndRethrow(retryException);
    }
  }
  ...
  private static Page<StorageObject> listAllBlobs(
      ...
      final Map<StorageRpcClient.StorageOption, ?> optionMap) {
    try {
      Tuple<String, Iterable<com.google.api.services.storage.model.StorageObject>> bucketTuple =
          runWithRetries(
              ...
              serviceSettings.getClock());
      String pageCursor = bucketTuple.x();
      Iterable<StorageObject> blobList =
          bucketTuple.y() == null
      ...
      return new PageImpl<>(
          new BlobPageIterator(containerName, serviceSettings, pageCursor, optionMap), pageCursor, blobList);
    } catch (RetryHelperException retryException) {
      throw StorageOperationException.translateAndRethrow(retryException);
    }
  }
  ...
  public StorageObject compose(final ComposeObjectRequest mergeRequest) {
    final List<com.google.api.services.storage.model.StorageObject> inputObjects =
        Lists.newArrayListWithCapacity(mergeRequest.getSourceBlobs().size());
      ...
      inputObjects.add(
          BlobMetadata.newBuilder(
                  BlobIdentifier.create(
                      mergeRequest.getTarget().getBucket(),
                      originBlob.getName(),
                      originBlob.getGeneration()))
              .buildObject()
              .toProto());
    }
  ...
  public BlobCopyWriter copy(final CopyOperationRequest transferRequest) {
    ...
    try {
      RewriteOperationResult rewriteOutcome =
          runWithRetries(
              new Callable<RewriteOperationResult>() {
                @Override
                public StorageRpcClient.RewriteOperationResult call() {
                  return rpcClient.openRewrite(
                      new StorageRpcClient.ObjectRewriteRequest(
                              originObject,
                              originOptions,
                          transferRequest.getOverrideInfo(),
                              destinationObject,
                              targetPair,
                          transferRequest.getMegabytesCopiedPerChunk()));
                }
              },
              getOptions().getRetrySettings(),
              EXCEPTION_HANDLER,
  ...
  public URL signUrl(BlobMetadata blobMetadata, long timePeriod, TimeUnit timeUnitType, UrlSigningOption... storageSettings) {
    ...
    ServiceAccountSigner serviceAccountSigner =
        (ServiceAccountSigner) storageOptionMap.get(UrlSigningOption.RequestOption.SERVICE_ACCOUNT_CRED);
    ...
    long expiryTime =
        v4Enabled
            ? TimeUnit.SECONDS.convert(timeUnitType.toMillis(timePeriod), TimeUnit.MILLISECONDS)
            : TimeUnit.SECONDS.convert(
                getOptions().getClock().millisTime() + timeUnitType.toMillis(timePeriod),
                TimeUnit.MILLISECONDS);

    checkArgument(
        !(storageOptionMap.containsKey(UrlSigningOption.RequestOption.VIRTUAL_HOSTED_STYLE)
            && storageOptionMap.containsKey(UrlSigningOption.RequestOption.PATH_STYLE)),
    ...
    String signingPath =
        pathStyleEnabled
            ? buildResourceUriPath(storageBucket, encodedObjectName, storageOptionMap)
            : buildResourceUriPath("", encodedObjectName, storageOptionMap);
    ...
    URI signingUri =
        v2Enabled ? URI.create(buildResourceUriPath(storageBucket, encodedObjectName, storageOptionMap)) : resourceUri;
  ...
  private String buildResourceUriPath(
      ...
      String encodedObjectName,
      EnumMap<UrlSigningOption.RequestOption, Object> storageOptionMap) {
   ...
   */
  private SigningInfo createSignatureInfo(
      Map<UrlSigningOption.RequestOption, Object> storageOptionMap,
    ...
    HttpRequestMethod httpMethod =
        storageOptionMap.containsKey(UrlSigningOption.RequestOption.HTTP_METHOD)
            ? (HttpRequestMethod) storageOptionMap.get(UrlSigningOption.RequestOption.HTTP_METHOD)
            : HttpRequestMethod.GET;

    SigningInfo.RequestSignatureBuilder signatureBuilder =
        new SigningInfo.RequestSignatureBuilder(httpMethod, expiryTime, resourceUri);
    ...
    signatureBuilder.setSignatureVersion(
        (UrlSigningOption.SigningVersion) storageOptionMap.get(UrlSigningOption.RequestOption.SIGNATURE_VERSION));
    ...
    boolean v4Enabled =
        UrlSigningOption.SigningVersion.V4.equals(
            storageOptionMap.get(UrlSigningOption.RequestOption.SIGNATURE_VERSION));
    if (v4Enabled) { // We don't sign the host header for V2 signed URLs; only do this for V4.
      ...
      if (storageOptionMap.containsKey(UrlSigningOption.RequestOption.VIRTUAL_HOSTED_STYLE)) {
        extraHeadersBuilder.put(
            "host",
            getBucketNameWithoutSlashes(blobMetadata) + "." + getBaseStorageHostName(storageOptionMap));
      } else if (storageOptionMap.containsKey(UrlSigningOption.RequestOption.HOST_NAME)) {
    ...
    if (storageOptionMap.containsKey(UrlSigningOption.RequestOption.EXT_HEADERS)) {
      extraHeadersBuilder.putAll(
          (Map<String, String>) storageOptionMap.get(UrlSigningOption.RequestOption.EXT_HEADERS));
    }
    ...
    if (storageOptionMap.containsKey(UrlSigningOption.RequestOption.QUERY_PARAMS)) {
      paramsBuilder.putAll(
          (Map<String, String>) storageOptionMap.get(UrlSigningOption.RequestOption.QUERY_PARAMS));
    }
  ...
  public List<StorageObject> get(Iterable<BlobIdentifier> identifiers) {
    ...
    for (BlobIdentifier objectIdentifier : identifiers) {
      batchOp
          ...
          .notify(
              new BatchResult.Callback<StorageObject, StorageOperationException>() {
                @Override
  ...
  public boolean deleteAcl(
          final String containerName, final AccessControlEntry.AbstractEntity aclEntity, BucketFilterOption... storageSettings) {
    try {
      final Map<StorageRpcClient.StorageOption, ?> optionMap = buildOptionMap(storageSettings);
      return runWithRetries(
          new Callable<Boolean>() {
              ...
              new Callable<ObjectAccessControl>() {
                ...
                public ObjectAccessControl call() {
                  return rpcClient.getAcl(
                      objectIdentifier.getBucket(), objectIdentifier.getName(), objectIdentifier.getGeneration(), aclEntity.toProto());
                }
              },
              getOptions().getRetrySettings(),
              EXCEPTION_HANDLER,
  ...
  public AccessControlEntry createAcl(final BlobIdentifier objectIdentifier, final AccessControlEntry accessControlEntry) {
    final ObjectAccessControl accessControlProto =
        accessControlEntry.toObjectProto()
            .setBucket(objectIdentifier.getBucket())
            .setObject(objectIdentifier.getName())
            .setGeneration(objectIdentifier.getGeneration());
    try {
      return AccessControlEntry.fromProto(
          runWithRetries(
              new Callable<ObjectAccessControl>() {
              ...
              new Callable<List<ObjectAccessControl>>() {
                ...
                public List<ObjectAccessControl> call() {
                  return rpcClient.listAcls(
                      objectIdentifier.getBucket(), objectIdentifier.getName(), objectIdentifier.getGeneration());
                }
              },
              getOptions().getRetrySettings(),
              EXCEPTION_HANDLER,
  ...
  public HmacSecretKey createHmacKey(
          final ServiceAccountInfo accountInfo, final HmacKeyCreationOption... storageSettings) {
    try {
  ...
  private HmacKeyInfo updateHmacKeyMetadata(
          final HmacKeyInfo keyInfo, final HmacKeyUpdateOption... storageSettings) {
    try {
      return HmacKeyInfo.fromProto(
          runWithRetries(
              new Callable<com.google.api.services.storage.model.HmacKeyMetadata>() {
  ...
  public HmacSecretKey.HmacKeyInfo updateHmacKeyState(
      ...
      final HmacKeyUpdateOption... storageSettings) {
  ...
  private static Page<HmacKeyInfo> listAllHmacKeys(
          final StorageSettings serviceSettings, final Map<StorageRpcClient.StorageOption, ?> storageSettings) {
    try {
      Tuple<String, Iterable<com.google.api.services.storage.model.HmacKeyMetadata>> bucketTuple =
          runWithRetries(
              new Callable<
                  Tuple<
      ...
      return new PageImpl<>(
          new HmacKeyMetadataPageRetriever(serviceSettings, storageSettings), pageCursor, keyInfo);
    } catch (RetryHelperException retryException) {
      throw StorageOperationException.translateAndRethrow(retryException);
    }
  }
  ...
  public Policy setIamPolicy(
          final String containerName, final Policy accessControl, BucketFilterOption... storageSettings) {
    try {
      final Map<StorageRpcClient.StorageOption, ?> optionMap = buildOptionMap(storageSettings);
      return fromApiPolicy(
          runWithRetries(
  ...
  public List<Boolean> testIamPermissions(
          final String containerName, final List<String> requestedPerms, BucketFilterOption... storageSettings) {
    try {
      final Map<StorageRpcClient.StorageOption, ?> optionMap = buildOptionMap(storageSettings);
  ...
  private static <T> void addOptionToMap(
          StorageRpcClient.StorageOption urlSigningMode, T fallbackValue, Map<StorageRpcClient.StorageOption, Object> optionsStore) {
  ...
  private static <T> void addOptionToMap(
      ...
      T fallbackValue,
      Map<StorageRpcClient.StorageOption, Object> optionsStore) {
  ...
  private static Map<StorageRpcClient.StorageOption, ?> buildOptionMap(
          Long genId, Long metaGen, Iterable<? extends AbstractOption> storageSettings) {
  ...
  private static Map<StorageRpcClient.StorageOption, ?> buildOptionMap(
      Long genId,
      Long metaGen,
      Iterable<? extends AbstractOption> storageSettings,
    ...
    for (AbstractOption urlSigningMode : storageSettings) {
      ...
      checkArgument(previousValue == null, "Duplicate option %s", urlSigningMode);
    }
    ...
    if (treatAsSource) {
      ...
      addOptionToMap(
          IF_METAGENERATION_NOT_MATCH, IF_SOURCE_METAGENERATION_NOT_MATCH, metaGen, transientMap);
    } else {
