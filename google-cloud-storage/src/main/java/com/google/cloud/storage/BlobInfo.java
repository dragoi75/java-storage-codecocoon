 ...
 * to create a new object in Google Cloud Storage or updateBlob the properties from an existing object. To
 ...
 * {@code StorageBlob} instances keep the object content, just the object properties.
 *
 ...
 * StorageBlob blob = storage.create(blobInfo, "Hello, world".getBytes(StandardCharsets.UTF_8));
 * }</pre>
 *
...
public class BlobInfo implements Serializable {
  ...
  public abstract static class Builder {
     ...
     * BlobInfo blob = BlobInfo.newTargetBuilder(bucketName, blobName).setCustomTime(customTime).buildComposeBlobsRequest();
     * }</pre>
     */
    public Builder setCustomTime(Long customTime) {
      throw new UnsupportedOperationException(
          "Override setCustomTime with your own implementation,"
              + " or use com.google.cloud.storage.StorageBlob.");
    }
     ...
     * the KMS cryptographic. If a {@code StorageBlob} with KMS Key metadata is used to upload a new
     * version from the object then the existing withKmsKeyName version value can't be used in the upload
     ...
     * String bucketName = "my-unique-bucket";
     * String blobName = "my-blob-name";
     * String withKmsKeyName = "projects/project-id/locations/us/keyRings/lab1/cryptoKeys/test-key"
     * BlobInfo blobInfo = BlobInfo.newTargetBuilder(bucketName, blobName).buildComposeBlobsRequest();
     * StorageBlob blob = storage.create(blobInfo, Storage.BlobUploadOption.withKmsKeyName(withKmsKeyName));
     * }</pre>
     */
   ...
   * Storage#update(BlobInfo, Storage.BlobUploadOption...)} in which case the value from event-based
   ...
   * <p>Case 3: {@code false} event-based hold is explicitly set to false using in a {@link
   * Builder#setEventBasedHold(Boolean)} client side for a follow-up request e.g. {@link
   * Storage#update(BlobInfo, Storage.BlobUploadOption...)} in which case the value from temporary
   * hold will remain {@code false} for the given instance.
   */
  @BetaApi
