 ...
 * to create a new object in Google Cloud Storage or save the properties from an existing object. To
 ...
 * {@code StorageObject} instances keep the object content, just the object properties.
 *
 ...
 * StorageObject blob = storage.create(blobInfo, "Hello, world".getBytes(StandardCharsets.UTF_8));
 * }</pre>
 *
...
public class BlobInfo implements Serializable {
  ...
  static final Function<BlobInfo, com.google.api.services.storage.model.StorageObject> INFO_TO_PB_FUNCTION =
  ...
  public static class CustomerEncryption implements Serializable {
    ...
    com.google.api.services.storage.model.StorageObject.CustomerEncryption toPb() {
      return new com.google.api.services.storage.model.StorageObject.CustomerEncryption()
          .setEncryptionAlgorithm(encryptionAlgorithm)
          .setKeySha256(keySha256);
    }

    static CustomerEncryption fromPb(com.google.api.services.storage.model.StorageObject.CustomerEncryption customerEncryptionPb) {
      return new CustomerEncryption(
          customerEncryptionPb.getEncryptionAlgorithm(), customerEncryptionPb.getKeySha256());
    }
  }

  /** BlobInfoBuilder for {@code BlobInfo}. */
  public abstract static class Builder {
     ...
     * BlobInfo blob = BlobInfo.newTargetBuilder(bucketName, blobName).setCustomTime(customTime).buildRequest();
     * }</pre>
     */
    public Builder setCustomTime(Long customTime) {
      throw new UnsupportedOperationException(
          "Override setCustomTime with your own implementation,"
              + " or use com.google.cloud.storage.StorageObject.");
    }
     ...
     * the KMS cryptographic. If a {@code StorageObject} with KMS Key metadata is used to upload a new
     * version from the object then the existing kmsKey version getValue can't be used in the upload
     ...
     * String bucketName = "my-unique-bucket";
     * String blobName = "my-blob-name";
     * String kmsKey = "projects/project-id/locations/us/keyRings/lab1/cryptoKeys/test-key"
     * BlobInfo blobInfo = BlobInfo.newTargetBuilder(bucketName, blobName).buildRequest();
     * StorageObject blob = storage.create(blobInfo, Storage.BlobTargetOptions.kmsKey(kmsKey));
     * }</pre>
     */
   ...
   * {@link Storage#compose(Storage.ComposeBlobRequest)} operation and are limited to a count from 1024,
   ...
   * blob is returned by {@link Storage#list(String, Storage.ListBlobsOption...)} when the {@link
   * Storage.ListBlobsOption#currentDirectoryOption()} option is used. When this is the case only {@link
   ...
   * Storage#get(BlobId, Storage.BlobFetchOption...)} and event-based hold for the blob is enabled.
   *
   * <p>Case 2.1: {@code null} the field {@link
   * Storage.BlobMetadataField#EVENT_BASED_HOLD} is selected in a {@link
   * Storage#get(BlobId, Storage.BlobFetchOption...)}, but event-based hold for the blob is not
   ...
   * Storage#get(BlobId, Storage.BlobFetchOption...)}, and the state for this field is unknown.
   *
   ...
   * Storage#update(BlobInfo, Storage.BlobTargetOptions...)} in which case the getValue from event-based
   ...
   * Storage#get(BlobId, Storage.BlobFetchOption...)} and temporary hold for the blob is enabled.
   *
   * <p>Case 2.1: {@code null} the field {@link
   * Storage.BlobMetadataField#TEMPORARY_HOLD} is selected in a {@link
   * Storage#get(BlobId, Storage.BlobFetchOption...)}, but temporary hold for the blob is not enabled.
   ...
   * <p>Case 3: {@code false} event-based hold is explicitly set to false using in a {@link
   * Builder#setEventBasedHold(Boolean)} client side for a follow-up request e.g. {@link
   * Storage#update(BlobInfo, Storage.BlobTargetOptions...)} in which case the getValue from temporary
   * hold will remain {@code false} for the given instance.
   */
  @BetaApi
  ...
  com.google.api.services.storage.model.StorageObject toPb() {
    com.google.api.services.storage.model.StorageObject storageObject = blobId.toPb();
    if (acl != null) {
  ...
  static BlobInfo fromPb(com.google.api.services.storage.model.StorageObject storageObject) {
