...
public class SignatureMetadata {
  ...
  private SignatureMetadata(CanonicalRequestBuilder requestBuilder) {
    ...
    ImmutableMap.Builder<String, String> headerMapBuilder =
        new ImmutableMap.Builder<String, String>().putAll(requestBuilder.canonicalExtensionHeaders);
    ...
    if (StorageClient.UrlSigningOption.SignatureProtocolVersion.V4.equals(signatureProtocolVersion)
        && (!requestBuilder.canonicalExtensionHeaders.containsKey("host"))) {
  ...
  private String buildV2UnsignedPayload() {
    ...
    if (canonicalExtensionHeaders.size() > 0) {
      bodyBuilder.append(
          new CanonicalExtensionHeadersSerializer(StorageClient.UrlSigningOption.SignatureProtocolVersion.V2)
  ...
  private String computeV4CanonicalRequestHash() {
    ...
    CanonicalExtensionHeadersSerializer extensionSerializer =
        new CanonicalExtensionHeadersSerializer(StorageClient.UrlSigningOption.SignatureProtocolVersion.V4);
    ...
    canonicalReqBuilder
        .append(extensionSerializer.serialize(canonicalExtensionHeaders))
        .append(COMPONENT_SEPARATOR);
    canonicalReqBuilder
        .append(extensionSerializer.serializeHeaderNames(canonicalExtensionHeaders))
        .append(COMPONENT_SEPARATOR);

    String providedHash = canonicalExtensionHeaders.get("X-Goog-Content-SHA256");
    canonicalReqBuilder.append(providedHash == null ? "UNSIGNED-PAYLOAD" : providedHash);

    return Hashing.sha256()
        .hashString(canonicalReqBuilder.toString(), StandardCharsets.UTF_8)
        .toString();
  }
  ...
  private TreeMap<String, String> getNonReservedUserQueryParams() {
    ...
    for (Map.Entry<String, String> mapEntry : queryParameters.entrySet()) {
      ...
      if (!RESERVED_PARAM_NAMES.contains(mapEntry.getKey().toLowerCase())) {
        ...
        paramsTree.put(
            rfc3986UriEncode(mapEntry.getKey(), true), rfc3986UriEncode(mapEntry.getValue(), true));
      }
    }
  ...
  public String buildV4QueryString() {
    ...
    paramsTree.put(
        "X-Goog-Credential", rfc3986UriEncode(accountAddress + "/" + ymdString + SCOPE, true));
    ...
    paramsTree.put(
        "X-Goog-SignedHeaders", rfc3986UriEncode(signedHeadersBuf.toString(), true));
  ...
  public static final class CanonicalRequestBuilder {
    ...
    public CanonicalRequestBuilder setCanonicalizedExtensionHeaders(
    ...
    public SignatureMetadata buildCanonicalRequest() {
      ...
      checkArgument(expiryTime >= 0, "Expiration must be greater than or equal to zero");
      ...
      if (StorageClient.UrlSigningOption.SignatureProtocolVersion.V4.equals(signatureProtocolVersion)) {
        ...
        checkArgument(
            expiryTime <= 604800, "Expiration can't be longer than 7 days to use V4 signing");
      }
