...
public class SignatureMetadata {
  ...
  private SignatureMetadata(CanonicalStringBuilder canonicalStringBuilder) {
    ...
    ImmutableMap.Builder<String, String> headersMapBuilder =
        new ImmutableMap.Builder<String, String>().putAll(canonicalStringBuilder.canonicalExtensionHeaders);
    ...
    if (Storage.UrlSigningOption.SignatureSchemeVersion.V4.equals(signatureScheme)
        && (!canonicalStringBuilder.canonicalExtensionHeaders.containsKey("host"))) {
  ...
  private String buildV2UnsignedPayload() {
    ...
    if (canonicalExtensionHeaders.size() > 0) {
      dataBuffer.append(
          new CanonicalExtensionHeadersSerializer(Storage.UrlSigningOption.SignatureSchemeVersion.V2)
  ...
  private String buildV4CanonicalRequestHash() {
    ...
    CanonicalExtensionHeadersSerializer extensionHeaderSerializer =
        new CanonicalExtensionHeadersSerializer(Storage.UrlSigningOption.SignatureSchemeVersion.V4);
    ...
    canonicalReqBuilder
        .append(extensionHeaderSerializer.serialize(canonicalExtensionHeaders))
        .append(COMPONENT_SEPARATOR);
    canonicalReqBuilder
        .append(extensionHeaderSerializer.serializeHeaderNames(canonicalExtensionHeaders))
        .append(COMPONENT_SEPARATOR);

    String externalHash = canonicalExtensionHeaders.get("X-Goog-Content-SHA256");
    canonicalReqBuilder.append(externalHash == null ? "UNSIGNED-PAYLOAD" : externalHash);
  ...
  private TreeMap<String, String> getNonReservedUserQueryParams() {
    ...
    for (Map.Entry<String, String> mapPair : queryParameters.entrySet()) {
      ...
      if (!RESERVED_PARAM_KEYS.contains(mapPair.getKey().toLowerCase())) {
        ...
        orderedParamTree.put(
            rfc3986UriEncode(mapPair.getKey(), true), rfc3986UriEncode(mapPair.getValue(), true));
      }
    }
  ...
  public String buildV4QueryString() {
    ...
    orderedParamTree.put(
        "X-Goog-Credential", rfc3986UriEncode(ownerEmail + "/" + dateYmd + SCOPE, true));
  ...
  public static final class CanonicalStringBuilder {
    ...
    public SignatureMetadata buildCanonicalString() {
      ...
      if (Storage.UrlSigningOption.SignatureSchemeVersion.V4.equals(signatureScheme)) {
        checkArgument(ownerEmail != null, "Account email required to use V4 signing");
