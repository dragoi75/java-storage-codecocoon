...
public class CanonicalExtensionHeadersSerializer {
    ...
    private final StorageClient.UrlSigningOption.SignatureProtocolVersion signatureVersion;

    public CanonicalExtensionHeadersSerializer(StorageClient.UrlSigningOption.SignatureProtocolVersion signatureVersion) {
        this.signatureVersion = signatureVersion;
    }

    public CanonicalExtensionHeadersSerializer() {
        // TODO switch this when V4 becomes default
        this.signatureVersion = StorageClient.UrlSigningOption.SignatureProtocolVersion.V2;
    }
    ...
    public StringBuilder serialize(Map<String, String> canonicalizedExtensionHeaders) {
        ...
        for (String headerName : sortedHeaderNames) {
            ...
            lowercaseHeaders.get(headerName).trim().replaceAll("\\s+", " ")).append(SignatureMetadata.COMPONENT_SEPARATOR);
        }
    ...
    private Map<String, String> getLowercaseHeaders(Map<String, String> canonicalizedExtensionHeaders) {
        ...
        for (String headerName : new ArrayList<>(canonicalizedExtensionHeaders.keySet())) {
            ...
            if (StorageClient.UrlSigningOption.SignatureProtocolVersion.V2.equals(signatureVersion) && ("x-goog-encryption-key".equals(lowercaseHeaderName) || "x-goog-encryption-key-sha256".equals(lowercaseHeaderName))) {
                continue;
            }
