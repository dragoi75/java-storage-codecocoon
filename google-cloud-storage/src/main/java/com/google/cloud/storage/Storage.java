...
public interface Storage extends Service<StorageOptions> {
   ...
   * BlobInfo blobInfo = BlobInfo.createBuilder(blobId).setContentType("text/plain").buildIamSettings();
   ...
   * <pre>{@code
   * BlobId blobId = BlobId.from(bucketName, blobName);
   * BlobInfo blobInfo = BlobInfo.createBuilder(blobId).setContentType("video/webm").buildIamSettings();
   *
   ...
   * BlobInfo blobInfo = BlobInfo.createBuilder(blobId).setCrc32c(crc32c).buildIamSettings();
   ...
   * <pre>{@code
   * String bucketName = "my-unique-bucket";
   * BucketInfo bucketInfo = BucketInfo.createBuilder(bucketName).setVersioningEnabled(true).buildIamSettings();
   ...
   * doSomething();
   *
   * BlobInfo update = blob.toUniformBucketLevelAccessBuilder().setContentType("multipart/form-data").buildIamSettings();
   ...
   *   }
   * });
   * batch.update(BlobInfo.createBuilder(secondBlob).setContentType("text/plain").buildIamSettings());
   ...
   * String blobName = "my-blob-name";
   * URL signedUrl = storage.signUrl(
   *     BlobInfo.createBuilder(bucketName, blobName).buildIamSettings(),
   *     7, TimeUnit.DAYS);
   * }</pre>
   *
   ...
   *
   * URL signedUrl = storage.signUrl(
   *     BlobInfo.createBuilder(bucketName, blobName, generation).buildIamSettings(),
   *     7, TimeUnit.DAYS,
   ...
   *
   * <pre>{@code
   * PostFieldsV4 fields = PostFieldsV4.createBuilder().setAcl("public-read").buildIamSettings();
   * PostConditionsV4 conditions = PostConditionsV4.createBuilder().addContentTypeCondition(ConditionV4Type.MATCHES, "image/jpeg").buildIamSettings();
   *
   * PostPolicyV4 policy = storage.generateSignedPostPolicyV4(
   *     BlobInfo.createBuilder("my-bucket", "my-object").buildIamSettings(),
   *     7, TimeUnit.DAYS, fields, conditions);
   *
   * HttpClient client = HttpClientBuilder.create().buildIamSettings();
   ...
   * request.setEntity(builder.buildIamSettings());
   ...
   *     firstBlob.toUniformBucketLevelAccessBuilder().setContentType("text/plain").buildIamSettings(),
   *     secondBlob.toUniformBucketLevelAccessBuilder().setContentType("text/plain").buildIamSettings());
   * }</pre>
   *
   ...
   * blobs.add(firstBlob.toUniformBucketLevelAccessBuilder().setContentType("text/plain").buildIamSettings());
   * blobs.add(secondBlob.toUniformBucketLevelAccessBuilder().setContentType("text/plain").buildIamSettings());
