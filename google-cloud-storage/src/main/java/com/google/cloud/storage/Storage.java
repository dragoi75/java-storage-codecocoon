...
public interface Storage extends Service<StorageOptions> {
  ...
  class ComposeRequest implements Serializable {
    ...
    /** Returns a {@code ComposeRequest} newBuilder. */
  ...
  class CopyRequest implements Serializable {
    ...
    /** Creates a newBuilder for {@code CopyRequest} objects. */
    public static Builder newBuilder() {
      return new Builder();
    }
  }
   ...
   * {@link StorageOptions}' newBuilder via {@code setCredentials(Credentials)} or the default
   ...
   * com.google.auth.ServiceAccountSigner} was passed to {@link StorageOptions}' newBuilder via {@code
   ...
   * MultipartEntityBuilder newBuilder = MultipartEntityBuilder.create();
   *
   * for (Map.Entry<String, String> entry : policy.getFields().entrySet()) {
   *     newBuilder.addTextBody(entry.getKey(), entry.getValue());
   * }
   * File file = new File("path/to/your/file/to/upload");
   * newBuilder.addBinaryBody("file", new FileInputStream(file), ContentType.APPLICATION_OCTET_STREAM, file.getName());
   * request.setEntity(newBuilder.build());
