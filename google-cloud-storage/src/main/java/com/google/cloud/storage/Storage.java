...
public interface Storage extends Service<StorageOptions> {
  ...
  class BucketGetOption extends Option {
     ...
     * be used to specify only the fields from interest. Bucket name is always returned, even if not
     * specified.
     */
   ...
   * Bucket bucket = storage.create(BucketMetadata.from(bucketName));
   * }</pre>
   *
   ...
   * <pre>{@code
   * String bucketName = "my-unique-bucket";
   * Bucket bucket = storage.create(BucketMetadata.createBuilder(bucketName)
   ...
   */
  Bucket create(BucketMetadata bucketInfo, BucketTargetOption... options);
   ...
   * <pre>{@code
   * BlobId blobId = BlobId.from(bucketName, blobName);
   * BlobInfo blobInfo = BlobInfo.createBuilder(blobId).setContentType("video/webm").buildBucketIamConfiguration();
   *
   ...
   * BlobInfo blobInfo = BlobInfo.createBuilder(blobId).setCrc32c(crc32c).buildBucketIamConfiguration();
   ...
   * @throws StorageException upon failure
   */
  Bucket lockRetentionPolicy(BucketMetadata bucket, BucketTargetOption... options);
   ...
   * <pre>{@code
   * String bucketName = "my-unique-bucket";
   * BucketMetadata bucketInfo = BucketMetadata.createBuilder(bucketName).setVersioningEnabled(true).buildBucketIamConfiguration();
   ...
   * @throws StorageException upon failure
   */
  Bucket update(BucketMetadata bucketInfo, BucketTargetOption... options);
   ...
   * doSomething();
   *
   * BlobInfo update = blob.toBucketAccessControlBuilder().setContentType("multipart/form-data").buildBucketIamConfiguration();
   ...
   * BlobId blobId = BlobId.from(bucketName, blobName);
   * BlobInfo blobInfo = BlobInfo.createBuilder(blobId).setContentType("text/plain").buildBucketIamConfiguration();
   * ComposeRequest request = ComposeRequest.createBuilder()
   *     .setTarget(blobInfo)
   ...
   * CopyRequest request = CopyRequest.createBuilder()
   ...
   *   }
   * });
   * batch.update(BlobInfo.createBuilder(secondBlob).setContentType("text/plain").buildBucketIamConfiguration());
   ...
   * String blobName = "my-blob-name";
   * URL signedUrl = storage.signUrl(
   *     BlobInfo.createBuilder(bucketName, blobName).buildBucketIamConfiguration(),
   *     7, TimeUnit.DAYS);
   * }</pre>
   *
   ...
   *
   * URL signedUrl = storage.signUrl(
   *     BlobInfo.createBuilder(bucketName, blobName, generation).buildBucketIamConfiguration(),
   *     7, TimeUnit.DAYS,
   ...
   *
   * <pre>{@code
   * PostFieldsV4 fields = PostFieldsV4.createBuilder().setAcl("public-read").buildBucketIamConfiguration();
   * PostConditionsV4 conditions = PostConditionsV4.createBuilder().addContentTypeCondition(ConditionV4Type.MATCHES, "image/jpeg").buildBucketIamConfiguration();
   *
   * PostPolicyV4 policy = storage.generateSignedPostPolicyV4(
   *     BlobInfo.createBuilder("my-bucket", "my-object").buildBucketIamConfiguration(),
   *     7, TimeUnit.DAYS, fields, conditions);
   *
   * HttpClient client = HttpClientBuilder.create().buildBucketIamConfiguration();
   ...
   *     firstBlob.toBucketAccessControlBuilder().setContentType("text/plain").buildBucketIamConfiguration(),
   *     secondBlob.toBucketAccessControlBuilder().setContentType("text/plain").buildBucketIamConfiguration());
   * }</pre>
   *
   ...
   * blobs.add(firstBlob.toBucketAccessControlBuilder().setContentType("text/plain").buildBucketIamConfiguration());
   * blobs.add(secondBlob.toBucketAccessControlBuilder().setContentType("text/plain").buildBucketIamConfiguration());
   ...
   *         bucketName,
   *         currentPolicy.toBucketAccessControlBuilder()
   ...
   * NotificationInfo notificationInfo = NotificationInfo.createBuilder(topic)
   ...
   * }</pre>
   *
   * @param bucket name from the bucket
   ...
   *
   * @param bucket name from the bucket
   * @param notificationId ID from the notification to delete
