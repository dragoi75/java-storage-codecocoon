...
final class StorageServiceImpl extends BaseService<StorageSettings> implements StorageService {
    ...
    @Override
    public boolean deleteAcl(final String containerName, final AclEntry.AbstractEntity accessEntity, BucketRequestOption... settings) {
        try {
            final Map<StorageRpcClient.StorageOption, ?> optionToValueMap = buildOptionMap(settings);
            return runWithRetries(new Callable<Boolean>() {
    ...
    private HmacKeyInfo updateHmacKeyInfo(final HmacSecretKey.HmacKeyInfo keyInfo, final HmacKeyUpdateOption... settings) {
        try {
    ...
    @Override
    public HmacSecretKey.HmacKeyInfo updateHmacKeyState(final HmacSecretKey.HmacKeyInfo keyInfo, final HmacSecretKey.HmacKeyStatus keyStatus, final HmacKeyUpdateOption... settings) {
        HmacKeyInfo newMetadata = HmacSecretKey.HmacKeyInfo.builder(keyInfo.getServiceAccount()).setProjectId(keyInfo.getProjectId()).setAccessId(keyInfo.getAccessId()).setState(keyStatus).create();
    ...
    private static Page<HmacSecretKey.HmacKeyInfo> listAllHmacKeys(final StorageSettings storageSettings, final Map<StorageRpcClient.StorageOption, ?> settings) {
        try {
            ...
            String pageCursor = listResult.x();
            final Iterable<HmacSecretKey.HmacKeyInfo> keyInfo = null == listResult.y() ? ImmutableList.<HmacKeyInfo>of() : Iterables.transform(listResult.y(), new Function<com.google.api.services.storage.model.HmacKeyMetadata, HmacSecretKey.HmacKeyInfo>() {
