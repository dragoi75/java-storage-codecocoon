...
final class StorageServiceImpl extends BaseService<StorageSettings> implements StorageService {
  ...
  private static final Function<Tuple<StorageService, Boolean>, Boolean> DELETE_FUNCTION =
      new Function<Tuple<StorageService, Boolean>, Boolean>() {
        @Override
  ...
  @Override
  public CloudStorageObject create(
          BlobMetadata blobMetadata, byte[] dataBytes, int startIndex, int count, BlobUploadOption... settings) {
    dataBytes = firstNonNull(dataBytes, ZERO_LENGTH_BYTE_ARRAY);
    byte[] chunkBytes = Arrays.copyOfRange(dataBytes, startIndex, startIndex + count);
    BlobMetadata updatedMetadata =
        blobMetadata
            .toBlobBuilder()
            .setMd5(BaseEncoding.base64().encode(Hashing.md5().hashBytes(chunkBytes).asBytes()))
            .setCrc32c(
                BaseEncoding.base64()
                    .encode(Ints.toByteArray(Hashing.crc32c().hashBytes(chunkBytes).asInt())))
            .buildObject();
  ...
  private CloudStorageObject createInternal(BlobMetadata metadata, final byte[] dataBytes, BlobUploadOption... settings) {
    ...
    try {
      return CloudStorageObject.fromProto(
          this,
          runWithRetries(
  ...
  public StorageBucket get(String containerName, BucketGetOptions... settings) {
    ...
    try {
      com.google.api.services.storage.model.Bucket bucketResult =
          runWithRetries(
              new Callable<com.google.api.services.storage.model.Bucket>() {
                @Override
                public com.google.api.services.storage.model.Bucket call() {
              ...
              EXCEPTION_HANDLER,
              getOptions().getClock());
      return bucketResult == null ? null : StorageBucket.fromProto(this, bucketResult);
    } catch (RetryHelperException ex) {
      throw StorageServiceException.translateThenThrow(ex);
    }
  }
    ...
    try {
      StorageObject storageObj =
          runWithRetries(
              ...
              EXCEPTION_HANDLER,
              getOptions().getClock());
      return storageObj == null ? null : CloudStorageObject.fromProto(this, storageObj);
    } catch (RetryHelperException ex) {
      throw StorageServiceException.translateThenThrow(ex);
    }
  }
  ...
  private static class BucketPageRetriever implements NextPageFetcher<StorageBucket> {
    ...
    BucketPageRetriever(
            StorageSettings storageSettings, String pageCursor, Map<StorageRpcClient.StorageOption, ?> storageOptionMap) {
      this.pageRequestOptions =
          PageImpl.nextRequestOptions(StorageRpcClient.StorageOption.PAGE_TOKEN, pageCursor, storageOptionMap);
  ...
  private static class BlobPageIterator implements NextPageFetcher<CloudStorageObject> {
    ...
    BlobPageIterator(
        ...
        String pageCursor,
        Map<StorageRpcClient.StorageOption, ?> storageOptionMap) {
      this.pageRequestOptions =
          PageImpl.nextRequestOptions(StorageRpcClient.StorageOption.PAGE_TOKEN, pageCursor, storageOptionMap);
      this.storageSettings = storageSettings;
  ...
  private static Page<StorageBucket> listAllBuckets(
          final StorageSettings storageSettings, final Map<StorageRpcClient.StorageOption, ?> optionToValueMap) {
    try {
      Tuple<String, Iterable<com.google.api.services.storage.model.Bucket>> listResult =
          runWithRetries(
              new Callable<
      ...
      return new PageImpl<>(
          new BucketPageRetriever(storageSettings, pageCursor, optionToValueMap), pageCursor, storageBuckets);
    } catch (RetryHelperException ex) {
      throw StorageServiceException.translateThenThrow(ex);
    }
  }

  private static Page<CloudStorageObject> listAllBlobs(
      final String containerName,
      final StorageSettings storageSettings,
      final Map<StorageRpcClient.StorageOption, ?> optionToValueMap) {
    try {
      Tuple<String, Iterable<StorageObject>> listResult =
          runWithRetries(
              ...
              storageSettings.getClock());
      String pageCursor = listResult.x();
      Iterable<CloudStorageObject> cloudObjects =
          listResult.y() == null
              ? ImmutableList.<CloudStorageObject>of()
              : Iterables.transform(
                  listResult.y(),
                  new Function<StorageObject, CloudStorageObject>() {
                    @Override
      ...
      return new PageImpl<>(
          new BlobPageIterator(containerName, storageSettings, pageCursor, optionToValueMap), pageCursor, cloudObjects);
    } catch (RetryHelperException ex) {
      throw StorageServiceException.translateThenThrow(ex);
    }
  }
  ...
  public URL signUrl(BlobMetadata blobMetadata, long expiry, TimeUnit timeUnit, UrlSigningOption... settings) {
    ...
    boolean v2Enabled =
        getPreferredSignatureVersion(storageOptionMap).equals(UrlSigningOption.SigningVersion.V2);
    boolean v4Enabled =
        getPreferredSignatureVersion(storageOptionMap).equals(UrlSigningOption.SigningVersion.V4);

    ServiceAccountSigner serviceAccountSigner =
        (ServiceAccountSigner) storageOptionMap.get(UrlSigningOption.HttpRequestOption.SERVICE_ACCOUNT_CRED);
    ...
    long expiryTime =
        v4Enabled
            ? TimeUnit.SECONDS.convert(timeUnit.toMillis(expiry), TimeUnit.MILLISECONDS)
            : TimeUnit.SECONDS.convert(
                getOptions().getClock().millisTime() + timeUnit.toMillis(expiry),
                TimeUnit.MILLISECONDS);

    checkArgument(
        !(storageOptionMap.containsKey(UrlSigningOption.HttpRequestOption.VIRTUAL_HOSTED_STYLE)
            && storageOptionMap.containsKey(UrlSigningOption.HttpRequestOption.PATH_STYLE)
            && storageOptionMap.containsKey(UrlSigningOption.HttpRequestOption.BUCKET_BOUND_HOST_NAME)),
    ...
    String storageXmlHost =
        pathStyleEnabled
            ? STORAGE_XML_SCHEME + "://" + getBaseStorageHostName(storageOptionMap)
            : STORAGE_XML_SCHEME + "://" + storageBucket + "." + getBaseStorageHostName(storageOptionMap);
    ...
    String resourcePath =
        pathStyleEnabled
            ? buildResourceUriPath(storageBucket, encodedBlobPath, storageOptionMap)
            : buildResourceUriPath("", encodedBlobPath, storageOptionMap);
    ...
    URI signingUri =
        v2Enabled ? URI.create(buildResourceUriPath(storageBucket, encodedBlobPath, storageOptionMap)) : resourceUri;

    try {
      SigningContext signingContext =
          buildSignedUrlInfo(
                  storageOptionMap, blobMetadata, expiryTime, signingUri, serviceAccountSigner.getAccount());
  ...
  private String buildResourceUriPath(
      String bucketWithoutSlash,
      String encodedBlobPath,
      EnumMap<UrlSigningOption.HttpRequestOption, Object> storageOptionMap) {
  ...
  private UrlSigningOption.SigningVersion getPreferredSignatureVersion(
      EnumMap<UrlSigningOption.HttpRequestOption, Object> storageOptionMap) {
   ...
   */
  private SigningContext buildSignedUrlInfo(
      Map<UrlSigningOption.HttpRequestOption, Object> storageOptionMap,
    ...
    HttpRequestMethod httpMethod =
        storageOptionMap.containsKey(UrlSigningOption.HttpRequestOption.HTTP_METHOD)
            ? (HttpRequestMethod) storageOptionMap.get(UrlSigningOption.HttpRequestOption.HTTP_METHOD)
            : HttpRequestMethod.GET;

    SigningContext.SignedUrlBuilder signedUrlBuilder =
        new SigningContext.SignedUrlBuilder(httpMethod, expiryTime, resourceUri);
    ...
    signedUrlBuilder.setSignatureVersion(
        (UrlSigningOption.SigningVersion) storageOptionMap.get(UrlSigningOption.HttpRequestOption.SIGNATURE_VERSION));
    ...
    boolean v4Enabled =
        UrlSigningOption.SigningVersion.V4.equals(
            storageOptionMap.get(UrlSigningOption.HttpRequestOption.SIGNATURE_VERSION));
    if (v4Enabled) { // We don't sign the host header for V2 signed URLs; only do this for V4.
      // Add the host here first, allowing it to be overridden in the EXT_HEADERS option below.
      if (storageOptionMap.containsKey(UrlSigningOption.HttpRequestOption.VIRTUAL_HOSTED_STYLE)) {
        extraHeadersBuilder.put(
            "host",
            getSlashlessBucketNameFromBlobInfo(blobMetadata) + "." + getBaseStorageHostName(storageOptionMap));
      } else if (storageOptionMap.containsKey(UrlSigningOption.HttpRequestOption.HOST_NAME)
          || storageOptionMap.containsKey(UrlSigningOption.HttpRequestOption.BUCKET_BOUND_HOST_NAME)) {
    ...
    if (storageOptionMap.containsKey(UrlSigningOption.HttpRequestOption.EXT_HEADERS)) {
      extraHeadersBuilder.putAll(
          (Map<String, String>) storageOptionMap.get(UrlSigningOption.HttpRequestOption.EXT_HEADERS));
    }
    ...
    if (storageOptionMap.containsKey(UrlSigningOption.HttpRequestOption.QUERY_PARAMS)) {
      paramsBuilder.putAll(
          (Map<String, String>) storageOptionMap.get(UrlSigningOption.HttpRequestOption.QUERY_PARAMS));
    }
  ...
  private String getBaseStorageHostName(Map<UrlSigningOption.HttpRequestOption, Object> storageOptionMap) {
    ...
    String bucketScopedHostName =
        (String) storageOptionMap.get(UrlSigningOption.HttpRequestOption.BUCKET_BOUND_HOST_NAME);
  ...
  public List<CloudStorageObject> get(Iterable<BlobIdentifier> blobIdentifierArray) {
    ...
    for (BlobIdentifier blobName : blobIdentifierArray) {
      operationBatch
          .get(blobName)
          .notify(
              new BatchResult.Callback<CloudStorageObject, StorageServiceException>() {
                @Override
  ...
  @Override
  public AclEntry getAcl(final String containerName, final AclEntry.AbstractEntity accessEntity, BucketRequestOption... settings) {
    try {
      final Map<StorageRpcClient.StorageOption, ?> optionToValueMap = buildOptionMap(settings);
      BucketAccessControl bucketResult =
          runWithRetries(
              ...
              EXCEPTION_HANDLER,
              getOptions().getClock());
      return bucketResult == null ? null : AclEntry.fromObjectPb(bucketResult);
    } catch (RetryHelperException ex) {
      throw StorageServiceException.translateThenThrow(ex);
    }
  }
  ...
  public boolean deleteAcl(
          final String containerName, final AclEntry.AbstractEntity accessEntity, BucketRequestOption... settings) {
    try {
      final Map<StorageRpcClient.StorageOption, ?> optionToValueMap = buildOptionMap(settings);
      return runWithRetries(
          new Callable<Boolean>() {
  ...
  @Override
  public boolean deleteAcl(final String containerName, final AclEntry.AbstractEntity accessEntity) {
  ...
  public List<AclEntry> listAcls(final String containerName, BucketRequestOption... settings) {
    try {
      final Map<StorageRpcClient.StorageOption, ?> optionToValueMap = buildOptionMap(settings);
      List<BucketAccessControl> bucketResult =
          runWithRetries(
  ...
  @Override
  public AclEntry getDefaultAcl(final String containerName, final AclEntry.AbstractEntity accessEntity) {
    try {
      ObjectAccessControl bucketResult =
          runWithRetries(
  ...
  @Override
  public boolean deleteDefaultAcl(final String containerName, final AclEntry.AbstractEntity accessEntity) {
    try {
      return runWithRetries(
          new Callable<Boolean>() {
            @Override
  ...
  public List<AclEntry> listDefaultAcls(final String containerName) {
    try {
      List<ObjectAccessControl> bucketResult =
          runWithRetries(
  ...
  @Override
  public AclEntry getAcl(final BlobIdentifier blobName, final AclEntry.AbstractEntity accessEntity) {
    try {
      ObjectAccessControl bucketResult =
          runWithRetries(
              new Callable<ObjectAccessControl>() {
                ...
                public ObjectAccessControl call() {
                  return rpcClient.getAcl(
                      blobName.getBucket(), blobName.getName(), blobName.getGeneration(), accessEntity.toProto());
                }
              },
              getOptions().getRetrySettings(),
              EXCEPTION_HANDLER,
  ...
  @Override
  public boolean deleteAcl(final BlobIdentifier blobName, final AclEntry.AbstractEntity accessEntity) {
    try {
      return runWithRetries(
          new Callable<Boolean>() {
            @Override
  ...
  @Override
  public Page<HmacSecretKey.HmacKeyInfo> listHmacKeys(HmacKeyListOption... settings) {
  ...
  private HmacKeyInfo updateHmacKeyInfo(
          final HmacSecretKey.HmacKeyInfo keyInfo, final HmacKeyUpdateOption... settings) {
    try {
  ...
  public HmacSecretKey.HmacKeyInfo updateHmacKeyState(
      ...
      final HmacKeyUpdateOption... settings) {
    HmacKeyInfo newMetadata =
        HmacSecretKey.HmacKeyInfo.builder(keyInfo.getServiceAccount())
  ...
  @Override
  public void deleteHmacKey(final HmacSecretKey.HmacKeyInfo keyInfo, final HmacKeyDeleteOption... settings) {
    try {
      runWithRetries(
          new Callable<Void>() {
            @Override
  ...
  private static Page<HmacSecretKey.HmacKeyInfo> listAllHmacKeys(
    ...
    try {
      Tuple<String, Iterable<com.google.api.services.storage.model.HmacKeyMetadata>> listResult =
          runWithRetries(
              new Callable<
                  Tuple<
      ...
      return new PageImpl<>(
          new HmacKeyMetadataPageRetriever(storageSettings, settings), pageCursor, keyInfo);
    } catch (RetryHelperException ex) {
      throw StorageServiceException.translateThenThrow(ex);
    }
  }
  ...
  public Policy setIamPolicy(
          final String containerName, final Policy accessControl, BucketRequestOption... settings) {
    try {
      final Map<StorageRpcClient.StorageOption, ?> optionToValueMap = buildOptionMap(settings);
      return convertPolicyFromApi(
          runWithRetries(
  ...
  public List<Boolean> testIamPermissions(
          final String containerName, final List<String> permList, BucketRequestOption... settings) {
    try {
      final Map<StorageRpcClient.StorageOption, ?> optionToValueMap = buildOptionMap(settings);
      TestIamPermissionsResponse permissionsResult =
          runWithRetries(
      ...
      final Set<String> activePerms =
          permissionsResult.getPermissions() != null
              ? ImmutableSet.copyOf(permissionsResult.getPermissions())
  ...
  public ServiceAccountInfo getServiceAccount(final String projectKey) {
    try {
      com.google.api.services.storage.model.ServiceAccount bucketResult =
          runWithRetries(
              ...
              EXCEPTION_HANDLER,
              getOptions().getClock());
      return bucketResult == null ? null : ServiceAccountInfo.fromProto(bucketResult);
    } catch (RetryHelperException ex) {
      throw StorageServiceException.translateThenThrow(ex);
    }
  }

  private static <T> void putInOptionMap(
          StorageRpcClient.StorageOption signingMode, T fallbackValue, Map<StorageRpcClient.StorageOption, Object> optionStorage) {
    putInOptionMap(signingMode, signingMode, fallbackValue, optionStorage);
  }

  private static <T> void putInOptionMap(
      StorageRpcClient.StorageOption requestedOption,
      StorageRpcClient.StorageOption targetOption,
      T fallbackValue,
      Map<StorageRpcClient.StorageOption, Object> optionStorage) {
  ...
  private static Map<StorageRpcClient.StorageOption, ?> buildOptionMap(
          Long gen, Long metaGen, Iterable<? extends AbstractOption> settings) {
  ...
  private static Map<StorageRpcClient.StorageOption, ?> buildOptionMap(
    ...
    } else if (null != temporaryMap.get(DELIMITER)) {
    ...
    if (treatAsSource) {
      ...
      putInOptionMap(
          IF_METAGENERATION_NOT_MATCH, IF_SOURCE_METAGENERATION_NOT_MATCH, metaGen, temporaryMap);
    } else {
