...
final class StorageServiceImpl extends BaseService<StorageSettings> implements Storage {
    ...
    public StorageObject create(BlobInfo blobMetadata, BlobUploadOption... settings) {
        BlobInfo updatedBlobInfo = blobMetadata.asBuilder().setMd5(ZERO_BYTES_MD5).setCrc32c(ZERO_BYTES_CRC32C).buildMetadata();
    ...
    public StorageObject create(BlobInfo blobMetadata, byte[] data, BlobUploadOption... settings) {
        ...
        BlobInfo updatedBlobInfo = blobMetadata.asBuilder().setMd5(BaseEncoding.base64().encode(Hashing.md5().hashBytes(data).asBytes())).setCrc32c(BaseEncoding.base64().encode(Ints.toByteArray(Hashing.crc32c().hashBytes(data).asInt()))).buildMetadata();
        return createInternal(updatedBlobInfo, data, settings);
    }

    @Override
    public StorageObject create(BlobInfo blobMetadata, byte[] data, int startOffset, int lengthBytes, BlobUploadOption... settings) {
        data = firstNonNull(data, ZERO_LENGTH_BYTES);
        byte[] subBytes = Arrays.copyOfRange(data, startOffset, startOffset + lengthBytes);
        BlobInfo updatedBlobInfo = blobMetadata.asBuilder().setMd5(BaseEncoding.base64().encode(Hashing.md5().hashBytes(subBytes).asBytes())).setCrc32c(BaseEncoding.base64().encode(Ints.toByteArray(Hashing.crc32c().hashBytes(subBytes).asInt()))).buildMetadata();
    ...
    public StorageObject create(BlobInfo blobMetadata, InputStream data, BlobWriteOptions... settings) {
        ...
        InputStream inputStream = firstNonNull(data, new ByteArrayInputStream(ZERO_LENGTH_BYTES));
    ...
    public StorageObject get(BlobIdentifier objectName, BlobGetOptions... settings) {
        ...
        try {
            com.google.api.services.storage.model.StorageObject storageItem = runWithRetries(new Callable<com.google.api.services.storage.model.StorageObject>() {
            ...
            return null == storageItem ? null : StorageObject.fromProto(this, storageItem);
        } catch (RetryHelperException retryEx) {
            throw StorageServiceException.translateAndRethrow(retryEx);
        }
    }
    ...
    private static class BucketPageIterator implements NextPageFetcher<StorageBucket> {
        ...
        BucketPageIterator(StorageSettings storageSettings, String pageToken, Map<StorageRpcClient.StorageOption, ?> optionMapping) {
            this.pageParams = PageImpl.nextRequestOptions(StorageRpcClient.StorageOption.PAGE_TOKEN, pageToken, optionMapping);
    ...
    private static class BlobPageRetriever implements NextPageFetcher<StorageObject> {
        ...
        BlobPageRetriever(String containerName, StorageSettings storageSettings, String pageToken, Map<StorageRpcClient.StorageOption, ?> optionMapping) {
            this.pageParams = PageImpl.nextRequestOptions(StorageRpcClient.StorageOption.PAGE_TOKEN, pageToken, optionMapping);
            this.storageSettings = storageSettings;
    ...
    private static Page<StorageBucket> listAllBuckets(final StorageSettings storageSettings, final Map<StorageRpcClient.StorageOption, ?> storageOptionsMap) {
        try {
            Tuple<String, Iterable<com.google.api.services.storage.model.Bucket>> bucketsPage = runWithRetries(new Callable<Tuple<String, Iterable<com.google.api.services.storage.model.Bucket>>>() {
            ...
            Iterable<StorageBucket> storageBucketCollection = null == bucketsPage.y() ? ImmutableList.<StorageBucket>of() : Iterables.transform(bucketsPage.y(), new Function<com.google.api.services.storage.model.Bucket, StorageBucket>() {
            ...
            return new PageImpl<>(new BucketPageIterator(storageSettings, pageToken, storageOptionsMap), pageToken, storageBucketCollection);
        } catch (RetryHelperException retryEx) {
            throw StorageServiceException.translateAndRethrow(retryEx);
        }
    }

    private static Page<StorageObject> listAllBlobs(final String containerName, final StorageSettings storageSettings, final Map<StorageRpcClient.StorageOption, ?> storageOptionsMap) {
        try {
            Tuple<String, Iterable<com.google.api.services.storage.model.StorageObject>> bucketsPage = runWithRetries(new Callable<Tuple<String, Iterable<com.google.api.services.storage.model.StorageObject>>>() {
            ...
            String pageToken = bucketsPage.x();
            Iterable<StorageObject> objectCollection = null == bucketsPage.y() ? ImmutableList.<StorageObject>of() : Iterables.transform(bucketsPage.y(), new Function<com.google.api.services.storage.model.StorageObject, StorageObject>() {
            ...
            return new PageImpl<>(new BlobPageRetriever(containerName, storageSettings, pageToken, storageOptionsMap), pageToken, objectCollection);
        } catch (RetryHelperException retryEx) {
            throw StorageServiceException.translateAndRethrow(retryEx);
        }
    }
    ...
    public StorageObject compose(final ComposeObjectsRequest composeOperation) {
        final List<com.google.api.services.storage.model.StorageObject> sourceObjects = Lists.newArrayListWithCapacity(composeOperation.getSourceBlobs().size());
        for (ComposeObjectsRequest.SourceBlobMetadata sourceMetadata : composeOperation.getSourceBlobs()) {
            sourceObjects.add(BlobInfo.newBuilder(BlobIdentifier.create(composeOperation.getTarget().getBucket(), sourceMetadata.getName(), sourceMetadata.getGeneration())).buildMetadata().toProto());
        }
        final com.google.api.services.storage.model.StorageObject destinationObject = composeOperation.getTarget().toProto();
        final Map<StorageRpcClient.StorageOption, ?> blobTargetOptions = createOptionMap(composeOperation.getTarget().getGeneration(), composeOperation.getTarget().getMetageneration(), composeOperation.getTargetOptions());
        try {
            return StorageObject.fromProto(this, runWithRetries(new Callable<com.google.api.services.storage.model.StorageObject>() {
    ...
    public ChunkedCopyWriter copy(final CopyOperationRequest copyOperation) {
        ...
        final Map<StorageRpcClient.StorageOption, ?> originOptions = createOptionMap(copyOperation.getSource().getGeneration(), null, copyOperation.getSourceOptions(), true);
        final com.google.api.services.storage.model.StorageObject destinationObject = copyOperation.getTarget().toProto();
        final Map<StorageRpcClient.StorageOption, ?> blobTargetOptions = createOptionMap(copyOperation.getTarget().getGeneration(), copyOperation.getTarget().getMetageneration(), copyOperation.getTargetOptions());
        try {
            RewriteOperationResult rewriteResult = runWithRetries(new Callable<StorageRpcClient.RewriteOperationResult>() {

                @Override
                public StorageRpcClient.RewriteOperationResult call() {
                    return storageClient.openRewrite(new StorageRpcClient.RewriteOperationRequest(originObject, originOptions, copyOperation.getOverrideInfo(), destinationObject, blobTargetOptions, copyOperation.getMegabytesCopiedPerChunk()));
                }
            }, getOptions().getRetrySettings(), EXCEPTION_HANDLER, getOptions().getClock());
    ...
    public URL signUrl(BlobInfo blobMetadata, long expiry, TimeUnit timeScale, UrlSigningOption... settings) {
        ...
        boolean legacySigningEnabled = getPreferredSignatureVersion(optionMapping).equals(UrlSigningOption.SignatureSchemeVersion.V2);
        boolean enhancedSigningEnabled = getPreferredSignatureVersion(optionMapping).equals(UrlSigningOption.SignatureSchemeVersion.V4);
        ServiceAccountSigner serviceAccountSigner = (ServiceAccountSigner) optionMapping.get(UrlSigningOption.RequestOption.SERVICE_ACCOUNT_CRED);
        ...
        long expiryEpoch = enhancedSigningEnabled ? TimeUnit.SECONDS.convert(timeScale.toMillis(expiry), TimeUnit.MILLISECONDS) : TimeUnit.SECONDS.convert(getOptions().getClock().millisTime() + timeScale.toMillis(expiry), TimeUnit.MILLISECONDS);
        checkArgument(!(optionMapping.containsKey(UrlSigningOption.RequestOption.VIRTUAL_HOSTED_STYLE) && optionMapping.containsKey(UrlSigningOption.RequestOption.PATH_STYLE) && optionMapping.containsKey(UrlSigningOption.RequestOption.BUCKET_BOUND_HOST_NAME)), "Only one of VIRTUAL_HOSTED_STYLE, PATH_STYLE, or BUCKET_BOUND_HOST_NAME SignUrlOptions can be" + " specified.");
        ...
        String xmlHost = pathStyleEnabled ? STORAGE_XML_SCHEME + "://" + getBaseStorageHostName(optionMapping) : STORAGE_XML_SCHEME + "://" + bucketId + "." + getBaseStorageHostName(optionMapping);
        ...
        String signedPath = pathStyleEnabled ? buildResourcePath(bucketId, escapedObjectName, optionMapping) : buildResourcePath("", escapedObjectName, optionMapping);
        ...
        URI signingUri = legacySigningEnabled ? URI.create(buildResourcePath(bucketId, escapedObjectName, optionMapping)) : resourceUri;
        try {
            SignatureMetadata signingMetadata = buildCanonicalSignature(optionMapping, blobMetadata, expiryEpoch, signingUri, serviceAccountSigner.getAccount());
    ...
    @Override
    public PostPolicyV4 generateSignedPostPolicyV4(BlobInfo blobMetadata, long expiry, TimeUnit timeScale, PostFieldsV4 formFields, PostConditionsV4 policyConditions, PostPolicyV4Parameter... settings) {
        EnumMap<UrlSigningOption.RequestOption, Object> optionMapping = Maps.newEnumMap(UrlSigningOption.RequestOption.class);
        ...
        String signerKey = serviceAccountSigner.getAccount() + "/" + dateYmdFormat.format(nowMillis) + "/auto/storage/goog4_request";
        ...
        for (Map.Entry<String, String> mapPair : formFields.getFieldsMap().entrySet()) {
            ...
            postConditionsBuilder.addCondition(ConditionV4Type.MATCHES, mapPair.getKey(), mapPair.getValue());
            policyFieldMap.put(mapPair.getKey(), mapPair.getValue());
        }
        PostConditionsV4 postConditions = postConditionsBuilder.withBucketCondition(ConditionV4Type.MATCHES, blobMetadata.getBucket()).withKeyCondition(ConditionV4Type.MATCHES, blobMetadata.getName()).addCondition(ConditionV4Type.MATCHES, "x-goog-date", formattedDate).addCondition(ConditionV4Type.MATCHES, "x-goog-credential", signerKey).addCondition(ConditionV4Type.MATCHES, "x-goog-algorithm", "GOOG4-RSA-SHA256").buildInstance();
        PostPolicyV4Document postPolicy = PostPolicyV4Document.create(expiryFormatter.format(nowMillis + timeScale.toMillis(expiry)), postConditions);
        String encodedPolicy = BaseEncoding.base64().encode(postPolicy.toJsonString().getBytes());
        String encodedSignature = BaseEncoding.base16().encode(serviceAccountSigner.sign(encodedPolicy.getBytes())).toLowerCase();
        for (PostPolicyV4.ConditionV4 postCondition : postConditions.getConditions()) {
            if (ConditionV4Type.MATCHES == postCondition.conditionCategory) {
    ...
    public PostPolicyV4 generateSignedPostPolicyV4(BlobInfo blobMetadata, long expiry, TimeUnit timeScale, PostFieldsV4 formFields, PostPolicyV4Parameter... settings) {
        return generateSignedPostPolicyV4(blobMetadata, expiry, timeScale, formFields, PostConditionsV4.builder().buildInstance(), settings);
    }

    public PostPolicyV4 generateSignedPostPolicyV4(BlobInfo blobMetadata, long expiry, TimeUnit timeScale, PostConditionsV4 postConditions, PostPolicyV4Parameter... settings) {
        return generateSignedPostPolicyV4(blobMetadata, expiry, timeScale, PostFieldsV4.builder().buildInstance(), postConditions, settings);
    }

    public PostPolicyV4 generateSignedPostPolicyV4(BlobInfo blobMetadata, long expiry, TimeUnit timeScale, PostPolicyV4Parameter... settings) {
        return generateSignedPostPolicyV4(blobMetadata, expiry, timeScale, PostFieldsV4.builder().buildInstance(), settings);
    }

    private String buildResourcePath(String bucketNameNoSlash, String escapedObjectName, EnumMap<UrlSigningOption.RequestOption, Object> optionMapping) {
    ...
    private UrlSigningOption.SignatureSchemeVersion getPreferredSignatureVersion(EnumMap<UrlSigningOption.RequestOption, Object> optionMapping) {
    ...
    private boolean usePathStyleForSignedUrl(EnumMap<UrlSigningOption.RequestOption, Object> optionMapping) {
        ...
        if (optionMapping.containsKey(UrlSigningOption.RequestOption.VIRTUAL_HOSTED_STYLE) || optionMapping.containsKey(UrlSigningOption.RequestOption.BUCKET_BOUND_HOST_NAME)) {
            return false;
        }
        return true;
    }
     ...
     */
    private SignatureMetadata buildCanonicalSignature(Map<UrlSigningOption.RequestOption, Object> optionMapping, BlobInfo blobMetadata, long expiryEpoch, URI resourceUri, String serviceAccountEmail) {
        HttpRequestMethod httpMethod = optionMapping.containsKey(UrlSigningOption.RequestOption.HTTP_METHOD) ? (HttpRequestMethod) optionMapping.get(UrlSigningOption.RequestOption.HTTP_METHOD) : HttpRequestMethod.GET;
        SignatureMetadata.CanonicalStringBuilder signatureStringBuilder = new SignatureMetadata.CanonicalStringBuilder(httpMethod, expiryEpoch, resourceUri);
        ...
        signatureStringBuilder.setSignatureVersion((UrlSigningOption.SignatureSchemeVersion) optionMapping.get(UrlSigningOption.RequestOption.SIGNATURE_VERSION));
        ...
        ImmutableMap.Builder<String, String> extensionHeadersBuilder = new ImmutableMap.Builder<String, String>();
        boolean enhancedSigningEnabled = UrlSigningOption.SignatureSchemeVersion.V4.equals(optionMapping.get(UrlSigningOption.RequestOption.SIGNATURE_VERSION));
        if (enhancedSigningEnabled) {
            ...
            if (!optionMapping.containsKey(UrlSigningOption.RequestOption.VIRTUAL_HOSTED_STYLE)) {
                if (optionMapping.containsKey(UrlSigningOption.RequestOption.HOST_NAME) || optionMapping.containsKey(UrlSigningOption.RequestOption.BUCKET_BOUND_HOST_NAME)) {
                    extensionHeadersBuilder.put("host", getBaseStorageHostName(optionMapping));
                }
            } else {
                extensionHeadersBuilder.put("host", getSlashlessBucketName(blobMetadata) + "." + getBaseStorageHostName(optionMapping));
            }
        }
        if (optionMapping.containsKey(UrlSigningOption.RequestOption.EXT_HEADERS)) {
            extensionHeadersBuilder.putAll((Map<String, String>) optionMapping.get(UrlSigningOption.RequestOption.EXT_HEADERS));
        }
        ...
        if (optionMapping.containsKey(UrlSigningOption.RequestOption.QUERY_PARAMS)) {
            paramsBuilder.putAll((Map<String, String>) optionMapping.get(UrlSigningOption.RequestOption.QUERY_PARAMS));
        }
        return signatureStringBuilder.setCanonicalizedExtensionHeaders((Map<String, String>) extensionHeadersBuilder.build()).setCanonicalizedQueryParams((Map<String, String>) paramsBuilder.build()).buildCanonicalString();
    }
    ...
    private String getBaseStorageHostName(Map<UrlSigningOption.RequestOption, Object> optionMapping) {
        ...
        String bucketScopedHostName = (String) optionMapping.get(UrlSigningOption.RequestOption.BUCKET_BOUND_HOST_NAME);
    ...
    public List<StorageObject> get(Iterable<BlobIdentifier> blobIdentifiers) {
        ...
        for (BlobIdentifier objectName : blobIdentifiers) {
            operationBatch.get(objectName).notify(new BatchResult.Callback<StorageObject, StorageServiceException>() {
    ...
    public List<StorageObject> update(Iterable<BlobInfo> blobMetadataArray) {
        ...
        for (BlobInfo blobMetadata : blobMetadataArray) {
            operationBatch.patch(blobMetadata).notify(new BatchResult.Callback<StorageObject, StorageServiceException>() {
    ...
    public List<Boolean> delete(Iterable<BlobIdentifier> blobIdentifiers) {
        ...
        for (BlobIdentifier objectName : blobIdentifiers) {
            operationBatch.deleteBlob(objectName).notify(new BatchResult.Callback<Boolean, StorageServiceException>() {
    ...
    public AclEntry getAcl(final String containerName, final BaseEntity entityInfo, BucketSourceRequestOption... settings) {
        try {
            ...
            return null == resultBucket ? null : AclEntry.fromProto(resultBucket);
        } catch (RetryHelperException retryEx) {
            throw StorageServiceException.translateAndRethrow(retryEx);
        }
    }
    ...
    @Override
    public boolean deleteAcl(final String containerName, final BaseEntity entityInfo, BucketSourceRequestOption... settings) {
        try {
            final Map<StorageRpcClient.StorageOption, ?> storageOptionsMap = createOptionMap(settings);
            return runWithRetries(new Callable<Boolean>() {
    ...
    public AclEntry getAcl(final BlobIdentifier objectName, final AclEntry.BaseEntity entityInfo) {
        try {
            ObjectAccessControl resultBucket = runWithRetries(new Callable<ObjectAccessControl>() {
                ...
                @Override
                public ObjectAccessControl call() {
                    return storageClient.getAcl(objectName.getBucket(), objectName.getName(), objectName.getGeneration(), entityInfo.toProto());
                }
            }, getOptions().getRetrySettings(), EXCEPTION_HANDLER, getOptions().getClock());
            return null == resultBucket ? null : AclEntry.fromProto(resultBucket);
        } catch (RetryHelperException retryEx) {
    ...
    public boolean deleteAcl(final BlobIdentifier objectName, final BaseEntity entityInfo) {
        try {
            return runWithRetries(new Callable<Boolean>() {
                ...
                @Override
                public Boolean call() {
                    return storageClient.deleteAcl(objectName.getBucket(), objectName.getName(), objectName.getGeneration(), entityInfo.toProto());
                }
            }, getOptions().getRetrySettings(), EXCEPTION_HANDLER, getOptions().getClock());
        } catch (RetryHelperException retryEx) {
            throw StorageServiceException.translateAndRethrow(retryEx);
    ...
    public List<AclEntry> listAcls(final BlobIdentifier objectName) {
        try {
            List<ObjectAccessControl> resultBucket = runWithRetries(new Callable<List<ObjectAccessControl>>() {
                ...
                @Override
                public List<ObjectAccessControl> call() {
                    return storageClient.listAcls(objectName.getBucket(), objectName.getName(), objectName.getGeneration());
                }
            }, getOptions().getRetrySettings(), EXCEPTION_HANDLER, getOptions().getClock());
            return Lists.transform(resultBucket, AclEntry.OBJECT_PB_TO_ACL_ENTRY_FN);
        } catch (RetryHelperException retryEx) {
    ...
    private HmacKeyInfo updateHmacKeyMetadata(final HmacKeyInfo keyInfo, final UpdateHmacKeyOptions... settings) {
        try {
            return HmacKeyInfo.fromProto(runWithRetries(new Callable<com.google.api.services.storage.model.HmacKeyMetadata>() {
    ...
    @Override
    public HmacSecretKey.HmacKeyInfo updateHmacKeyState(final HmacKeyInfo keyInfo, final HmacSecretKey.HmacKeyStatus keyStatus, final UpdateHmacKeyOptions... settings) {
        HmacSecretKey.HmacKeyInfo newMetadata = HmacSecretKey.HmacKeyInfo.newHmacKeyBuilder(keyInfo.getServiceAccount()).setProjectId(keyInfo.getProjectId()).setAccessId(keyInfo.getAccessId()).setState(keyStatus).buildHmacKeyInfo();
    ...
    private static Page<HmacSecretKey.HmacKeyInfo> listAllHmacKeys(final StorageSettings storageSettings, final Map<StorageRpcClient.StorageOption, ?> settings) {
        try {
            Tuple<String, Iterable<com.google.api.services.storage.model.HmacKeyMetadata>> bucketsPage = runWithRetries(new Callable<Tuple<String, Iterable<com.google.api.services.storage.model.HmacKeyMetadata>>>() {
            ...
            String pageToken = bucketsPage.x();
            final Iterable<HmacKeyInfo> keyInfo = null == bucketsPage.y() ? ImmutableList.<HmacKeyInfo>of() : Iterables.transform(bucketsPage.y(), new Function<com.google.api.services.storage.model.HmacKeyMetadata, HmacKeyInfo>() {
            ...
            return new PageImpl<>(new HmacKeyMetadataPaginator(storageSettings, settings), pageToken, keyInfo);
        } catch (RetryHelperException retryEx) {
            throw StorageServiceException.translateAndRethrow(retryEx);
        }
    }
    ...
    @Override
    public Policy setIamPolicy(final String containerName, final Policy encodedPolicy, BucketSourceRequestOption... settings) {
        try {
            final Map<StorageRpcClient.StorageOption, ?> storageOptionsMap = createOptionMap(settings);
            return fromApiPolicy(runWithRetries(new Callable<com.google.api.services.storage.model.Policy>() {
    ...
    @Override
    public List<Boolean> testIamPermissions(final String containerName, final List<String> requestedActions, BucketSourceRequestOption... settings) {
        try {
            final Map<StorageRpcClient.StorageOption, ?> storageOptionsMap = createOptionMap(settings);
            ...
            final Set<String> grantedActions = null != iamResult.getPermissions() ? ImmutableSet.copyOf(iamResult.getPermissions()) : ImmutableSet.<String>of();
            return Lists.transform(requestedActions, new Function<String, Boolean>() {
    ...
    private static <T> void putOptionIfAbsent(StorageRpcClient.StorageOption signingMode, T fallbackValue, Map<StorageRpcClient.StorageOption, Object> optionMap) {
        putOptionIfAbsent(signingMode, signingMode, fallbackValue, optionMap);
    }

    private static <T> void putOptionIfAbsent(StorageRpcClient.StorageOption requestedOption, StorageRpcClient.StorageOption insertOption, T fallbackValue, Map<StorageRpcClient.StorageOption, Object> optionMap) {
    ...
    private static Map<StorageRpcClient.StorageOption, ?> createOptionMap(Long expectedGeneration, Long expectedMetaGeneration, Iterable<? extends AbstractOption> settings) {
        return createOptionMap(expectedGeneration, expectedMetaGeneration, settings, false);
    }

    private static Map<StorageRpcClient.StorageOption, ?> createOptionMap(Long expectedGeneration, Long expectedMetaGeneration, Iterable<? extends AbstractOption> settings, boolean treatAsSource) {
        ...
        if (!treatAsSource) {
            ...
            putOptionIfAbsent(IF_METAGENERATION_NOT_MATCH, IF_SOURCE_METAGENERATION_NOT_MATCH, expectedMetaGeneration, interimMap);
        }
    ...
    private static Map<StorageRpcClient.StorageOption, ?> createOptionMap(Long expectedGeneration, Long expectedMetaGeneration, AbstractOption... settings) {
