...
final class StorageServiceImpl extends BaseService<StorageClientOptions> implements CloudStorageClient {
    ...
    private static class BucketPageRetriever implements NextPageFetcher<StorageBucket> {
        ...
        BucketPageRetriever(StorageClientOptions clientConfig, String pageCursor, Map<StorageRpcClient.StorageOption, ?> optionsLookup) {
            this.requestParams = PageImpl.nextRequestOptions(StorageRpcClient.StorageOption.PAGE_TOKEN, pageCursor, optionsLookup);
    ...
    private static class BlobPageIterator implements NextPageFetcher<StorageObject> {
        ...
        BlobPageIterator(String containerName, StorageClientOptions clientConfig, String pageCursor, Map<StorageRpcClient.StorageOption, ?> optionsLookup) {
            this.requestParams = PageImpl.nextRequestOptions(StorageRpcClient.StorageOption.PAGE_TOKEN, pageCursor, optionsLookup);
            this.clientConfig = clientConfig;
    ...
    @Override
    public AccessControlEntry getAcl(final String containerName, final ProtoEntity protoEntity, BucketOption... clientConfig) {
        try {
        ...
        try {
            List<ObjectAccessControl> result = runWithRetries(new Callable<List<ObjectAccessControl>>() {
                ...
                public List<ObjectAccessControl> call() {
                    return rpcClient.listAcls(objectName.getBucket(), objectName.getName(), objectName.getGeneration());
                }
            }, getOptions().getRetrySettings(), EXCEPTION_HANDLER, getOptions().getClock());
    ...
    public URL signUrl(BlobMetadata blobMetadata, long expiryDuration, TimeUnit timeMeasure, UrlSigningOption... clientConfig) {
        ...
        boolean versionTwoEnabled = getPreferredSignatureVersion(optionsLookup).equals(UrlSigningOption.SignatureProtocolVersion.V2);
        boolean versionFourEnabled = getPreferredSignatureVersion(optionsLookup).equals(UrlSigningOption.SignatureProtocolVersion.V4);
        ServiceAccountSigner serviceSigner = (ServiceAccountSigner) optionsLookup.get(UrlSigningOption.RequestOption.SERVICE_ACCOUNT_CRED);
        if (null == serviceSigner) {
        ...
        long expiryTimestamp = versionFourEnabled ? TimeUnit.SECONDS.convert(timeMeasure.toMillis(expiryDuration), TimeUnit.MILLISECONDS) : TimeUnit.SECONDS.convert(getOptions().getClock().millisTime() + timeMeasure.toMillis(expiryDuration), TimeUnit.MILLISECONDS);
        checkArgument(!(optionsLookup.containsKey(UrlSigningOption.RequestOption.VIRTUAL_HOSTED_STYLE) && optionsLookup.containsKey(UrlSigningOption.RequestOption.PATH_STYLE) && optionsLookup.containsKey(UrlSigningOption.RequestOption.BUCKET_BOUND_HOST_NAME)), "Only one of VIRTUAL_HOSTED_STYLE, PATH_STYLE, or BUCKET_BOUND_HOST_NAME SignUrlOptions can be" + " specified.");
        ...
        String xmlHost = enablePathStyle ? STORAGE_XML_SCHEME + "://" + getBaseStorageHostName(optionsLookup) : STORAGE_XML_SCHEME + "://" + containerName + "." + getBaseStorageHostName(optionsLookup);
        ...
        String servicePath = enablePathStyle ? buildResourceUriPath(containerName, escapedObjectName, optionsLookup) : buildResourceUriPath("", escapedObjectName, optionsLookup);
        ...
        URI signingUri = versionTwoEnabled ? URI.create(buildResourceUriPath(containerName, escapedObjectName, optionsLookup)) : fileLocation;
        try {
            SignatureDetails signatureDetails = buildCanonicalSignature(optionsLookup, blobMetadata, expiryTimestamp, signingUri, serviceSigner.getAccount());
    ...
    @Override
    public AccessControlEntry getDefaultAcl(final String containerName, final ProtoEntity protoEntity) {
        try {
    ...
    public StorageObject compose(final ComposeBlobsRequest composeReq) {
        final List<com.google.api.services.storage.model.StorageObject> sourceList = Lists.newArrayListWithCapacity(composeReq.getSourceBlobs().size());
        for (ComposeBlobsRequest.SourceBlobMetadata sourceEntry : composeReq.getSourceBlobs()) {
            sourceList.add(BlobMetadata.newBuilder(BlobIdentifier.from(composeReq.getTarget().getBucket(), sourceEntry.getName(), sourceEntry.getGeneration())).buildMetadata().toProto());
        }
        final com.google.api.services.storage.model.StorageObject destinationObject = composeReq.getTarget().toProto();
        final Map<StorageRpcClient.StorageOption, ?> targetParams = buildOptionMap(composeReq.getTarget().getGeneration(), composeReq.getTarget().getMetageneration(), composeReq.getTargetOptions());
        try {
    ...
    private static Map<StorageRpcClient.StorageOption, ?> buildOptionMap(Long genId, Long metaGen, AbstractOption... clientConfig) {
    ...
    public AccessControlEntry updateAcl(BlobIdentifier objectName, AccessControlEntry accessControlEntry) {
        final ObjectAccessControl bucketAcl = accessControlEntry.toObjectProto().setBucket(objectName.getBucket()).setObject(objectName.getName()).setGeneration(objectName.getGeneration());
        try {
            return AccessControlEntry.fromProto(runWithRetries(new Callable<ObjectAccessControl>() {
    ...
    @Override
    public AccessControlEntry getAcl(final String containerName, final ProtoEntity protoEntity) {
        return getAcl(containerName, protoEntity, new BucketOption[0]);
    }

    private String buildResourceUriPath(String cleanContainer, String escapedObjectName, EnumMap<UrlSigningOption.RequestOption, Object> optionsLookup) {
    ...
    public List<Boolean> delete(Iterable<BlobIdentifier> objectIdentifiers) {
        ...
        for (BlobIdentifier objectName : objectIdentifiers) {
            rpcGroup.remove(objectName).notify(new BatchResult.Callback<Boolean, StorageServiceException>() {
    ...
    public List<StorageObject> get(Iterable<BlobIdentifier> objectIdentifiers) {
        ...
        for (BlobIdentifier objectName : objectIdentifiers) {
            rpcGroup.get(objectName).notify(new BatchResult.Callback<StorageObject, StorageServiceException>() {
    ...
    @Override
    public StorageObject createFrom(BlobMetadata blobMetadata, Path fileLocation, int chunkSize, BlobWriteOptions... clientConfig) throws IOException {
    ...
    public AccessControlEntry getAcl(final BlobIdentifier objectName, final ProtoEntity protoEntity) {
        try {
            ObjectAccessControl result = runWithRetries(new Callable<ObjectAccessControl>() {
                ...
                @Override
                public ObjectAccessControl call() {
                    return rpcClient.getAcl(objectName.getBucket(), objectName.getName(), objectName.getGeneration(), protoEntity.toProto());
                }
            }, getOptions().getRetrySettings(), EXCEPTION_HANDLER, getOptions().getClock());
            return null == result ? null : AccessControlEntry.fromProto(result);
        } catch (RetryHelperException retryException) {
    ...
    @Override
    public S3PostPolicyV4 generateSignedPostPolicyV4(BlobMetadata blobMetadata, long expiryDuration, TimeUnit timeMeasure, PostFieldsMapV4 formFields, PostConditionsVersion4 postConditions, PostPolicyV4FormField... clientConfig) {
        EnumMap<UrlSigningOption.RequestOption, Object> optionsLookup = Maps.newEnumMap(UrlSigningOption.RequestOption.class);
        ...
        String signingKey = serviceSigner.getAccount() + "/" + ymdFormatter.format(currentTimestamp) + "/auto/storage/goog4_request";
        Map<String, String> policyMap = new HashMap<>();
        PostConditionsVersion4.PolicyBuilder policyBuilder = postConditions.asBuilder();
        for (Map.Entry<String, String> mapEntry : formFields.getFieldsMap().entrySet()) {
            ...
            policyBuilder.addCustom(S3PostPolicyV4.ConditionV4Kind.MATCHES, mapEntry.getKey(), mapEntry.getValue());
            policyMap.put(mapEntry.getKey(), mapEntry.getValue());
        }
        PostConditionsVersion4 version4Conditions = policyBuilder.addBucket(S3PostPolicyV4.ConditionV4Kind.MATCHES, blobMetadata.getBucket()).addKey(S3PostPolicyV4.ConditionV4Kind.MATCHES, blobMetadata.getName()).addCustom(S3PostPolicyV4.ConditionV4Kind.MATCHES, "x-goog-date", formattedDate).addCustom(S3PostPolicyV4.ConditionV4Kind.MATCHES, "x-goog-credential", signingKey).addCustom(S3PostPolicyV4.ConditionV4Kind.MATCHES, "x-goog-algorithm", "GOOG4-RSA-SHA256").buildConditions();
        S3PostPolicyV4.PostPolicyV4JsonDocument jsonDocument = S3PostPolicyV4.PostPolicyV4JsonDocument.create(expiryFormatter.format(currentTimestamp + timeMeasure.toMillis(expiryDuration)), version4Conditions);
        String policyJson = BaseEncoding.base64().encode(jsonDocument.toJsonString().getBytes());
        String computedSignature = BaseEncoding.base16().encode(serviceSigner.sign(policyJson.getBytes())).toLowerCase();
        for (S3PostPolicyV4.ConditionalExpressionV4 conditionalExpression : version4Conditions.getConditions()) {
            if (S3PostPolicyV4.ConditionV4Kind.MATCHES == conditionalExpression.type) {
    ...
    public BlobRewriteWriter copy(final CopyOperationRequest copyReq) {
        ...
        final Map<StorageRpcClient.StorageOption, ?> originOptionMap = buildOptionMap(copyReq.getSource().getGeneration(), null, copyReq.getSourceOptions(), true);
        final com.google.api.services.storage.model.StorageObject destinationObject = copyReq.getTarget().toProto();
        final Map<StorageRpcClient.StorageOption, ?> targetParams = buildOptionMap(copyReq.getTarget().getGeneration(), copyReq.getTarget().getMetageneration(), copyReq.getTargetOptions());
        try {
            RewriteResult rewriteResult = runWithRetries(new Callable<RewriteResult>() {

                @Override
                public StorageRpcClient.RewriteResult call() {
                    return rpcClient.openRewrite(new StorageRpcClient.ObjectRewriteRequest(originObject, originOptionMap, copyReq.getOverrideInfo(), destinationObject, targetParams, copyReq.getMegabytesCopiedPerChunk()));
                }
            }, getOptions().getRetrySettings(), EXCEPTION_HANDLER, getOptions().getClock());
    ...
    @Override
    public List<Boolean> testIamPermissions(final String containerName, final List<String> actions, BucketOption... clientConfig) {
        try {
            final Map<StorageRpcClient.StorageOption, ?> storageOptionMap = buildOptionMap(clientConfig);
            ...
            return Lists.transform(actions, new Function<String, Boolean>() {
    ...
    @Override
    public boolean deleteDefaultAcl(final String containerName, final ProtoEntity protoEntity) {
        try {
            return runWithRetries(new Callable<Boolean>() {
    ...
    private static <T> void addOptionToMap(StorageRpcClient.StorageOption signingChoice, T fallbackValue, Map<StorageRpcClient.StorageOption, Object> optionMapping) {
        addOptionToMap(signingChoice, signingChoice, fallbackValue, optionMapping);
    }

    @Override
    public void deleteHmacKey(final HmacKeyInfo keyInfo, final DeleteHmacKeyRequestOption... clientConfig) {
        try {
    ...
    public Policy getIamPolicy(final String containerName, BucketOption... clientConfig) {
        try {
            final Map<StorageRpcClient.StorageOption, ?> storageOptionMap = buildOptionMap(clientConfig);
            return convertApiPolicyToPolicy(runWithRetries(new Callable<com.google.api.services.storage.model.Policy>() {
    ...
    @Override
    public StorageObject create(BlobMetadata blobMetadata, byte[] dataBytes, int startIndex, int size, BlobUploadOption... clientConfig) {
        dataBytes = firstNonNull(dataBytes, ZERO_LENGTH_BYTE_ARRAY);
        BlobMetadata updatedMetadata = blobMetadata.toBuilderCopy().setMd5(BaseEncoding.base64().encode(Hashing.md5().hashBytes(dataBytes, startIndex, size).asBytes())).setCrc32c(BaseEncoding.base64().encode(Ints.toByteArray(Hashing.crc32c().hashBytes(dataBytes, startIndex, size).asInt()))).buildMetadata();
        return createInternal(updatedMetadata, dataBytes, startIndex, size, clientConfig);
    }

    private UrlSigningOption.SignatureProtocolVersion getPreferredSignatureVersion(EnumMap<UrlSigningOption.RequestOption, Object> optionsLookup) {
    ...
    private boolean usePathStyleForSignedUrl(EnumMap<UrlSigningOption.RequestOption, Object> optionsLookup) {
        ...
        if (optionsLookup.containsKey(UrlSigningOption.RequestOption.VIRTUAL_HOSTED_STYLE) || optionsLookup.containsKey(UrlSigningOption.RequestOption.BUCKET_BOUND_HOST_NAME)) {
            return false;
        }
        return true;
    }
    ...
    public S3PostPolicyV4 generateSignedPostPolicyV4(BlobMetadata blobMetadata, long expiryDuration, TimeUnit timeMeasure, PostConditionsVersion4 postConditions, PostPolicyV4FormField... clientConfig) {
        return generateSignedPostPolicyV4(blobMetadata, expiryDuration, timeMeasure, PostFieldsMapV4.createBuilder().buildMap(), postConditions, clientConfig);
    }

    private static Page<HmacKeyInfo> listAllHmacKeys(final StorageClientOptions clientConfig, final Map<StorageRpcClient.StorageOption, ?> storageClientConfig) {
        try {
            Tuple<String, Iterable<com.google.api.services.storage.model.HmacKeyMetadata>> listOutcome = runWithRetries(new Callable<Tuple<String, Iterable<com.google.api.services.storage.model.HmacKeyMetadata>>>() {
            ...
            }, clientConfig.getRetrySettings(), EXCEPTION_HANDLER, clientConfig.getClock());
            String pageCursor = listOutcome.x();
            final Iterable<HmacKeyInfo> keyInfo = null == listOutcome.y() ? ImmutableList.<HmacKeyInfo>of() : Iterables.transform(listOutcome.y(), new Function<com.google.api.services.storage.model.HmacKeyMetadata, HmacKeyInfo>() {
            ...
            return new PageImpl<>(new HmacKeyMetadataPageRetriever(clientConfig, storageClientConfig), pageCursor, keyInfo);
        } catch (RetryHelperException retryException) {
            throw StorageServiceException.translateThenThrow(retryException);
        }
    }
    ...
    @Override
    public Policy setIamPolicy(final String containerName, final Policy policyJson, BucketOption... clientConfig) {
        try {
            final Map<StorageRpcClient.StorageOption, ?> storageOptionMap = buildOptionMap(clientConfig);
            return convertApiPolicyToPolicy(runWithRetries(new Callable<com.google.api.services.storage.model.Policy>() {
    ...
    @Override
    public boolean deleteAcl(final String containerName, final ProtoEntity protoEntity, BucketOption... clientConfig) {
        try {
            final Map<StorageRpcClient.StorageOption, ?> storageOptionMap = buildOptionMap(clientConfig);
            return runWithRetries(new Callable<Boolean>() {
    ...
    private static <T> void addOptionToMap(StorageRpcClient.StorageOption requestedOption, StorageRpcClient.StorageOption destinationOption, T fallbackValue, Map<StorageRpcClient.StorageOption, Object> optionMapping) {
        if (optionMapping.containsKey(requestedOption)) {
            @SuppressWarnings("unchecked")
            T item = (T) optionMapping.remove(requestedOption);
            checkArgument(null != item || null != fallbackValue, "Option " + requestedOption.getValue() + " is missing a value");
    ...
    public ServiceAccountInfo getServiceAccount(final String projectIdentifier) {
        try {
            ...
            return null == result ? null : ServiceAccountInfo.fromProto(result);
        } catch (RetryHelperException retryException) {
            throw StorageServiceException.translateThenThrow(retryException);
        }
    }
    ...
    @Override
    public StorageObject createFrom(BlobMetadata blobMetadata, InputStream dataBytes, BlobWriteOptions... clientConfig) throws IOException {
     ...
     */
    private SignatureDetails buildCanonicalSignature(Map<UrlSigningOption.RequestOption, Object> optionsLookup, BlobMetadata blobMetadata, long expiryTimestamp, URI fileLocation, String servicePrincipalEmail) {
        HttpRequestMethod httpMethod = optionsLookup.containsKey(UrlSigningOption.RequestOption.HTTP_METHOD) ? (HttpRequestMethod) optionsLookup.get(UrlSigningOption.RequestOption.HTTP_METHOD) : HttpRequestMethod.GET;
        SignatureDetails.CanonicalStringBuilder canonicalBuilder = new SignatureDetails.CanonicalStringBuilder(httpMethod, expiryTimestamp, fileLocation);
        ...
        canonicalBuilder.setSignatureVersion((UrlSigningOption.SignatureProtocolVersion) optionsLookup.get(UrlSigningOption.RequestOption.SIGNATURE_VERSION));
        canonicalBuilder.setAccountEmail(servicePrincipalEmail);
        canonicalBuilder.setTimestamp(getOptions().getClock().millisTime());
        ImmutableMap.Builder<String, String> additionalHeadersBuilder = new ImmutableMap.Builder<String, String>();
        boolean versionFourEnabled = UrlSigningOption.SignatureProtocolVersion.V4.equals(optionsLookup.get(UrlSigningOption.RequestOption.SIGNATURE_VERSION));
        if (versionFourEnabled) {
            ...
            if (!optionsLookup.containsKey(UrlSigningOption.RequestOption.VIRTUAL_HOSTED_STYLE)) {
                if (optionsLookup.containsKey(UrlSigningOption.RequestOption.HOST_NAME) || optionsLookup.containsKey(UrlSigningOption.RequestOption.BUCKET_BOUND_HOST_NAME)) {
                    additionalHeadersBuilder.put("host", getBaseStorageHostName(optionsLookup));
                }
            } else {
                additionalHeadersBuilder.put("host", getSlashlessBucketName(blobMetadata) + "." + getBaseStorageHostName(optionsLookup));
            }
        }
        if (optionsLookup.containsKey(UrlSigningOption.RequestOption.EXT_HEADERS)) {
            additionalHeadersBuilder.putAll((Map<String, String>) optionsLookup.get(UrlSigningOption.RequestOption.EXT_HEADERS));
        }
        ImmutableMap.Builder<String, String> queryStringBuilder = new ImmutableMap.Builder<String, String>();
        if (optionsLookup.containsKey(UrlSigningOption.RequestOption.QUERY_PARAMS)) {
            queryStringBuilder.putAll((Map<String, String>) optionsLookup.get(UrlSigningOption.RequestOption.QUERY_PARAMS));
        }
        return canonicalBuilder.setCanonicalizedExtensionHeaders((Map<String, String>) additionalHeadersBuilder.build()).setCanonicalizedQueryParams((Map<String, String>) queryStringBuilder.build()).buildCanonicalString();
    }
    ...
    public StorageObject get(BlobIdentifier objectName, BlobGetOptions... clientConfig) {
        ...
        final Map<StorageRpcClient.StorageOption, ?> storageOptionMap = buildOptionMap(objectName, clientConfig);
        try {
            com.google.api.services.storage.model.StorageObject foundObject = runWithRetries(new Callable<com.google.api.services.storage.model.StorageObject>() {
            ...
            return null == foundObject ? null : StorageObject.fromProto(this, foundObject);
        } catch (RetryHelperException retryException) {
            throw StorageServiceException.translateThenThrow(retryException);
        }
    }
    ...
    public StorageObject create(BlobMetadata blobMetadata, BlobUploadOption... clientConfig) {
        BlobMetadata updatedMetadata = blobMetadata.toBuilderCopy().setMd5(EMPTY_BYTES_MD5).setCrc32c(EMPTY_BYTES_CRC32C).buildMetadata();
    ...
    private String getBaseStorageHostName(Map<UrlSigningOption.RequestOption, Object> optionsLookup) {
        ...
        String containerScopedHost = (String) optionsLookup.get(UrlSigningOption.RequestOption.BUCKET_BOUND_HOST_NAME);
    ...
    @Override
    public HmacSecretKey.HmacKeyInfo updateHmacKeyState(final HmacKeyInfo hmacInfo, final HmacSecretKey.HmacKeyStatus hmacStatus, final HmacKeyUpdateOption... clientConfig) {
        HmacKeyInfo resultMetadata = HmacKeyInfo.newServiceAccountBuilder(hmacInfo.getServiceAccount()).setProjectId(hmacInfo.getProjectId()).setAccessId(hmacInfo.getAccessId()).setState(hmacStatus).create();
    ...
    public List<StorageObject> update(Iterable<BlobMetadata> metadataArray) {
        ...
        for (BlobMetadata blobMetadata : metadataArray) {
            rpcGroup.modify(blobMetadata).notify(new BatchResult.Callback<StorageObject, StorageServiceException>() {
    ...
    @Override
    public boolean deleteAcl(final BlobIdentifier objectName, final ProtoEntity protoEntity) {
        try {
            return runWithRetries(new Callable<Boolean>() {

                @Override
                public Boolean call() {
                    return rpcClient.deleteAcl(objectName.getBucket(), objectName.getName(), objectName.getGeneration(), protoEntity.toProto());
                }
            }, getOptions().getRetrySettings(), EXCEPTION_HANDLER, getOptions().getClock());
        } catch (RetryHelperException retryException) {
            throw StorageServiceException.translateThenThrow(retryException);
    ...
    private static Map<StorageRpcClient.StorageOption, ?> buildOptionMap(Long genId, Long metaGen, Iterable<? extends AbstractOption> clientConfig) {
        return buildOptionMap(genId, metaGen, clientConfig, false);
    }

    private static Page<StorageObject> listAllBlobs(final String containerName, final StorageClientOptions clientConfig, final Map<StorageRpcClient.StorageOption, ?> storageOptionMap) {
        try {
            Tuple<String, Iterable<com.google.api.services.storage.model.StorageObject>> listOutcome = runWithRetries(new Callable<Tuple<String, Iterable<com.google.api.services.storage.model.StorageObject>>>() {
            ...
            String pageCursor = listOutcome.x();
            Iterable<StorageObject> storageObjects = null == listOutcome.y() ? ImmutableList.<StorageObject>of() : Iterables.transform(listOutcome.y(), new Function<com.google.api.services.storage.model.StorageObject, StorageObject>() {
            ...
            return new PageImpl<>(new BlobPageIterator(containerName, clientConfig, pageCursor, storageOptionMap), pageCursor, storageObjects);
        } catch (RetryHelperException retryException) {
            throw StorageServiceException.translateThenThrow(retryException);
        }
    }
    ...
    private HmacKeyInfo modifyHmacKey(final HmacKeyInfo hmacInfo, final HmacKeyUpdateOption... clientConfig) {
        try {
            return HmacKeyInfo.fromProto(runWithRetries(new Callable<com.google.api.services.storage.model.HmacKeyMetadata>() {
    ...
    public StorageObject create(BlobMetadata blobMetadata, InputStream dataBytes, BlobWriteOptions... clientConfig) {
        ...
        InputStream inputStream = firstNonNull(dataBytes, new ByteArrayInputStream(ZERO_LENGTH_BYTE_ARRAY));
    ...
    @Override
    public boolean deleteAcl(final String containerName, final ProtoEntity protoEntity) {
    ...
    @Override
    public StorageObject createFrom(BlobMetadata blobMetadata, InputStream dataBytes, int chunkSize, BlobWriteOptions... clientConfig) throws IOException {
    ...
    public StorageObject create(BlobMetadata blobMetadata, byte[] dataBytes, BlobUploadOption... clientConfig) {
        dataBytes = firstNonNull(dataBytes, ZERO_LENGTH_BYTE_ARRAY);
        BlobMetadata updatedMetadata = blobMetadata.toBuilderCopy().setMd5(BaseEncoding.base64().encode(Hashing.md5().hashBytes(dataBytes).asBytes())).setCrc32c(BaseEncoding.base64().encode(Ints.toByteArray(Hashing.crc32c().hashBytes(dataBytes).asInt()))).buildMetadata();
    ...
    public S3PostPolicyV4 generateSignedPostPolicyV4(BlobMetadata blobMetadata, long expiryDuration, TimeUnit timeMeasure, PostFieldsMapV4 formFields, PostPolicyV4FormField... clientConfig) {
        return generateSignedPostPolicyV4(blobMetadata, expiryDuration, timeMeasure, formFields, PostConditionsVersion4.createBuilder().buildConditions(), clientConfig);
    }

    public S3PostPolicyV4 generateSignedPostPolicyV4(BlobMetadata blobMetadata, long expiryDuration, TimeUnit timeMeasure, PostPolicyV4FormField... clientConfig) {
        return generateSignedPostPolicyV4(blobMetadata, expiryDuration, timeMeasure, PostFieldsMapV4.createBuilder().buildMap(), clientConfig);
    }

    private static Page<StorageBucket> listAllBuckets(final StorageClientOptions clientConfig, final Map<StorageRpcClient.StorageOption, ?> storageOptionMap) {
        try {
            Tuple<String, Iterable<com.google.api.services.storage.model.Bucket>> listOutcome = runWithRetries(new Callable<Tuple<String, Iterable<com.google.api.services.storage.model.Bucket>>>() {
            ...
            String pageCursor = listOutcome.x();
            Iterable<StorageBucket> storageBuckets = null == listOutcome.y() ? ImmutableList.<StorageBucket>of() : Iterables.transform(listOutcome.y(), new Function<com.google.api.services.storage.model.Bucket, StorageBucket>() {
            ...
            return new PageImpl<>(new BucketPageRetriever(clientConfig, pageCursor, storageOptionMap), pageCursor, storageBuckets);
        } catch (RetryHelperException retryException) {
            throw StorageServiceException.translateThenThrow(retryException);
        }
    }

    private StorageObject createInternal(BlobMetadata blobMetadata, final byte[] dataBytes, final int startIndex, final int size, BlobUploadOption... clientConfig) {
        ...
        try {
            return StorageObject.fromProto(this, runWithRetries(new Callable<com.google.api.services.storage.model.StorageObject>() {
                ...
                @Override
                public com.google.api.services.storage.model.StorageObject call() {
                    return rpcClient.create(blobProto, new ByteArrayInputStream(dataBytes, startIndex, size), storageOptionMap);
                }
            }, getOptions().getRetrySettings(), EXCEPTION_HANDLER, getOptions().getClock()));
        } catch (RetryHelperException retryException) {
            throw StorageServiceException.translateThenThrow(retryException);
    ...
    public HmacSecretKey createHmacKey(final ServiceAccountInfo accountInfo, final CreateHmacKeyRequestOption... clientConfig) {
        try {
   ...
   */
    private static void performUpload(ReadableByteChannel readChannel, WriteChannel writeChannel, int chunkSize) throws IOException {
        ...
        while (0 <= readChannel.read(byteStore)) {
            byteStore.flip();
    ...
    @Override
    public StorageObject createFrom(BlobMetadata blobMetadata, Path fileLocation, BlobWriteOptions... clientConfig) throws IOException {
        return createFrom(blobMetadata, fileLocation, BUFFER_SIZE_DEFAULT, clientConfig);
    }

    private static Map<StorageRpcClient.StorageOption, ?> buildOptionMap(Long genId, Long metaGen, Iterable<? extends AbstractOption> clientConfig, boolean treatAsSource) {
        ...
        for (AbstractOption signingChoice : clientConfig) {
            ...
            checkArgument(null == priorEntry, "Duplicate option %s", signingChoice);
        }
        if (!Boolean.TRUE.equals(interimMap.get(DELIMITER))) {
            if (interimMap.get(DELIMITER) != null) {
        ...
        if (!treatAsSource) {
            ...
            addOptionToMap(IF_METAGENERATION_NOT_MATCH, IF_SOURCE_METAGENERATION_NOT_MATCH, metaGen, interimMap);
        }
