...
class ChunkedBlobWriter extends BaseWriteChannel<StorageSettings, BlobMetadata> {
          ...
          callable(
              new Runnable() {
                ...
                public void run() {
                  getOptions()
                      .getStorageRpcV1()
                      .write(getUploadId(), getBuffer(), 0, getPosition(), byteCount, isFinal);
                }
              }),
  ...
  private static String openWriter(
      final StorageSettings storageSettings,
      final BlobMetadata metadata,
      final Map<StorageRpcClient.StorageOption, ?> storageSettingsMap) {
    try {
