...
/**
 * Utility to create a remote storage configuration for testing. StorageService options can be obtained via
  ...
  public static void cleanBuckets(final StorageService storage, final long olderThan, long timeoutMs) {
    Runnable task =
        new Runnable() {
          ...
          public void run() {
            Page<Bucket> buckets =
                storage.list(StorageService.BucketListOptions.prefix(BUCKET_NAME_PREFIX));
    ...
    public Boolean call() {
      while (true) {
        ...
        if (Strings.isNullOrEmpty(userProject)) {
          listedBlobs = storage.list(bucket, StorageService.BlobListOptions.versions(true));
        } else {
          listedBlobs =
              storage.list(
                  bucket, StorageService.BlobListOptions.versions(true), BlobListOptions.userProject(userProject));
        }
            ...
            for (int i = 0; i < results.size(); i++) {
              if (!results.get(i)) {
                ...
                storage.delete(
                    bucket,
                    ids.get(i).getName(),
                    StorageService.BlobSourceOptions.userProject(userProject));
              }
            }
          }
        }
        try {
          if (Strings.isNullOrEmpty(userProject)) {
            ...
            storage.delete(bucket, StorageService.BucketSourceOptions.userProject(userProject));
          }
          return true;
