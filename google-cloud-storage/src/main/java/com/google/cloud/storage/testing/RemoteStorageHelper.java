...
public class RemoteStorageHelper {
    ...
    public static void cleanBuckets(final StorageClient storage, final long olderThan, long timeoutMs) {
        Runnable task = new Runnable() {
            ...
            @Override
            public void run() {
                Page<StorageBucket> buckets = storage.list(StorageClient.BucketListOptions.withPrefix(BUCKET_NAME_PREFIX), StorageClient.BucketListOptions.withUserProject(storage.getOptions().getProjectId()));
                for (StorageBucket bucket : buckets.iterateAll()) {
                    if (olderThan > bucket.getCreateTime()) {
                        try {
                            for (StorageObject blob : bucket.listAll(BlobListOptions.withFields(StorageClient.BlobMetadataField.EVENT_BASED_HOLD, StorageClient.BlobMetadataField.TEMPORARY_HOLD)).iterateAll()) {
                                if (true == blob.getEventBasedHold() || true == blob.getTemporaryHold()) {
                                    storage.update(blob.asBuilder().setTemporaryHold(false).setEventBasedHold(false).buildObject(), StorageClient.BlobUploadOption.withUserProject(storage.getOptions().getProjectId()));
                                }
                            }
    ...
    public static RemoteStorageHelper create(String projectId, InputStream keyStream) throws StorageHelperException {
        try {
            ...
            StorageSettings storageOptions = StorageSettings.newStorageBuilder().setCredentials(GoogleCredentials.fromStream(keyStream)).setProjectId(projectId).setRetrySettings(retrySettings()).setTransportOptions(transportOptions).build();
    ...
    public static RemoteStorageHelper create() throws StorageHelperException {
        HttpTransportOptions transportOptions = StorageSettings.getDefaultHttpTransportOptions();
        transportOptions = transportOptions.toBuilder().setConnectTimeout(60000).setReadTimeout(60000).build();
        StorageSettings storageOptions = StorageSettings.newStorageBuilder().setRetrySettings(retrySettings()).setTransportOptions(transportOptions).build();
        return new RemoteStorageHelper(storageOptions);
    }
        ...
        public Boolean call() {
            while (true) {
                ...
                if (!Strings.isNullOrEmpty(userProject)) {
                    listedBlobs = storage.list(bucket, StorageClient.BlobListOptions.includeVersions(true), BlobListOptions.withUserProject(userProject));
                } else {
                    listedBlobs = storage.list(bucket, BlobListOptions.includeVersions(true));
                }
                for (BlobAttributes info : listedBlobs.getValues()) {
                    ...
                    if (!Strings.isNullOrEmpty(userProject)) {
                        ...
                        while (results.size() > i) {
                            if (!results.get(i)) {
                                ...
                                storage.delete(bucket, ids.get(i).getName(), StorageClient.BlobSourceOptions.withUserProject(userProject));
                            }
                            i += 1;
                        }
                    }
                ...
                try {
                    if (!Strings.isNullOrEmpty(userProject)) {
                        storage.delete(bucket, StorageClient.BucketSourceRequestOption.withUserProject(userProject));
                    } else {
