                  ...
                  for (StorageObject blob :
                    ...
                    if (Boolean.TRUE.equals(blob.getEventBasedHold())
                      ...
                      storage.update(
                          ...
                          Storage.BlobTargetOptions.setUserProject(
    ...
    public Boolean call() {
      while (true) {
        ...
        if (Strings.isNullOrEmpty(userProject)) {
          listedBlobs = storage.list(bucket, ListBlobsOption.includeVersions(true));
        } else {
          listedBlobs =
              storage.list(
                  bucket, ListBlobsOption.includeVersions(true), ListBlobsOption.withUserProject(userProject));
        }
        ...
        try {
          if (Strings.isNullOrEmpty(userProject)) {
            ...
            storage.delete(bucket, Storage.BucketReadOption.withUserProject(userProject));
          }
          return true;
