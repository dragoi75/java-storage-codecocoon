...
/**
 * Utility to create a remote storage configuration for testing. CloudStorage options can be obtained via
...
public class RemoteStorageHelper {
  ...
  public static void cleanBuckets(final CloudStorage storage, final long olderThan, long timeoutMs) {
    Runnable task =
        new Runnable() {
          @Override
          public void run() {
            Page<Bucket> buckets =
                storage.list(CloudStorage.BucketListOptions.withPrefix(BUCKET_NAME_PREFIX));
                      ...
                      bucket
                          .list(
                              BlobListOptions.withFields(
                                  CloudStorage.BlobMetadataField.EVENT_BASED_HOLD,
                                  CloudStorage.BlobMetadataField.TEMPORARY_HOLD))
                          .iterateAll()) {
   ...
   * separate thread to send the delete requests. Use {@link #forceDelete(CloudStorage storage, String
   ...
   */
  public static Boolean forceDelete(CloudStorage storage, String bucket, long timeout, TimeUnit unit)
  ...
  public static Boolean forceDelete(
          CloudStorage storage, String bucket, long timeout, TimeUnit unit, String userProject)
      throws InterruptedException, ExecutionException {
   ...
   */
  public static void forceDelete(CloudStorage storage, String bucket) {
  ...
  private static class DeleteBucketTask implements Callable<Boolean> {
    ...
    public DeleteBucketTask(CloudStorage storage, String bucket) {
      this.storage = storage;
      this.bucket = bucket;
      this.userProject = "";
    }

    public DeleteBucketTask(CloudStorage storage, String bucket, String userProject) {
      this.storage = storage;
      this.bucket = bucket;
    ...
    public Boolean call() {
      while (true) {
        ...
        if (Strings.isNullOrEmpty(userProject)) {
          ...
          listedBlobs =
              storage.list(
                  bucket, BlobListOptions.includeVersions(true), BlobListOptions.withUserProject(userProject));
        }
            ...
            for (int i = 0; i < results.size(); i++) {
              if (!results.get(i)) {
                ...
                storage.delete(
                    bucket,
                    ids.get(i).getName(),
                    CloudStorage.BlobSourceOptions.withUserProject(userProject));
              }
            }
          }
        }
        try {
          if (Strings.isNullOrEmpty(userProject)) {
            ...
            storage.delete(bucket, CloudStorage.BucketSourceOptions.withUserProject(userProject));
          }
          return true;
