...
public class RemoteStorageHelper {
    ...
    private static class DeleteBucketTask implements Callable<Boolean> {
        ...
        public Boolean call() {
            while (true) {
                ...
                try {
                    if (!Strings.isNullOrEmpty(userProject)) {
                        storage.delete(bucket, CloudStorageClient.BucketOption.userProjectId(userProject));
                    } else {
    ...
    public static RemoteStorageHelper create() throws StorageHelperException {
        ...
        StorageClientOptions storageOptions = StorageClientOptions.newServiceClientBuilder().setRetrySettings(retrySettings()).setTransportOptions(transportOptions).build();
            ...
            public void run() {
                ...
                for (StorageBucket bucket : buckets.iterateAll()) {
                    if (olderThan > bucket.getCreateTime()) {
                        try {
                            for (StorageObject blob : bucket.listAll(BlobListOptions.withFields(CloudStorageClient.BlobMetadataField.EVENT_BASED_HOLD, CloudStorageClient.BlobMetadataField.TEMPORARY_HOLD)).iterateAll()) {
                                if (Boolean.TRUE.equals(blob.getEventBasedHold()) || Boolean.TRUE.equals(blob.getTemporaryHold())) {
                                    storage.update(blob.toBuilderCopy().setTemporaryHold(false).setEventBasedHold(false).buildMetadata(), CloudStorageClient.BlobUploadOption.userProjectId(storage.getOptions().getProjectId()));
                                }
                            }
    ...
    public static RemoteStorageHelper create(String projectId, InputStream keyStream) throws StorageHelperException {
        try {
            HttpTransportOptions transportOptions = StorageClientOptions.getDefaultHttpTransportOptions();
            transportOptions = transportOptions.toBuilder().setConnectTimeout(60000).setReadTimeout(60000).build();
            StorageClientOptions storageOptions = StorageClientOptions.newServiceClientBuilder().setCredentials(GoogleCredentials.fromStream(keyStream)).setProjectId(projectId).setRetrySettings(retrySettings()).setTransportOptions(transportOptions).build();
            return new RemoteStorageHelper(storageOptions);
