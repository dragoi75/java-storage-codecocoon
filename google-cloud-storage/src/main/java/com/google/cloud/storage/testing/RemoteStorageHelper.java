...
public class RemoteStorageHelper {
        ...
        new Runnable() {
          ...
          public void run() {
            Page<StorageBucket> buckets =
                storage.list(
                    CloudStorageClient.BucketListOptions.withPrefix(BUCKET_NAME_PREFIX),
                    CloudStorageClient.BucketListOptions.withUserProject(storage.getOptions().getProjectId()));
                  ...
                  for (StorageBlob blob :
                      bucket
                          .listObjects(
                              CloudStorageClient.BlobListOptions.withFields(
                                  CloudStorageClient.BlobMetadataField.EVENT_BASED_HOLD,
                                  CloudStorageClient.BlobMetadataField.TEMPORARY_HOLD))
                          .iterateAll()) {
                    if (Boolean.TRUE.equals(blob.getEventBasedHold())
                      ...
                      storage.update(
                          ...
                          CloudStorageClient.BlobUploadOptions.withUserProject(
   ...
   */
  public static Boolean forceDelete(CloudStorageClient storage, String bucket, long timeout, TimeUnit unit)
  ...
  public static Boolean forceDelete(
          CloudStorageClient storage, String bucket, long timeout, TimeUnit unit, String userProject)
      throws InterruptedException, ExecutionException {
    ...
    public Boolean call() {
      while (true) {
        ...
        if (Strings.isNullOrEmpty(userProject)) {
          ...
          listedBlobs =
              storage.list(
                  bucket, CloudStorageClient.BlobListOptions.withVersions(true), BlobListOptions.withUserProject(userProject));
        }
            ...
            for (int i = 0; i < results.size(); i++) {
              if (!results.get(i)) {
                ...
                storage.delete(
                    bucket,
                    ids.get(i).getName(),
                    CloudStorageClient.BlobReadOption.withUserProject(userProject));
              }
            }
          }
        }
