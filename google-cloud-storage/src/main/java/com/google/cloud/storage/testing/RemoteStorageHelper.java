...
public class RemoteStorageHelper {
        ...
        new Runnable() {
          ...
          public void run() {
            Page<StorageBucket> buckets =
                storage.list(
                    StorageClient.BucketListOptions.namePrefix(BUCKET_NAME_PREFIX),
                    StorageClient.BucketListOptions.userProjectId(storage.getOptions().getProjectId()));
              ...
              if (bucket.getCreateTime() < olderThan) {
                try {
                  for (StorageBlob blob :
                      bucket
                          .listBlobs(
                              BlobListOptions.selectFields(
                                  StorageClient.BlobMetadataField.EVENT_BASED_HOLD,
                                  StorageClient.BlobMetadataField.TEMPORARY_HOLD))
                          .iterateAll()) {
                    if (blob.getEventBasedHold() == true || blob.getTemporaryHold() == true) {
                      storage.update(
                          blob.toBlobInfoBuilder().setTemporaryHold(false).setEventBasedHold(false).buildStorageObject(),
                          StorageClient.BlobUploadOption.withUserProject(
   ...
   */
  public static Boolean forceDelete(StorageClient storage, String bucket, long timeout, TimeUnit unit)
  ...
  public static Boolean forceDelete(
          StorageClient storage, String bucket, long timeout, TimeUnit unit, String userProject)
      throws InterruptedException, ExecutionException {
  ...
  public static RemoteStorageHelper create(String projectId, InputStream keyStream)
    ...
    try {
      ...
      StorageSettings storageOptions =
    ...
    public Boolean call() {
      while (true) {
        ...
        if (Strings.isNullOrEmpty(userProject)) {
          ...
          listedBlobs =
              storage.list(
                  bucket, BlobListOptions.includeVersions(true), StorageClient.BlobListOptions.userProjectId(userProject));
        }
            ...
            for (int i = 0; i < results.size(); i++) {
              if (!results.get(i)) {
                ...
                storage.delete(
                    bucket,
                    ids.get(i).getName(),
                    StorageClient.BlobSourceOptions.userProjectId(userProject));
              }
            }
          }
        }
