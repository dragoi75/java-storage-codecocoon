...
public class RemoteStorageHelper {
                        ...
                        try {
                            for (StorageObject blob : bucket.listObjects(BlobListOptions.withFields(Storage.BlobMetadataField.EVENT_BASED_HOLD, Storage.BlobMetadataField.TEMPORARY_HOLD)).iterateAll()) {
                                if (true == blob.getEventBasedHold() || true == blob.getTemporaryHold()) {
                                    storage.update(blob.asBuilder().setTemporaryHold(false).setEventBasedHold(false).buildMetadata(), Storage.BlobUploadOption.withUserProject(storage.getOptions().getProjectId()));
                                }
                            }
    ...
    public static RemoteStorageHelper create(String projectId, InputStream keyStream) throws StorageHelperException {
        try {
            ...
            StorageSettings storageOptions = StorageSettings.createBuilder().setCredentials(GoogleCredentials.fromStream(keyStream)).setProjectId(projectId).setRetrySettings(retrySettings()).setTransportOptions(transportOptions).build();
    ...
    public static RemoteStorageHelper create() throws StorageHelperException {
        HttpTransportOptions transportOptions = StorageSettings.getDefaultHttpTransportOptions();
        transportOptions = transportOptions.toBuilder().setConnectTimeout(60000).setReadTimeout(60000).build();
        StorageSettings storageOptions = StorageSettings.createBuilder().setRetrySettings(retrySettings()).setTransportOptions(transportOptions).build();
        return new RemoteStorageHelper(storageOptions);
    }
    ...
    private static class DeleteBucketTask implements Callable<Boolean> {
        ...
        public Boolean call() {
            while (true) {
                    ...
                    if (!Strings.isNullOrEmpty(userProject)) {
                        ...
                        while (results.size() > i) {
                            if (!results.get(i)) {
                                ...
                                storage.delete(bucket, ids.get(i).getName(), Storage.BlobSourceSettings.withUserProject(userProject));
                            }
                            i += 1;
                        }
                    }
                ...
                try {
                    if (!Strings.isNullOrEmpty(userProject)) {
                        storage.delete(bucket, Storage.BucketSourceRequestOption.withUserProject(userProject));
                    } else {
