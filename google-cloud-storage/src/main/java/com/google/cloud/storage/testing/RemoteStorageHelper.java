        ...
        new Runnable() {
          ...
          public void run() {
            Page<StorageBucket> buckets =
                storage.list(
                    StorageClient.BucketListOptions.withPrefix(BUCKET_NAME_PREFIX),
                    StorageClient.BucketListOptions.withUserProject(storage.getOptions().getProjectId()));
                ...
                try {
                  for (StorageObject blob :
                    ...
                    if (blob.getEventBasedHold() == true || blob.getTemporaryHold() == true) {
                      storage.update(
                          blob.asBuilder().setTemporaryHold(false).setEventBasedHold(false).buildObject(),
    ...
    public Boolean call() {
      while (true) {
        ...
        if (Strings.isNullOrEmpty(userProject)) {
          ...
          listedBlobs =
              storage.list(
                  bucket, StorageClient.BlobListOptions.includeVersions(true), BlobListOptions.withUserProject(userProject));
        }
            ...
            for (int i = 0; i < results.size(); i++) {
              if (!results.get(i)) {
                ...
                storage.delete(
                    bucket,
                    ids.get(i).getName(),
                    StorageClient.BlobSourceOptions.withUserProject(userProject));
              }
            }
          }
        }
