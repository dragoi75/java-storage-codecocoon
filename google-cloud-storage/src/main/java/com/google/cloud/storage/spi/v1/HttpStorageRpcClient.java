...
public class HttpStorageRpcClient implements StorageRpcClient {
    ...
    public HttpStorageRpcClient(StorageSettings storageSettings) {
        ...
        storageService = new Storage.Builder(httpTransport, new JacksonFactory(), requestInit).setRootUrl(storageSettings.getHost()).setApplicationName(storageSettings.getApplicationName()).build();
    }
    ...
    private class DefaultRpcBatchExecutor implements RpcBatchRequest {
        ...
        @Override
        public void addDeleteOperation(StorageObject targetObject, CompletionHandler<Void> completionHandler, Map<StorageOption, ?> storageSettings) {
            try {
                if (BATCH_SIZE_LIMIT == batchSizeCurrent) {
        ...
        @Override
        public void addPatchOperation(StorageObject targetObject, CompletionHandler<StorageObject> completionHandler, Map<StorageOption, ?> storageSettings) {
            try {
                if (BATCH_SIZE_LIMIT == batchSizeCurrent) {
                    batchQueue.add(storageService.batch());
                    batchSizeCurrent = 0;
        ...
        @Override
        public void addGetRequest(StorageObject targetObject, CompletionHandler<StorageObject> completionHandler, Map<StorageOption, ?> storageSettings) {
            try {
                if (BATCH_SIZE_LIMIT == batchSizeCurrent) {
                    batchQueue.add(storageService.batch());
                    batchSizeCurrent = 0;
        ...
        public void submitBatch() {
            ...
            try {
                ...
                for (BatchRequest batchRequest : batchQueue) {
                    ...
                    batchRequest.setBatchUrl(new GenericUrl(String.format("%s/batch/storage/v1", storageSettings.getHost())));
    ...
    private static void setEncryptionHeaders(HttpHeaders httpMetadata, String hdrPrefix, Map<StorageOption, ?> storageSettings) {
        ...
        if (null != secretValue) {
            ...
            httpMetadata.set(hdrPrefix + "key-sha256", baseEncoder.encode(digestFn.hashBytes(baseEncoder.decode(secretValue)).asBytes()));
        }
    }
    ...
    private Span createSpan(String operationName) {
        return requestTracer.spanBuilder(operationName).setRecordEvents(httpCensusModule.isRecordEvents()).startSpan();
    }
    ...
    public Bucket create(Bucket container, Map<StorageOption, ?> storageSettings) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            return storageService.buckets().insert(this.storageSettings.getProjectId(), container).setProjection(DEFAULT_PROJECTION).setPredefinedAcl(StorageOption.PREDEFINED_ACL.getString(storageSettings)).setPredefinedDefaultObjectAcl(StorageOption.PREDEFINED_DEFAULT_OBJECT_ACL.getString(storageSettings)).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    @Override
    public StorageObject create(StorageObject targetObject, final InputStream dataStream, Map<StorageOption, ?> storageSettings) {
        Span batchSpan = createSpan(HttpStorageRpcSpans.SPAN_NAME_CREATE_OBJECT);
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            Storage.Objects.Insert uploadRequest = storageService.objects().insert(targetObject.getBucket(), targetObject, new InputStreamContent(targetObject.getContentType(), dataStream));
    ...
    private Storage.Objects.Patch createPatchCall(StorageObject targetObject, Map<StorageOption, ?> storageSettings) throws IOException {
        return storageService.objects().patch(targetObject.getBucket(), targetObject.getName(), targetObject).setProjection(DEFAULT_PROJECTION).setPredefinedAcl(StorageOption.PREDEFINED_ACL.getString(storageSettings)).setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(storageSettings)).setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(storageSettings)).setIfGenerationMatch(StorageOption.IF_GENERATION_MATCH.getLong(storageSettings)).setIfGenerationNotMatch(StorageOption.IF_GENERATION_NOT_MATCH.getLong(storageSettings)).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings));
    }
    ...
    public boolean delete(Bucket container, Map<StorageOption, ?> storageSettings) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            storageService.buckets().delete(container.getName()).setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(storageSettings)).setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(storageSettings)).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings)).execute();
            return true;
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            StorageServiceException storageServiceError = translateException(ioException);
    ...
    private Storage.Objects.Delete createDeleteCall(StorageObject storageItem, Map<StorageOption, ?> storageSettings) throws IOException {
        return storageService.objects().delete(storageItem.getBucket(), storageItem.getName()).setGeneration(storageItem.getGeneration()).setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(storageSettings)).setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(storageSettings)).setIfGenerationMatch(StorageOption.IF_GENERATION_MATCH.getLong(storageSettings)).setIfGenerationNotMatch(StorageOption.IF_GENERATION_NOT_MATCH.getLong(storageSettings)).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings));
    }
    ...
    @Override
    public StorageObject compose(Iterable<StorageObject> sourceItems, StorageObject destination, Map<StorageOption, ?> targetOptionMap) {
        ...
        for (StorageObject storageItem : sourceItems) {
            ...
            if (null != genNumber) {
                ...
                objectEntry.setObjectPreconditions(new ObjectPreconditions().setIfGenerationMatch(genNumber));
            }
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            return storageService.objects().compose(destination.getBucket(), destination.getName(), composeOperation).setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(targetOptionMap)).setIfGenerationMatch(StorageOption.IF_GENERATION_MATCH.getLong(targetOptionMap)).setUserProject(StorageOption.USER_PROJECT.getString(targetOptionMap)).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    public byte[] load(StorageObject sourceRef, Map<StorageOption, ?> storageSettings) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            Storage.Objects.Get getOperation = storageService.objects().get(sourceRef.getBucket(), sourceRef.getName()).setGeneration(sourceRef.getGeneration()).setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(storageSettings)).setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(storageSettings)).setIfGenerationMatch(StorageOption.IF_GENERATION_MATCH.getLong(storageSettings)).setIfGenerationNotMatch(StorageOption.IF_GENERATION_NOT_MATCH.getLong(storageSettings)).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings));
    ...
    private Get buildReadRequest(StorageObject sourceRef, Map<StorageOption, ?> storageSettings) throws IOException {
        Get getOperation = storageService.objects().get(sourceRef.getBucket(), sourceRef.getName()).setGeneration(sourceRef.getGeneration()).setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(storageSettings)).setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(storageSettings)).setIfGenerationMatch(StorageOption.IF_GENERATION_MATCH.getLong(storageSettings)).setIfGenerationNotMatch(StorageOption.IF_GENERATION_NOT_MATCH.getLong(storageSettings)).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings));
        setEncryptionHeaders(getOperation.getRequestHeaders(), ENCRYPTION_PREFIX, storageSettings);
    ...
    @Override
    public long read(StorageObject sourceRef, Map<StorageOption, ?> storageSettings, long offset, OutputStream destStream) {
        ...
        try {
            ...
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            StorageServiceException storageServiceError = translateException(ioException);
            if (HTTP_SC_RANGE_NOT_SATISFIABLE == storageServiceError.getCode()) {
                return 0;
            }
            throw storageServiceError;
        } finally {
    ...
    @Override
    public Tuple<String, byte[]> read(StorageObject sourceRef, Map<StorageOption, ?> storageSettings, long offset, int byteCount) {
        Span batchSpan = createSpan(HttpStorageRpcSpans.SPAN_NAME_READ);
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
    ...
    @Override
    public void write(String uploadIdentifier, byte[] writeBuffer, int bufferOffset, long destinationOffset, int size, boolean isFinal) {
        ...
        try {
            ...
            HttpRequest request = storageService.getRequestFactory().buildPutRequest(targetUrl, new ByteArrayContent(null, writeBuffer, bufferOffset, size));
    ...
    public String open(StorageObject storageItem, Map<StorageOption, ?> storageSettings) {
        ...
        try {
            ...
            serverPort = 0 < serverPort ? serverPort : targetUrl.toURL().getDefaultPort();
            ...
            HttpRequest request = httpRequestFactory.buildPostRequest(targetUrl, new JsonHttpContent(jsonParserFactory, storageItem));
            HttpHeaders headersCollection = request.getHeaders();
            headersCollection.set("X-Upload-Content-Type", firstNonNull(storageItem.getContentType(), "application/octet-stream"));
            String secretValue = StorageOption.CUSTOMER_SUPPLIED_KEY.getString(storageSettings);
            if (null != secretValue) {
                BaseEncoding baseEncoder = BaseEncoding.base64();
                HashFunction digestFn = Hashing.sha256();
                ...
                headersCollection.set("x-goog-encryption-key-sha256", baseEncoder.encode(digestFn.hashBytes(baseEncoder.decode(secretValue)).asBytes()));
            }
    ...
    public String open(String presignedLink) {
        ...
        try {
            ...
            HttpRequestFactory httpRequestFactory = storageService.getRequestFactory();
            HttpRequest request = httpRequestFactory.buildPostRequest(targetUrl, new ByteArrayContent("", rawBytes, 0, rawBytes.length));
            HttpHeaders headersCollection = request.getHeaders();
    ...
    private RewriteOperationResult performRewrite(RewriteOperationRequest getOperation, String authKey) {
        try {
            ...
            return new RewriteOperationResult(getOperation, operationResult.getResource(), operationResult.getObjectSize().longValue(), operationResult.getDone(), operationResult.getRewriteToken(), operationResult.getTotalBytesRewritten().longValue());
        } catch (IOException ioException) {
    ...
    public BucketAccessControl getAcl(String container, String principalId, Map<StorageOption, ?> storageSettings) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            return storageService.bucketAccessControls().get(container, principalId).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings)).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            StorageServiceException storageServiceError = translateException(ioException);
            if (HTTP_NOT_FOUND == storageServiceError.getCode()) {
    ...
    public boolean deleteAcl(String container, String principalId, Map<StorageOption, ?> storageSettings) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            storageService.bucketAccessControls().delete(container, principalId).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings)).execute();
            return true;
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            StorageServiceException storageServiceError = translateException(ioException);
    ...
    public BucketAccessControl createAcl(BucketAccessControl bucketAccessControl, Map<StorageOption, ?> storageSettings) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            return storageService.bucketAccessControls().insert(bucketAccessControl.getBucket(), bucketAccessControl).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings)).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    public BucketAccessControl patchAcl(BucketAccessControl bucketAccessControl, Map<StorageOption, ?> storageSettings) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            return storageService.bucketAccessControls().patch(bucketAccessControl.getBucket(), bucketAccessControl.getEntity(), bucketAccessControl).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings)).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    public List<BucketAccessControl> listAcls(String container, Map<StorageOption, ?> storageSettings) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            return storageService.bucketAccessControls().list(container).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings)).execute().getItems();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    public ObjectAccessControl patchDefaultAcl(ObjectAccessControl bucketAccessControl) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            return storageService.defaultObjectAccessControls().patch(bucketAccessControl.getBucket(), bucketAccessControl.getEntity(), bucketAccessControl).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    public ObjectAccessControl getAcl(String container, String storageItem, Long genNumber, String principalId) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            return storageService.objectAccessControls().get(container, storageItem, principalId).setGeneration(genNumber).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            StorageServiceException storageServiceError = translateException(ioException);
            if (HTTP_NOT_FOUND == storageServiceError.getCode()) {
    ...
    public boolean deleteAcl(String container, String storageItem, Long genNumber, String principalId) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            storageService.objectAccessControls().delete(container, storageItem, principalId).setGeneration(genNumber).execute();
            return true;
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            StorageServiceException storageServiceError = translateException(ioException);
    ...
    public ObjectAccessControl createAcl(ObjectAccessControl bucketAccessControl) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            return storageService.objectAccessControls().insert(bucketAccessControl.getBucket(), bucketAccessControl.getObject(), bucketAccessControl).setGeneration(bucketAccessControl.getGeneration()).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    public ObjectAccessControl patchAcl(ObjectAccessControl bucketAccessControl) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            return storageService.objectAccessControls().patch(bucketAccessControl.getBucket(), bucketAccessControl.getObject(), bucketAccessControl.getEntity(), bucketAccessControl).setGeneration(bucketAccessControl.getGeneration()).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    public List<ObjectAccessControl> listAcls(String container, String storageItem, Long genNumber) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            return storageService.objectAccessControls().list(container, storageItem).setGeneration(genNumber).execute().getItems();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    public HmacKey createHmacKey(String accountEmail, Map<StorageOption, ?> storageSettings) {
        ...
        try {
            return storageService.projects().hmacKeys().create(projectIdentifier, accountEmail).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings)).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    public Tuple<String, Iterable<HmacKeyMetadata>> listHmacKeys(Map<StorageOption, ?> storageSettings) {
        ...
        try {
            HmacKeysMetadata credentialsMetadata = storageService.projects().hmacKeys().list(projectIdentifier).setServiceAccountEmail(StorageOption.SERVICE_ACCOUNT_EMAIL.getString(storageSettings)).setPageToken(StorageOption.PAGE_TOKEN.getString(storageSettings)).setMaxResults(StorageOption.MAX_RESULTS.getLong(storageSettings)).setShowDeletedKeys(StorageOption.SHOW_DELETED_KEYS.getBoolean(storageSettings)).execute();
            return Tuple.<String, Iterable<HmacKeyMetadata>>of(credentialsMetadata.getNextPageToken(), credentialsMetadata.getItems());
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    public HmacKeyMetadata getHmacKey(String credentialKey, Map<StorageOption, ?> storageSettings) {
        ...
        try {
            return storageService.projects().hmacKeys().get(projectIdentifier, credentialKey).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings)).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    public HmacKeyMetadata updateHmacKey(HmacKeyMetadata keyMetadata, Map<StorageOption, ?> storageSettings) {
        ...
        try {
            return storageService.projects().hmacKeys().update(projectIdentifier, keyMetadata.getAccessId(), keyMetadata).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings)).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    public void deleteHmacKey(HmacKeyMetadata keyMetadata, Map<StorageOption, ?> storageSettings) {
        ...
        try {
            storageService.projects().hmacKeys().delete(projectIdentifier, keyMetadata.getAccessId()).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings)).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    public Policy getIamPolicy(String container, Map<StorageOption, ?> storageSettings) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            Storage.Buckets.GetIamPolicy policyRequest = storageService.buckets().getIamPolicy(container).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings));
            if (StorageOption.REQUESTED_POLICY_VERSION.getLong(storageSettings) != null) {
                policyRequest.setOptionsRequestedPolicyVersion(StorageOption.REQUESTED_POLICY_VERSION.getLong(storageSettings).intValue());
            }
    ...
    public Policy setIamPolicy(String container, Policy accessControl, Map<StorageOption, ?> storageSettings) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            return storageService.buckets().setIamPolicy(container, accessControl).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings)).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    @Override
    public TestIamPermissionsResponse testIamPermissions(String container, List<String> allowedActions, Map<StorageOption, ?> storageSettings) {
        Span batchSpan = createSpan(HttpStorageRpcSpans.SPAN_NAME_TEST_BUCKET_IAM_PERMISSIONS);
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            return storageService.buckets().testIamPermissions(container, allowedActions).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings)).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
    ...
    public Bucket lockRetentionPolicy(Bucket container, Map<StorageOption, ?> storageSettings) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            return storageService.buckets().lockRetentionPolicy(container.getName(), StorageOption.IF_METAGENERATION_MATCH.getLong(storageSettings)).setUserProject(StorageOption.USER_PROJECT.getString(storageSettings)).execute();
        } catch (IOException ioException) {
            batchSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw translateException(ioException);
        } finally {
