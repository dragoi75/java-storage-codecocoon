...
public class HttpStorageRpcClient implements StorageRpcClient {
  ...
  public HttpStorageRpcClient(StorageSettings settings) {
    ...
    storageBackend =
        new Storage.Builder(httpTransport, new JacksonFactory(), requestSetup)
            .setRootUrl(settings.getHost())
  ...
  private class DefaultRpcBatcher implements RpcBatchBuilder {
    ...
    public void addDeletion(
            StorageObject objectToDelete, ResultCallback<Void> resultHandler, Map<StorageOption, ?> settings) {
      try {
        if (currentBatchCount == BATCH_MAX_SIZE) {
    ...
    public void addUpdate(
        StorageObject objectToDelete,
        ResultCallback<StorageObject> resultHandler,
        Map<StorageOption, ?> settings) {
      try {
        if (currentBatchCount == BATCH_MAX_SIZE) {
          batchQueue.add(storageBackend.batch());
          currentBatchCount = 0;
      ...
      try {
        ...
        for (BatchRequest batchRequest : batchQueue) {
          ...
          batchRequest.setBatchUrl(
              new GenericUrl(String.format("%s/batch/storage/v1", settings.getHost())));
  ...
  private static void setEncryptionHeaders(
          HttpHeaders httpFields, String fieldPrefix, Map<StorageOption, ?> settings) {
    ...
    if (secretValue != null) {
      ...
      httpFields.set(
          fieldPrefix + "key-sha256",
          encoder.encode(digestFunction.hashBytes(encoder.decode(secretValue)).asBytes()));
    }
  }
  ...
  public Bucket create(Bucket container, Map<StorageOption, ?> settings) {
    ...
    try {
      return storageBackend
          .buckets()
          .insert(this.settings.getProjectId(), container)
          .setProjection(DEFAULT_PROJECTION)
          .setPredefinedAcl(StorageOption.PREDEFINED_ACL.getString(settings))
          .setPredefinedDefaultObjectAcl(StorageOption.PREDEFINED_DEFAULT_OBJECT_ACL.getString(settings))
          .execute();
    } catch (IOException ioException) {
      currentSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
      throw toStorageException(ioException);
  ...
  public StorageObject create(
          StorageObject objectToDelete, final InputStream inputStream, Map<StorageOption, ?> settings) {
    Span currentSpan = startTraceSpan(HttpStorageRpcSpans.SPAN_NAME_CREATE_OBJECT);
    Scope currentScope = telemetry.withSpan(currentSpan);
    try {
      Storage.Objects.Insert createCall =
          storageBackend
              .objects()
              .insert(
                  objectToDelete.getBucket(),
                      objectToDelete,
                  new InputStreamContent(objectToDelete.getContentType(), inputStream));
      ...
      return createCall
          .setProjection(DEFAULT_PROJECTION)
          .setPredefinedAcl(StorageOption.PREDEFINED_ACL.getString(settings))
          .setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(settings))
          .setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(settings))
          .setIfGenerationMatch(StorageOption.IF_GENERATION_MATCH.getLong(settings))
          .setIfGenerationNotMatch(StorageOption.IF_GENERATION_NOT_MATCH.getLong(settings))
          .setUserProject(StorageOption.USER_PROJECT.getString(settings))
          .setKmsKeyName(StorageOption.KMS_KEY_NAME.getString(settings))
          .execute();
    } catch (IOException ioException) {
      currentSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
      throw toStorageException(ioException);
  ...
  public Tuple<String, Iterable<Bucket>> list(Map<StorageOption, ?> settings) {
    ...
    Scope currentScope = telemetry.withSpan(currentSpan);
    try {
      Buckets containerCollection =
          storageBackend
              .buckets()
              .list(this.settings.getProjectId())
              .setProjection(DEFAULT_PROJECTION)
              .setPrefix(StorageOption.PREFIX.getString(settings))
              .setMaxResults(StorageOption.MAX_RESULTS.getLong(settings))
              .setPageToken(StorageOption.PAGE_TOKEN.getString(settings))
              .setFields(StorageOption.FIELDS.getString(settings))
              .setUserProject(StorageOption.USER_PROJECT.getString(settings))
              .execute();
    ...
    try {
      Objects itemCollection =
          storageBackend
              ...
              .list(container)
              .setProjection(DEFAULT_PROJECTION)
              .setVersions(StorageOption.VERSIONS.getBoolean(settings))
              .setDelimiter(StorageOption.DELIMITER.getString(settings))
              .setPrefix(StorageOption.PREFIX.getString(settings))
              .setMaxResults(StorageOption.MAX_RESULTS.getLong(settings))
              .setPageToken(StorageOption.PAGE_TOKEN.getString(settings))
              .setFields(StorageOption.FIELDS.getString(settings))
              .setUserProject(StorageOption.USER_PROJECT.getString(settings))
              .execute();
      Iterable<StorageObject> storedEntities =
          Iterables.concat(
              firstNonNull(itemCollection.getItems(), ImmutableList.<StorageObject>of()),
              itemCollection.getPrefixes() != null
                  ? Lists.transform(itemCollection.getPrefixes(), storageObjectFromPrefix(container))
  ...
  private Storage.Objects.Get getCall(StorageObject blobItem, Map<StorageOption, ?> settings)
      throws IOException {
    ...
    return fetchRequest.setGeneration(blobItem.getGeneration())
        .setProjection(DEFAULT_PROJECTION)
        .setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(settings))
        .setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(settings))
        .setIfGenerationMatch(StorageOption.IF_GENERATION_MATCH.getLong(settings))
  ...
  public Bucket patch(Bucket container, Map<StorageOption, ?> settings) {
    ...
    try {
      ...
      return storageBackend
          ...
          .setProjection(viewMode == null ? DEFAULT_PROJECTION : viewMode)
          .setPredefinedAcl(StorageOption.PREDEFINED_ACL.getString(settings))
          .setPredefinedDefaultObjectAcl(StorageOption.PREDEFINED_DEFAULT_OBJECT_ACL.getString(settings))
          .setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(settings))
          .setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(settings))
  ...
  private Storage.Objects.Patch buildPatchCall(StorageObject objectToDelete, Map<StorageOption, ?> settings)
      throws IOException {
    return storageBackend
        .objects()
        .patch(objectToDelete.getBucket(), objectToDelete.getName(), objectToDelete)
        .setProjection(DEFAULT_PROJECTION)
        .setPredefinedAcl(StorageOption.PREDEFINED_ACL.getString(settings))
        .setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(settings))
        .setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(settings))
  ...
  private Storage.Objects.Delete buildDeleteCall(StorageObject blobEntry, Map<StorageOption, ?> settings)
      throws IOException {
    return storageBackend
        .objects()
        .delete(blobEntry.getBucket(), blobEntry.getName())
        .setGeneration(blobEntry.getGeneration())
        .setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(settings))
        .setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(settings))
        .setIfGenerationMatch(StorageOption.IF_GENERATION_MATCH.getLong(settings))
        .setIfGenerationNotMatch(StorageOption.IF_GENERATION_NOT_MATCH.getLong(settings))
  ...
  public StorageObject compose(
          Iterable<StorageObject> inputBlobs, StorageObject destinationBlob, Map<StorageOption, ?> destinationParams) {
    ...
    for (StorageObject originObject : inputBlobs) {
      ...
      if (versionNumber != null) {
        ...
        entryItem.setObjectPreconditions(
            new ObjectPreconditions().setIfGenerationMatch(versionNumber));
      }
    ...
    try {
      return storageBackend
          .objects()
          .compose(destinationBlob.getBucket(), destinationBlob.getName(), composePayload)
  ...
  @Override
  public long read(
          StorageObject sourceObject, Map<StorageOption, ?> settings, long offset, OutputStream destinationStream) {
    ...
    try {
      ...
      currentSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
      StorageServiceException storageError = toStorageException(ioException);
      if (storageError.getCode() == RANGE_NOT_SATISFIABLE_STATUS) {
        return 0;
      }
      throw storageError;
    } finally {
  ...
  public Tuple<String, byte[]> read(
          StorageObject sourceObject, Map<StorageOption, ?> settings, long offset, int byteCount) {
    Span currentSpan = startTraceSpan(HttpStorageRpcSpans.SPAN_NAME_READ);
    Scope currentScope = telemetry.withSpan(currentSpan);
    try {
  ...
  public void write(
    ...
    try {
      ...
      HttpRequest preparedRequest =
          storageBackend
              .getRequestFactory()
              .buildPutRequest(requestUrl, new ByteArrayContent(null, writeBuffer, bufferOffset, chunkLength));
  ...
  public String open(StorageObject blobItem, Map<StorageOption, ?> settings) {
    ...
    try {
      ...
      portNumber = portNumber > 0 ? portNumber : requestUrl.toURL().getDefaultPort();
      ...
      HttpRequest preparedRequest =
          httpRequestFactory.buildPostRequest(requestUrl, new JsonHttpContent(jsonParserFactory, blobItem));
      HttpHeaders httpHeaders = preparedRequest.getHeaders();
      httpHeaders.set(
          "X-Upload-Content-Type",
          firstNonNull(blobItem.getContentType(), "application/octet-stream"));
      String secretValue = StorageOption.CUSTOMER_SUPPLIED_KEY.getString(settings);
      if (secretValue != null) {
        BaseEncoding encoder = BaseEncoding.base64();
        HashFunction digestFunction = Hashing.sha256();
    ...
    try {
      ...
      HttpRequest preparedRequest =
          httpRequestFactory.buildPostRequest(
                  requestUrl, new ByteArrayContent("", payloadBytes, 0, payloadBytes.length));
      HttpHeaders httpHeaders = preparedRequest.getHeaders();
  ...
  private RewriteOperationResponse performRewrite(ObjectRewriteRequest getOperation, String authToken) {
    try {
      ...
      Long maxBytesPerCall =
          getOperation.megabytesRewrittenPerCall != null ? getOperation.megabytesRewrittenPerCall * ONE_MEGABYTE : null;
      ...
      return new RewriteOperationResponse(
              getOperation,
  ...
  public ObjectAccessControl getAcl(String container, String blobItem, Long versionNumber, String entityId) {
    ...
    try {
      return storageBackend
          ...
          .get(container, blobItem, entityId)
          .setGeneration(versionNumber)
          .execute();
    } catch (IOException ioException) {
      currentSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
      StorageServiceException storageError = toStorageException(ioException);
  ...
  public boolean deleteAcl(String container, String blobItem, Long versionNumber, String entityId) {
    ...
    try {
      storageBackend
          .objectAccessControls()
          .delete(container, blobItem, entityId)
          .setGeneration(versionNumber)
          .execute();
      return true;
    } catch (IOException ioException) {
    ...
    try {
      HmacKeysMetadata hmacKeysInfo =
          storageBackend
              ...
              .setServiceAccountEmail(StorageOption.SERVICE_ACCOUNT_EMAIL.getString(settings))
              .setPageToken(StorageOption.PAGE_TOKEN.getString(settings))
              .setMaxResults(StorageOption.MAX_RESULTS.getLong(settings))
              .setShowDeletedKeys(StorageOption.SHOW_DELETED_KEYS.getBoolean(settings))
              .execute();
    ...
    try {
      Storage.Buckets.GetIamPolicy iamPolicyRequest =
          storageBackend
              ...
              .setUserProject(StorageOption.USER_PROJECT.getString(settings));
      if (null != StorageOption.REQUESTED_POLICY_VERSION.getLong(settings)) {
        iamPolicyRequest.setOptionsRequestedPolicyVersion(
            StorageOption.REQUESTED_POLICY_VERSION.getLong(settings).intValue());
      }
  ...
  public TestIamPermissionsResponse testIamPermissions(
          String container, List<String> permissionList, Map<StorageOption, ?> settings) {
    Span currentSpan = startTraceSpan(HttpStorageRpcSpans.SPAN_NAME_TEST_BUCKET_IAM_PERMISSIONS);
    Scope currentScope = telemetry.withSpan(currentSpan);
    try {
      return storageBackend
          .buckets()
          .testIamPermissions(container, permissionList)
          .setUserProject(StorageOption.USER_PROJECT.getString(settings))
          .execute();
    } catch (IOException ioException) {
      currentSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
  ...
  public Bucket lockRetentionPolicy(Bucket container, Map<StorageOption, ?> settings) {
    ...
    try {
      return storageBackend
          .buckets()
          .lockRetentionPolicy(container.getName(), StorageOption.IF_METAGENERATION_MATCH.getLong(settings))
          .setUserProject(StorageOption.USER_PROJECT.getString(settings))
          .execute();
    } catch (IOException ioException) {
      currentSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
