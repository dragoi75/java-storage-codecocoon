...
public class HttpStorageRpcClient implements StorageRpcClient {
    ...
    public StorageObject create(StorageObject targetObject, final InputStream dataStream, Map<StorageOption, ?> storageSettings) {
        ...
        Scope spanScope = requestTracer.withSpan(batchSpan);
        try {
            Insert uploadRequest = storageService.objects().insert(targetObject.getBucket(), targetObject, new InputStreamContent(targetObject.getContentType(), dataStream));
    ...
    private Get getCall(StorageObject storageItem, Map<StorageOption, ?> storageSettings) throws IOException {
        Get fetchRequest = storageService.objects().get(storageItem.getBucket(), storageItem.getName());
