...
public class HttpStorageRpcClient implements StorageServiceRpc {
    ...
    @Override
    public StorageObject writeWithResponse(String uploadIdentifier, byte[] sourceBytes, int sourceOffset, long destinationOffset, int byteCount, boolean isFinal) {
        ...
        try {
            ...
            GenericUrl endpoint = new GenericUrl(uploadIdentifier);
            HttpRequest outboundRequest = objectStore.getRequestFactory().buildPutRequest(endpoint, new ByteArrayContent(null, sourceBytes, sourceOffset, byteCount));
    ...
    public String open(StorageObject storedItem, Map<StorageOption, ?> clientConfig) {
        ...
        try {
            ...
            if (null != encryptionKeyName && encryptionKeyName.contains("cryptoKeyVersions")) {
            ...
            HttpRequest outboundRequest = httpReqFactory.buildPostRequest(endpoint, new JsonHttpContent(jsonParserFactory, storedItem));
            ...
            if (null != secretToken) {
                ...
                headersMap.set("x-goog-encryption-key-sha256", binaryEncoder.encode(digestCalculator.hashBytes(binaryEncoder.decode(secretToken)).asBytes()));
            }
    ...
    public String open(String signedUri) {
        ...
        try {
            ...
            HttpRequestFactory httpReqFactory = objectStore.getRequestFactory();
            HttpRequest outboundRequest = httpReqFactory.buildPostRequest(endpoint, new ByteArrayContent("", payloadBytes, 0, payloadBytes.length));
            HttpHeaders headersMap = outboundRequest.getHeaders();
    ...
    private RewriteResult performRewrite(ObjectRewriteRequest getReq, String authKey) {
        try {
            ...
            Long maxBytesPerCall = null != getReq.megabytesRewrittenPerCall ? getReq.megabytesRewrittenPerCall * BYTES_PER_MEGABYTE : null;
            Storage.Objects.Rewrite transformOperation = objectStore.objects().rewrite(getReq.source.getBucket(), getReq.source.getName(), getReq.target.getBucket(), getReq.target.getName(), getReq.overrideInfo ? getReq.target : null).setSourceGeneration(getReq.source.getGeneration()).setRewriteToken(authKey).setMaxBytesRewrittenPerCall(maxBytesPerCall).setProjection(DEFAULT_PROJECTION).setIfSourceMetagenerationMatch(StorageOption.IF_SOURCE_METAGENERATION_MATCH.getLong(getReq.sourceOptions)).setIfSourceMetagenerationNotMatch(StorageOption.IF_SOURCE_METAGENERATION_NOT_MATCH.getLong(getReq.sourceOptions)).setIfSourceGenerationMatch(StorageOption.IF_SOURCE_GENERATION_MATCH.getLong(getReq.sourceOptions)).setIfSourceGenerationNotMatch(StorageOption.IF_SOURCE_GENERATION_NOT_MATCH.getLong(getReq.sourceOptions)).setIfMetagenerationMatch(StorageOption.IF_METAGENERATION_MATCH.getLong(getReq.targetOptions)).setIfMetagenerationNotMatch(StorageOption.IF_METAGENERATION_NOT_MATCH.getLong(getReq.targetOptions)).setIfGenerationMatch(StorageOption.IF_GENERATION_MATCH.getLong(getReq.targetOptions)).setIfGenerationNotMatch(StorageOption.IF_GENERATION_NOT_MATCH.getLong(getReq.targetOptions)).setDestinationPredefinedAcl(StorageOption.PREDEFINED_ACL.getString(getReq.targetOptions)).setUserProject(projectId).setDestinationKmsKeyName(StorageOption.KMS_KEY_NAME.getString(getReq.targetOptions));
            ...
            return new RewriteResult(getReq, operationResponse.getResource(), operationResponse.getObjectSize().longValue(), operationResponse.getDone(), operationResponse.getRewriteToken(), operationResponse.getTotalBytesRewritten().longValue());
        } catch (IOException ioException) {
    ...
    public BucketAccessControl createAcl(BucketAccessControl accessControlEntry, Map<StorageOption, ?> clientConfig) {
        ...
        Scope traceScope = traceAgent.withSpan(traceSpan);
        try {
            return objectStore.bucketAccessControls().insert(accessControlEntry.getBucket(), accessControlEntry).setUserProject(StorageOption.USER_PROJECT.getString(clientConfig)).execute();
        } catch (IOException ioException) {
            traceSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw toStorageException(ioException);
        } finally {
    ...
    public Tuple<String, Iterable<HmacKeyMetadata>> listHmacKeys(Map<StorageOption, ?> clientConfig) {
        ...
        try {
            HmacKeysMetadata hmacKeysInfo = objectStore.projects().hmacKeys().list(projectKey).setServiceAccountEmail(StorageOption.SERVICE_ACCOUNT_EMAIL.getString(clientConfig)).setPageToken(StorageOption.PAGE_TOKEN.getString(clientConfig)).setMaxResults(StorageOption.MAX_RESULTS.getLong(clientConfig)).setShowDeletedKeys(StorageOption.SHOW_DELETED_KEYS.getBoolean(clientConfig)).execute();
    ...
    @Override
    public TestIamPermissionsResponse testIamPermissions(String containerRef, List<String> accessRights, Map<StorageOption, ?> clientConfig) {
        Span traceSpan = createSpan(HttpStorageRpcSpanNames.SPAN_TEST_BUCKET_PERMISSIONS);
        Scope traceScope = traceAgent.withSpan(traceSpan);
        try {
            return objectStore.buckets().testIamPermissions(containerRef, accessRights).setUserProject(StorageOption.USER_PROJECT.getString(clientConfig)).execute();
        } catch (IOException ioException) {
            traceSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw toStorageException(ioException);
        } finally {
    ...
    public Bucket lockRetentionPolicy(Bucket containerRef, Map<StorageOption, ?> clientConfig) {
        ...
        Scope traceScope = traceAgent.withSpan(traceSpan);
        try {
            return objectStore.buckets().lockRetentionPolicy(containerRef.getName(), StorageOption.IF_METAGENERATION_MATCH.getLong(clientConfig)).setUserProject(StorageOption.USER_PROJECT.getString(clientConfig)).execute();
        } catch (IOException ioException) {
            traceSpan.setStatus(Status.UNKNOWN.withDescription(ioException.getMessage()));
            throw toStorageException(ioException);
        } finally {
